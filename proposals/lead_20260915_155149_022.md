...
**feat(writeback): write a launched run's work queued before dispatch, and the items its nodes deliver**

*Project description:*
## What

Change the settlement write-back so a launched run marks everything it will work as claimed before it
starts any of it, releases that claim when the work will not happen, and does the same for items on
other boards that a node says it delivers.

This builds on the onetaskgraph release carrying two things, stated literally in that node and restated
here:

- **The status category `queued`.** Spelled `queued`, between `todo` and `in-progress`. `local-md` reads
  the word `queued` as that category, and `github-projects` maps it by default to the board option
  `Queued`.
- **The verb `onetaskgraph task status set <source>:<native-id> <category>`.**
  - It writes one existing task's status and nothing else.
  - Under `--json`, success prints `{qualified id, status: {category, name}}`.
  - Failure prints the failure document the other verbs write, and exits non-zero.
  - It refuses by name an unqualified id, an unknown source, a missing task, a disabled category, and a
    source that cannot write a status.

### 1. A plan-node field, `delivers`

- **Shape.** An optional list of qualified task ids (`<source>:<native-id>`), stored on the node's task
  as the reserved metadata key `onepipeline.delivers`. Absent means the node delivers nothing.
- **Refusals where the plan is read,** and where an edit introduces the problem:
  - a value that is not a list of qualified ids, refused naming the node and the offending value;
  - one ticket named by two nodes, refused naming both nodes and the ticket. The rule is one ticket,
    one node, by the manager's ruling.
- **Edits.** `add`, `retry` and `requeue` carry it like every other node field.
- **Its own task.** The write-back owns it on the node's own task, like the other declared fields, and
  writes it back unchanged.
- **Where the reserved keys live.** Read `src/taskgraph.rs`, which names `onepipeline.id`,
  `onepipeline.repo` and the `onepipeline.deps` refusal, for where reserved keys are parsed and
  refused.
...
### 3. The status each delivered item is written

Each delivered item is written through `onetaskgraph task status set`, and only when its status
changes:

- `queued` while its node has not started, and again when the node is requeued or retried back to
  pending, or the run is adopted with it pending;
- `in-progress` while its node is Running or CompleteDraft;
- `done` when its node settles Done;
- `todo` when its node settles failed, provider-failed, cancelled, parked or skipped, or the run closes
  out with the node never started.

A delivered write that fails is handled exactly as a failed projection is today:

- it is reported to the planner through the existing unprojected surface;
- it is retried on the existing schedule, or attempted again when the graph changes if the store
  refused it (see `FailureClass`);
- it never changes a scheduling decision, a settlement, or the run's exit.

### 4. Launch ordering

Today the reconcile loop calls `start_ready` before `writeback.publish` in the same pass, and the copy
runs on a worker thread, so the first projection lands after the first dispatch. Change it:

- **Every driver waits before its first dispatch,** whether it launched the run or adopted it. It waits
  for one attempt of its first whole projection, including the delivered items' `queued` writes,
  bounded by the store command deadline.
- **If that attempt fails,** dispatch proceeds and the existing unprojected surface is raised.
- **Later projections** stay off the loop, as they are.

### 5. The store version gate

Add a named constant beside `WRITEBACK_MEMBERS_FROM` in `src/cli.rs`, naming the first onetaskgraph
release carrying `queued` and `task status set`. For a store reporting an older version:

- unstarted nodes are written `todo`, as today;
- no delivered item is written;
- the run reports once why its delivered items are not written.

### 6. The pinned store and the documents

- **`justfile`.** `onetaskgraph-version` names the store release this depends on, so every store
  journey runs against it.
- **`docs/contract.md`.** The sentence beginning "Status writes back; the journal does not move" says
  what is true: `queued` while a node waits, the launch wait, the release of the claim at closeout, and
  the delivered items.
- **`docs/contract-divergences.md`.** The entry recording the projection's words (§50) and the reserved
  keys includes `queued` and `onepipeline.delivers`, and its counts are updated.

  `every_word_and_key_this_projection_writes_is_named_by_the_divergence` and
  `the_projected_categories_remain_named_by_the_approved_contract` in `src/writeback.rs` reconcile those
  documents with the code. Keep them as the drift gate and extend their lists.

### A shared interface is never changed unilaterally

The field name `delivers` and its key `onepipeline.delivers`, the words above, and the release-on-closeout
rule are what this host's adoption node builds against and documents for managers. If you find a
departure you want (a missing field, a different word, a better decomposition), keep building against
the agreed surface and put the proposal to your manager over `$ORCHESTRATOR_ASK_MANAGER`. The manager
decides whether the contract is amended.

## Why

In the user's words: "actually also want to add a queued status and when the dag starts everything in
that graph goes to queued and then as is already happening they go to in progress once the node is
actually running + completed once it's merged. this will prevent another manager picking up already in
flight follow-ups or other tickets in general, that are planned in a dag but are not running yet."

The engine is the only thing that knows when a run has claimed work. Today it writes unstarted nodes as
`todo`, after its first dispatch. So a manager, or an agent sent to pick up accepted tickets (which
selects `Todo`), sees claimed work as available. The same agent reads a second board of follow-up
tickets that a plan's nodes deliver, and no run marks those at all.

## Acceptance criteria

- **The plan field.**
  - A plan node's `delivers` is read from and written to the reserved task metadata key
    `onepipeline.delivers` as a list of qualified task ids, and a node without it delivers nothing.
  - A value that is not a list of qualified ids is refused where the plan is read and where an edit
    introduces it, naming the node and the value.
  - A plan in which two nodes name the same ticket in `delivers` is refused where the plan is read, and
    so is an edit that introduces such a duplicate. The refusal names both nodes and the ticket.
  - `add`, `retry` and `requeue` carry the field.
  - A projection writes it back unchanged on the node's own task.
- **The node's own words while driven.** Pending, Ready, Waiting and Blocked are written `queued`.
  Running and CompleteDraft are written `in progress`, Done `done`, Failed `failed`, provider-failed
  `provider-failed`, Cancelled `cancelled`, Parked `parked`, and Skipped `skipped`.
- **Releasing and re-queuing the claim.** At closeout, whether the run settled or was stopped, a node
  that never started is written `todo`. A driver adopting that run writes it `queued` on its first
  projection.
- **The delivered item's words.**
  - Each item a node delivers is written, through `onetaskgraph task status set`: `queued` while the node
    has not started, including again after a requeue, a retry back to pending, or an adopt; `in-progress`
    while the node runs; `done` when it settles done; and `todo` when it settles failed,
    provider-failed, cancelled, parked or skipped, or the run closes out with it never started.
  - An item is written only when its status changes.
  - The item's title, content, labels and metadata are read back unchanged after every write.
- **A failed delivered write.** It raises the existing unprojected planner surface and is retried on the
  existing schedule, or when the graph changes if refused. The run's node statuses, settlement and exit
  status are what they would be without it.
- **Launch ordering.**
  - Before its first dispatch, every driver (launch or adopt) makes one attempt at its first whole
    projection, including delivered items' `queued` writes, bounded by the store command deadline.
  - When that attempt succeeds, every task of the plan and every delivered item reads `queued` at the
    moment the first node is dispatched.
  - When it fails, the first dispatch still happens and the unprojected surface is raised.
- **The version gate.** A named constant beside `WRITEBACK_MEMBERS_FROM` holds the lowest store version
  whose binary offers `queued` and `task status set`, and the engine uses both only from that version.
  Against a store reporting a lower version, unstarted nodes are written `todo`, no delivered item is
  written, and the run says why exactly once. A test over the gate proves both sides of the boundary.
- **The pinned store.** The `justfile`'s `onetaskgraph-version` names a store version whose installed
  binary offers `queued` and `task status set`, the store journeys above run against that install, and
  `the_release_the_checks_install_meets_the_floor_and_is_named_once` passes.
- **The documents.** `docs/contract.md` and `docs/contract-divergences.md` name `queued`,
  `onepipeline.delivers`, the launch wait, the release of the claim at closeout, and the delivered
  items' words, with counts that match the code. The tests in `src/writeback.rs` that reconcile those
  documents with the projection's words and reserved keys pass with `queued` and `onepipeline.delivers`
  in their lists.
- **The journeys.** Driving the real `onepipeline` binary and the real `onetaskgraph` store over
  `local-md` sources, end-to-end journeys in `tests/e2e/` prove each of these:
  - **At the first dispatch.** At the moment the first node is dispatched, every task of the launched
    plan reads `queued`, and so does an item that a node names in `delivers` on a second source. This
    is observed from the store as the dispatched worker would read it, not from the projection record
    alone.
  - **Running and done.** A running node's task reads `in progress`, and a settled-done node's task reads
    `done`. Its delivered item reads `in-progress` and then `done`.
  - **A failed node.** A failed node's delivered item reads `todo`.
  - **A stopped run.** A run stopped with nodes never started leaves their tasks and delivered items at
    `todo`, and adopting it reads them `queued` again before its first dispatch.
  - **A refused delivered write.** A store refusing a delivered write raises the planner surface, and
    the run settles with its node statuses unchanged.
  - **A failed first projection.** A first projection that fails does not prevent the first dispatch.
- **Checks.** The unit tests of the modules this change touches, the end-to-end journeys above, and the
  repository's judged lint over this change's diff pass on the finished tree.
- **Proof and claims.** The behavior this node adds is proven end to end by journeys that drive the real
  binary against the real store, not by inspection. Every claim the dispatch makes about the finished
  work, including what it verified and the evidence for it, is true of the tree as it finally stands.

## Additional info

**Where to start reading.** Line numbers are from `v0.31.0`, so read the current tree before relying on
them.

- `src/writeback.rs`: `projected()`, `ProjectedStatus`, `every_projected_word`, `WholeBecause`,
  `Writeback::publish`, `wait_briefly`, and the module docs on ownership and on "A projection carries
  what changed".
- `src/engine.rs`: `Writeback::start` near the top of the loop, and the pass where `start_ready` runs
  before `writeback.publish`.
- `src/cli.rs`: `WRITEBACK_MEMBERS_FROM`, the precedent for a store-version gate.
- `src/taskgraph.rs`: the reserved `onepipeline.` keys, and `CHECKED_MINIMUM`.
- `tests/e2e/store.rs`: `projected_words` and the settlement-word journey, the pattern for reading words
  back off the real store.

**What this does not change.** The launch record, the member-copy logic and the retry schedule. A
delivered item is not a member of the launched project, so it never enters a `project copy`.

**Where the consumer lives.** This host's follow-up tickets live on a GitHub Projects board, addressed
as `followups:<native-id>`, and their `Todo` means accepted. This node proves the delivered writes on
`local-md` sources. The host proves them against its board fixture after adopting this release.

**Why this node waits on onetaskgraph's crate.** This node consumes onetaskgraph's `crate` target,
unlike the host's default `pypi` wheel. The manager approved this as an exception to the brief's "naming
no `consumes`", for this node alone. This repository's checks install the store with
`cargo install onetaskgraph` at the `justfile`'s `onetaskgraph-version`. In onetaskgraph's
`.github/workflows/release.yml`, `publish-crates` and `publish-python` are sibling jobs that do not wait
on each other, so a released wheel does not mean the crate is installable yet. The harness renders the
released version into this task. Take it from there.
...
{"onepipeline.adoption":"published","onepipeline.consumes":{"otg-queued-status":"crate"},"onepipeline.id":"op-queued-projection","onepipeline.max_turns":200,"onepipeline.persona":"engineer","onetaskgraph.item_kind":"task","orchestrator.plan-review":{"by":"planning-closeout","key":"334bf89e623cb785910539ed04c57ff0d50c2998122568c139c4d3991da91142","reviewed_at":"2026-09-15T12:09:08.875727+00:00"}}
-->
