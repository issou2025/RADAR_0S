...
**fix(workspace): fail closed on unreadable session records, and scan open run roots once**

*Project description:*
## What

Two defects in `crates/onevcs/src/workspace.rs`'s record listing, on one branch because
both reshape `workspace::all()` and its callers.

**Unreadable state read as absence** (https://github.com/nickderobertis/onevcs/issues/175). `workspace::all()`
(`:672-695`) returns an empty collection when the sessions directory cannot be read
(`let Ok(entries) = std::fs::read_dir(&directory) else { return Ok(Vec::new()); }`) and
skips any record that fails to load. Placement, reclaim, holder and sweep decisions then
mistake unreadable state for no state, and can reuse or remove a workspace that should have
been kept. Make `all()` propagate a directory read error and a record-load error, naming the
record — an **absent** sessions directory on a fresh host still reads as empty — and make
every **destructive** caller fail closed: the sweep through `spent_records` (`:738`, feeding
`sweep.rs:225`), pool prune and shed (`pool.rs:410`, `:940`), `supersede` (`:1236`), and
maintenance's `claim` (`pool/maintain.rs:296`). Read-only callers (`holders`, `status.rs`,
`preserve.rs`, `vcs.rs` recoverable, `resumable`) propagate the error rather than answering
from partial state; `status.rs:2501`'s `.ok()` stops swallowing it.

**Quadratic ownership checks** (https://github.com/nickderobertis/onevcs/issues/176). `Record::tree_is_its_own`
(`:521-533`) calls `all()?` for every record on a slot that is not open, and `holders`
(`:716`) and `spent_records` (`:738`) call `all()` and then `spent` per record, so a host
with many closed pooled sessions does quadratic record-directory work. Load the open run
roots **once** per top-level holder or sweep scan, and pass that snapshot into a pure
ownership predicate used for every record — threaded through `spent`,
`holds_unpublished_work`, `adopt` (`:1798`) and `close` (`:1922`).

**The contract other nodes read** (this plan's `op-relink-onevcs` adapts to it): the
public operations onepipeline calls — `session_holders`, `recoverable`, `pool_maintain`,
`workspace_capacity`, `preserve`, `session` — return `Err` naming the unreadable directory
or record instead of acting on partial state; an absent directory is still empty. State
it in `docs/contract.md`.

## Why

A sweep or placement that treats an unreadable session record as absent can remove or
reuse a workspace that holds live work, and there is no workaround — nothing tells the
operator the state was unreadable. The quadratic rescan makes every holder and sweep read
slower as closed pooled sessions accumulate, which on this host is every run.

## Acceptance criteria

- `workspace::all()` returns an error naming the directory when the sessions directory
  exists but cannot be read, and an error naming the record when a record cannot be
  loaded; an absent sessions directory still yields an empty list.
- Every destructive caller — sweep, pool prune and shed, supersede, maintenance claim —
  refuses to act when the listing errs, and no read-only caller answers from partial
  state; journeys drive the real CLI over an unreadable sessions directory and over a
  malformed record and prove a sweep and a prune remove nothing and report the error.
- A holder or sweep scan loads the open run roots once and decides every record's
  ownership through a pure predicate over that snapshot. A test drives the real holder and
  sweep operations over growing numbers of closed pooled records and proves two things:
  the sessions directory is listed a number of times that does not grow with the record
  count, and every ownership decision is the one made before the change.
- `docs/contract.md` states the error behaviour of the public operations and
  `tests/contract.rs` holds it.
- The tests covering what this change touches — the `sweep`, `holders`, `pool` and
  `maintain` e2e modules, `tests/contract.rs`, and the unit tests in `workspace.rs` — are
  green over the finished tree, and so is the judged lint over this change's diff.
- The branch carries GitHub closing metadata associating it with issue #175, #176 of this repository and with no other issue; whether the issue actually closes is decided downstream when the change lands, and is not this dispatch's bar.
- Every claim this dispatch makes about the finished work is true of the tree as it
  finally stands.

## Additional info

onevcs has no local hooks; its merge path is the CI in `.github/workflows/ci.yml`. `just test-one <name>` and `just test-quick` are the narrow runners. `docs/contract.md` states the public surface and `crates/onevcs/tests/contract.rs` holds it: a change to a public type is stated in both. `crates/onevcs/tests/e2e/refusing_fs.rs` looks like an existing helper for an unreadable directory. `ov-maintenance-types` follows this node in `pool/maintain.rs`'s `claim`. Put `Closes #175`, `Closes #176` in the body of the commit that resolves each ticket, so the landed change closes it; the issues live in this repository. onevcs releases from conventional commits through release-plz with `semver_check` on, and this plan's onepipeline relink waits for the release carrying this node, so a releasing type is required: `fix`, or `feat!` where the public API breaks (the semver check will say so).

A contract this task states is fixed for the run: other nodes of this plan build
against it. If you find a reason to depart from it — a missing field, a wrong shape, a
better decomposition — put the proposal to your manager over
`$ORCHESTRATOR_ASK_MANAGER` and keep building against the agreed surface until the
answer arrives; never change a shared interface on your own.

Iterate with the named tests and `just lint-llm-diff <base>` rather than the whole
gate; the repository's merge path runs the complete bar downstream on the change this
branch becomes. Items on the followups board are not yours to move — its credential is
masked from every dispatch, and the manager reads the board after the change lands.
Anything you notice outside this task is a follow-up draft, not part of this branch.
...
{"onepipeline.execution_checkout":"nickderobertis__onevcs","onepipeline.id":"ov-workspace-scan","onepipeline.max_turns":24,"onepipeline.node":"ov-workspace-scan","onepipeline.persona":"engineer","onetaskgraph.delivers":["followups:I_kwDOTx3GGM8AAAABSPGuDA","followups:I_kwDOTx3GGM8AAAABSPGwKw"],"onetaskgraph.item_kind":"task"}
-->
