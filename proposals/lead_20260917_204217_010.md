...
**fix(sdk): ship py.typed in the skilltest-sdk and skilltest-pytest wheels**

*Project description:*
## Amendment

Where this amendment and any other part of this task disagree, the amendment wins.

The branch `onevcs/s-d7206562b4df` already carries the whole change across three commits (`8a00dc3`, `f20e2e0`, `6d19a86`): the markers, the classifiers, the wheel-held tests in both packages, and the renamed stripped-wheel tests. Continue on that branch; the previous dispatch was failed on one reading of a criterion, not on the tree.

The criterion that the checks exercising this change are green over the finished tree is held to this for the `skilltest-sdk` project's `test-e2e` target: every test it runs passes except the two in `sdks/python/tests/test_oneharness.py` that fail on `main` before this change because the repository-root `oneharness.toml`'s `run_mode = "fallback"` is refused by the bundled oneharness 0.3.11 — failures this branch neither creates nor alters, which the report names with that reason and which stay in the follow-up draft rather than being repaired or suppressed here. Every other target the criterion names is green in full.

The judged lint over this change's diff against `main` is green — no finding, or every finding answered at its own site with a substantive reason and accounted for in the report — where the run's judged tier may be configured by a copy of the tree's `llmlint.yml` whose only difference is the unreachable `monorepo.llmlint.yml@1` plugin pin replaced by `project-graph.llmlint.yml@1`, the report saying so, and `llmlint.yml` itself is unchanged on the branch. A finding at a line the diff did not write, true of the tree on `main` before this change and neither created nor altered by it — `projects_stay_in_one_uv_workspace` at `plugins/pytest/pyproject.toml` and `sdks/python/pyproject.toml` — is answered by the report naming it as pre-existing with the reason, and carries no suppression directive.

## What

Make both Python packages of this repository typed packages under the invariant below,
and hold each wheel to it in the repository's own gate:

- `sdks/python` — `skilltest-sdk`, `hatchling`, `packages = ["skilltest_sdk"]`;
- `plugins/pytest` — `skilltest-pytest`, `hatchling`, `packages = ["skilltest_pytest"]`.

For each:

- **The marker.** An empty `py.typed` beside the package's `__init__.py`
  (`sdks/python/skilltest_sdk/py.typed`, `plugins/pytest/skilltest_pytest/py.typed`).
  `scripts/gen-contract.sh` writes four named files into `skilltest_sdk/` (`_report.py`,
  `_validation.py`, `_case.py`, `_error.py`) and nothing else there; the marker is
  outside what it owns, and a test shows that running the real generator
  (`scripts/gen-contract.sh`) leaves the marker in place.
- **The classifier.** `Typing :: Typed` in each `pyproject.toml`'s
  `[project].classifiers` (neither declares a list today; add it).
- **The wheel-held check.** A test in each package's `tests/` directory, reached by that
  project's `test-e2e` target, builds the wheel with `uv build --wheel` into a temporary
  directory — the way `scripts/build-python-dist.sh` and `scripts/build-python-wheel.sh`
  build what `publish.yml` uploads — and asserts that `<package>/py.typed` is a member of
  the wheel and that the wheel's `METADATA` carries `Classifier: Typing :: Typed`. Read
  the wheel as the zip it is; do not assert on the source tree. For `skilltest-sdk` the
  pure `py3-none-any` wheel is the one to build (an empty `_bin`); the platform wheels are
  that wheel retagged, so the marker in the pure wheel is the marker in every one.

Nothing about either package's runtime changes, and `skilltest-pytest`'s exact
`skilltest-sdk==` pin, the lockstep version, and every manifest version stay as they
are: the release pipeline writes them.

### The baseline invariant (fixed for this plan; every node restates it from here)

1. A Python project that publishes an installable distribution of importable modules —
   one whose `pyproject.toml` names a pure-Python build backend (`hatchling.build`,
   `uv_build`, `setuptools.build_meta`, `flit_core.buildapi`, `pdm.backend`,
   `poetry.core.masonry.api`) and does not opt out — ships an empty `py.typed` marker
   file inside the package directory it distributes, so the built wheel carries
   `<package>/py.typed` for every importable package it ships.
2. That project declares the `Typing :: Typed` trove classifier in
   `[project].classifiers`.
3. That project's own verification holds the **built wheel** to both: a check in the
   repository's gate builds the wheel the way its release does and fails when the
   wheel lacks the marker or its METADATA lacks the classifier. Where a generator
   writes the package, the marker lives outside what the generator owns and a test
   shows regeneration keeps it.
4. Exempt: a manifest with no `[build-system]`; a backend outside the pure-Python set
   (maturin under any bindings included); `[tool.uv] package = false`; the
   `Private :: Do Not Upload` classifier.
5. The baseline audit checks 1 as tree presence — a `py.typed` beside an `__init__.py`
   under the manifest's directory — and 2 as manifest content, and never builds
   anything. Clause 3 is the repository's own and the audit does not run it.

The authoritative statement lives in dero-skills' `references/languages/python.md` once
the `baseline-typed-packaging` node lands; the audit derives its check from it and each
repository's wheel check is that repository's proof of clause 3. If you find the
invariant short of what your package needs, do not work around it or change it on your
own: ask the manager over `$ORCHESTRATOR_ASK_MANAGER` and keep building against it.

A shared interface is never changed unilaterally while this plan runs: the invariant
above, the file names it fixes and the registration it names hold every node together.
A departure you want — a different placement, a wider or narrower firing rule, a better
decomposition — is a proposal over `$ORCHESTRATOR_ASK_MANAGER`, and you keep building
against the agreed surface until the manager amends it.

## Why

The user found a `# type: ignore[import-untyped]  # Package omits py.typed.` over an SDK
import in another repository and asked "why do i see type ignore on python sdk import? is
this a problem across all our python sdks?" Measured against the latest wheels on PyPI,
`skilltest-sdk` 0.10.0 and `skilltest-pytest` 0.10.0 ship no `py.typed` — both are
`hatchling` packages, and every `hatchling` SDK the user publishes was missing it while
every `uv_build` one had it. Both are fully annotated (pydantic report models generated
from `schemas/`, a typed runner and stream), so a consumer running mypy gets
`import-untyped` and every imported name as `Any`. The user's direction: "i want to make
sure we handle this properly in all of them", held to the built wheel rather than a file
in the tree.

## Acceptance criteria

- The `py3-none-any` wheel `uv build --wheel` produces in `sdks/python` contains
  `skilltest_sdk/py.typed`, and the wheel `uv build --wheel` produces in `plugins/pytest`
  contains `skilltest_pytest/py.typed`; each wheel's `METADATA` carries
  `Classifier: Typing :: Typed`.
- A test in each package's `tests/`, reached by that project's `test-e2e` target, builds
  that package's wheel into a temporary directory and asserts both facts on the wheel's
  contents, and fails when either is absent.
- A test shows that running the real contract generator leaves
  `sdks/python/skilltest_sdk/py.typed` in place, and `scripts/gen-contract.sh --check`
  still passes: the generated artifacts are unchanged.
- The checks that exercise this change are green over the finished tree: the
  `skilltest-sdk` and `skilltest-pytest` projects' `format-check`, `lint`, `typecheck`
  and `test-e2e` targets, the contract drift gate (`scripts/gen-contract.sh --check`),
  and the judged lint over this change's diff.
- Every claim this dispatch makes about the finished work — what it changed, what it
  verified, and the evidence for it — is true of the tree as it finally stands.

## Additional info

`AGENTS.md` at the root is the repository's own rules, including "Publishing": the
whole repository shares one lockstep version that semantic-release computes from the
squash-merged PR title, and `pr-title.yml` enforces a conventional title. This node's
title is a `fix`, which cuts a patch release of all six packages; that release is what
carries the marker to PyPI, and nothing in this plan waits on it. Never hand-edit a
manifest version.

The `test-e2e` targets set `SKILLTEST_BIN` to `target/debug/skilltest`, so `cargo build`
the CLI and the fake provider once before running them, as the targets' commands assume.
Iterate with the one Nx target that reports something (`pnpm exec nx run
skilltest-sdk:test-e2e`), and take the repository's diff-scoped judged lint for the
last check. The cheapest evidence that a wheel test can fail is to induce the absence
once — remove a marker, run the test, quote the failure, restore — and say in your
report that the quoted run is the induced one. Commit each piece as it works.

Publication is through the harness after you settle; the branch is verified by the
repository's own required checks downstream of you. Finish the branch, commit everything,
leave the tree clean, and report.
...
{"onepipeline.amendment":"Where this amendment and any other part of this task disagree, the amendment wins.\n\nThe branch `onevcs/s-d7206562b4df` carries the whole typed-markers change (`8a00dc3`, `f20e2e0`, `6d19a86`) and its change request is https://github.com/nickderobertis/skilltest/pull/37. Continue on that branch.\n\nThe repository's required check `check (ubuntu-latest)` refuses that branch for a failure the branch did not create: `sdks/python/tests/test_oneharness.py` fails because the repository-root `oneharness.toml` carries a `run_mode` key the oneharness the SDKs bundle no longer accepts (`TOML parse error at line 26`), and the same tree on `main` fails the same way under today's bundled oneharness. So, beside the typed-markers change, the branch carries exactly one further change: the repository-root `oneharness.toml` parses under the oneharness the SDKs bundle and expresses the routing it expressed before — the key that oneharness no longer reads is removed, or written in the spelling that oneharness documents for the same behaviour — so that `sdks/python/tests/test_oneharness.py` passes in full and the `skilltest-sdk` project's `test-e2e` target is green in full. That change is its own conventional commit whose subject says what it fixes, and the report names it and the reason. Nothing under `.github/workflows/` changes, and `llmlint.yml` is unchanged on the branch.\n\nThe judged lint over this change's diff against `main` is green — no finding, or every finding answered at its own site with a substantive reason and accounted for in the report — where the run's judged tier may be configured by a copy of the tree's `llmlint.yml` whose only difference is the unreachable `monorepo.llmlint.yml@1` plugin pin replaced by `project-graph.llmlint.yml@1`, the report saying so. A finding at a line the diff did not write, true of the tree on `main` before this change and neither created nor altered by it — `projects_stay_in_one_uv_workspace` at `plugins/pytest/pyproject.toml` and `sdks/python/pyproject.toml` — is answered by the report naming it as pre-existing with the reason, and carries no suppression directive.\n","onepipeline.branch":"onevcs/s-d7206562b4df","onepipeline.execution_checkout":"nickderobertis__skilltest","onepipeline.id":"skilltest-py-typed-2","onepipeline.persona":"engineer","onepipeline.resume":{"branch":"onevcs/s-d7206562b4df"},"onetaskgraph.item_kind":"task"}
-->
