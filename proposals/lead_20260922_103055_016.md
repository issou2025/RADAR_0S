...
**fix(deps): link the engine and bus releases without the agent crate**

*Project description:*
## What

Relink onepipeline-ui onto the onepipeline release that node `op-own-vocabulary`
produced and onto the onemessagebus release that node `omb-retire-agent` produced.
After this the read API links one engine and one bus core, and no crate in its graph
links `onemessagebus-agent`. Today this crate pins `onepipeline = "=0.42.0"` and
`onemessagebus = "0.7.0"`, so it pulls the agent crate in through the engine.

- Move the exact `onepipeline = "=…"` requirement and the `onemessagebus` requirement
  to those two releases, and refresh `Cargo.lock`. The comments above both
  requirements must stay true. `tests/packaging.rs` reconciles the lockfile's edges,
  so one copy of the bus is linked.
- The engine moves from `0.42.0` to the new release in one step. Adapt this crate to
  whatever the engine's public API changed across those releases, keeping the read
  API's served shapes as `docs/contract.md` states them. Any change to a served shape
  that the engine forces is named in the change request, never made silently.
- `src/filter.rs` says the stack reads its grammar "through one implementation,
  `onemessagebus-agent`'s", and `tests/contract.rs` holds this crate's copy of the
  grammar to "the bus's own matcher document". Both are re-pointed at the grammar's
  new owner, onepipeline: its `onepipeline::filter::{EventFilter, Matcher}` and its
  `docs/contract.md` or `schemas/events.json` (`agent.event-filter@1`). The copy stays
  held to a source that exists at the linked release.
- Any other prose here that names `onemessagebus-agent` names its replacement.
...
- `Cargo.toml` requires onepipeline at exactly the release `op-own-vocabulary`
  produced and onemessagebus at the release `omb-retire-agent` produced. `Cargo.lock`
  has no `onemessagebus-agent` package, exactly one `onemessagebus` package and exactly
  one `onepipeline` package. `tests/packaging.rs` passes.
- The filter-grammar contract test holds this crate's grammar to onepipeline's filter
  grammar at the linked release, not to `onemessagebus-agent`, and it passes.
- The read API's contract tests and e2e journeys pass over the finished tree against
  the new engine, driving the real server. If the engine upgrade changed a served shape,
  the worker's draft change request is published and its description names each
  changed shape.
- No manifest or lockfile of this repository names `onemessagebus-agent`. No code
  imports `onemessagebus_agent`. No document or comment names it as the current home
  of the filter grammar or of any other shape.
- The tests covering what this diff touches, and this repository's judged lint over
  this diff, are green on the finished tree.
...
The change request may be published early: if the engine upgrade changes a served shape,
open it as a draft and name each changed shape in its description, so the drafter
finishes from it. The operational notes below say how.

Follow this repository's `fix(deps)` precedent for relinking the engine (for example
`a49d074`, "link the engine and sibling releases that …"). The host that runs this plan
holds its `config/onepipeline-ui.version` and `config/onepipeline.version` pins to one
engine and currently declares the two as divergent. Its adoption node (`aio-adopt`)
depends on this release so it can retire that declaration. So this node's deliverable is
a release whose binary links exactly the engine `op-own-vocabulary` produced.

This node's cross-repository references name the releases to link.
...
{"onepipeline.adoption":"published","onepipeline.consumes":{"omb-retire-agent":"crate","op-own-vocabulary":"crate"},"onepipeline.execution_checkout":"nickderobertis__onepipeline-ui","onepipeline.id":"opui-relink","onepipeline.max_turns":24,"onepipeline.persona":"engineer","onetaskgraph.item_kind":"task","orchestrator.plan-review":{"by":"review-plan","key":"46c8ae52be780e5023488d7074d127f73dcf7557391c6c2bf68c64a68085bf53","reviewed_at":"2026-09-22T07:27:20.024275+00:00"}}
-->
