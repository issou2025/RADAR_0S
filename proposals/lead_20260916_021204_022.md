...
`Cargo.toml:526` declares the workspace requirement and `Cargo.toml:686-695` explains why
`oneharness-core` is taken at one copy across the workspace. The dependency's release is
named for you in this task's cross-repository references and in the note this run delivers
when the release arrives; take the version from there and from nowhere else.
...
for the fix in this stack before. What has to be true when you are done is that the lockfile
resolves `oneharness-core` to the named release.
...
seams are one type. If the named release falls outside what those siblings accept, that is a
fork to raise over `$ORCHESTRATOR_ASK_MANAGER` — not something to resolve by allowing a
second copy into the graph.

Nothing else is in scope: no behaviour of this repository changes, and no test of this
repository is rewritten to suit the new engine unless the relink makes an existing one
untrue, in which case the repair is part of this change.
...
anywhere until this relink happens and this host's engine pin moves. Without this node the
fix arrives whenever somebody happens to bump the dependency, which is not a plan.
...
- The check that reconciles this build's linked engine versions is green over the finished
  tree, and so is every oneharness-facing test of this repository.
- No behaviour of this repository is changed by this node, and any existing test the relink
  makes untrue is repaired here rather than left failing or skipped.
- The commit body names the oneharness issue this relink puts in force,
  https://github.com/nickderobertis/oneharness/issues/1290, as the work it carries.
...
the change this branch becomes; nothing here asks you to have run them first.

This node starts from the work of the sibling node that changed this repository's test
inputs and its label-strict store source, because that node also edits the manifest and the
lockfile. Its work is on the base you start from.
...
{"onepipeline.adoption":"published","onepipeline.consumes":{"oh-auth-classifier":"core"},"onepipeline.execution_checkout":"nickderobertis__onepipeline","onepipeline.id":"op-relink-oneharness","onepipeline.persona":"engineer","onetaskgraph.item_kind":"task"}
-->
