...
**fix(sdk): take a hyphen-leading credential in the python smoke**

*Project description:*
## What

Land on `main` the fix for the Python SDK's installed smoke check refusing a credential that
begins with `-`, in the form PR #57 already carries as commit `80f015b` — the smoke's own
parser reading the token after `--credential` exactly as given, the way the Node and Rust
smoke checks already do — together with its test that proves each of the three installed
clients against a real supervisor configured under such a credential.

### The cause

`tools/release-artifacts` proves each shipped client by installing it into a throwaway
environment and running its smoke check against a real supervisor, handing it the credential
the supervisor generated for itself: 32 random bytes as unpadded URL-safe base64, whose
alphabet includes `-`, so one in sixty-four begins with it. `python/printobserver-sdk/smoke/
smoke.py` parses its arguments with `argparse`, which reads `--credential -qx…` as two
options, and the check stops before it makes a request:

```
usage: printobserver-sdk-smoke [-h] --server SERVER --credential CREDENTIAL …
printobserver-sdk-smoke: error: argument --credential: expected one argument
```

Seen on PR #57's Linux gate (run 35227537584, job 105223161668,
`test_shipped_artifacts_journey.py::test_python_proofs_can_be_repeated[prove-client-python-smoke: contract]`),
on PR #58's Linux gate (run 35186163528, job 105088701184, the same test) and macOS gate
(run 35214921093), and in `tools/release-artifacts/tests/test_clients.py` on #58's run
35234929838. It is a defect of `main`: a user running the installed smoke check with their
own `-`-leading credential meets the same refusal.

### The fix, and why this form

The manager has ruled the fix belongs in the smoke's own parser, not in how one caller
spells the argument, because a user runs the installed check with a credential of their own.
PR #57's `80f015b` is that fix:

- `python/printobserver-sdk/smoke/smoke.py` reads `--server`, `--credential`, `--print-id`
  and `--image-id` as the token following each name, whatever it begins with, with a usage
  error (exit 2) naming the argument when one is absent or has no value.
- `tools/release-artifacts/tests/test_clients.py` gains
  `test_each_installed_client_takes_a_credential_beginning_with_a_hyphen`, parametrized over
  all three clients, which starts a real supervisor configured under a `-`-leading credential
  (`World(…, credential=HYPHEN_LEADING)`) and runs each installed smoke check against it.
- `tools/release-artifacts/src/release_artifacts/world.py` takes the optional `credential`
  the test needs; `suppressions.toml` has one site string updated for the smoke's renamed
  line.

**Take that commit rather than re-implementing it**: cherry-pick `80f015b` onto your
branch (the branch `origin/onevcs/s-02aab8fbdadf` carries it; fetch it read-only). At the
time of planning it conflicted on `main` only in the import block at the top of
`test_clients.py`, where #57's earlier commits had widened the imports — resolve to what
`main`'s test module actually needs plus `World`; if `main` has moved and the conflict is
wider, resolve it and say where. Then read the result as if you had written it: the test must exercise
all three clients (it does, through `CLIENTS`), the parser must refuse a missing value with
exit 2 and a message naming the argument, and nothing else in #57's branch is needed for it
to pass. Keep the commit's subject and its account of the cause; add your own commit only
for what the cherry-pick did not already do.

PR #58 carries a different fix for the same failure (`5558986`, `4ed4720`: the installer
spells `--credential=<value>` for the pypi target only, plus a leading-dash test of its own);
the manager has ruled it drops that on rebase. This node does not touch
`release_artifacts/installing.py`'s argument spelling, so #58's hunks there conflict with
nothing and are simply unnecessary once rebased.

## Why

The user's words: "let's make sure to fix flaky tests too — they're super expensive when
the gate is so long." This one fires on one push in sixty-four, on every platform cell at
once, and each time it looks like a broken client rather than an unlucky credential; it has
cost both live pull requests a two-hour round. And it is a real defect for an operator whose
credential starts with a dash, so the fix belongs in the shipped smoke check, once, on
`main`, rather than twice on two branches in two different forms.

## Acceptance criteria

- The installed Python smoke check (`python/printobserver-sdk/smoke/smoke.py`) takes the
  value of `--credential` — and of `--server`, `--print-id` and `--image-id` — exactly as the
  token following the name, whatever character it begins with, and refuses an absent name or
  a missing value with exit status 2 and a message naming the argument.
- `tools/release-artifacts/tests/test_clients.py` proves each of the three installed clients
  (`crate:printobserver-sdk`, `pypi:printobserver-sdk`, `npm:@printobserver/sdk`) against a
  real supervisor configured under a credential beginning with `-`, driving the installed
  smoke check the way `prove_client` drives it; its Python case, run against the smoke check
  as it stands at revision `365896e` (the head of `main` this plan was written against, which
  carries the defect), fails with `argument --credential: expected one argument`, and against
  the finished tree passes — the former cited as the prior-state evidence it is.
- The finished smoke parser and the new test correspond to PR #57's `80f015b`: the same
  parser behaviour (each named argument read as the token after its name, exit 2 naming a
  missing one) and the same test over the three clients; the argument spelling in `release_artifacts/installing.py` is
  unchanged; `suppressions.toml` changes only in the one site string the moved smoke line
  requires; `llmlint.yml`, `repo-policy.toml` and the CI matrix are untouched.
- No test's coverage is removed or narrowed, no test is marked ignored, and no assertion is wrapped in a retry that could mask a failure.
- The leading-dash cases hold beyond one run: the finished tree's
  `test_each_installed_client_takes_a_credential_beginning_with_a_hyphen` cases for all three
  clients, against a real supervisor, are shown passing repeatedly on this Linux host while
  the rest of the repository's parallel test tier runs beside them, with the number of runs
  and the commit they ran on stated, and no run among them failing.
- The tests over the code this node touches are green over the finished tree:
  `tools/release-artifacts`' tests including the new one, the Python SDK's own tests, and the
  `check-repo` checks over the tree (the client generation and suppression rules read these
  files); the Python linter and formatter checks over `tools/release-artifacts` and the Python
  SDK are clean, the Python type checker reports nothing, and the judged lint over this change's own diff
  leaves no finding unanswered — each named with what it reported.
...
**Reading order.** `python/printobserver-sdk/smoke/smoke.py` on `main`;
`tools/release-artifacts/src/release_artifacts/installing.py` (`prove_client`, `smoke_check`,
how the credential is handed to each client's check) and `world.py` (`World`, `start`, the
credential the supervisor writes for its clients); `tools/release-artifacts/tests/test_clients.py`;
then `git show 80f015b` in full, and the branch it sits on (`git fetch origin
onevcs/s-02aab8fbdadf` — read only). The failing logs:
`gh api --allow-escape-sequences repos/nickderobertis/printobserver/actions/jobs/105223161668/logs`
and job 105088701184.

**Running the proof.** The release-artifacts tests are a Python project under Nx
(`tools/release-artifacts/project.json` names the target; `uv run -q pytest
tools/release-artifacts/tests/test_clients.py -k hyphen` runs the new test alone). They
build and install real artifacts and start a real supervisor, so they take minutes; run them
in the foreground or with a sentinel inside one turn. `just check-repo` is the deterministic
repository-checks tier and is what reads `suppressions.toml`.

**This is a `fix(sdk)`, not a `test(...)`**: the shipped smoke check changes, so the release
automation is right to cut a release from it. Keep the title as given.
...
**Iterate with** the tests above, `uv run -q ruff check` / `ruff format --check` over the
touched projects, `uv run -q ty check` where the project runs it, `just check-repo`, and the
judged lint over your diff. The whole gate runs downstream on the merge path; nothing here
asks you to have run it first.

**A shared interface is never changed unilaterally.** The smoke check's command line
(`--server`, `--credential`, `--print-id`, `--image-id`) and the credential's own contract
(printable ASCII, not empty, neither beginning nor ending with a space) are fixed for this
run; a departure you want is a proposal to your manager over `$ORCHESTRATOR_ASK_MANAGER`, and
you keep building against the agreed surface while you wait.
...
{"onepipeline.execution_checkout":"nickderobertis__printobserver-isolated","onepipeline.id":"python-smoke-credential","onepipeline.persona":"engineer","onetaskgraph.item_kind":"task","orchestrator.plan-review":{"by":"review-plan","key":"e428d531b87db8bd3c667218da859d72e2978ae549bb89c54ae49e52bf415f4b","reviewed_at":"2026-09-17T17:54:03.894388+00:00"}}
-->
