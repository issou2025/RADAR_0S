...
**fix(linear): wait for deleted documents to become unreadable**

*Project description:*
## What

Ticket: https://github.com/nickderobertis/onetaskgraph/issues/1308 — *Linear live cleanup reads
deleted documents without settling*.

**Root cause (from the ticket).** The Linear live journey waits for created artifacts to become
visible but checks deletion visibility immediately: `crates/onetaskgraph-linear/tests/live.rs:755-769`
(basis `9158e053c7a6bb88ad8874cc7ab24b6769b9757d`) deletes documents and refuses at once when a
read still finds one. A release pull request failed because a document deleted moments earlier was
still readable through Linear's index.

**The accepted fix.** Use the existing bounded settling helper (`settle::settled` under
`settle::LINEAR_INDEX`, or a sibling helper in `tests/settle/mod.rs` built on it) to wait until each
deleted document becomes unreadable, and fail with the helper's naming diagnostic only once the
bound is spent.
...
Linear index lag fails onetaskgraph's required live lane after the behaviour under test has
already succeeded, which blocks onetaskgraph releases — and this host's plan store is
onetaskgraph, so a blocked release holds every fix this host is waiting on from it.

## Acceptance criteria

- The live journey's post-deletion check waits, through a bounded settling helper in
  `tests/settle/mod.rs`, until each deleted document reads as absent, instead of refusing on the
  first read.
- `tests/settle_gate.rs` drives that helper through the real plugin against a loopback workspace
  in which a deleted document stays readable for some reads and then disappears (the wait passes),
  and one in which it never disappears (the wait fails once its bound is spent, naming the
  document).
- The live test target compiles with the journey calling that helper.
...
**Stacked.** `otg-1309` and then `otg-1311` stack on this branch, because all three touch
`tests/live.rs` and `tests/settle/mod.rs`.
...
{"onepipeline.id":"otg-1308","onepipeline.max_turns":200,"onepipeline.persona":"engineer","onetaskgraph.delivers":["followups:I_kwDOUBBgxs8AAAABRkMuVg"],"onetaskgraph.item_kind":"task","orchestrator.plan-review":{"by":"planning-closeout","key":"98fe5c07c370059a51ff73492deedf2ab8e6fd38993ee870fd3d8d6ee9965442","reviewed_at":"2026-09-16T15:22:01.095449+00:00"}}
-->
