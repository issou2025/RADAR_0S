...
Adopt this workstream's cargo build contract in `skilltest` (github.com/nickderobertis/skilltest; checkout **no registered checkout on this host** — see the project record).

skilltest is a Cargo workspace of `crates/skilltest-cli` and `crates/skilltest-core` beside plugin and SDK projects; no crate outside the workspace.

**What exists today:** no `.cargo/config.toml`; the root `Cargo.toml` carries `[profile.dev] opt-level = 0` with a comment (the e2e suite spawns the binary many times). That table stays; `debug = 1` goes in the config file and the two merge. Add one sentence to that comment pointing at `.cargo/config.toml` for the debuginfo level.
...
- `scripts/e2e-lib.sh`, `scripts/gen-contract.sh`, `plugins/vitest/tests/helpers.ts` and `plugins/pytest/tests/conftest.py` resolve `target/debug/skilltest` at the clone root; `scripts/demo-gif.py` reads `target/release/skilltest`
- `just coverage` is `cargo llvm-cov nextest --workspace --features fake-provider`; the `justfile` gate runs `scripts/gen-contract.sh --check`; `.github/workflows/ci.yml`, the `e2e-*.yml` workflows, `publish.yml`, `release.yml`
...
- The checks that read the target directory, spawn a built binary or run under coverage are green over the finished tree (their commands are under `## Additional info`): the core crate's test target; the CLI build and the contract check, a reader of `target/debug/skilltest`.
- Coverage collection through this repository's own coverage interface still works over the finished tree, reporting usable per-file line results under the line-table debuginfo: the coverage tier with its floor is green (its command is under `## Additional info`).
...
- the core crate's test target: `just nx run skilltest-core:test`
- the CLI build and the contract check, a reader of `target/debug/skilltest`: `just nx run skilltest-cli:build && bash scripts/gen-contract.sh --check`
- coverage: the coverage tier with its floor: `just coverage`
...
{"onepipeline.id":"skilltest","onepipeline.persona":"engineer","onetaskgraph.item_kind":"task","orchestrator.plan-review":{"by":"planning-closeout","key":"13a65e40160d3a80e7b199ee7c525e87aaba058e217c230a0962a4e049d7ef0f","reviewed_at":"2026-09-20T09:43:09.764635+00:00"}}
-->
