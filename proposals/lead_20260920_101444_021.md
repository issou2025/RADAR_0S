...
Adopt this workstream's cargo build contract in `onepipeline` (github.com/nickderobertis/onepipeline; checkout `onepipeline` (registered, `~/projects/onepipeline`)).

onepipeline is a Cargo workspace whose members are the root package and `crates/testfakes`; no crate outside the workspace.
...
- `npm/test/launcher.test.mjs`, `npm/test/publish-gate.test.mjs` and `npm/test/workspace-nx.test.mjs` resolve `target/debug/onepipeline` at the clone root
- `tests/linked_engines.rs` writes fixture workspaces under `target/linked-engines/<case>` with their own `.cargo/config.toml` (a `[source]` replacement) and runs `cargo generate-lockfile` / `cargo update --dry-run` there with the working directory inside the clone — cargo now merges the root file's `[build]` and `[profile]` tables with the fixture's `[source]` table; neither command builds, so no target-directory lock is contended
- `.config/nextest.toml` profiles (nextest's, not cargo's — unrelated), `.github/workflows/ci.yml`, `published-smoke.yml`, `release.yml`
...
- The checks that read the target directory, spawn a built binary or run under coverage are green over the finished tree (their commands are under `## Additional info`): the `linked_engines` integration test, which writes fixture build configs under `target/` and resolves lockfiles there; the Node launcher tests, readers of `target/debug/onepipeline`.
...
- the `linked_engines` integration test, which writes fixture build configs under `target/` and resolves lockfiles there: `cargo nextest run --locked --test linked_engines`
- the Node launcher tests, readers of `target/debug/onepipeline`: `cargo build --locked && bash scripts/nx.sh run onepipeline-npm:test`
- coverage: a scoped run through the crate's coverage interface reporting per-file line results: `cargo llvm-cov nextest --locked -E 'kind(lib)' --summary-only`
...
{"onepipeline.id":"onepipeline","onepipeline.persona":"engineer","onetaskgraph.item_kind":"task","orchestrator.plan-review":{"by":"planning-closeout","key":"88975b6a76c98c830ea880f2f2e8e9afab3c06aa81161b3118637cc1b5877f4e","reviewed_at":"2026-09-20T09:43:09.705323+00:00"}}
-->
