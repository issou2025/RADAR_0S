...
Adopt this workstream's cargo build contract in `printobserver` (github.com/nickderobertis/printobserver; checkout `nickderobertis__printobserver` (registered; execution checkout `nickderobertis__printobserver-isolated`)).

printobserver is a Cargo workspace of twelve `crates/*` members beside Python and Node projects; no crate outside the workspace.

**What exists today:** no `.cargo/config.toml`; `Cargo.toml` carries `[workspace.package]`, `[workspace.dependencies]` and `[workspace.lints]` and no profile table.
...
No cargo invocation in the tree names its own target directory inside the clone.

**Builds that happen outside the clone or already name the contract's directory** (read them; none changes):
- `tests/repo-e2e/tests/test_release_tag_journey.py` copies the tree into a temporary directory and **writes** `.cargo/config.toml` there (a `[source.crates-io] replace-with` pointing at a stand-in registry). With a tracked file of that name, that write replaces the copy's contract file; the copy builds outside the clone either way. Read that helper and decide: compose the registry tables on top of the copied file's contents (so the journey builds the copy under the contract, which is the cheaper build), or leave the overwrite and say why in the report
...
- `tests/repo-e2e/tests/test_windows_lint_journey.py` sets `CARGO_TARGET_DIR` to `REPO_ROOT/target`, the directory the contract names; `test_registry_proof_journey.py`, `test_service_manager_journey.py`, `test_server_shutdown_journey.py`, `test_windows_service_installer_journey.py`, `tools/release-artifacts/src/release_artifacts/build.py` and its tests resolve `target/debug/...`, `target/release/...` and `target/package` at the clone root; `tools/repo-checks/tests/treecopy.py` keeps its schema lock file under `target`; `test_bootstrap_journey.py` lists `target` among build products
- `just check-repo` (`uv run -q python -m repo_checks`) reads `repo-policy.toml` and the Nx targets; `just coverage` is `repo_checks coverage` over `cargo llvm-cov` per crate; the committed `.githooks/commit-msg` admits `build` as a non-release type
- `.github/workflows/ci.yml` (platform matrix), `install-path.yml`, `platform-*.yml`, `release-plz.yml`
...
- No `CARGO_TARGET_DIR`, `--target-dir` or `build.target-dir` anywhere in the `justfile`, `project.json`, `nx.json`, workflows, scripts or tests names any target directory but `<clone>/target` itself for a build inside the clone (a build in a temporary directory outside the clone, and the directories the build tool or its own tooling creates under `<clone>/target` for a build it drives, are the only exceptions); every invocation that named one before is inventoried in the report with what replaced it, and nothing introduces a directory shared across worktrees or repositories.
- `.gitignore` ignores the root `target` directory and carries no entry for a nested target directory the contract makes dead.
- The checks that read the target directory, spawn a built binary or run under coverage are green over the finished tree (their commands are under `## Additional info`): the repository-policy checks; one crate's test target and the CLI crate's build target; the release-tag journey, which writes a `.cargo/config.toml` into its copy of the tree.
- Coverage collection through this repository's own coverage interface still works over the finished tree, reporting usable per-file line results under the line-table debuginfo: one crate's test target, which runs under coverage instrumentation without reporting, followed by the Rust-only summary report over the profiles it wrote is green (its command is under `## Additional info`).
...
- the repository-policy checks: `just check-repo`
- one crate's test target and the CLI crate's build target: `bunx nx run printobserver-types:test && bunx nx run printobserver:build`
- the release-tag journey, which writes a `.cargo/config.toml` into its copy of the tree: `uv run -q pytest tests/repo-e2e/tests/test_release_tag_journey.py`
- coverage: one crate's test target, which runs under coverage instrumentation without reporting, followed by the Rust-only summary report over the profiles it wrote: `bunx nx run printobserver-types:test && cargo llvm-cov report --summary-only`
...
{"onepipeline.execution_checkout":"nickderobertis__printobserver-isolated","onepipeline.id":"printobserver","onepipeline.persona":"engineer","onetaskgraph.item_kind":"task","orchestrator.plan-review":{"by":"planning-closeout","key":"8345349a971abda247bd5cde3ed699dfe480c58b5fca1c86086cdfab82e3d6cc","reviewed_at":"2026-09-20T09:43:09.741022+00:00"}}
-->
