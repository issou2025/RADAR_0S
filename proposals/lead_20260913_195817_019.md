...
**feat(deps): link the onevcs, onejudge and oneagentgraph fixes**

*Project description:*
## What

**Read against** `nickderobertis/onepipeline` `origin/main` at `6b8f49d` — the v0.29.1 release commit, one commit past the `8242960` issue #1004 read, which changed only the version in `Cargo.toml`, so every onepipeline line the issue cites is unchanged; the producers it links were read at onevcs `e0f307c`, onejudge `ad9cc13` and oneagentgraph `7545a7c`. Every path and line this task cites was checked against that tree; if the tree has moved when you start, re-read what moved rather than trusting a cited line number.

Link into the engine the library fixes that
`nickderobertis/ai-orchestrator` issue #1004 root causes 2 and 3 land in other repositories,
so that a dispatch runs them. A dispatch publishes through the onevcs the engine links, and
settles through the oneagentgraph and onejudge it links. Move the workspace requirements in
`Cargo.toml` — `onevcs` (about `:446`), `onejudge` (about `:478`), `oneagentgraph` (about
`:445`) — to the releases that carry those fixes, together with `onevcs-testing` wherever
the new onevcs requires it. Then resolve `Cargo.lock` to one copy of each.

**What those releases carry**, stated in full in their own nodes' tasks and restated here
rather than redecided:

- **onevcs (`onevcs-no-verdict`).**
  - A required check concluding `cancelled` or `stale` is no verdict. A publication keeps
    reading and ends `checks-unsettled` rather than `checks-failed` when no later verdict
    arrives. `failure`, `timed_out`, `action_required` and `startup_failure` still refuse.
    The `FailureKind` vocabulary is unchanged.
  - `onevcs release acknowledge <REFERENCE> --target <NAME> --version <VERSION> [--supersede]`
    accepts an automated target whose landing has no established baseline. `release status`
    and the library's `release_status` then answer released at that version, marked
    acknowledged.
- **onejudge (`onejudge-named-artifacts`), through oneagentgraph
  (`oneagentgraph-link-onejudge`).** `user.artifacts` is an optional list of file or directory
  paths, relative ones resolved against the judge's evidence worktree, with overrides
  `--artifact <PATH>` and `ONEJUDGE_ARTIFACTS`. It is rendered into the judge-side prompts
  only when non-empty. The retry limit and git evidence tools are unchanged.

**What the engine owes on top of the version move** is proof that each fix reaches a dispatch.

- **Release hold.** A node held `published` on a producer whose landing has no baseline is
  released once that landing's release is acknowledged through onevcs. This is the #855
  incident, where the hold read `release_status` and waited with no timeout.
- **Publication closeout.** A closeout whose required check concludes `cancelled` does not
  settle `checks-failed`.
- **Judge.** A dispatched `kind: onejudge` member whose persona delta carries
  `user.artifacts` gives its judge a prompt naming them.

Where the engine's own text names onevcs's outcome for a check, or the hold's not-answered
state, bring it in line in `docs/contract-divergences.md`. How the engine classifies
`checks-unsettled` for retries is not changed by this node. If the version move forces a code
change beyond that, it is part of this node.

## Why

In the run that closed #855:

- A cancelled check cost a publication attempt.
- A `published` hold never cleared on a release that was already on PyPI.
- A design-document judge looped on a git question that could not see its subject.

The operator wants none of these to happen to a dispatched node again. A dispatched node runs
the libraries this engine links, not the CLIs a host installs, so none of the three fixes
reaches a dispatch until the engine links the releases that carry them.

## Acceptance criteria

- `Cargo.toml` requires, and `Cargo.lock` resolves, onevcs, onejudge and oneagentgraph
  versions that contain the fixes above, with a single copy of each and of `oneharness-core` in
  the tree.
- A node held `published` on a dependency whose landing has no release baseline stays held
  until that landing's release is acknowledged through onevcs, and is then released. This is
  proven end to end through the compiled engine with a sandboxed onevcs home.
- A publication closeout over a required check concluding `cancelled`, with no later verdict,
  keeps reading the change's checks after that conclusion — the host double records further reads
  of them — and then settles `checks-unsettled`: not `checks-failed`, not a landing, and not a wait
  with no end. This is proven end to end through the compiled engine with the onevcs host double.
- A dispatched `kind: onejudge` member whose persona delta names `user.artifacts` gives its
  judge a prompt naming them, proven end to end through the compiled engine with the harness
  doubled.
- The existing publication, release-hold and dispatch journeys stay green.
- `docs/contract-divergences.md` says nothing about onevcs's check outcome or a missing baseline
  that the linked onevcs contradicts.
- The tests covering the changed dependencies and any code they forced pass over the finished
  tree, and the judged lint over this node's diff leaves no finding unanswered.
...
- Read first:
  - `AGENTS.md` and `tests/AGENTS.md`.
  - The comment blocks above each moved requirement in `Cargo.toml`. They record why each
    version was taken; add this raise to them in the same voice.
  - The engine code that reads `release_status` for a held node, and the publication closeout
    that maps onevcs's failure kinds.
- The references block the engine appends to this task names each consumed release; take the
  versions from there.
- `onepipeline-monitor-cursor`, `onepipeline-retire-revision-pin`, `onepipeline-writeback-divergence`
  and `onepipeline-teardown-diagnosis` run in parallel in this repository and touch none of these
  requirements; they edit `docs/contract-divergences.md` entries 58, 44 and 50 in place. A conflict in the release changelog is release-plz's to resolve on the merge
  path.
- Tests: one e2e module runs with `cargo nextest run --locked -E 'binary(e2e) and
  test(/^<module>::/)'`, and `just lint-llm-diff <base>` is the judged tier. The merge path runs
  the full gate.
- If a proof above needs a double the linked `onevcs-testing` release does not offer, say so to
  the manager over `$ORCHESTRATOR_ASK_MANAGER` rather than weakening the proof.
- The title must stay a releasing type. This host adopts the engine through the
...
{"onepipeline.adoption":"published","onepipeline.consumes":{"oneagentgraph-link-onejudge":"crate","onejudge-named-artifacts":"crate","onevcs-no-verdict":"crate"},"onepipeline.id":"onepipeline-link-fixes","onepipeline.max_turns":200,"onepipeline.persona":"engineer","onetaskgraph.item_kind":"task","orchestrator.plan-review":{"by":"review-plan","key":"ea63bbba4d0719f59481e790c27bf57edf30fafafc7e24c175a67b12ebf0060e","reviewed_at":"2026-09-13T17:30:18.984841+00:00"}}
-->
