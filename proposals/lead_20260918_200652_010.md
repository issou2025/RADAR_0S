...
**fix(release): hold a published node whose dependency could not be resolved this pass**

*Project description:*
## What

Resolve https://github.com/nickderobertis/onepipeline/issues/366, accepted at severity
`critical`: a release hold treats an unresolvable dependency the same as no dependency, so
a `published`-adoption node can dispatch at once against work no release carries.

`src/release.rs::resolve` returns an empty `Vec` whenever any of a node's out-of-repository
dependencies is `Resolution::Unreadable`, and `held` then drops a node from the held set
whenever its cached `dependencies` entry is empty or absent — reading "could not be
resolved this pass" as "has no out-of-repository dependency". Two real triggers reach that
one path: a dependency reference that desyncs after a retry-rename (`src/graph.rs` says a
`retry` clones a node; `release.rs`'s `node.consumes.get(dep)` is a direct lookup with no
fallback for the renamed id), and a dependency whose landing is relayed later in the same
reconciliation pass the consumer becomes ready in, so `resolve` cannot yet describe it on
the consumer's first ready pass. Both were observed: `node-unheld` followed at once by
`node-dispatched` with no `release-probed` ever emitted, and `node-dispatched` recorded
before the hold that should have applied was released.

Distinguish the two in `resolve`: when any dependency is `Resolution::Unreadable`, record
an explicit unresolved marker for the node rather than an empty set or no entry, and have
`held` hold the node whenever that marker is present — not only when the cached entry is
non-empty — until a later pass resolves every dependency. The hold reports what it is
waiting on the way every release hold does (the `node-held` reason and the recurring
`release-wait` surface), naming the dependency it could not resolve, so an operator reads
a hold rather than silence. Read whether the retry-rename case also needs the `consumes`
lookup to follow the rename; if the marker alone closes both triggers, say so in your
report with the evidence, and if it does not, close the rename too. Nothing changes for a
node whose dependencies all resolve, and a node with genuinely no out-of-repository
dependency is still not held.

## Why

The user accepted this at severity `critical` because it is the release-adoption
mechanism failing open: a node whose whole purpose is to wait for a release carrying its
dependency's work launched immediately, silently, with no probe and no diagnostic trail,
and only a worker noticing an inconsistency mid-dispatch caught it. This host's adoption
nodes — two of them in this very plan — rely on that hold to say when a fix is really in
force.

## Acceptance criteria

- A `published` node with an out-of-repository dependency that cannot be resolved on a
  reconciliation pass is held on that pass, its hold reason names the unresolved
  dependency, the recurring release-wait surface names that dependency on every pass the
  hold persists, and it is released only once every dependency resolves and answers
  released.
- A `published` node readied in the same pass its dependency's landing is relayed in is
  held before any dispatch is recorded, and a release probe runs for it before it launches.
- A `published` node depending on a retry-renamed node is held and probed exactly as one
  depending on the original would be.
- A node whose dependencies all resolve behaves as today, and a node with no
  out-of-repository dependency is never held by this change.
- The branch's history carries a closing reference naming
  https://github.com/nickderobertis/onepipeline/issues/366, so the change that carries the work closes it.
- Both triggers are proven by end-to-end journeys driving the real binary through a
  release probe double — the same-pass landing and the retry-rename — rather than by
...
Iterate with this repository's diff-scoped judged lint and the `holds`, `adoption` and
`crossdag` e2e modules rather than the whole gate; the required checks run downstream on
the change this branch becomes. `tests/e2e/adoption.rs` and `tests/e2e/holds.rs` already
drive `published` nodes through the probe double `tests/e2e/probe.sh` and are where the
two new journeys belong.

A sibling node of this plan (`op-views`) changes how the status view renders a release hold
from the journal's `node-held` record; it does not touch `src/release.rs`, and nothing here
touches `src/views.rs`.

This repository's releases are cut from conventional commits, and this host's engine pin
adopts the release that carries this fix, so a commit type that cuts no release would leave
the chain waiting. `fix` is the type.
...
{"onepipeline.branch":"onevcs/s-46c8a9047ff6","onepipeline.context":"Correction to the note above: there is no incomplete-step commit on onevcs/s-46c8a9047ff6. Its HEAD is c295947 (test(adoption): count the same-pass journey in the edge rule's reason, a three-line change to tests/e2e/adoption.rs committed at 19:24Z), on top of d283d7e, the fix. The previous dispatch's worktree is gone, so if the judge's 'uncommitted edit' was something made after c295947 it was not preserved; treat the branch as it stands as the tree, run the touched modules' tests and the judged diff lint on it, and commit whatever those still name. If they name nothing, the branch is finished and the report is what remains.","onepipeline.execution_checkout":"nickderobertis__onepipeline","onepipeline.id":"op-release-hold-3","onepipeline.persona":"engineer","onepipeline.resume":{"branch":"onevcs/s-46c8a9047ff6"},"onetaskgraph.delivers":["followups:I_kwDOTx8aUc8AAAABR7a3kg"],"onetaskgraph.item_kind":"task"}
-->
