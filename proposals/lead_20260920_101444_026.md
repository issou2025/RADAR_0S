...
Adopt this workstream's cargo build contract in `oneharness-ui` (github.com/nickderobertis/oneharness-ui; checkout `nickderobertis__oneharness-ui` (registered)).

oneharness-ui is a TypeScript-first repository carrying a Cargo workspace whose one member is `apps/desktop-shell` (the Tauri shell); no crate outside the workspace. GitHub reports its primary language as TypeScript, which is why the brief names it beside the Rust list.

**What exists today:** no `.cargo/config.toml`; the root `Cargo.toml` carries `[workspace.package]` and `[profile.release]`, which stay untouched.
...
- `CARGO_TARGET_DIR="$ROOT/target/oneharness-ui-upstream-build"` in `scripts/build-compatible-cli.sh`, a `cargo install --git` of a pinned upstream `oneharness` revision built with `RUSTFLAGS` unset because the upstream crate does not compile under this repository's `-D warnings`

Consolidate it: point `scripts/build-compatible-cli.sh`'s `CARGO_TARGET_DIR` at `$ROOT/target` — the variable stays, because a `cargo install --git` with no target directory set builds in a temporary one it then discards, which is worse than any sub-directory — and keep `RUSTFLAGS` unset for that build as today. Every dependency the upstream crate shares with this workspace is then rebuilt on each alternation between the two flag sets; say so in the report with one measured alternation.
...
- `scripts/run-desktop-e2e.mjs`, `scripts/build-sidecar.mjs`, `scripts/release-assets.mjs`, `scripts/upload-release.sh`, `apps/desktop-shell/tests/e2e/capabilities.ts` and the `check.yml`, `release.yml` and `desktop-e2e.yml` workflows read `target/release/...`, `target/release/bundle` and `target/release-assets` at the clone root
- `scripts/build-test-provider.mjs` compiles a fixture with bare `rustc` into `target/oneharness-ui-test`, which `scripts/test-rust.sh` also uses for an empty history store; `packages/oneharness-bridge/test/*.integration.test.ts` read the fixture from there
- `apps/desktop-shell/tests/e2e/fixture.ts` passes `CARGO_TARGET_DIR` through to the application under test when the environment carries one
- every `just` recipe runs Nx under `RUSTFLAGS="-D warnings"`; `scripts/test-rust.sh` runs `cargo llvm-cov --workspace` for `desktop-shell:test`
...
- No `CARGO_TARGET_DIR`, `--target-dir` or `build.target-dir` anywhere in the `justfile`, `project.json`, `nx.json`, workflows, scripts or tests names any target directory but `<clone>/target` itself for a build inside the clone (a build in a temporary directory outside the clone, and the directories the build tool or its own tooling creates under `<clone>/target` for a build it drives, are the only exceptions); every invocation that named one before is inventoried in the report with what replaced it, and nothing introduces a directory shared across worktrees or repositories. The behaviours the removed sub-directories protected, named under `## What`, still hold: the pinned upstream `oneharness` CLI still builds from source with `RUSTFLAGS` unset and reports its pinned version and the report quantifies the cost the removed isolation hid with one measured alternation taken before the change and one after — main build, then the upstream build, then the main build again — giving each step's wall-clock time and the crates it recompiled, so the two can be compared.
- `.gitignore` ignores the root `target` directory and carries no entry for a nested target directory the contract makes dead.
- The checks that read the target directory, spawn a built binary or run under coverage are green over the finished tree (their commands are under `## Additional info`): the desktop shell's Rust test tier (the workspace under coverage instrumentation); the desktop shell's build; the compatible upstream CLI source build under the one target directory; the bridge integration tests, readers of the rustc-built fixture under `target/oneharness-ui-test`.
- Coverage collection through this repository's own coverage interface still works over the finished tree, reporting usable per-file line results under the line-table debuginfo: the desktop shell's test tier, which is the workspace under coverage instrumentation is green (its command is under `## Additional info`).
...
- the desktop shell's Rust test tier (the workspace under coverage instrumentation): `bunx nx run desktop-shell:test`
- the desktop shell's build: `bunx nx run desktop-shell:build`
- the compatible upstream CLI source build under the one target directory: `bash scripts/build-compatible-cli.sh`
- the bridge integration tests, readers of the rustc-built fixture under `target/oneharness-ui-test`: `bunx nx run oneharness-bridge:test`
- coverage: the desktop shell's test tier, which is the workspace under coverage instrumentation: `bunx nx run desktop-shell:test`
- the judged lint over this branch's diff and the format check: `just lint-llm-diff <base> && just format-check`
...
{"onepipeline.id":"oneharness-ui","onepipeline.persona":"engineer","onetaskgraph.item_kind":"task","orchestrator.plan-review":{"by":"planning-closeout","key":"043c8043122db0d74abd8393a79bcc297f57d8fd926d6eebcbd894d258a11d20","reviewed_at":"2026-09-20T09:43:09.620023+00:00"}}
-->
