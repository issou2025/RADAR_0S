...
**fix(local-md): read the canonical in-progress word and resolve a file-relative root**

*Project description:*
## What

Three accepted follow-up tickets against onetaskgraph. The first two both change
`crates/onetaskgraph-local-md/src/lib.rs`, so they could never run in parallel; the third is
a documentation correction in the same area. One commit per ticket.

**1. local-md omits the canonical in-progress status spelling** —
https://github.com/nickderobertis/onetaskgraph/issues/1140, accepted at severity `high`.
`crates/onetaskgraph-local-md/src/lib.rs:75-88` builds the default status mapping with the
display aliases `in progress` and `doing` but without the normalized category's own
canonical spelling, `in-progress`. A task that persists that word therefore reads back as
`unknown` even though the CLI exposes `in-progress` as the normalized category, so
status-based selection and automation can omit actively running work or treat it as
unknown, and an author has to use a display alias to keep the classification. Every other
canonical category word — `draft`, `backlog`, `todo`, `queued`, `done`, `cancelled` —
already classifies as itself.

Add `in-progress` to the default mapping, document in `docs/local-md.md` that a canonical
normalized category spelling is accepted, and add a table-driven plugin test that reads
every normalized category word back as its own category while the existing aliases keep
working. Drive it off the category type's own variants rather than a written list, so a
category added later without a mapping fails that test rather than passing unnoticed —
`queued` was added recently and is exactly the case that list would have missed.
`in_progress` is not a canonical category word and needs no mapping; if you decide to accept
it anyway, say so in your report with the reason.

**2. Relative local-md roots resolve against the process working directory** —
https://github.com/nickderobertis/onetaskgraph/issues/1144, accepted at severity `high`. A
`local-md` root read out of `onetaskgraph.yaml` keeps no usable relationship to that file's
directory by the time the plugin is instantiated: `crates/onetaskgraph-core/src/config/discovery.rs:34-60,110-131`
loads the nearest document found by walking upward from the working directory, and
`crates/onetaskgraph-local-md/src/lib.rs:352-357` canonicalizes `config.root` directly
without rebasing it against the document's path. A command run from a worktree or another
directory therefore reads or writes a different store than the configuration's author
intended, and every cross-directory launcher has to inject an absolute root instead.

**The contract this establishes, which the node states rather than re-decides.** Preserve
the origin of a file-supplied path through effective configuration, and resolve a relative
`local-md` root against the directory containing the configuration document that supplied
it. A relative root supplied through the environment layer or a flag keeps resolving against
the process working directory, because there is no document to rebase it on — state that
rule explicitly in the documentation rather than leaving it to be inferred, and say it in one
place that both the core configuration layer and the plugin are read against. The reconciling
check is the CLI journeys the ticket asks for: one from a subdirectory of the checkout and
one from a sibling worktree, each proving the same configured store is read.

That environment-layer half is load-bearing for a real consumer. The `ai-orchestrator`
harness exports absolute source roots today as the workaround for this defect, and it will
keep exporting them afterwards for a different reason: a dispatched planner works in a
worktree that carries its own copy of `onetaskgraph.yaml`, so document-relative resolution
would send the plan it authors into the worktree's own `.plans`, which is the failure that
export exists to prevent. Do not change environment-layer semantics to be
document-relative, and do not assume any launcher's absolute export is now redundant.

**3. The reference subprocess host is documented as a shipped runtime** —
https://github.com/nickderobertis/onetaskgraph/issues/1238, the onetaskgraph half.
`README.md:486` says `onetaskgraph-source` ships beside the main binary, while
`.github/workflows/release.yml:25-46` packages only the user-facing CLI. That wording led a
downstream repository's offline journey to treat the reference host as a runtime dependency
and resolve an unrelated stale local build from its search path. Stop documenting it as
shipped beside the CLI, and keep the wording accurate about what it is: a reference
implementation of the plugin side and a test host that re-runs this repository's own plugin
journeys over a real pipe.

**The binary stays.** The ticket offered dropping it and the manager has decided against
that: `crates/onetaskgraph/tests/common/mod.rs:40-50` and
`crates/onetaskgraph/tests/e2e/fixtures.rs:408-426` use it to repeat journeys through a real
subprocess boundary, which is exactly the reference coverage the ticket asked to be weighed.
This piece is a documentation correction and nothing else: do not remove the binary target,
and do not add the host to the release artifacts.

## Why

The user accepted all three by moving them to `Todo` after a follow-up run verified each
against a real basis. The first makes a task that says it is in progress read as unknown, so
an operator's own automation can skip work that is actually running. The second is why every
launcher in this stack carries an absolute-root workaround, and why a plan authored in one
directory can be invisible to a process in another — the failure looks like a plan that
vanished. The third is a sentence that cost a downstream repository's test chain a
dependency on an executable nobody ships, and the user asked on the issue what the host even
is, which is a sign the wording misleads a reader about the SDK boundary.

## Acceptance criteria

- The local-md default status mapping reads the canonical `in-progress` word back as the
  in-progress category, and every other normalized category word still reads back as its own
  category.
- The existing display aliases for work in progress still classify as they did.
- A table-driven plugin test covers every normalized category word by reading the category
  type's own variants, so a category added later with no mapping fails that test.
- `docs/local-md.md` states that a canonical normalized category spelling is accepted, and no
  longer implies that the canonical in-progress word maps to unknown.
- A relative `local-md` root supplied by a configuration document resolves against the
  directory containing that document.
- A relative root supplied through the environment layer or a flag resolves against the
  process working directory, and the documentation states that rule for both layers in the
  one place a reader of either the configuration layer or the plugin would look.
- A CLI journey run from a subdirectory of a checkout and a CLI journey run from a sibling
  worktree each read the same configured store, and each fails if a root is resolved against
  the process working directory instead of against the document.
- `README.md` no longer says the reference subprocess host ships beside the CLI, and it says
  what the host is: the reference implementation of the plugin side and the test host for this
  repository's own journeys.
- The reference host is still a binary target of the CLI crate and is still absent from the
  release artifacts.
- Each of the three tickets is resolved by its own commit whose body carries a `Closes` line
  naming that ticket's issue: 1140, 1144, 1238.
- Every behaviour this change adds or alters is proven by a test or journey that drives the
  real plugin and the real CLI rather than by inspection, and no test substitutes a double for
  the layer under test.
- The tests that cover the crates and the documents this change touches are green over the
...
Iterate with this repository's diff-scoped judged lint rather than its whole gate, and run
the crate tests and journeys that cover what you touched rather than everything. The
...
This repository's releases are cut from conventional commits, so each commit's type decides
what release carries it. Two consumers are waiting on the release that carries the first two
tickets — this host's own plan store adopts it — so a commit type that cuts no release would
leave the fix unreleased.

The second ticket's rule reaches a consumer outside this repository, and the paragraph above
about environment-layer semantics is the agreement rather than a suggestion. If you find you
cannot satisfy it as stated, ask over `$ORCHESTRATOR_ASK_MANAGER` rather than choosing
different semantics; a shared interface in this plan is never changed unilaterally, and a
proposal is answered by the manager while you carry on with whatever does not depend on it.
...
{"onepipeline.execution_checkout":"nickderobertis__onetaskgraph","onepipeline.id":"otg-local-md","onepipeline.persona":"engineer","onetaskgraph.item_kind":"task"}
-->
