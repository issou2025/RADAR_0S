...
**feat(cli)!: retire the onemessagebus-agent crate and its agent profile**

*Project description:*
## What

Delete `crates/onemessagebus-agent` from this repository along with everything that
exists only because of it. After this change the bus compiles in no word of the agent
stack. This is the first node of the plan and has nothing ahead of it. Every consumer
(onejudge, onevcs, oneagentgraph, onepipeline, onepipeline-ui) stops linking the crate
and links the release this change produces, each in its own node. The `0.8.0` release
of `onemessagebus-agent` stays on crates.io for anyone still pinned to it, and the
consumers copy the fixtures they now own from the tag `onemessagebus-agent-v0.8.0`. So
deleting the fixtures here loses nothing.

What goes, all of it:

- **The crate.** Everything under `crates/onemessagebus-agent/`: `src/`
  (`lib.rs`, `event.rs`, `note.rs`, `registry.rs`), `tests/` (including `recorded/`
  and `golden/`), `AGENTS.md`, `README.md`, `project.json`, `Cargo.toml`. It also
  leaves the workspace `members`, the workspace dependency table, `Cargo.lock`, and
  every Nx `project.json` or `nx.json` entry that names it.
- **The release target.** The `crate:onemessagebus-agent` / `agent-crate` target in
  `release-targets.toml`, its `[[package]]` in `release-plz.toml`, the step in
  `.github/workflows/release.yml` that publishes it, and every release and
  smoke script that names it (`scripts/smoke-published.sh`, `sdk-install/`). This must
  stay consistent with `npm/test/release-targets.test.mjs` and
  `crates/onemessagebus-repo/tests/release_declaration.rs`, which hold the declaration
  to the release configuration and to onevcs's schema.
- **The CLI's agent profile.** `crates/onemessagebus-cli` stops linking the crate.
  `src/profile.rs` offers `open` alone, and that is the default. `--profile agent` is
  refused by the existing generic refusal (`` `agent` is not a profile this build links;
  choose one of: open ``), and the refusal names no stack crate. The CLI's `registry()`
  becomes the core's own schemas — `onemessagebus::transport::register_protocol`'s and
  the resident protocol's — with no `agent.*` id. `deliver`'s spool pre-check keeps its
  current behaviour for a schema this build does not know. Keep the `--profile` flag on
  `events emit` and `events merge`, so an existing `--profile open` caller keeps
  working.
- **The SDK models for the agent families.** The generated
  `python/onemessagebus-sdk/src/onemessagebus/_generated/messages/agent_*.py` files
  (`agent_event_envelope_v1`, `_v2`, `agent_event_filter_v1`, `agent_labels_v1`,
  `agent_note_v1`, `agent_artifact_ref_v1`) and their exports. The matching
  `npm/onemessagebus-sdk/src/generated/messages/agent-*.ts` files and their entries in
  the generated index and schemas modules. The `vocabulary.json` manifest, which now
  describes `open`. The SDK clients' `events` methods default to `open` as the CLI
  does. Regenerate with the repository's own generators
  (`python/onemessagebus-sdk/scripts/generate.py`,
  `npm/onemessagebus-sdk/scripts/generate.mjs`) rather than by deleting files by hand.
  The parity suites between the Rust registry and the two SDKs (`parity/`,
  `cross-language/`, `crates/onemessagebus-e2e/tests/cross_language.rs`) must agree
  afterwards.
- **The journeys that used the agent words.** The e2e crate's journeys that drove
  `agent.note@1`, the agent envelope or `--profile agent` are rewritten, not dropped:
  `crates/onemessagebus-e2e/tests/e2e/{inbox,events,resident,schema,support}.rs` and
  `tests/cross_language.rs`. They are re-pointed at a message family the bus's own
  tests declare (registered through a `--registry` directory or a linked bundle), so
  each journey still proves the core behaviour it proved, and no product's schema is
  involved. The `events` journey that proved `--profile agent` is the default becomes
  a journey proving `open` is the default and `agent` is refused with the generic
  refusal.
- **The prose.** `docs/contract.md` loses Contract N (the note) and the agent-specific
  parts of Contracts W, F, R and E. The core's generic shape stays, stated over a
  vocabulary rather than over the agent one, and the same goes for `docs/wire.md`,
  `docs/inbox.md`, `README.md`, `AGENTS.md`, `crates/onemessagebus/README.md` and
  `src/lib.rs` docs, `llmlint.yml`, and the Python and npm SDK READMEs. Where a document
  used the agent vocabulary as its worked example, it now uses a neutral one. Every
  test that holds a document to the code (`crates/onemessagebus-cli/tests/docs.rs` and
  its siblings) still holds.

The change request must say plainly that **reading or writing an agent-stack stream
that carries `phase` with the `onemessagebus` CLI ends with this release**. Such
streams are now read through onepipeline's and onevcs's own readers. The core's `Open`
vocabulary still refuses an unknown top-level key, and that does not change. A generic
bus feature that reads a vocabulary as data off a linked bundle is a follow-up the
planner already drafted. It is not part of this node.

### The contracts this plan fixes (every node restates them from here)

These are agreed for the whole run, and the manager ruled on each while the plan was
written. The bus crate `onemessagebus-agent` at tag `onemessagebus-agent-v0.8.0` in
`github.com/nickderobertis/onemessagebus` is the **reference shape** for everything
below: each shape moves out of it **byte for byte on the wire**, and every fixture that
proves this is copied from that tag unchanged.

1. **The bus release comes first (node `omb-retire-agent`).** The onemessagebus
   release that deletes `crates/onemessagebus-agent` is cut before any consumer moves.
   Each consumer then drops `onemessagebus-agent` **and** links that new bus release
   (the core `onemessagebus` crate, at the release that node produces) in the same change, so
   no crate ever resolves two bus cores. The last `onemessagebus-agent` release
   (`0.8.0`) stays on crates.io, untouched. The bus core's public API does not change
   in this plan. After every consumer node lands, each consumer's `Cargo.lock` has no
   `onemessagebus-agent` package and exactly one `onemessagebus` package.
2. **The note contract belongs to onejudge (node `oj-own-note`).** `onejudge::note`
   (`crates/onejudge/src/note.rs`) *declares* every item `onemessagebus_agent::note`
   exports at `0.8.0`, where today it re-exports them: `Note`,
   `Addressee::{Worker,Supervisor,Both}`, `Party::{Worker,Supervisor}`, `Criterion`,
   `CriterionRefused`, `NoteText`, `NoteRefused`, `DeliveredNote`,
   `Accepted::{Queued,Interrupted,JudgedWith}`, `Undelivered`, `Criteria`,
   `Notes` (= `onemessagebus::Sender<Note, Accepted>`), `NoteInbox`
   (= `onemessagebus::Inbox<Note, Accepted>`), `NoteInboxExt`, `prelude`,
   `worker_block`, `supervisor_block`. It keeps the same names, the same serde shapes
   and the same refusal words, and it stays re-exported at the crate root wherever it
   is today. `Note` stays registered as **`agent.note@1`**. onejudge publishes that
   schema in a **new bundle `schemas/note.json` at bundle version `1`**, holding
   `agent.note@1` alone. The type is the one source. A test fails when the committed
   file differs from the bundle generated from the type, the same way
   `schemas/judge-seat-frames.json` is held today. `judge-seat-frames.json` does not
   change: its version is the command-provider protocol version (`8`) the host pins.
3. **Each producer declares its own vocabulary over the core's open `Source`.**
   - **onevcs (node `ov-own-vocabulary`)** declares `onevcs::Phase`: the same four
     variants `Development, Integrate, Review, Release`, `#[serde(rename_all =
     "kebab-case")]`, `every()` and `as_str()`. onevcs owns it. It also declares a
     `Vocabulary` impl whose `Source` is the core's open `onemessagebus::Source`,
     with the word `vcs` exported as `pub const SOURCE_WORD: &str = "vcs"`. That impl's
     dimensions are `{ phase: Option<Phase> }`, omitted when absent. Its typed
     `Labels` holds the six reserved keys `run_id` (text), `round` (integer), `node`,
     `step`, `member`, `persona` (text), in that wire order, with extras flattened
     after them. Its matcher fields are `phase` plus the five text labels, with no
     `round`. Its write version is `1`. The public paths stay the same:
     `onevcs::{Envelope, EventFilter, EventMatcher, Labels, Phase, Source,
     ArtifactRef}` and `onevcs::event::*`, now as onevcs's own types.
   - **oneagentgraph (node `oag-own-vocabulary`)** does the same with
     `pub const SOURCE_WORD: &str = "agentgraph"`. It never links onevcs at runtime,
     so its `phase` dimension and matcher field hold an **open word** (a string
     newtype it does not enumerate). It never stamps one. So every filter it accepts
     today it still accepts, and a matcher that names a phase still never matches one
     of its envelopes. The public paths stay the same:
     `oneagentgraph::event::{Envelope, EventFilter, Labels, MatchFields, Matcher,
     Source}`.
   - **onepipeline (node `op-own-vocabulary`)** is the one crate that names every
     producer. `src/vocabulary.rs` declares the agent-stack vocabulary as its own
     `Vocabulary` impl `Agent` (`NAME = "agent"`, `DEFAULT_SOURCE = "pipeline"`).
     It has the closed `pub enum Source { Agentgraph, Vcs, Pipeline }` under
     `#[serde(rename_all = "lowercase")]`, dimensions `{ phase: Option<Phase> }` with
     `Phase` re-exported from `onevcs::Phase`, and typed `Labels`/`MatchFields` as
     above. Write versions are `pipeline` → 2 and `agentgraph`/`vcs` → 1. The file
     also holds `pub const EVENT_ENVELOPE_FAMILY: &str = "agent.event-envelope";`,
     `pub const EVENT_ENVELOPE_READS: &[u32] = &[2, 1];`, and the `Envelope` /
     `EventFilter` / `Matcher` / `Emitter` / `Reader` / `Merge` aliases over `Agent`.
     `src/event.rs` keeps `pub const ENVELOPE_VERSION: u32 = EVENT_ENVELOPE_READS[0];`
     and keeps exporting `onepipeline::event::{Envelope, Labels, Phase, Source,
     ArtifactRef}`. `onepipeline::filter::{EventFilter, Matcher}` keep their paths.
     The ai-orchestrator drift gate reads these spellings in these files at the
     adopted tag.
   - **The drift check for the restated labels and source words lives in
     onepipeline.** A contract test there fails when a sibling's `Labels` with every
     reserved key set does not cross into onepipeline's `Labels` with no extras, or
     when `onevcs::SOURCE_WORD` / `oneagentgraph::event::SOURCE_WORD` differ from
     `Source::Vcs.as_str()` / `Source::Agentgraph.as_str()`. The recorded streams in 5
     round-trip through onepipeline's reader. onepipeline crosses onevcs envelopes and
     filters **at their JSON form**, as it already does for oneagentgraph's.
4. **Schema ids keep the `agent.` namespace.** `agent.` already names the stack
   rather than any crate: onejudge publishes `agent.onejudge-frame.*@8`,
   oneagentgraph registers `agent.agentgraph.*@1`, and onepipeline publishes
   `agent.planner-surface@1` and `agent.reply-envelope@{2,3}`, none of them from an
   agent crate. onejudge owns `agent.note@1`. onepipeline owns
   `agent.event-envelope@1`, `agent.event-envelope@2`, `agent.event-filter@1`,
   `agent.artifact-ref@1` (the core's `ArtifactRef`, registered under this id by
   onepipeline) and `agent.labels@1`. onepipeline publishes those five in a
   **new bundle `schemas/events.json` at bundle version `1.0.0`**. It is generated
   from the types and held by a test that fails when the committed file differs.
   `schemas/planner-channel.json` does not change. Every registry that lists these
   ids today still lists them with the same documents. Only who registers them
   changes.
5. **The fixtures move; they are not lost.** Each is copied byte for byte from
   `onemessagebus` at tag `onemessagebus-agent-v0.8.0`, path
   `crates/onemessagebus-agent/tests/...`, and carries its provenance section from
   that crate's `README.md` beside it:
   - `golden/onejudge-0.8.1-note.json` → onejudge
     `crates/onejudge/tests/golden/onejudge-0.8.1-note.json`.
   - `recorded/onevcs-session.ndjson` → onevcs `crates/onevcs/tests/recorded/`.
   - `recorded/oneagentgraph-run.ndjson` → oneagentgraph `tests/recorded/`.
   - All four of `recorded/{oneagentgraph-run.ndjson, onevcs-session.ndjson,
     onepipeline-events.jsonl, onepipeline-relayed-drift.jsonl}` → onepipeline
     `tests/recorded/bus/`. onepipeline's existing `tests/golden/envelope-v1.json` and
     `envelope-v2.json` stay where they are.
6. **The bus knows nothing about the agent stack.** After `omb-retire-agent`, no
   source, phase, label, note or schema id of any product is compiled into the bus.
   The CLI's `--profile` offers `open` alone, which is the default. `--profile agent`
   is refused by the existing generic refusal (`` `agent` is not a profile this build
   links; choose one of: open ``), and that refusal names no stack crate. Reading or
   writing an agent-stack stream that carries `phase` with the bus CLI ends with that
   release. Those streams are read through onepipeline's and onevcs's own readers.

**A shared interface is never changed unilaterally while this plan runs.** The items
above bind every node. If you want a departure — a missing field, a different file, a
renamed id, a better decomposition — put it to the manager over
`$ORCHESTRATOR_ASK_MANAGER` and keep building against the agreed surface until the
manager amends it.

**Dependency directions are fixed.** The bus core never depends on anything of the
stack. onevcs and onejudge stay peers, and neither links the other. oneagentgraph
does not link onevcs at runtime. onepipeline is the one crate that names every
producer. If sharing a type would invert one of these, that type has the wrong home.


## Why

onemessagebus is meant to be a generic message bus. The user has been factoring out the
schemas it holds for other libraries under one rule: a protocol one program owns lives
in that program and is published as a schema bundle, never as a module of the bus.
onemessagebus#111 deleted the onejudge codec, and #131 deleted onepipeline's planner
channel. `crates/onemessagebus-agent` is what is left of that pattern. Everything in it
is vocabulary some *other* library owns, and it sits in the bus only because that was a
convenient place to share it. The user's rulings: the note contract "should live in the
library just like we recently factored out onejudge schemas"; on the stack vocabulary,
"onepipeline would be the only place we need this source right so it should be defined
there"; and "make sure it's an explicit goal to delete the agent crate". When nothing is
left in it, the crate goes, along with its release target, its generated SDK models and
its CLI profile. The whole move keeps every byte on the wire the same, because streams
and notes already written must still read.


## Acceptance criteria

- `crates/onemessagebus-agent/` does not exist. The workspace `Cargo.toml` and
  `Cargo.lock` name no `onemessagebus-agent` package, and no crate, test, example or
  script in the repository imports `onemessagebus_agent`.
- `release-targets.toml` declares no `crate:onemessagebus-agent` (`agent-crate`)
  target, `release-plz.toml` declares no package of that name, and no workflow step
  publishes it. `npm/test/release-targets.test.mjs` and
  `crates/onemessagebus-repo/tests/release_declaration.rs` pass over the edited
  declaration.
- The CLI's `--profile` accepts `open` alone, and `open` is the default for
  `events emit` and `events merge`. `--profile agent` exits with the invalid-input
  code and the generic refusal naming `open` as the only choice, and that refusal
  names no stack crate. An e2e journey in `crates/onemessagebus-e2e` drives the real
  binary and proves both.
- The schema registry the built binary reports lists no id in the `agent.`
  namespace. The Python and TypeScript SDKs carry no generated model for an `agent.*`
  family: no `agent_*` module under `_generated/messages/`, no `agent-*` module under
  `src/generated/messages/`, and no `agent.*` entry in either generated index. Their
  vocabulary manifest describes `open`. Regenerating either SDK with its generator
  leaves the tree unchanged.
- The Python SDK's and the TypeScript SDK's events clients, called with no profile,
  use `open`: a journey through each real SDK client, driving the built binary, emits
  an envelope with a source word the old agent profile did not have and reads it back,
  and a call naming `agent` is refused with the generic refusal.
- No source word, phase word, reserved label key, note shape or `agent.*` schema id of
  the agent stack is compiled into any crate, SDK or generated file of this
  repository. A search of `crates/`, `python/`, `npm/`, `scripts/`, `sdk-install/` and
  `docs/` for `onemessagebus-agent`, `onemessagebus_agent`, `agent.event-envelope`,
  `agent.event-filter`, `agent.labels`, `agent.artifact-ref` and `agent.note` matches
  nothing outside `CHANGELOG.md`.
- The inbox, spool, queue, resident, schema and cross-language journeys that used the
  agent note or envelope still prove the core behaviour they proved, now over a
  message family the bus's own tests declare. They pass over the finished tree.
- `docs/contract.md`, `docs/wire.md`, `docs/inbox.md`, `README.md`, `AGENTS.md` and the
  SDK READMEs describe no agent profile. The tests that hold documents to the code
  pass over the finished tree.
- The tests and SDK parity suites that cover the crates, SDKs and scripts this diff
  touches, and the repository's judged lint over this diff, are green on the finished
  tree. The CLI behaviour above is proven end to end by journeys that drive the real
...
- The worker's draft change request is published, and its description states that
  reading or writing an agent-stack stream carrying `phase` with the `onemessagebus`
  CLI ends with this release.

## Additional info

The change request may be published early: open it as a draft once the branch carries
the work, and write the statement about agent-stack streams into its description, so the
drafter finishes from it. The operational notes below say how.

Commit this as a breaking change (`!`) so release-plz cuts a new minor release of the
shared workspace version. Every consumer node in this plan links that release. The core
`onemessagebus` crate's public API does not change here. If deleting the crate turns up
something in the core that only the agent crate used, keep the core as it is and name
it in your report. Removing core API is a different change.

Nothing in this repository links any consumer. The manager reads the `onemessagebus`
CLI's commands on the host that runs this plan; nothing there names `--profile` or runs
`events emit` or `events merge`.

Iterate with the repository's own affected tier (`just check-affected`) and the
diff-scoped judged lint. The required checks run downstream on the change this branch
becomes.
...
{"onepipeline.execution_checkout":"nickderobertis__onemessagebus","onepipeline.id":"omb-retire-agent","onepipeline.max_turns":40,"onepipeline.persona":"engineer","onetaskgraph.item_kind":"task","orchestrator.plan-review":{"by":"review-plan","key":"af5b58fd5386ae8e32a9c812a4962b69fe15819250e8f02ad3bd8baf8dfdd101","reviewed_at":"2026-09-22T07:26:41.734377+00:00"}}
-->
