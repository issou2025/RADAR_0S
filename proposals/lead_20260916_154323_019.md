...
**feat(dispatch): refresh a node's environment from a hook before launch**

*Project description:*
## What

Add a generic **pre-dispatch environment-provider hook** to onepipeline: a command a launch names,
run immediately before each node launch, returning environment additions for that child, after
which the engine validates the current harness configs' `env_from` sources against the refreshed
environment and refuses the launch with a specific diagnostic when the hook or a key fails.

This is the upstream half of ai-orchestrator ticket
https://github.com/nickderobertis/ai-orchestrator/issues/1109 — *live harness config can outpace a
driver's environment*. A live driver re-reads harness configuration (`oneharness.*.toml`) from the
checkout at dispatch time but keeps only the environment captured when the driver started. When a
merged routing change added `ORCHESTRATOR_CLAUDE_PRIMARY_BACKUP_CONFIG_DIR` to an `env_from` entry
while a run was live, the next dispatch failed provider startup: the driver had never inherited the
new variable, and oneharness correctly refused an absent `env_from` source. The user asked *"Can we
just reload the env with each dispatch?"* and *"Should this live upstream in onepipeline? Or have
some kind of hooks to support it?"*, and accepted this design: the generic mechanism upstream here,
the repository-specific composition (which of ai-orchestrator's shell helpers produce the
variables) in ai-orchestrator. **onepipeline must not source or know any ai-orchestrator helper or
`ORCHESTRATOR_*` variable.**

**The contract (fixed by the manager; state it in `docs/contract.md` beside the run-end hooks).**

- **Naming it.** `onepipeline start --dispatch-env-hook COMMAND` and
  `--dispatch-env-hook-timeout SECONDS`; the launch config takes `dispatch_env_hook` and
  `dispatch_env_hook_timeout`, keys of the launch-config `schema_version` one above the newest the
  base declares, refused by name in a document declaring an earlier one; a flag beats the config.
  The timeout is a positive whole number of seconds, `60` when unnamed, zero refused wherever it is
  read. Both are retained in the launch record (omitted when absent, so an older record reads as
  naming no hook) and replayed by `adopt`, which takes neither flag.
- **When it runs.** Immediately before **every node-scope dispatch attempt** — a first dispatch, a
  re-asked dispatch, a retry, a requeue, and a lifecycle node's worker launch alike — and **never**
  for the dag-scope observer launch.
- **How it is spawned.** The way a run-end hook is: the command itself, no shell, no arguments,
  with the launch record's `dir` as its working directory, the driver's environment plus
  `ONEPIPELINE_HOOK=dispatch-env`, `ONEPIPELINE_RUN_ID`, `ONEPIPELINE_RUN_ROOT` (absolute) and
  `ONEPIPELINE_NODE_ID`. It is awaited for up to the timeout; one still running then has its process
  tree ended.
- **What it prints.** Exactly one JSON document on stdout:
  `{"version": 1, "env": {"NAME": "value", ...}}`. Every value is a string and every name a valid
  environment variable name. A document with any other top-level member, a non-string value, an
  invalid name, or anything that is not that one document is **malformed**. Its stderr is kept in
  the run's `hooks/dispatch-env.log`.
- **What it changes.** The `env` members are overlaid on the driver's environment **for that one
  child launch only**; the driver's own environment is unchanged.
- **Validation.** After overlaying, and before launching, the engine reads every oneharness config
  that launch will read — each member's `oneharness_config` after `--node-set` overrides, as the file
  is on disk at that moment — and checks that every `env_from` source variable those configs name
  is present in the refreshed child environment.
- **Refusal.** A hook that exits non-zero, cannot start, times out or prints a malformed document,
  or a missing `env_from` source, **refuses the launch**: nothing is dispatched, and the node settles
  under the existing outcome word `infrastructure-failure`, with its existing retry semantics. No
  new outcome word. The refusal's detail names **either** the hook ending (the exit code, `timeout`,
  `could-not-start` or `malformed` with what was malformed) **or** the config file, the harness
  variant and the missing key by name.
- **Secrets.** A hook's `env` **values** are never written anywhere the run keeps — journal, events,
  logs, `result.json`, launch record, views or surfaces. Anything recorded about a hook's output
  names variables only.
- **Absent.** A launch naming no hook behaves exactly as today: no spawn, no added environment, no
  new validation.

Beyond the contract: `onepipeline start --help`, the launch-config reference, and the contract
document's machine-readable block (the one the run-end hooks already have) describe the hook; the
tests that hold that block to the implementation cover the new keys.
...
A live run on this host is supervised for hours while this repository's harness routing keeps
merging. Today a routing change that introduces a new environment indirection makes the next
dispatch of every live run fail before its first turn, and the only remedy is re-adopting the run
under a freshly launched driver. The user wants every dispatch to pick up the current
environment, and wants the mechanism generic in onepipeline so other callers get the same seam
without onepipeline learning this host's variables.

## Acceptance criteria

- `onepipeline start` accepts `--dispatch-env-hook COMMAND` and `--dispatch-env-hook-timeout
  SECONDS` (default `60`, zero refused), the launch config accepts `dispatch_env_hook` and
  `dispatch_env_hook_timeout` at a new schema version (refused by name under an earlier one), a flag
  beats the config, both are retained in the launch record, and `adopt` replays them.
- Before every node-scope dispatch attempt, and never for the dag-scope observer, the named hook
  is spawned with no shell and no arguments, in the launch record's `dir`, with
  `ONEPIPELINE_HOOK=dispatch-env`, `ONEPIPELINE_RUN_ID`, `ONEPIPELINE_RUN_ROOT` and
  `ONEPIPELINE_NODE_ID` set, and the `env` of its `{"version": 1, "env": {...}}` document reaches
  that child launch's environment and no other process's.
- A journey launches a node whose oneharness config names an `env_from` variable absent from the
  driver's environment and supplied only by the hook, and the dispatched child is handed that
  variable; a second dispatch after the hook's output changes is handed the new value.
- A hook exiting non-zero, timing out, or printing a malformed document (another top-level
  member, a non-string value) refuses the launch and settles the node `infrastructure-failure` with
  a detail naming that ending; a hook whose output leaves an `env_from` source missing refuses the
  launch and the detail names the config file, the variant and the missing key — each proven by a
  test.
- No value a hook prints appears in the run's journal, events, logs, `result.json`, launch record
  or views, proven by a test whose hook prints a sentinel value and which searches the run's
  directory for it.
- A launch naming no hook dispatches exactly as before: no hook spawn and no new validation,
  proven by the existing dispatch journeys staying green unchanged.
- `docs/contract.md` states the contract above, and the check that holds its machine-readable
  block to the implementation covers the new flags, keys, environment names and document.
...
- The checks that exercise this change — the test modules that exercise the code this change touches, and the repository's judged lint over this change's diff where the repository runs one — are green over the finished tree.
...
**Who consumes this.** `aio-adopt-engine` (ai-orchestrator) adopts the release this becomes,
writes a hook that prints this exact document, passes `--dispatch-env-hook` on every `start`, and
drives a real launch against it. So the document shape, the flag names, the environment names and
the `infrastructure-failure` settlement are a contract: changing any of them is a proposal to your
manager, never a unilateral edit.

**Stacked on `op-325`.** That node changes how the e2e `World` builds a store command's
environment in `tests/e2e/harness.rs`; your journeys likely use the same harness, so you start
from its branch.
...
{"onepipeline.id":"op-dispatch-env-hook","onepipeline.max_turns":200,"onepipeline.persona":"engineer","onetaskgraph.item_kind":"task","orchestrator.plan-review":{"by":"planning-closeout","key":"708c87f6e69a15463463b1c354040826369c6513ab80c2ddc8418d606adec1ad","reviewed_at":"2026-09-16T15:22:01.084584+00:00"}}
-->
