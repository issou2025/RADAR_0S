...
**fix(deps): relink oneharness-core for the Claude login-refusal classifier**

*Project description:*
## Resuming a preserved branch

Three earlier dispatches of this node worked this branch; you continue it in your own
worktree rather than starting over, and the branch already carries the whole change. In
order: `431ae25` moves the workspace's `oneharness-core` requirement and the lockfile's
resolution onto the release named in this task's references (one copy across the workspace,
and the engine-currency check green over it); two merges bring the base up to `727a2ba`;
and `96cc24a` adds the journey in the dispatch e2e module that drives a Claude
`Not logged in` terminal record through the real graph against the linked core and asserts
the identity is stepped past as unauthenticated — observed failing against the previous core
(`rate-limit` where `auth` was expected) and passing against the linked one. The judged lint
over the diff was green on that tree (12 passed, 0 failed, 1 ignored: a site-scoped ignore
above the login-refusal record in the fake Claude binary, with its reason). The change
request that carries the branch is https://github.com/nickderobertis/onepipeline/pull/314,
whose `cross (windows-latest)` leg fails on `views::host_never_renders_a_dispatch_of_a_run_that_was_stopped`
exactly as the base itself does at `727a2ba`; that failure is main's, is recorded in a
follow-up draft, and is not yours to fix — the manager re-runs that leg on the host.

The third dispatch was refused by its judge on one point only: its final run of the
oneharness-facing test selection reported 239 of 241 passing, taken with the host's load
average near 25, and the two that failed —
`dispatch::a_dispatch_settles_on_its_terminal_event_while_the_graphs_final_reaper_runs` and
`views::an_observer_this_host_cannot_ask_about_is_never_reported_dead` — are timing
journeys that touch nothing this branch changes and each passed when run again (the second
three times of three). A single run reporting every one of them green is what the branch
still owes, and it is the only thing it owes. Nothing about the manifest, the lockfile, the
journey or the fake record is to be reworked; if reading the tree shows one of those claims
untrue, repair that and say so.

## What

Move this workspace's `oneharness-core` dependency onto the newly published engine release
that carries the Claude login-refusal auth classifier
(https://github.com/nickderobertis/oneharness/issues/1290), and make the lockfile resolve
to it, so every consumer of this engine — including the harness that dispatches this plan —
reads a Claude `Not logged in` result as an authentication failure rather than as a rate
limit. The branch you resume already does this; your dispatch verifies it and takes it to
publication.

**The fix is the resolution, not the requirement.** A widened `Cargo.toml` requirement with
the lockfile untouched changes nothing about what this build links, and has been mistaken
for the fix in this stack before.

**One copy, still.** `oneagentgraph` and `onejudge` each require `oneharness-core` at a
minor version, and this workspace deliberately holds one copy so the types crossing those
seams are one type.

Nothing else is in scope: the only behaviour that changes is the one the linked core
brings, and no test of this repository is rewritten to suit the new engine.

## Why

The user accepted the oneharness ticket so that **this host** stops misreading an
unauthenticated Claude identity as rate-limited — an operator sent to wait out a limit that
does not exist, or to look for quota, when the remedy is to authenticate a named
configuration directory. A dispatch reaches `oneharness-core` only through the engine
release that links it, so the classifier fixed in its own repository is not in force
anywhere until this relink happens and this host's engine pin moves.

## Acceptance criteria

- The workspace's `oneharness-core` requirement names the engine release this task's
  cross-repository references name for the oneharness dependency.
- The lockfile resolves `oneharness-core` to that release, so what this build links has
  moved rather than only what it permits.
- The resolved dependency graph holds exactly one copy of `oneharness-core`, and the
  requirements `oneagentgraph` and `onejudge` place on it are satisfied by that one copy.
- The check that reconciles this build's linked engine versions is green over the finished
  tree.
- A journey in this repository's own end-to-end suite drives a Claude login refusal through
  a real dispatch against the linked core and holds that the identity is read as
  unauthenticated rather than rate-limited; the report cites the run in which that journey
  fails against the previous core and the run in which it passes against the linked one,
  saying which is which.
- Every oneharness-facing test of this repository has a passing result over the finished
  tree, and the report records, for each run it cites, the run's result beside the host's
  load at the time and the number of tests that run kept in flight at once.
- Beyond what the linked core itself changes — a Claude login refusal now read as
  unauthenticated where the previous core read it as rate-limited — no behaviour of this
  repository's own code is changed by this node, and any existing test the relink makes
  untrue is repaired here rather than left failing or skipped.
- The finished change records that it carries
  https://github.com/nickderobertis/oneharness/issues/1290, so the ticket is traceable to
  the change that puts it in force.
- The judged lint over this change's diff is green over the finished tree, and every
  suppression the diff writes and every suppression already standing over a line the diff
  changes is accounted for in the report with its site and reason.
- Every claim this dispatch makes about the finished work is true of the tree as it finally
  stands.

## Additional info

Iterate with `just lint-llm-diff <base>` and the oneharness-facing test selection rather
than this repository's whole gate. The repository's own required checks run downstream on
the change this branch becomes; nothing here asks you to have run them first, and the
Windows failure that leg shows today is the base's own, handled by the manager.

This repository's releases are cut from conventional commits, and this host's engine pin
adopts the release that carries this relink, so a commit type that cuts no release would
leave the whole chain waiting indefinitely.
...
The timing journeys in the oneharness-facing selection hold their bounds only when the host
has the capacity to run them, and this host is running several dispatches at once. Take that
selection with fewer tests in flight than the runner's default, or once the load average has
fallen, and record both beside the result; a failure of one of those journeys under load is
evidence about the host, not about this branch, and is answered by a run that passes, not by
an explanation.
...
{"onepipeline.adoption":"published","onepipeline.branch":"onevcs/s-1e2be82c486f","onepipeline.consumes":{"oh-auth-classifier-r2":"core"},"onepipeline.execution_checkout":"nickderobertis__onepipeline","onepipeline.id":"op-relink-oneharness-r2","onepipeline.max_turns":6,"onepipeline.persona":"engineer","onepipeline.resume":{"branch":"onevcs/s-1e2be82c486f"},"onetaskgraph.item_kind":"task"}
-->
