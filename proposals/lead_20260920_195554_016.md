...
**feat(views): a Stop-hook mode for unwatched, free space on host and status, and an ask verb**

*Project description:*
## What

Three supervisory surfaces the consumer host (`ai-orchestrator`) has been composing
around this engine in scripts of its own. Each becomes the engine's, as a general
feature, on one branch. Read the consumer's current implementation of each before
designing yours — the file named under each heading is the spec of the behaviour and
the record of what it cost to get wrong — then design it as this crate would, not as a
transplant.

### 1. `onepipeline unwatched --hook claude-code-stop`

`onepipeline unwatched` answers `6` for a run its session owns that nothing is watching
(`src/unwatched.rs`, divergence entry 68). The consumer turns that into a claude-code
`Stop` hook with a 427-line Python adapter
(`ai-orchestrator/scripts/stop-unwatched-guard.py`, whose module docstring is the
specification of every ending below). The engine already models the launcher
(`ONEPIPELINE_LAUNCHER=claude-code`, the launching session on
`ONEPIPELINE_LAUNCHER_SESSION`), so the hook is the engine's to ship. The contract:

- **Input.** With `--hook claude-code-stop`, the verb reads claude-code's Stop payload
  as one JSON object on stdin and asks about the session its `session_id` field names —
  **never** the session the environment carries, because a dispatched worker inherits
  its manager's `ONEPIPELINE_LAUNCHER_SESSION` and the worker's own session owns no run,
  which is what keeps the hook silent inside a dispatch. `--session` and `--hook` are
  mutually exclusive. The `stop_hook_active` field (true when this stop follows a block
  of this hook's own) is the continuation signal below.
- **Output, always on stdout, always exit 0.** Exactly one of: a decision object
  `{"decision": "block", "reason": "<the verb's own report lines>"}` when a run the
  session owns is *proven* unwatched and this is not a repeat of a block already made
  on the identical report; a warning object `{"systemMessage": "<one sentence naming
  what could not be answered and the command to ask it by hand>"}` for every ending
  that is not evidence — the runs root unreadable, the question refused, a memory it
  cannot keep, an engine error — which ends the turn and blocks nothing; or nothing at
  all for nothing to report, a payload it cannot read, or a blank/absent `session_id`.
  Never a diagnostic on stdout beside the object (the harness reads stdout as the
  decision), and nothing on stderr that the ordinary `unwatched` would not write.
- **Block once per condition.** A block is recorded before it is made, keyed by a digest
  of the report, in one file per session under
  `${XDG_STATE_HOME:-$HOME/.local/state}/onepipeline/stop-hook/<sha256(session_id)>`
  (a relative `XDG_STATE_HOME` is ignored, as the specification says). On a
  continuation (`stop_hook_active: true`) whose remembered digest equals the current
  report's, the hook emits nothing — the manager was told and did nothing, and a
  second identical block would hold the session open forever. A memory that cannot be
  read or written is a `systemMessage`, never a block. A session with nothing to report
  has its memory removed.
- **Cost.** Bounded like the verb: no run's merged event store is read.

### 2. Free space on `host` and `status`

`onepipeline host` and `onepipeline status <run>` report what is running and nothing
about what it is running *in*; the consumer pipes both through
`ai-orchestrator/scripts/supervision-readings.py` to add it, and its docstring records
the incident (a host at 197G/197G reported as two unrelated test failures). Add to
both views, **above** the `providers:` block that `oneagentgraph health` contributes —
because the consumer's own watch guidance cuts the report at that line and reads only
what is above it — one line per **distinct filesystem** among the runs root and the
linked onevcs's workspaces root (`$ONEVCS_HOME/workspaces`, `~/.onevcs/workspaces` by
default), deduplicated by device so one disk holding both is one line naming both:
...
A root that does not exist yet is measured at its nearest existing ancestor (a fresh
checkout is not a host whose free space is unknown), and a filesystem that cannot be
read says so on its line rather than being left out. Read it; never act on it. Extend
`docs/contract.md`'s description of the two views' content accordingly, through the
register (below).

### 3. `onepipeline ask`

A dispatched agent asks its manager a blocking question over the run's planner
channel. Today that is `ai-orchestrator/scripts/ask-manager.sh`, 190 lines that
JSON-encode a plain-text question into a `planner-question` frame and exec
`onemessagebus ask` with the run's channel, asker and bus policy — every one of which
the engine already knows. Add `onepipeline ask`:

- **Input**: the question as `onepipeline ask <TEXT…>` (several words joined by one
  space), `onepipeline ask --file <PATH>`, or on stdin when neither is given; a blank
  question, or one carrying a NUL byte, is refused at exit 2 naming the form it came in.
- **Environment**: `ONEPIPELINE_RUN_ID` (required; refused at exit 2 when absent, naming
  it), `ONEPIPELINE_RUNS_DIR` (default `runs`), `ONEPIPELINE_CHANNEL_ASKER` (who asks;
  optional; blank refused). Flags: `--about <NODE>` (the node the question is about, at
  most 512 bytes, no control characters) and `--timeout <SECONDS>` (the reply window;
  default the `reply_window_seconds` the run's launch record's `bus_config` names, or
  the bus's own default when it names none).
- **Behaviour**: raise one frame of kind `planner-question`, source `proposal`, message
  the text, on the run's `surfaces` queue at `<runs>/<run>/channel`, **under the bus
  policy the run's launch record carries** (`bus_config` in `launch.json`) — the same
  policy the manager's reply verbs read — blocking, through the linked onemessagebus
  (the library, not a spawned CLI).
- **Output**: stdout is the bus's one-line JSON answer verbatim —
  `{"answer":"reply",…}` at exit 0; `timeout`, `abandoned` or `refused` at exit 1 —
  and nothing else on stdout. An elapsed wait is never a ruling.

### 4. Where each is recorded and proven

This repository's approved contract is not edited to match the code; each surface goes
into `docs/contract-divergences.md` as an entry in the register's own shape (a
proposal for the Views line or the dispatch surface, what this build does, and why),
the way entries 58 and 68 recorded `watch` and `unwatched`. Where the register's
convention puts a type into `tests/contract.rs`, do that. Each behaviour is proven by a
journey driving the compiled binary: the hook over real Stop payloads against a runs
root with an unwatched run, a watched one, a continuation, and an unreadable memory;
the free-space lines over a runs root and a workspaces root on one device and on a
root that does not exist yet; `ask` over a real channel with a replying listener, a
timeout, and each refusal.

## Why

The user's direction for ai-orchestrator: *"Basically everything else in the repo
[outside planning and follow-ups] should be very light or no wrapper on CLI tools and
the repo should be primarily config."* These three are 1,273 lines of that repository
standing in for engine verbs — a hook adapter that is the only thing enforcing its
watch invariant, a view filter that is the only thing saying whether the disk is full,
and a shim that is the one way a dispatch asks its manager anything. Each is a fact
about a run or about the engine's own channel, which is what makes it the engine's:
*"a worker or manager who hits a gap in a library surfaces it ... and never papers over
it with ai-orchestrator code."*

## Acceptance criteria

- `onepipeline unwatched --hook claude-code-stop`, fed a Stop payload naming a session
  that owns a run nothing is watching, writes exactly one `{"decision":"block",…}` object
  whose `reason` is the verb's own report lines, exits 0, and records the block; fed
  the same payload with `stop_hook_active: true` and the condition unchanged, writes
  nothing and exits 0; fed a payload naming a session with nothing to report, writes
  nothing, exits 0 and leaves no memory; fed an unreadable payload or a blank
  `session_id`, writes nothing and exits 0; and with the runs root unreadable, or the
  memory unwritable, writes exactly one `{"systemMessage":…}` object and exits 0. In
  every case the session asked about is the payload's, with `ONEPIPELINE_LAUNCHER_SESSION`
  set to a different session in the environment.
- `onepipeline host` and `onepipeline status <run>` each print, above the `providers:`
  block, one `free space:` line per distinct filesystem among the runs root and the
  linked onevcs's workspaces root, naming both roots on one line when they share a
  device, measuring a not-yet-existing root at its nearest existing ancestor, and
  saying so on the line for a filesystem that cannot be read; the rest of each view is
  unchanged.
- `onepipeline ask` accepts the three input forms, raises one `planner-question` frame
  on the run's `surfaces` queue under the launch record's bus policy, blocks for the
  reply window, and prints the bus's one-line answer, unaltered, as its only stdout with exit 0
  only for `answer: reply` and exit 1 for `timeout`, `abandoned` and `refused`; a
  missing `ONEPIPELINE_RUN_ID`, a blank question and a NUL-carrying question are each
  refused at exit 2 naming the cause.
- Each of the three is recorded in `docs/contract-divergences.md` in the register's
  entry shape, and each is proven by a journey driving the compiled binary rather than
  an in-process call, with no double for the layer under test.
- The tests covering the modules this change touches (`unwatched`, `views`, `cli`, the
  channel) are green over the finished tree, and so is the judged lint over this
  change's diff.
...
The consumer's three implementations are the specifications, and their docstrings
carry the reasoning; read them from the registered checkout at
`~/ai-orchestrator/scripts/stop-unwatched-guard.py`,
`~/ai-orchestrator/scripts/supervision-readings.py` and
`~/ai-orchestrator/scripts/ask-manager.sh` (read-only — that checkout is another
manager's publication checkout; write nothing there). The consumer will call the hook
as `"$CLAUDE_PROJECT_DIR/.venv/bin/onepipeline unwatched --hook claude-code-stop"` from
its tracked `.claude/settings.json`, and `ask` from a two-line adapter behind
`$ORCHESTRATOR_ASK_MANAGER`; the field names, flags and exit statuses above are what it
reads, so a departure you want is a proposal over `$ORCHESTRATOR_ASK_MANAGER`, not an
edit.

A sibling node of this plan (`op-publication-verbs`) adds two more verbs on top of this
branch; keep your CLI additions in the shape `src/cli.rs` already uses so it stacks
cleanly. Iterate with the affected test modules and this repository's lint over your
diff; `just check` and the full gate run on the merge path after you settle.
...
{"onepipeline.execution_checkout":"nickderobertis__onepipeline","onepipeline.id":"op-supervision-verbs","onepipeline.persona":"engineer","onetaskgraph.item_kind":"task","orchestrator.plan-review":{"by":"planning-closeout","key":"135ed0632d287111089c80d42e4df694c4d64144de0f5281381829f7c3817e04","reviewed_at":"2026-09-20T19:30:11.763339+00:00"}}
-->
