...
**docs(divergences): say a board projection needs an unknown status mapping**

*Project description:*
## What

**Read against** `nickderobertis/onepipeline` `origin/main` at `6b8f49d` — the v0.29.1 release commit, one commit past the `8242960` issue #1004 read, which changed only the version in `Cargo.toml`, so every onepipeline line the issue cites is unchanged — and `nickderobertis/onetaskgraph` at `0c2e2a6`. Every path and line this task cites was checked against that tree; if the tree has moved when you start, re-read what moved rather than trusting a cited line number.

Root cause 8 of `nickderobertis/ai-orchestrator` issue #1004, onepipeline half: the engine's
own record of its write-back words reasons from the store that accepts them and not the store
that refuses them. Read the issue's entry for the incident.

Divergence entry 50 of `docs/contract-divergences.md` ("A settled node reaches its board under a
word for something else", starting near line 2457; the passage near lines 2483-2497) reasons
that a store reading an outsider word as `unknown` "keeps the name … so the copy is accepted".
That is true of the `local-md` source and false of `github-projects`, which ships `unknown`
disabled, writes a board option rather than a word, and refuses the copy until
`status_mapping.unknown` names an option. Correct the entry in place to say so, and to say that
a board projection needs that mapping configured. Add no new numbered entry: sibling nodes of
this plan edit that file in parallel.

**The contract this entry describes, which this node does not change** (declared once in
`src/writeback.rs` `ProjectedStatus`; restated by `onetaskgraph-unknown-status` and
`aio-board-unknown-status`): the engine projects eight words — `todo`, `in progress`, `done`,
`cancelled`, which are onetaskgraph category words, and `failed`, `provider-failed`, `parked`,
`skipped`, which are not and which onetaskgraph classifies `unknown`. The `github-projects`
source keeps `unknown` disabled by default, and the remedy is mapping it to a board Status option;
a closed state is not a remedy, because it reads back as `done` or `cancelled` and a copy would
never settle (`onetaskgraph-unknown-status` stops offering one).

## Why

The operator is closing every root cause of issue #1004 so that none of them has to be
repaired by hand again. In the run that closed #855 every settlement write-back to the `plans`
board was refused from the first failed node onward, while onepipeline's own divergence entry
said that could not happen. The operator wants the record to be true, so the next reader of that
entry configures the board instead of trusting it.

## Acceptance criteria

- Divergence entry 50 states that the `github-projects` source ships `unknown` disabled, writes a
  board option rather than a word, and refuses the copy until `status_mapping.unknown` names a
  board Status option, and that a board projection therefore needs that mapping; it keeps the
  `local-md` behaviour, attributed to `local-md`. It is complete when each of those three
  `github-projects` statements, the needed mapping and the `local-md` attribution is present as a
  statement rather than a placeholder, and each `github-projects` statement names by file the
  onetaskgraph code it describes (`crates/onetaskgraph-github-projects/src/lib.rs`). Those
  assertions describe another repository's behaviour: the check they are compared against is
  onetaskgraph's own test of the `unknown` refusal — the disabled-status refusal tests in
  `crates/onetaskgraph-github-projects/tests/plugin.rs` and the end-to-end refusal test
  `onetaskgraph-unknown-status` adds under `crates/onetaskgraph/tests/e2e/` — and that
  comparison's later result is not this dispatch's bar.
- The entry recommends no closed state for `unknown`.
- `src/writeback.rs` is unchanged, so `ProjectedStatus` still declares the same eight words.
- Any test in the repository that reads `docs/contract-divergences.md` passes over the finished
...
- Read first: `AGENTS.md` (the contract rules: `docs/contract.md` is approved text, and every
  departure is a divergence entry), `docs/contract-divergences.md` entry 50, `src/writeback.rs`
  `ProjectedStatus`, and in the onetaskgraph checkout
  (`/home/nick/.ai-orchestrator/repos/nickderobertis__onetaskgraph`)
  `crates/onetaskgraph-github-projects/src/lib.rs` around `shipped_default`,
  `StatusMapping::resolve`, and the refusal near line 2783.
- The words are held by `tests/e2e/store.rs:2942`; this node leaves that test untouched. The
  judged tier: `just lint-llm-diff <base>`. The merge path runs the full gate.
- Siblings in this repository run in parallel and edit other entries or files:
  `onepipeline-monitor-cursor` (entry 58), `onepipeline-retire-revision-pin` (entry 44),
  `onepipeline-teardown-diagnosis` and `onepipeline-link-fixes`.
- The eight words are consumed by other nodes. If you find the contract above wrong, surface a
  proposal to the manager over `$ORCHESTRATOR_ASK_MANAGER` and keep building against it.
- No node of this plan waits on a release carrying this change: a documentation correction is in
  force by merging.
...
{"onepipeline.id":"onepipeline-writeback-divergence","onepipeline.max_turns":50,"onepipeline.persona":"engineer","onetaskgraph.item_kind":"task","orchestrator.plan-review":{"by":"review-plan","key":"ae9cd95cdedbbbd8bc4ee05be85da198b983e1a2c2ef6615d2c43eab2b796fff","reviewed_at":"2026-09-13T17:31:16.589610+00:00"}}
-->
