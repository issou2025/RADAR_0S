...
- A listing performed during repeated in-place replacements of a record succeeds, its
  ...
  node's bar.
- A query scoped to one project succeeds when a file under another project's directory
  vanishes during the walk or fails to parse, and it returns exactly that project's
  records; a record inside the scoped project that fails to parse still fails the query as
  malformed, naming that file.
...
{"onepipeline.execution_checkout":"nickderobertis__onetaskgraph","onepipeline.id":"otg-local-md","onepipeline.persona":"engineer","onetaskgraph.delivers":["followups:I_kwDOUBBgxs8AAAABRy2dWQ","followups:I_kwDOUBBgxs8AAAABRy2ijQ","followups:I_kwDOUBBgxs8AAAABR7a_rA"],"onetaskgraph.item_kind":"task"}
-->
