...
**feat(schema): link schema bundles by pinned URL through an on-disk cache, with schemas verbs**

*Project description:*
## What

Teach `onemessagebus` to **link a schema bundle by URL**, pinned to a version, through an on-disk cache — the
way `llmlint` links a plugin — so a bus configuration can name the frame grammar another program publishes
without the bus transcribing it or linking that program. This node is additive: no existing verb, key or
behaviour changes, and the `onejudge` codec is left exactly as it is (the node after this one retires it).

Read first: `AGENTS.md`, `docs/contract.md`, `docs/cli.md`, `docs/sdk-parity.md`,
`crates/onemessagebus/src/config.rs`, `schema.rs`, `sdk_schema.rs` (`RegistryDocument`), `capability.rs`
(`CAPABILITIES`), `crates/onemessagebus-cli/src/registry_dir.rs` and `cli.rs`, and the Python and TypeScript
SDK clients. For the behaviour being copied, read llmlint's README section "Plugins: shared rule sets" and its
`src/io/plugins.rs` (checkout `github.com/nickderobertis/llmlint`); where this task and llmlint differ, this
task wins.

### Contract L — schema links (this node fixes it; every later node restates it from here)

**The bundle document.** A linked URL serves one JSON object:

```json
{"version": "8", "description": "optional text", "schemas": [{"id": "agent.example-frame.hello@8", "schema": {"type": "object"}}]}
```

- `version` (required): a string of one to three dot-separated non-negative integers (`^\d+(\.\d+){0,2}$`) —
  the version the document **declares**, which a pin is asserted against and a cache entry is keyed by.
- `schemas` (required, non-empty): each entry is exactly the registry document shape `RegistryDocument` already
  is — `{"id": <SchemaId>, "schema": <JSON Schema object>}`. Ids within one bundle are unique.
- `description` (optional): text. Any other top-level key is refused naming it.
- The type is public in the core crate as `onemessagebus::SchemaBundle` (serde + `JsonSchema`), so a publisher
  in another repository generates its bundle through this one declaration rather than restating the shape.

**The link.** A link is a string `<location>` or `<location>@<pin>`. The pin is the text after the last `@`
that follows the location's last `/`, when that text matches `^\d+(\.\d+){0,2}$`; otherwise the link is
unpinned and the `@` is part of the location. Locations:

- `https://…` — any host.
- `http://…` — only when the host is `localhost`, `127.0.0.1` or `[::1]`; any other `http://` host is refused
  naming the link. This exists so suites can prove revalidation against a loopback server with no network.
- `file:///absolute/path`, or a bare path (absolute, or relative to the directory of the configuration file
  that names it) — read on every resolution and never cached; the pin is still asserted.

**The pin** is llmlint's prefix range over the declared version, with missing declared components read as 0:
`@8` accepts any declared version whose first component is 8 (`8`, `8.1`, `8.1.2`); `@8.1` accepts first two
components `8.1`; `@8.1.2` accepts exactly `8.1.2`. A resolved document whose declared version does not satisfy
the pin is a **hard error** naming the link, the pin and the declared version — never a silent fallback.

**The cache** lives at `$ONEMESSAGEBUS_SCHEMA_CACHE_DIR`, else `$XDG_CACHE_HOME/onemessagebus/schemas`, else
`$HOME/.cache/onemessagebus/schemas`. An entry is keyed by (location without the pin, declared version) and
records the body, the validators the origin returned (`ETag`, `Last-Modified`), and when it was last confirmed.
Resolution of a **pinned remote** link:

1. With no cached entry satisfying the pin: fetch; store under the version the body declares; use it. A fetch
   that fails refuses, naming the link and why.
2. With a satisfying entry confirmed within the freshness window (`ONEMESSAGEBUS_SCHEMA_TTL` seconds, default
   `3600`; `0` revalidates on every resolution): use the newest satisfying entry with no request.
3. With a satisfying entry older than the window: a conditional request. `304` re-confirms it; a `200` whose
   body satisfies the pin is stored as its own entry and used; a `200` whose body does not satisfy the pin is
   the hard error above. A revalidation that cannot be made — offline, a transport failure, a non-success
   status — reuses the cached entry and says so on stderr: the cache is a speed-up, never a network dependency.
4. `ONEMESSAGEBUS_SCHEMA_REFRESH=1` forces an unconditional fetch that replaces what the cache holds.

An **unpinned remote** link is fetched on every resolution and never cached. HTTP is a pure-Rust client (no
system OpenSSL, no spawned `curl`) honouring `HTTP_PROXY` / `HTTPS_PROXY` / `NO_PROXY`, with bounded connect and
read timeouts the documentation states.

**Where links are named.** The configuration file gains one optional top-level key:
...
`Config::load` parses each link and refuses a malformed one, but **resolves nothing**: resolution is an explicit
library call, so a library consumer that loads a configuration for its queues (the engine does) never touches the
network or the cache for links it does not use. Every command-line verb that loads a configuration makes that call
and resolves each link before it does anything else and registers every entry
of every resolved bundle into the registry it uses, beside the profile's schemas and any `--registry` directory;
an entry whose id is already registered with a different document is refused as the registry refuses it today.
`serve` resolves before it reads its first frame, and for `serve` alone a satisfying cached entry is used
**without revalidation** whatever its age — a serving session never waits on the network at start when the
cache holds what it needs — and only a link with nothing cached is fetched. `--registry <dir>` keeps working
unchanged for local documents.

**The verbs**, each a capability in `CAPABILITIES`:

- `schemas [--format json|text]` — list the cache: its directory, then one entry per cached (location, declared
  version) with its confirmation time (RFC 3339, UTC). JSON: `{"cache": <dir>, "entries": [{"url", "version",
  "confirmed_at"}]}`. An empty or absent cache is an empty list, exit 0.
- `schemas clear [--format json|text]` — remove every entry and report how many; exit 0, also when there were none.
- `schemas fetch [<link>...] [--config PATH] [--format json|text]` — resolve each named link, or every link the
  configuration names when none is named, **revalidating regardless of the freshness window** (conditional
  where an entry exists), and report per link the declared version it resolved to and whether it was fetched,
  confirmed, or reused after a failed revalidation. Exit 0 when every link ends resolved to a version satisfying
  its pin; otherwise non-zero, naming each link that did not. This is the verb a host runs at session setup to
  warm the cache.

## Why

The user found `onemessagebus` carrying a hand transcription of `onejudge`'s protocol v6 "field for field" and
said it "seems really weird to me to have anything specific to onejudge in one message bus". The repair is that
the program that owns a protocol publishes its grammar and a bus configuration names it — by a URL pinned to a
version, cached so a serving session never waits on the network. Linking is the half of that repair that has
to exist before the generic judge-seat binding can validate a frame against a grammar the bus does not carry.

## Acceptance criteria

- `onemessagebus::SchemaBundle` is public in the core crate with the fields Contract L states, and a bundle with
  an unknown top-level key, a malformed `version`, an empty `schemas`, a duplicated id, or an entry that is not a
  registry document is refused naming what is wrong — each proven by the core crate's tests.
- Link parsing is proven by the core crate's tests for every Contract L case: a pinned and an unpinned
  `https://` link; an `@` inside a path segment that is not a pin; `http://` accepted for `localhost`,
  `127.0.0.1` and `[::1]` and refused naming the link for any other host; `file://`; an absolute bare path; and a
  relative bare path resolved against the configuration file's directory.
- The pin rule is proven for `@8`, `@8.1` and `@8.1.2` against declared versions that satisfy and that do not
  (including `8` against `@8.0`, satisfied), and a document whose declared version does not satisfy its pin
  fails with the link, the pin and the declared version named.
- The cache rules are proven against the compiled binary in `onemessagebus-e2e`, with a loopback HTTP server the
  journey starts itself and no request leaving the host: a first resolution fetches and stores under the declared
  version; a resolution inside the window makes no request; one past the window sends a conditional request and
  a `304` re-confirms; a `200` with a newer satisfying version is stored beside the old entry and used; a `200`
  with a non-satisfying version is the hard error; a server that is down past the window reuses the entry and
  says so on stderr; `ONEMESSAGEBUS_SCHEMA_TTL=0` revalidates every time; `ONEMESSAGEBUS_SCHEMA_REFRESH=1`
  refetches unconditionally; an unpinned link is fetched on every resolution and leaves no cache entry; and the
  cache directory honours `ONEMESSAGEBUS_SCHEMA_CACHE_DIR` and `XDG_CACHE_HOME`.
- The transport requirements Contract L states are proven functionally, inside the dispatch and with no
  request leaving the host, against servers the journeys start themselves: an `https://` link is fetched from a
  loopback TLS server whose certificate the journey trusts through the client's own root store (so the pure-Rust
  client's HTTPS path is the one exercised, not a plain-HTTP stand-in); with `HTTPS_PROXY` naming a loopback
  proxy the journey runs, the fetch reaches the bundle through that proxy and the proxy's log shows it, and
  with the bundle's host in `NO_PROXY` the same fetch bypasses the proxy and its log shows nothing; a server
  that accepts the connection and never answers ends the resolution with a refusal naming the read timeout
  within the bound the documentation states, and a connection that never completes establishing — a loopback
  listener whose accept backlog the journey has filled so the client's connect is never accepted, or a
  loopback proxy that accepts the TCP connection and never answers the `CONNECT` — ends with a refusal naming
  the connect timeout within the connect bound; a closed port is not that evidence, since it refuses at once.
  Each bound is measured by the journey rather than assumed.
- A `file://` link and a bare-path link are proven in the same journeys to be read on every resolution, never
  cached, and still held to their pin.
- `Config::load` on a configuration naming `schemas:` links that are unreachable and uncached succeeds and makes no
  request, proven by the core crate's tests; a malformed link is refused at load naming it.
- A configuration naming `schemas:` registers every entry of every linked bundle for the verbs that load it,
  proven end to end by `schema check` and `send` (or `validate`) accepting a record that satisfies a linked schema
  and refusing one that violates it, with the linked bundle served over `file://`; a linked entry contradicting an
  already-registered id is refused naming the id.
- `serve` with a warmed cache and a loopback server that is down starts and answers its first frame with no
  request made and no revalidation attempted, whatever the entry's age; with nothing cached and the server down
  it refuses before reading a frame, naming the link.
- `schemas`, `schemas clear` and `schemas fetch` behave as Contract L states, each proven against the compiled
  binary in `onemessagebus-e2e`, including `schemas fetch` exiting non-zero naming a link it could not resolve,
  exiting 0 when a failed revalidation reused a satisfying entry, and resolving every link a `--config` names when
  none is given.
- The three verbs are capabilities in `CAPABILITIES`, each with a method in the Python and the TypeScript SDK
  client; the SDK coverage check (every capability has one method in each client and no method beside them) and
  both SDKs' generate-checks pass over the finished tree, and `docs/sdk-parity.md` is the parity audit's regenerated
  output; each SDK method is exercised by that SDK's own journey test through the real
  binary.
- The schema-linking contract is documented once, in a new `docs/schema-links.md` stating Contract L (document,
  link, pin, cache, where links are named, the three verbs), with `docs/cli.md`, `docs/contract.md` and the
  configuration's key table pointing at it rather than restating it; the repository's documentation tests that
  read those files pass.
- No existing behaviour changes: every behaviour the crates' existing test suites and `onemessagebus-e2e` journeys
  cover is still covered by a test in the finished tree and still passes with the same expected values; the only
  expected values that move are those of the tests enumerating the capability list or the configuration's keys,
  which now name this node's additions.
- The checks that exercise this change are green over the finished tree: the tests of the crates it touches, the
  `onemessagebus-e2e` journeys, both SDKs' test tiers, the workspace lint and format check, the dependency audit
  (for the HTTP client added), and the judged lint over this branch's own diff.
...
- **Criteria revised after plan review, on the manager's rulings.** The existing-behaviour criterion made edit
  history the bar (tests passing "unmodified"); it now states the property that reached for — covered behaviour
  still covered and passing — because the review refused that shape in the `oneagentgraph` node and reads this one
  by the same meaning. The report-truth property is added as its own criterion. The note on the plan's one
  networked journey is updated because that journey no longer exists.
- **You own Contract L.** The node after this one (`bus-judge-seat-binding`) and the `onejudge` and
  `ai-orchestrator` nodes restate it from this task and from `docs/schema-links.md`. Once this plan is running a
  shared interface is never changed unilaterally: if you find the contract wrong or incomplete, raise a proposal
  with `"$ORCHESTRATOR_ASK_MANAGER"` and keep building against the agreed surface until the manager rules.
- The HTTP client is your choice within the contract. llmlint uses `ureq` with rustls; `deny.toml` holds the
  license and advisory policy it must pass.
- The only network this node's checks may touch is a loopback server a test starts. No test here fetches from the
  internet, and no test anywhere in this plan does: the `ai-orchestrator` node proves its link offline too, and the
  one real fetch of a published bundle is that host's session-setup warm step, a later comparison no criterion
  rests on.
- Keep the new verbs out of the `onejudge` codec's path entirely; nothing about `serve --codec onejudge` changes
  here.
- This is a `feat` release of the workspace: both published crates move together on it.
- The checks above are this repository's recipes: `cargo test -p onemessagebus -p onemessagebus-cli`,
  `just test-e2e`, `just python-sdk-check`, `just node-sdk-check`, `just sdk-coverage`, `just parity-audit`,
  `just lint`, `just fmt-check`, `just deps-check` and `just lint-llm-diff`. Running the pieces separately, and only
  the ones your change touches as you iterate, is fine; the full `just gate` is not owed.
...
{"onepipeline.branch":"onevcs/s-23813bc45bd7","onepipeline.id":"bus-schema-links-2","onepipeline.max_turns":200,"onepipeline.persona":"engineer","onepipeline.resume":{"branch":"onevcs/s-23813bc45bd7"},"onetaskgraph.item_kind":"task"}
-->
