...
Adopt this workstream's cargo build contract in `oneharness` (github.com/nickderobertis/oneharness; checkout `nickderobertis__oneharness` (registered)).

oneharness is a Cargo workspace of the root package and `crates/oneharness-core`; no crate outside the workspace.
...
- `target/sdk-schema-generator` as `CARGO_TARGET_DIR` for the `sdk-schema`-feature example build (`scripts/check-capability-surface.sh`, `scripts/sdk-generator.mjs` through `scripts/sdk-coverage.mjs`, `scripts/parity-audit.mjs`, `npm/oneharness-sdk/scripts/generate.mjs`, and `npm/oneharness-sdk/test/generate.test.ts`), which keeps a feature-flagged build from thrashing the main tree's fingerprints

Consolidate it: remove every `target/sdk-schema-generator` override (`scripts/check-capability-surface.sh`'s default, `scripts/sdk-coverage.mjs`, `scripts/parity-audit.mjs`, `npm/oneharness-sdk/scripts/generate.mjs`, and the expectations in `npm/oneharness-sdk/test/generate.test.ts`) so the `sdk-schema`-feature example builds into `<clone>/target`. What the sub-directory protected was build time — alternating the feature set on `oneharness-core` recompiles that crate — not correctness; measure one `just check` alternation before and after and put the numbers in the report.

**Builds that happen outside the clone or already name the contract's directory** (read them; none changes):
- `scripts/check-build-mock-harness.sh` runs recipes with `CARGO_TARGET_DIR` under a `mktemp -d` directory and a failing `RUSTC_WRAPPER` — outside the clone, unchanged
- `python/oneharness-sdk/test/test_generate.py` sets `CARGO_TARGET_DIR` to `ROOT/target`, the same directory the contract names
...
- `scripts/e2e-lib.sh`, `scripts/smoke.sh`, `scripts/check-control-probes.sh`, `scripts/e2e-variants-test.sh` and `just npm-e2e` resolve `target/debug/oneharness` (and the mock harness) at the clone root; `ONEHARNESS_LIVE_INSTALL_DIR` is `target/e2e-install/bin`; `scripts/check-lf-contracts.sh` works under `target/lf-contracts`; `just python-sdk-check` writes `target/python-sdk.coverage`
- `just coverage` adds `-C linker=…` through `RUSTFLAGS` and runs `cargo llvm-cov` over the workspace (skipped on Windows)
- `.github/workflows/ci.yml` and the `e2e-*.yml` workflows, `release.yml`
...
- No `CARGO_TARGET_DIR`, `--target-dir` or `build.target-dir` anywhere in the `justfile`, `project.json`, `nx.json`, workflows, scripts or tests names any target directory but `<clone>/target` itself for a build inside the clone (a build in a temporary directory outside the clone, and the directories the build tool or its own tooling creates under `<clone>/target` for a build it drives, are the only exceptions); every invocation that named one before is inventoried in the report with what replaced it, and nothing introduces a directory shared across worktrees or repositories. The behaviours the removed sub-directories protected, named under `## What`, still hold: the schema generator, the capability-surface check and both SDK generators still build and run the `sdk-schema` example and the report quantifies the cost the removed isolation hid with one measured alternation taken before the change and one after — main build, then that build, then the main build again — giving each step's wall-clock time and the crates it recompiled, so the two can be compared.
- `.gitignore` ignores the root `target` directory and carries no entry for a nested target directory the contract makes dead.
- The checks that read the target directory, spawn a built binary or run under coverage are green over the finished tree (their commands are under `## Additional info`): the debug build and the capability-surface check, whose schema generator now builds into the clone-root target directory; the SDK generation checks for Node and Python, whose generators now build into the clone-root target directory; the mock-harness build diagnostic check, which builds under a temporary target directory; the binary-consuming journeys that resolve `target/debug/oneharness`: the control-probe check and the Node e2e.
- Coverage collection through this repository's own coverage interface still works over the finished tree, reporting usable per-file line results under the line-table debuginfo: a scoped run through the workspace's coverage interface, with the linker `RUSTFLAGS` the `coverage` recipe sets, reporting per-file line results is green (its command is under `## Additional info`).
...
- the debug build and the capability-surface check, whose schema generator now builds into the clone-root target directory: `just build && bash scripts/check-capability-surface.sh`
- the SDK generation checks for Node and Python, whose generators now build into the clone-root target directory: `just sdk-check && just python-sdk-check`
- the mock-harness build diagnostic check, which builds under a temporary target directory: `bash scripts/check-build-mock-harness.sh`
- the binary-consuming journeys that resolve `target/debug/oneharness`: the control-probe check and the Node e2e: `cargo build --locked && bash scripts/check-control-probes.sh && just npm-e2e`
- coverage: a scoped run through the workspace's coverage interface, with the linker `RUSTFLAGS` the `coverage` recipe sets, reporting per-file line results: `RUSTFLAGS="-C linker=<COVERAGE_LINKER from the justfile>" cargo llvm-cov nextest -p oneharness-core --features <FEATURES from the justfile> --locked --summary-only`
...
{"onepipeline.id":"oneharness","onepipeline.persona":"engineer","onetaskgraph.item_kind":"task","orchestrator.plan-review":{"by":"planning-closeout","key":"3df5d75c2aeafb90ac2fd0c8e70735dd2c95e8b985c2dea54e340a2bd72ffc3d","reviewed_at":"2026-09-20T09:43:09.631660+00:00"}}
-->
