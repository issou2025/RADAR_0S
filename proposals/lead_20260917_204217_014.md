...
**test(e2e): run no background git maintenance in a fixture repository**

*Project description:*
## What

Stop git from starting a detached background repack inside the repositories the e2e suite
creates as fixtures, so that a tool walking such a repository's `.git` during a test — the
`release-plz update` the release-path journeys drive — never meets an object directory that
was there a moment ago. The change is to the shared fixture, `tests/repo-e2e/tests/journey.py::
GateCopy.__init__`, and it supersedes PR #58's `bebb3c2` (`git repack -ad` in `tagged()`).

### The failure

`test_release_path_journey.py::test_the_drafting_tool_finds_no_absent_registry_entry_to_compare_against`
failed PR #57's Linux gates twice in a day — run 35143963219 (aarch64, job 104955653624) and
run 35202822408 (x86_64, job 105141515203) — and its sibling
`test_a_tree_at_a_version_a_tag_already_names_cannot_be_drafted_from` failed #58's macOS
x86_64 gate (run 35200987773, job 105135860851), each with release-plz reporting

```
cannot copy directory ".../pytest-4/test_the_drafting_tool_finds_n0/copy1" to Utf8TempDir { … }
invalid entry
.../copy1/.git/objects/18: IO error for operation on .../copy1/.git/objects/18: No such file or directory
```

(`objects/09` and `objects/2a` in the other two). `release-plz update` copies the whole
project aside before it computes versions (`release_plz_core::copy_dir`, an `ignore` walker
over the tree and then over `.git`), and a loose-object fanout directory it had just listed
was gone when it descended into it.

### The cause, read off git's source

`GateCopy.__init__` makes every fixture by `git init` + `git add -A` + `git commit` of the
tracked tree: about 760 loose objects, no pack. `git commit` ends by running `git maintenance
run --auto` (this host's git 2.43 traces exactly that: `run_command: git maintenance run
--auto --quiet`). Since **git 2.54** ("git maintenance starts using the geometric strategy by
default", its release notes) that command's default strategy for a manual run is
`geometric`, whose `geometric-repack` task's auto condition is `too_many_loose_objects(100)`
— `maintenance.geometric-repack.auto` defaults to 100 — and whose background phase runs
**detached** (`daemonize()` in `maintenance_run_tasks` under `gc.autoDetach`, default on). So
on the hosted runners (git 2.55.0 on `ubuntu-24.04`; the macOS runners' Homebrew git is past
2.54 too) every fixture commit forks a repack that packs the loose objects and then deletes
them and their emptied `objects/XX` directories, while the test goes on to `git tag` and
`release-plz update` walks the same `.git`. Under the old `gc` strategy — this host's 2.43,
threshold 6912 loose objects — the auto condition never fires, which is why no run here
reproduces it. `git repack -ad` in `tagged()` (#58's `bebb3c2`) happens to leave nothing loose
for the condition to count, for the one fixture it touches; it names the wrong reason
("Git auto-maintenance cannot remove a loose object" is the right instinct with the old
threshold in mind) and covers one caller of a fixture every journey uses.

### The fix

A fixture repository runs no automatic maintenance at all: `GateCopy.__init__` sets
`maintenance.auto=false` (and `gc.auto=0`, so an older git's `gc` task cannot fire either) in
the copy's own `.git/config` before its first commit — a repository-local setting, so nothing
about the host's or the runner's git configuration is assumed, and every command any journey
later runs in the copy (`git tag`, the hook journeys' commits, release-plz's own git calls)
inherits it. That makes the fixture quiescent by construction on every git version; #58's
`bebb3c2` is then unnecessary and drops on rebase (it conflicts with nothing here — a different
file — and the manager carries the note to that branch).

Audit the other fixture repositories the suites create (`test_registry_proof_journey.py`,
`tools/release-artifacts/tests/test_dispatched.py`, `tools/repo-checks/tests/test_cli.py`,
`tools/repo-checks/tests/test_suppressions.py`) for the same shape — a repository whose
`.git` some tool walks, copies or reads as a directory during the test — and apply the same
setting where one exists; say in the report which you changed and which you judged not to
need it (a fixture nothing walks, or one under a hundred objects, is not at risk).

### The proof, on a host whose git cannot reproduce it

The property is that git spawns no maintenance from the fixture's own commands, and that is
visible on git 2.43 too: with `GIT_TRACE2_EVENT` (or `GIT_TRACE`) set while a `GateCopy` is
made and tagged, the trace before the change shows `maintenance run --auto` as a child of the
commit, and after the change it shows none. Write that as a test in `tests/repo-e2e/tests`
that drives the real fixture (`gate_copy()`) under the trace and asserts no `maintenance`
command appears, and a second assertion that the copy's repository configuration carries
`maintenance.auto=false`. Cite the before-trace as the induced/prior-state evidence it is.
Whether the hosted runners' git 2.55 then stops repacking is the merge path's to show, and is
not this node's bar.

## Why

The user's words: "let's make sure to fix flaky tests too — they're super expensive when
the gate is so long." This one refuses a two-hour round on a race between the test's own
fixture and a git the runner image upgraded underneath it, and it will get worse as every
runner moves past 2.54; both live pull requests have lost rounds to it, and the fix on #58
covers one fixture caller with a reason that does not hold.

## Acceptance criteria

- Every repository `GateCopy` creates carries `maintenance.auto=false` and `gc.auto=0` in its
  own repository-local configuration from before its first commit, so no Git command a journey
  runs in a copy starts automatic maintenance, on any Git version.
- A test in `tests/repo-e2e/tests` drives the real fixture (`gate_copy()` and its tagging)
  under Git's trace and proves that none of the fixture's own Git commands spawned a
  `maintenance` command, and that the copy's configuration carries the setting; the report
  cites the trace of the fixture *before* the change showing `maintenance run --auto` spawned
  by its commit, as prior-state evidence.
- Every other fixture repository in the suites whose `.git` a tool walks, copies or reads
  during a test carries the same setting, and the report lists each fixture the audit
  considered with the decision for it.
- `test_release_path_journey.py` is not restructured: `tagged()` and `drafted()` keep their
  shape, and no repack step is added on `main`.
- No test's coverage is removed or narrowed, no test is marked ignored, and no assertion is wrapped in a retry that could mask a failure; `llmlint.yml`, `repo-policy.toml` and the CI
  matrix are untouched.
- The fixture's quiescence holds across repeated runs: at the finished tree, the new fixture
  test and the `test_release_path_journey.py` and `test_release_tag_journey.py` modules are
  shown passing repeatedly on this Linux host, with the number of runs and the commit they
  ran on stated and no run among them failing, and Git's trace of the fixture's own commands
  in those runs shows no maintenance process spawned.
- The tests over the code this node touches are green over the finished tree: the new test,
  `tests/repo-e2e/tests/test_release_path_journey.py` and
  `tests/repo-e2e/tests/test_release_tag_journey.py` in full (both drive release-plz over a
  fixture), and the tests of any other project whose fixture changed; the Python linter and
  formatter checks over `tests/repo-e2e` are clean, the Python type checker reports nothing, and the
  judged lint over this change's own diff leaves no finding unanswered — each named with what
  it reported.
...
**Reading order.** `tests/repo-e2e/tests/journey.py` (`copy_tracked`, `GateCopy`, `shell_run`),
`conftest.py::gate_copy`, `test_release_path_journey.py` (`tagged`, `drafted`, the two tests
named above), then `git show bebb3c2` (fetch `origin/onevcs/s-ffbf18677e45` read-only). The
git side: `git log -1 v2.54.0 -- Documentation/RelNotes/2.54.0.adoc` is not needed — the
sentence is "git maintenance starts using the geometric strategy by default" in that file,
and `builtin/gc.c` at `v2.55.0` has `geometric_repack_auto_condition` (default `auto_value =
100`) and the detached background phase in `maintenance_run_tasks`; `gh api
repos/git/git/contents/builtin/gc.c?ref=v2.55.0 --jq .content | base64 -d` reads it. The
failing logs: `gh api --allow-escape-sequences
repos/nickderobertis/printobserver/actions/jobs/105141515203/logs` (and 104955653624,
105135860851).

**Seeing the spawn here.** `GIT_TRACE=1 git -c user.email=… commit …` in a scratch copy prints
`run_command: git maintenance run --auto --quiet` on this host's git 2.43 and `git
count-objects -v` afterwards shows the objects still loose; that pair is the before-state
citation. `GIT_TRACE2_EVENT=<file>` gives the same as JSON if you prefer to assert on it.

**Running the proof.** The two release-path/-tag journey modules drive the real release-plz
(pinned by `repo-policy.toml`; `just install-tools` puts it on PATH) and take some minutes
each; `uv run -q pytest tests/repo-e2e/tests/test_release_path_journey.py` runs one. The
whole e2e tier is not asked of you.

**How many runs convince.** This race is a version race, not a load race — the repack is a
detached process git starts on its own — so the counted runs need no other suite running
beside them; they need Git's trace on. This workstream's manager reads the count against at
least three separate runs of the named modules and the fixture test, five preferred, each
run's commit and outcome listed in the report. The release-journey modules take some
minutes each; keep every invocation inside the turn that started it, with a sentinel and log
named for this node if you background one.

**Iterate with** those modules, `uv run -q ruff check tests/repo-e2e`, `uv run -q ruff format
--check tests/repo-e2e`, `uv run -q ty check tests/repo-e2e`, and the judged lint over your
diff. The whole gate runs downstream on the merge path; nothing here asks you to have run it
first.

**A shared interface is never changed unilaterally.** `GateCopy`'s constructor signature and
what a copy contains (the tracked tree, one commit on `main`, the optional `node_modules`
link) are fixed for this run; a departure you want is a proposal to your manager over
...
{"onepipeline.execution_checkout":"nickderobertis__printobserver-isolated","onepipeline.id":"fixture-repository-quiescence","onepipeline.persona":"engineer","onetaskgraph.item_kind":"task","orchestrator.plan-review":{"by":"review-plan","key":"917720cc61ee9d9bda25e9adf6697ccfc31dd06e90c1eab728f6115f4ef89c20","reviewed_at":"2026-09-17T17:56:00.708376+00:00"}}
-->
