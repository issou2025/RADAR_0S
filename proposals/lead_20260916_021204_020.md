...
**fix(tests): declare the scripts the crate's tests drive and serve local-md in process**

*Project description:*
## What

Two accepted follow-up tickets about this repository's own test inputs and test chain. They
touch different files and are one node because the first is minutes of work. One commit per
ticket.

**1. Cached tests omit scripts they execute** —
https://github.com/nickderobertis/onepipeline/issues/300, accepted at severity `high`.
`nx.json:19-33` defines the `crateSource` named input, which `project.json` uses for the
cached `test` and `test-rest` targets, and it omits the repository scripts those Rust
integration tests execute: `tests/record_channel_answers.rs:1,61` runs
`scripts/record-channel-answers.sh`, `tests/linked_engines.rs:81,421-425` runs
`scripts/linked-engines.sh`, and `tests/published_smoke.rs:35,88` runs
`scripts/smoke-published.sh`. A script-only change therefore replays a stale green instead
of exercising the changed script, which weakens the deterministic gate.

Add **exactly those three script paths** to `crateSource`. Three constraints the ticket and
the user's own comment settle, and each is a constraint rather than a preference:

- Do not broaden the input to `scripts/**/*`.
- Do not add a journey per script.
- Do not add a cache-invalidation test.

What stands in their place is one check you run once and put in your report for your judge
to read: the resolved inputs of the cached test target, showing those three scripts present.

**2. The store journey's label-strict source spawns a second executable** —
https://github.com/nickderobertis/onetaskgraph/issues/1238, the onepipeline half of a
ticket whose other half is a documentation correction in onetaskgraph and is not yours.
`crates/testfakes/src/bin/label-strict-source.rs:1-24,263-305` spawns
`onetaskgraph-source` — a reference and test host that onetaskgraph does **not** ship —
purely to put the real `local-md` plugin behind the test fake's label refusal rule, and
`tests/e2e/store.rs:766-800` configures that chain through
`tests/e2e/harness.rs:3872-3892`, which hunts that executable beside the installed CLI or
on the search path and has resolved an unrelated stale local build. The ticket's answer is
to stop needing the second executable, not to have it shipped: *"Do not add the reference
host to release artifacts merely to satisfy this test."*

Change the label-strict source to provide `local-md` **in process** — linking the
`onetaskgraph-core` and `onetaskgraph-local-md` crates, which are published to crates.io at
each onetaskgraph release, or an equivalent in-process fixture — so the journey still proves
the destination's refusal behaviour over a real store while resolving no second executable.
Link them at the release this repository's justfile already names in its one constant for
the onetaskgraph it reads plans through, so one place still says which onetaskgraph this
build's checks use. The journey's own subject — a label-strict destination accepting the
settlement projection, run after run — must be unchanged: this is a change of how the
destination is stood up, not of what it proves. Remove the harness helper and the host
setting that exist only for the spawned host, so nothing is left hunting an executable
nobody installs.

## Why

The user accepted both. The first is this repository's deterministic gate quietly lying: a
changed script with a cached green is a check that did not run, and the user's own comment
on the issue asked for exactly the narrow fix and no more ceremony. The second already cost
a real journey: the store journey reached a stale host and failed during source
configuration, before it tested settlement projection at all, and the diagnosis took a
follow-up run to reach — a test-only dependency on an executable that is not part of any
release is a failure that reads as a product defect.

## Acceptance criteria

- `nx.json`'s `crateSource` named input names exactly the three scripts the crate's tests
  execute, and no wider script pattern.
- No journey per script and no cache-invalidation test is added for that input.
- This dispatch's report carries the output of one check showing the cached test target's
  resolved inputs with those three scripts present.
- The label-strict test source serves the real `local-md` plugin in process and spawns no
  second executable.
- Nothing in the test chain resolves, or looks for, the reference subprocess host: the
  harness helper and the source setting that existed for it are gone.
- The label-strict store journey still proves what it proved before — a destination that
  refuses a write whose labels differ from the ones it holds still accepts the settlement
  projection, run after run — and still fails if a projection drops a label.
- The onetaskgraph crates the test source links are named at the release this repository's
  own single constant for onetaskgraph names, so one place still decides which onetaskgraph
  this build's checks read.
- Each of the two tickets is resolved by its own commit whose body carries a `Closes` line
  naming it: onepipeline issue 300 and onetaskgraph issue 1238.
- The store journey and the crate tests that cover what this change touches are green over
  the finished tree, and so is the judged lint over this change's diff.
- Every claim this dispatch makes about the finished work is true of the tree as it finally
  stands.

## Additional info

Iterate with `just lint-llm-diff <base>` and a filtered end-to-end selection over the store
journey rather than this repository's whole gate. The repository's own required checks run
downstream on the change this branch becomes; nothing here asks you to have run them first.

A `Closes` line naming an issue of another repository does not close it on merge — GitHub
only closes issues of the repository the commit lands in. Write it anyway, because it is the
record of which ticket this commit answers; the manager closes the onetaskgraph issue when
both halves have landed.

This node adds crate dependencies, so it changes `Cargo.toml` and the lockfile. The node
that relinks `oneharness-core` starts from your work for that reason; keep your dependency
additions to what this ticket needs.
...
{"onepipeline.execution_checkout":"nickderobertis__onepipeline","onepipeline.id":"op-test-inputs","onepipeline.persona":"engineer","onetaskgraph.item_kind":"task"}
-->
