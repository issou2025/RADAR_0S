...
**fix(tests): provision strace, bound hook journeys, snapshot the unwatched fixture, close the validator race**

*Project description:*
## What

Four accepted tickets about this repository's own test tier and its provisioning — none of
them changes what the engine does, and every one has failed a valid publication on a host
or a CI leg. One commit per ticket. This node is deliberately not one this host's engine
adoption waits on: a fix to the suite reaches nobody through a release.

**1. E2E journeys require `strace` without provisioning it** —
https://github.com/nickderobertis/onepipeline/issues/337, accepted at severity `high`.
`tests/e2e/channel.rs`, `tests/e2e/listing.rs` and `tests/e2e/unwatched.rs` each run the
binary under `strace` on Linux — the channel one to make a pushed process's log unreadable
and prove the refusal path, the other two to observe what a listing opens — and every one
refuses rather than passes when the tracer will not run. Nothing installs or preflights
it: `scripts/session-setup.sh` reports the toolchain, `just bootstrap` installs cargo
tools, and `.github/workflows/ci.yml`'s Linux legs install nothing beyond the Rust tools.
Provision and preflight `strace` exactly where those Linux-only journeys run — the Linux CI
legs that execute the e2e tier, and the local bootstrap and session-setup path on Linux —
and keep the journeys compiled out on other platforms as they are today. A preflight
names the missing tool and the install command rather than letting the panic inside the
journey say it.

**2. Run-end-hook journeys outlive the test runner's timeout** —
https://github.com/nickderobertis/onepipeline/issues/338, accepted at severity `medium`.
Most launches in `tests/e2e/run_end_hooks.rs` inherit the 600-second product default hook
timeout while `.config/nextest.toml` terminates a test after 360 seconds, so a stuck hook
ends as an opaque runner `TIMEOUT` before the engine can record its own timed-out-hook
ending. Give every run-end-hook journey an explicit 120-second hook timeout through the
launch surface those journeys already use, leaving the product default untouched, so the
engine's own timeout evidence lands well inside the runner's bound; the journey that asks
about the default itself keeps asking about the default.

**3. The unwatched fixture copy races transient run files** —
https://github.com/nickderobertis/onepipeline/issues/339, accepted at severity `high`.
`tests/e2e/unwatched.rs::cloned_run` clones a live run directory file by file while that
run can still atomically replace temporary or cache files, so the required Windows leg
fails with `NotFound` after enumerating a name that is then renamed away. The assertions
consume a stable subset — the launch record and the summary document it rewrites, and
the journal it deliberately regrows — so replace the whole-directory clone with a fixture
builder that writes only the state files the unwatched verb reads, taken through
`RunPaths`'s own accessors rather than a directory walk, excluding transient runtime state
by construction rather than by a filename list.

**4. Edits unit tests race on `ETXTBSY`** —
https://github.com/nickderobertis/onepipeline/issues/364, accepted at severity `medium`.
`src/edits.rs`'s test helper `validator` writes a shell script with `fs::write`, sets its
mode separately, and the caller spawns it at once; under the parallel test runner another
thread forking between the write and the exec inherits the open write descriptor and the
exec fails `ETXTBSY`. Create the script with its executable mode at creation and fully
closed before the helper returns, so no other thread can observe the open descriptor, and
show the helper's result is spawnable immediately under concurrent forks.

## Why

The user accepted all four because each one rejects correct work for a reason that has
nothing to do with the change under test: a whole e2e tier red on a valid Linux host
because a system tool nobody declared is absent; a stuck hook reported as a bare runner
timeout with no diagnostic; the required Windows leg failing on release-only changes until
somebody re-runs it into quiescence; and a unit test that fails one run in several hundred
and passes on re-run. Each costs a publication attempt or a person's attention, and each
fix is confined to the suite.

## Acceptance criteria

- On a Linux host or CI leg that runs the e2e tier, `strace` is provisioned by the
  repository's own bootstrap or CI provisioning before the tier runs, and a preflight
  names the tool and its install command when it is absent; on other platforms the
  journeys that need it stay compiled out and nothing is installed.
- Every run-end-hook journey runs under an explicit hook timeout short enough that a stuck
  hook is ended and recorded by the engine before the runner's bound, the product default
  is unchanged, and the journey about the default still asks about the default.
- The unwatched fixture is assembled from exactly the state files the verb reads, obtained
  through the run-paths accessors, and copying it while the template run is still writing
  transient files cannot fail on a vanished entry.
- The validator script helper returns a path that is executable and whose descriptors are
  closed, and spawning it immediately under concurrent forks succeeds.
- The branch's history carries, for each of these tickets, a closing reference naming its
  issue (337, 338, 339, 364), so the change that carries the work is what closes it.
- Every behaviour this change adds is proven by the real journeys and tests it changes
  running green, with the race cases exercised under real concurrency rather than by
  inspection, and no test substitutes a double for the layer under test.
...
Iterate with this repository's diff-scoped judged lint and the e2e modules and unit tests
you touched rather than the whole gate; the required checks run downstream on the change
this branch becomes. The Windows leg cannot be run here — what you can prove locally is
the fixture's construction and the race it closes; say so in your report.

`.config/nextest.toml` records why the runner's bound is 360 seconds; leave that bound
where it is.
...
{"onepipeline.branch":"onevcs/s-bb1f58f2905e","onepipeline.execution_checkout":"nickderobertis__onepipeline","onepipeline.id":"op-test-fixtures-4","onepipeline.persona":"engineer","onepipeline.resume":{"branch":"onevcs/s-bb1f58f2905e"},"onepipeline.settlement":{"branch":"onevcs/s-bb1f58f2905e","detail":"onevcs: invalid input: git fetch origin --prune failed (exit 128): fatal: unable to access 'https://github.com/nickderobertis/onepipeline.git/': Could not resolve host: github.com","outcome":"publication-failed","status":"failed"},"onetaskgraph.delivers":["followups:I_kwDOTx8aUc8AAAABRp2SGg","followups:I_kwDOTx8aUc8AAAABRp2p-Q","followups:I_kwDOTx8aUc8AAAABRp24lg","followups:I_kwDOTx8aUc8AAAABR7avfg"],"onetaskgraph.item_kind":"task"}
-->
