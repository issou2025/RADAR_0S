...
**refactor(event): take the wire envelope, the filter grammar, the stream emitter and redaction from onemessagebus**

*Project description:*
## What

Make `nickderobertis/onevcs` the first consumer of `onemessagebus`: its `crates/onevcs/src/event.rs` (the
envelope, `Labels`, `Source`, `Phase`, `ArtifactRef`, `EventFilter`/`EventMatcher`) and the envelope half of
`crates/onevcs/src/stream.rs` (`Stream`'s NDJSON writing and `seq` rules, `EventStream`'s filtered reading,
`redact`, `PAYLOAD_LIMIT`, `REDACTED`, the credential tables) become **re-exports of, and thin calls into,
`onemessagebus` and `onemessagebus-agent`** — with every byte `onevcs` writes and every value it reads unchanged.
No behaviour changes; this is a relocation, and its bar is byte identity.

Read first: `AGENTS.md`, `docs/contract.md` (the "Shared event envelope" section at line ~1867 and the event-kind
and phase sections), `crates/onevcs/src/{event.rs,stream.rs}`, `crates/onevcs/tests/contract.rs`, and the
released `onemessagebus`'s `docs/contract.md`, `docs/wire.md` and `docs/queues.md` — the contracts this node
consumes are that repository's Contracts W, F, R and E, stated in the `bus-scaffold-wire-core` task and restated in
those documents; this node restates none of them and changes none. **A shared interface is never changed
unilaterally**: a departure this node wants is a proposal to the manager over the ask seam, while the work continues
against the agreed surface.

**What moves, and to what:**

- `Envelope`, `Labels`, `Source`, `ArtifactRef`, `ArtifactId`, `EventFilter`, `EventMatcher` → `pub use
  onemessagebus_agent::event::{Envelope, Labels, Source, ArtifactRef, ArtifactId, EventFilter, Matcher as
  EventMatcher}` under `onevcs::event`, so every public path a Rust consumer (`onepipeline`) imports today still
  resolves. `EventKind` stays `onevcs`'s own closed enum (its vocabulary) with `impl From<EventKind> for
  onemessagebus::Kind`.
- `Phase` → `pub use onemessagebus_agent::Phase;` and `Phase::of(kind)`, `Phase::every()` stay here as functions
  over `onevcs`'s kinds (the manager's ruling: the profile owns the closed enum, `onevcs` keeps the mapping).
- `stream::Stream` → a thin wrapper over `onemessagebus::Emitter` with a file sink: the single-writer in-memory
  `seq` and the shared from-the-file `seq` (`Stream::releases`) become `Emitter::new` and `Emitter::shared`; `emit`,
  `emit_push`, `emit_with`, `label` keep their signatures. `ENVELOPE_VERSION` stays `1` for this producer.
- `stream::EventStream` (filtered reading, `open_filtered`, `read`) → `onemessagebus::Reader` + the profile's
  `EventFilter`; `attributed`, `Line` and the lenient line handling keep their behaviour.
- `redact`, `REDACTED`, `PAYLOAD_LIMIT`, `CREDENTIAL_WORDS`, `CREDENTIAL_PREFIXES` → `onemessagebus::{Redactor,
  REDACTED, MAX_PAYLOAD_TEXT_BYTES}`; `store_artifact`/`read_artifact` stay `onevcs`'s (the artifact *store* is the
  producing library's), returning the bus's `ArtifactRef`.
- **`docs/contract.md`**: this task is the planner's ruling on the section titled "Shared event envelope
  (duplicate these types in this crate; there is deliberately no shared util crate)": its title and its first
  sentence change to say the types are `onemessagebus-agent`'s, re-exported here, and that `onemessagebus`'s
  `docs/contract.md` is the one source of the fenced text, which stays here as a copy marked as one; the fenced
  examples stay and `tests/contract.rs` keeps driving them, now through the re-exported types. Record it as this
  repository's convention requires (the contract is "not edited to match the code" — this edit is the ruling
  arriving, and the task says so in the commit body).
- `Cargo.toml`: `onemessagebus` and `onemessagebus-agent` as dependencies of `crates/onevcs`, resolved from the
  registry per the adoption instructions the release renders into this task's references block; `deny.toml`'s
  allow-list gains nothing new unless the bus brings a licence it lacks.

**What does not move:** `Phase::of`, the artifact store, the session/release stream *paths* (`path_for`), the
kinds, every verb. Nothing under `onevcs` gains a bus queue in this node; the stream is an emitter, not a queue.

### Working notes

- The bar is the recorded fixtures: capture, **before** any change, the NDJSON an `onevcs` session stream and a
  release stream write across the e2e suite's journeys (the suite already drives them), and hold the finished tree
  to byte identity with those captures on the same journeys — `ts` and ids aside where the journeys already
  normalize them.
- `tests/contract.rs`'s envelope and filter blocks keep passing; the blocks the bus now owns are driven through the
  re-exports, so a bus that stops matching this repository's text still fails here.
- Commit as you go on the branch the session opened.

## Why

The user asked to "extract a particular rust library … to be able to refactor everything out", and `onevcs` holds
one of the three drifted copies of the one envelope — the copy whose `EventMatcher` grew a `phase` field the other
two never got. Adopting the bus here is what makes that field one grammar rather than three, and the byte-identity
bar is what makes the adoption invisible to every run reading an `onevcs` stream on this host.

## Acceptance criteria

- Every type `onevcs::event` and `onevcs::stream` export at 0.21.0 still resolves at the same path over the finished
  tree, with `Envelope`, `Labels`, `Source`, `Phase`, `ArtifactRef`, `ArtifactId`, `EventFilter` and `EventMatcher`
  being re-exports of `onemessagebus_agent` types and `Redactor`, `Emitter` and `Reader` calls replacing the local
  implementations; `crates/onevcs/src/event.rs` and `stream.rs` declare no envelope, label, filter, matcher,
  redaction table or payload-bound of their own.
- The NDJSON a session stream and a release stream write on the e2e suite's journeys is byte-identical to the
  capture taken from the unchanged tree on the same journeys, timestamps and generated ids aside where those
  journeys already normalize them, and two processes appending to one release stream still produce no duplicate
  `seq` and no gap.
- Every filtered read the suite performs answers the same envelopes as before, `Phase::of` and `Phase::every`
  answer as at 0.21.0, and redaction over a payload holding every credential word and prefix produces the same
  bytes as at 0.21.0.
- `docs/contract.md`'s envelope section says the types are `onemessagebus-agent`'s and names that repository's
  contract as the source; every fenced block in it is driven by `tests/contract.rs`; no document in the tree still
  says a shared util crate is deliberately absent.
...
Release 0.21.0 is what this branch's base carries (`Cargo.toml`'s workspace version and `CHANGELOG.md`); every
"as at 0.21.0" above is measured off the unchanged tree in the worktree before the change, never off a registry.
Run the crate-scoped tests and the lint over your diff as you iterate.
...
{"onepipeline.adoption":"published","onepipeline.consumes":{"bus-ask-validators-codec":"crate"},"onepipeline.id":"onevcs-adopt-bus","onepipeline.max_turns":170,"onepipeline.persona":"engineer","onetaskgraph.item_kind":"task"}
-->
