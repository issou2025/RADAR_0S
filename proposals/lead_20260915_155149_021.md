...
**feat(status): add a queued status category and a verb that writes one task's status**

*Project description:*
## What

Two additions to onetaskgraph, shipped together in one release.
...
- **Position.** It sits between `todo` and `in-progress` wherever the vocabulary is listed in order.
- **The `todo` doc comment.** It currently reads "Queued, not yet started". Reword it so the two
  categories no longer describe each other.

Every place the vocabulary is spelled out today has to carry it. Measured at `v0.2.31`:

- `crates/onetaskgraph-plugin-api/src/work.rs`: the `StatusCategory` enum.
- `crates/onetaskgraph-github-projects/src/lib.rs`:
  - `CATEGORIES` and `category_position`;
  - `category_name`;
  - `shipped_default`, which should map `queued` to the board option `Queued`, the way `todo` maps to
    `Todo`;
  - `StatusMapping::resolve`, `column_for` and `resolved_target`, which need no new rule. The existing
    rule that one option cannot read back as two categories already covers `queued`.
- `crates/onetaskgraph-local-md/src/lib.rs`: `default_statuses` gains the word `queued`.
- `crates/onetaskgraph-linear/src/lib.rs`: `workflow_state_types`, `project_status_types` and `status`.
  - Linear has no queued state.
  - Choose the mapping that never lets a `queued` filter silently match items that read back as another
    category. Narrowing to nothing, as `draft` does, is the precedent.
  - Say why in the code, where the existing comments explain the other categories.
- `crates/onetaskgraph-in-memory`.
- `crates/onetaskgraph/src/cli.rs`: the `StatusArg` enum.
- `docs/plugin-protocol.md`: §4.5's `statuses` list, and §6, how a new wire value crosses the protocol.
- `docs/local-md.md`, and the README where it lists categories.
- The generated Python and TypeScript SDKs, regenerated from the schema the binary emits, never edited
  by hand.

**The protocol decision is yours**, under the rule §6 already states: a peer is never handed a shape it
was not written for without a refusal by name. Decide whether `queued` needs a protocol version, and
make the document and the implementation say the same thing.

### 2. A verb that writes only a task's status

```
onetaskgraph task status set <source>:<native-id> <category>
```

- **What it writes.** It sets the status of one existing task and nothing else. Title, content, labels,
  metadata, dependencies, project membership and comments stay exactly as they were.
- **Success.** Under `--json` it prints a document naming the qualified id and the resulting status as
  the store reads it back (`category` and `name`).
- **Failure.** Under `--json` it prints the same failure document every other verb writes, and it exits
  non-zero.
- **Refusals, each by name:**
  - an id that is not qualified;
  - an unknown source;
  - a task that does not exist;
  - a category disabled for that source, in the same wording a copy uses for that refusal;
  - a source that cannot write a status.
- **The plugin API.** Back the verb with a new `TaskSource` method whose default implementation refuses
  as unsupported, the way `write_task`'s default does (`crates/onetaskgraph-plugin-api/src/source.rs`).
  Document it in `docs/plugin-protocol.md` beside the other writes, and expose it in both SDKs.
- **Per plugin:**
  - **`local-md`** rewrites only the record's `status`, leaving the file otherwise byte-identical.
  - **`github-projects`** writes only what the category's target needs:
    - a column target sets the item's `Status` field option;
    - a closed target closes the issue with its state reason;
    - a column category written onto a closed issue reopens it, so a later `todo` reads back as
      `todo`.

    It never rewrites the issue's title, body or labels.
  - **`in-memory`** supports it.
  - **`linear`** either implements it or refuses it by name as unsupported, whichever that plugin's
    write support already implies.

### Who depends on this

onepipeline's settlement write-back will spawn this verb, and write `queued` through `project copy`, for
every node of a launched run and for every item a node names as the one it delivers. So the command
line and its JSON documents are the contract that consumer builds against. They are stated here and
restated there, and neither side changes them alone. If you find a better shape, keep building against
this one and put the proposal to your manager over `$ORCHESTRATOR_ASK_MANAGER`. The manager decides
whether the contract is amended.
...
Several managers share the same GitHub Projects boards, and an agent sent to pick up accepted tickets
selects `Todo`. Today an item a launched run has claimed still reads `Todo` until its node starts. The
store's seven categories have no room for "claimed, not yet started" without collapsing it into `Todo`
or `In Progress`, and the user wants those as separate columns. Separately, the tickets a run works live
on a second board, where the only write today is a total replacement from a source record. Moving a
ticket's status without owning its content needs a write that touches nothing else.

## Acceptance criteria

- **The category.** `queued` is a `StatusCategory`, spelled `queued` on the wire, in the emitted schema
  and in `--status`, and placed between `todo` and `in-progress` in every ordered listing of the
  vocabulary. That covers the plugin API enum, `github-projects`' `CATEGORIES`, the CLI's `StatusArg`,
  `docs/plugin-protocol.md` and both generated SDKs. Its documentation describes an item claimed by work
  that will do it and not yet started, and `todo`'s documentation no longer describes a queued item.
- **Local Markdown.** A record whose status is the word `queued`, in any letter case, reads as category
  `queued` with no configuration, and a task written at `queued` reads back as `queued`.
- **GitHub Projects mapping:**
  - The shipped default maps `queued` to the board option `Queued`.
  - `status_mapping.queued` accepts a board option name as the other column categories do.
  - A mapping that sends `queued` and any other category to one option is refused as configuration.
  - An open item at the `Queued` option reads back as category `queued` named `Queued`.
  - Writing `queued` to a board that lacks that option is refused, naming `Queued`.

  The crate's tests prove each of these against its HTTP fixture.
- **The other plugins.** The in-memory source stores and filters `queued`. The Linear source's handling
  of `queued` is explicit and commented, and its tests prove that a `queued` filter never returns an
  item that reads back as another category.
- **The protocol.** `docs/plugin-protocol.md` states how `queued` crosses the plugin protocol, including
  what a peer written against the earlier vocabulary is handed. The implementation does what it says,
  and a test holds it.
- **What the verb writes.** `onetaskgraph task status set <source>:<native-id> <category>` sets an
  existing task's status, and `task show` then reads that category back. It is proven through the real
  binary:
  - on a `local-md` source, where the record's file is byte-identical to its prior content apart from
    its status;
  - on a `github-projects` source against the crate's HTTP fixture, where the requests it makes are only
    the `Status` field update, or the issue close or reopen that the category's target requires. No
    request changes an issue's title, body or labels.

  In both cases the task's title, content, labels, metadata, dependencies, project and comments are
  read back unchanged.
- **Closed items on GitHub Projects.** Writing `done` or `cancelled` closes the issue with the state
  reason the mapping names. Writing a column category onto a closed issue reopens it, and the item reads
  back as that category.
- **The verb's JSON and refusals.**
  - Under `--json`, success prints a document naming the qualified id and the resulting `status`
    (`category` and `name`).
  - Failure prints the failure document the other verbs write, with a non-zero exit.
  - An unqualified id, an unknown source, a task that does not exist, a category disabled for the
    source, and a source that cannot write a status are each refused by name.
  - The disabled-category refusal uses the same wording a copy uses for the same refusal.
- **The plugin API and the SDKs.** The status-only write is a `TaskSource` method whose default refuses
  as unsupported, and it is documented in `docs/plugin-protocol.md`. The Python and TypeScript SDKs
  expose the verb as generated from the binary's schema, and the repository's SDK generation check
  passes over them.
- **The documents.** `docs/local-md.md`, the README, and the `github-projects` documentation of
  `status_mapping` name `queued`, its default option `Queued`, and the new verb.
- **Checks.** The tests of every crate this change touches, the CLI end-to-end tests that drive the new
  verb, the SDK generation check, and the repository's judged lint over this change's diff all pass on
  the finished tree.
- **Proof and claims.** The behavior this node adds is proven end to end by tests that drive the real
  binary or the real plugin against its fixture, not by inspection. Every claim the dispatch makes about
  the finished work, including what it verified and the evidence for it, is true of the tree as it
  finally stands.

## Additional info

**Where the release goes.** onepipeline's checks install this repository's `crate` target at a pinned
version, and this host installs the `pypi` wheel. Both publish from one version, so nothing here needs a
special release arrangement.

**What earlier work settled.** `github-projects` refuses `draft` because GitHub draft issues cannot have
sub-issues (`resolved_target`), and `unknown` is disabled by default. `queued` is an ordinary column
category like `todo`, not a special case of either.

**Where to start reading.** Line references above are from `v0.2.31`, so read the current tree before
relying on them. The status words a settlement write-back sends today, and why four of them classify as
`unknown`, are recorded in onepipeline's `docs/contract-divergences.md` §50. `queued` becomes the fifth
of its words that is a category.
...
{"onepipeline.id":"otg-queued-status","onepipeline.max_turns":200,"onepipeline.persona":"engineer","onetaskgraph.item_kind":"task","orchestrator.plan-review":{"by":"planning-closeout","key":"8884fc48a7a92c3bc8e03da0a36b68128e8c7e2d44597a043fe4da005cefea76","reviewed_at":"2026-09-15T12:09:08.879222+00:00"}}
-->
