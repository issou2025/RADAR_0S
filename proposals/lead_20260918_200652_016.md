...
**fix(core): fall through a clean-exit rate limit, honour a base bin override for a variant, and gate rustdoc**

*Project description:*
## What

Three accepted tickets against `crates/oneharness-core`, in one node because a stack of
three would buy no parallelism. One commit per ticket, the rustdoc one last because it
touches doc comments across the crate.

**1. A clean-exit rate-limit record stops the fallback chain** —
https://github.com/nickderobertis/oneharness/issues/1297, accepted at severity `high`.
`crates/oneharness-core/src/domain/fallback.rs::startup_failure_reason` permits
`FailureKind::RateLimit` fall-through only for `Status::Nonzero`, while `Auth` and `Quota`
fall through on `Status::Ok` too, so a provider-declared rate limit in a zero-exit record
stops the chain and leaves an identity with quota unused. Permit the rate-limit
fall-through for `Status::Ok` as well as `Status::Nonzero` when the run performed no
billable work — the `RunWork` the function already reads is the proof — and add the
clean-exit regression journey beside the existing fall-through journeys.

**2. `ONEHARNESS_BIN_<ID>` does not cover a variant selection** —
https://github.com/nickderobertis/oneharness/issues/1308, accepted at severity
`critical`. `crates/oneharness-core/src/io/detect.rs::BinOverrides::binary_for` derives
its env-var key by uppercasing and replacing `-` in the member id, and never touches the
`:` of a variant-qualified id like `claude-code:hooked`, so the derived key can match no
variable a caller can set and the real harness runs instead. The config-file lookup
`bin_for` (`domain/config.rs`) falls back from a variant to its base; the env path is the
only gap, and it is on the real dispatch path (`io/run.rs` preserves composed ids into
`detect::resolve_named`). In this run it meant a real, unbudgeted Claude turn ran inside a
test believed fully faked. Make the env-var lookup match the config-file semantics: a
variant-qualified selection is covered by `ONEHARNESS_BIN_<BASE>` — the base harness's
own key — with any variant-specific spelling the derivation can produce checked first,
and add a regression test asserting an `ONEHARNESS_BIN_<BASE>` override is honoured for a
variant-qualified selection of that base. Say in `docs/harness-usage.md` (or wherever the
override is documented) which key a caller sets.

**3. rustdoc warnings are not gated** —
https://github.com/nickderobertis/oneharness/issues/1306, accepted at severity `low`.
`cargo doc -p oneharness-core --no-deps` reports 41 warnings on today's `main` — public
items linking to private ones (`UsageReport` to `UsageReportWire`, `SpawnControls::cancel`
to `Finish::Terminate`, `run_job_streaming_cancellable` to `CANCEL_POLL_SLICE`) and
unresolved links — and neither `just check` nor `just gate` runs rustdoc. Repair the
public doc comments so every intra-doc link resolves in the public documentation build
(link the public concept, or describe the private item in words, rather than exposing a
private item to make a link resolve), and add the warnings-as-errors public documentation
build for `oneharness-core` to `just check` so a broken link cannot recur.

## Why

The user accepted these because the first two change what a dispatch on this host does
when a provider answers badly: an identity rate-limited on a clean exit stops the chain
where another identity had quota, and an override meant to fake every member of a harness
silently let a real, billed Claude turn run inside a test — the critical one. The third
keeps the published API documentation navigable and stops the drift the other two fixes
would otherwise add to. The first reaches this host only through the engine release that
relinks `oneharness-core`, which a sibling node of this plan does.

## Acceptance criteria

- A provider error record classified `rate_limit` with `Status::Ok` and no billable work
  falls through to the next identity exactly as a `quota` or `auth` record does, a
  `Status::Nonzero` rate limit still falls through, and a record proving billable work still
  stops the chain.
- A variant-qualified harness selection honours the `ONEHARNESS_BIN_<BASE>` override of
  its base harness, matching the config-file `bin` fallback; when both a variant-specific
  override and the base override are set for one selection, the variant-specific one wins;
  and the documented override keys are the ones the code reads.
- The public documentation build of `oneharness-core` with warnings as errors is clean, and
  the deterministic check tier runs it so a new unresolved or private-item link fails the deterministic
  tier.
- No private item is made public merely to make a link resolve.
- The branch's history carries, for each of these tickets, a closing reference naming its
  issue (1297, 1308, 1306), so the change that carries the work is what closes it.
- Every behaviour this change adds is proven by a test or journey driving the real fallback
  and detection code paths — a clean-exit rate-limit record through the chain, a
  variant-qualified selection under a base override — rather than by inspection, and no
...
- The tests that cover the modules this change touches are green over the finished tree,
  and so are the judged lint over this change's diff and the repository's workflow lint
  (the deterministic checks over its scripts, workflows and generated documents).
- The finished tree carries, as its own `test`-typed commit changing `tests/fixtures/codex-rate-limits.schema.json` alone, the snapshot of `v2/GetAccountRateLimitsResponse.json` as codex-cli 0.154.0 generates it: the checked-in snapshot is byte-for-byte what that release's generator emitted at the time of the refresh, which is the completeness the repository's codex-usage-schema check defines and compares; whether a codex-cli release newer than 0.154.0 later diverges from that snapshot is that check's later comparison and outside this node's bar. The parser's field test `codex_schema_snapshot_still_declares_every_field_the_parser_reads` passes against the refreshed snapshot.
- Every claim this dispatch makes about the finished work is true of the tree as it finally
  stands.

## Manager amendment (wins over ## Additional info where they disagree)

Beside the three tickets, the finished tree carries the refresh of `tests/fixtures/codex-rate-limits.schema.json` for the codex-cli release installed on this host, as its own `test`-typed commit limited to what the repository's own codex-usage-schema check prescribes, with the parser's field test passing against the refreshed snapshot and the repository's workflow lint green. This refresh is in scope and is not a departure from the tickets. The copilot-login-probe timing bound is enforcement configuration outside this node: if the merge path refuses the push on that check alone, say so in the report rather than touching the check.
...
Iterate with this repository's diff-scoped judged lint, the core crate's unit and
fallback journeys, and the rustdoc build rather than the whole gate; the required checks
run downstream on the change this branch becomes. `docs/testing-patterns.md` and
`docs/mock-spy-design.md` say how a fake harness binary is stood up in this repository's
journeys.

This repository's releases are cut from conventional commits, and a sibling node of this
plan relinks the `core` target that carries the first two fixes into the engine this host
runs, so a commit type that cuts no release would leave that node waiting. `fix` is the
type for all three.
...
{"onepipeline.branch":"onevcs/s-f8a05f3dc282","onepipeline.execution_checkout":"nickderobertis__oneharness","onepipeline.id":"oh-core-3","onepipeline.persona":"engineer","onepipeline.resume":{"branch":"onevcs/s-f8a05f3dc282"},"onetaskgraph.delivers":["followups:I_kwDOS0pFSM8AAAABRp2G9w","followups:I_kwDOS0pFSM8AAAABR7arYQ","followups:I_kwDOS0pFSM8AAAABR1MtPg"],"onetaskgraph.item_kind":"task"}
-->
