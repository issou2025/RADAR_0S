...
**feat(writeback): bound the settlement copy per item, and let a launch set the per-item budget**

*Project description:*
## What

Make the settlement write-back's `project copy` deadline a **per-item budget multiplied by
the item count the call site already holds**, and make the per-item figure a launch-level
setting of `onepipeline start` with the shipped value as its default — so the liveness
backstop scales with the plan instead of being outgrown by it. Issue
https://github.com/nickderobertis/ai-orchestrator/issues/855 §1 is the specification; this
node delivers its "Accepted fix" and nothing wider.

### What the code does today

`src/writeback.rs` bounds every store command the worker spawns with one constant:

```rust
const COMMAND_LIMIT: Duration = Duration::from_secs(60);
```

enforced in `bounded_output` as a **total wall-clock** deadline — the child is killed and
`"{name} exceeded {} seconds"` is returned — for three commands: `project-show`,
`task-list` (one call per page), and `project-copy`. Only the copy is linear in plan size:
it writes one item per node, and the snapshot it copies already knows how many —
`snapshot.nodes` is the map the `Unprojected` surface reports as `items:` when the copy
fails. A run of 34 items exceeded the 60 s and its settlement never reached the board.

### The contract this node lands

State this literally, once, and hold every copy to it — the details are below so that the
consumer of this setting (this host's `just orchestrate`, and any `adopt`) and its producer
restate it from here rather than decide it again.

- **A launch-level setting, nameable three ways**, resolved once at launch in this order and
  retained in the launch record so `adopt` replays what its launch resolved:
  1. the flag `--writeback-item-budget <SECONDS>` on `onepipeline start`;
  2. the environment variable `ONEPIPELINE_WRITEBACK_ITEM_BUDGET`;
  3. the launch-config key `writeback_item_budget`, arriving at launch-config
     **schema version 5** (`LAUNCH_CONFIG_SCHEMA_VERSION` moves from 4 to 5; the versions
     read keep 4, 3, 2 and 1; a document declaring an earlier version while carrying the key
     is refused **by the key's own name**, and a key present and blank is refused by name
     too — `BlankValue::Refused`, as the two hook keys are);
  4. the shipped default, **10 seconds per item**.
- **The value is a positive whole number of seconds.** Zero is refused wherever it is
  read — flag, variable, config key — naming the spelling that carried it, because a budget
  of zero kills every copy; it never falls through to the rung below.
- **The copy's deadline is `max(60 s, budget × item count)`**, where the item count is the
  number of nodes in the snapshot being projected — the same list the `Unprojected` surface
  names. The 60 s stays as the floor, and stays the whole deadline for `project-show` and
  each `task-list` page, which are not linear in plan size. Keep the comment beside the
  constant saying it is a liveness backstop for an unreachable store and not a latency
  target; that is still what it is.
- **The refusal names the budget it computed and the count it multiplied by**, so the
  one line on the driver's stderr and the `Unprojected` reason built from it say, for
  instance, `project-copy exceeded 340 seconds (34 items × 10 seconds per item)` — and,
  where the floor governed, say so: `project-copy exceeded 60 seconds (the 60 second floor;
  2 items × 10 seconds per item is less)`. Nothing
  else about how the failure is reported changes: the issue does not ask for the failure to
  be surfaced anywhere new, and this node does not widen into that.
- **`LaunchRecord` carries it** as a `u64` field defaulting to `0` on read, with an accessor
  answering `None` for `0` — the shape `heartbeat_interval` / `pacemaker_interval` already
  take — so a record written by an older build still reads and resolves to the default.

### How this repository requires an interface change to land

`docs/contract.md` is the approved contract, committed **verbatim**, and is never edited to
match the code. A new flag and a new config key are an interface change, and the way this
repository lands one is the way divergence entries **41** (`node_validator`) and **45**
(`envelope_reviewer`) did — follow them exactly:

- Add a new **OPEN** divergence entry at the end of `docs/contract-divergences.md` (the next
  number after the last one there), opening with the proposal for the planner who owns the
  contract, stating what the code does today and why (the incident above), and closing with
  a fenced JSON block that is **the one source** every copy of this contract is held to. Give
  it the shape entry 45's block takes — an object naming `flag`, `environment`,
  `config_key`, `config_schema_version`, `precedence`, and add `default_seconds`,
  `floor_seconds`, and one worked example of the computed deadline (item count, budget,
  resulting seconds, and the refusal text) — and update the record's opening paragraphs that
  list which entries are open.
- Extend `tests/contract.rs` with a test modelled on
  `the_envelope_reviewer_surface_is_what_the_divergence_record_names`: read the block through
  `divergence_block`, assert the flag is one `start` parses, the config key parses at the
  stated version and is `None` at every earlier version this build reads, the environment
  name is the constant the engine reads, the precedence list is the order the driver
  resolves, and the default and floor are the constants the code carries.
- Add an e2e under `tests/e2e/` (a module beside `node_validator.rs`, wired into
  `tests/e2e/main.rs`) that reads the three spellings out of that same block and drives
  them against the real binary: the precedence between flag, variable and config key
  observable on the launch record `adopt` replays, and the behaviour itself through the
  store double `crates/testfakes/src/bin/fake-onetaskgraph.rs` (extend it with a scripted
  hold on `project copy`, the way `onetaskgraph.refuse-reads` scripts a refusal) — a copy
  that outlasts the 60 s floor still lands under a budget the item count lifts above it,
  and a copy held past what a deliberately tiny budget allows (which the floor then
  governs) is killed and reported with the computed budget, the item count, and the floor
  in the refusal. Both journeys wait past 60 s by construction; `tests/e2e/store.rs` is
  where the existing write-back journeys live and how they wait for minutes-long
  schedules; reuse its helpers and its rendezvous settings.

Read this repository's `AGENTS.md`, `tests/AGENTS.md` and `src/AGENTS.md` first; each is
binding here. The releases are cut by `release-plz` from the squash subject, so this node's
title is a `feat` and cuts a minor release on its own; no node performs a release.

## Why

The write-back is the only thing that keeps the board in step with a run, and the backstop
is the thing that kills it. `root-causes-539-fixes` settled 18/18 with every node landed and
its settlement never reached the board: the run's own surface read *reason: project-copy
exceeded 60 seconds*. Write-back is best-effort and off the reconcile loop, so the run
settles identically whether or not the projection landed, and the only signal is one line
on a detached driver's stderr that nobody opens. A green run already proves nothing about
the board; this is the mechanism that makes that true more often as plans grow. The user
asked for what remains of #855 to be dispatched, and every one of its four entries remains.

## Acceptance criteria

- The `project copy` the write-back worker spawns is bounded by `max(60 s, budget × item
  count)`, with the item count taken from the snapshot being projected, while `project show`
  and each `task list` page stay bounded by the fixed 60 s; when the copy is killed, the
  reason names the seconds it was allowed, the item count, the per-item budget, and
  whether the floor was what governed.
- The per-item budget is nameable as the flag `--writeback-item-budget`, the environment
  variable `ONEPIPELINE_WRITEBACK_ITEM_BUDGET`, and the launch-config key
  `writeback_item_budget` at launch-config schema version 5; the flag beats the variable,
  the variable beats the key, and the shipped default beneath all three is 10 seconds; a
  value of zero and a blank config key are refused by name at the spelling that carried
  them; a launch config declaring version 4 or below while naming the key is refused by the
  key's name, and a config of any version this build reads that omits the key still loads.
- The resolved value is retained in the launch record and is what an adopted run uses; a
  launch record written without the field reads and resolves to the default.
- `docs/contract.md` is unchanged. A new open divergence entry in
  `docs/contract-divergences.md` states the proposal, the incident, and a fenced JSON block
  naming the flag, the environment variable, the config key, the schema version it arrives
  at, the precedence, the default, the floor, and a worked example of the computed deadline;
  the record's opening paragraphs list the new entry among the open ones.
- A test in `tests/contract.rs` holds that block against the types: the flag parses on
  `start`, the key parses at the stated version and at no earlier one, the environment name
  and the precedence are the ones the driver resolves, and the default and the floor are the
  constants the code carries.
- An e2e in `tests/e2e/` drives the real binary against the store double: a `project copy`
  that outlasts the 60 s floor lands when the item count lifts the budget above it; a copy
  held past what a deliberately tiny budget allows is killed and reported with the
  computed budget, the item count, and the floor that governed; and the precedence
  between the three spellings is observable on the launch record. The double's new
  scripted hold is documented in its module comment beside its siblings.
- The checks that exercise this change are green over the finished tree:
  `tests/contract.rs`, the unit tests of the write-back and launch-config modules in
  `src/`, the e2e modules this node adds or touches, the repository's clippy over all
  targets, and the judged lint over the node's diff — with nothing skipped and no
  suppression added to make any of them pass.
...
**Read the consumer before you settle the surface.** `ai-orchestrator`'s `just orchestrate`
(`justfile` and `scripts/` in that repository) is what passes flags to `onepipeline start`,
and it names none of this today; the default is what it will run under after adoption, which
is why the default and the floor are the two numbers this node has to get right.

**The `bounded_output` change is the whole behavioural fix; keep it that size.** Thread a
`Duration` into `bounded_output` (or give the copy its own computed deadline) rather than
rewriting the worker. `Writeback::start` already receives the `LaunchRecord`, which is where
the resolved budget rides to the worker thread.

**Precedent to copy, not to reinvent:** `driver::start`'s resolution of `node_validator` (the
flag, then `engine::configured_node_validator()`, then `declared.node_validator`), the
`KEYS_BY_VERSION` table and `LAUNCH_CONFIG_SCHEMA_VERSIONS_READ` in `src/filter.rs`, and
`LaunchRecord`'s `heartbeat_interval` field with its `pacemaker_interval` accessor in
`src/ledger.rs`. `tests/e2e/node_validator.rs` and `tests/e2e/envelope_reviewer.rs` read
their spellings out of their divergence blocks; do the same.

**The checks to run, and their scope.** `just --list` names the recipes. What exercises
this change is `tests/contract.rs`, the unit tests in the `src/` modules you touch, and the
e2e binary (`just test-e2e`, or a `cargo nextest` filter over the modules you add while
iterating), plus the repository's clippy over all targets and the diff-scoped judged lint.
The whole offline tier and the credentialled `smoke-real` are the merge path's to run, not
yours.

**A shared interface is never changed unilaterally.** The surface above is what this plan's
adoption node and this host's launcher will restate. If you find a departure you want — a
different spelling, a different default, a per-command budget — surface it as a proposal in
your report and keep building against the surface stated here; the manager decides whether
to amend the contract.

**Commit on the branch the session opened**, as a `feat(writeback): …` subject; publication
is the lifecycle's after you settle.
...
{"onepipeline.id":"op-writeback-budget","onepipeline.persona":"engineer","onetaskgraph.item_kind":"task"}
-->
