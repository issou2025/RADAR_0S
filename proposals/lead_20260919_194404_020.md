...
**feat(scheduler): hold a node for workspace capacity per identity, and requeue on a pool refusal**

*Project description:*
## What

Adopt the `onevcs` release that carries the worktree pool (this task's cross-repository
references block names the landing and the release; `onevcs-testing` is cut from the
same workflow and moves beside it, in this repository's own pinning voice — read the
root `Cargo.toml`'s comments on the sibling floors and `just engines-current`), and
teach the scheduler three things about it. Nothing about git enters this crate; every
call is the linked library's.

**1. The capacity read, per identity, before a queued node comes forward.** Where
`start_ready` would dispatch a lifecycle node (one with a `repo`), it first asks
`onevcs::workspace_capacity(&request)` with the same `SessionRequest`
`vcs::request_for` composes for it. `admitted: false` holds the node this pass — it is
skipped, not `break`-ed past, so nodes of other identities behind it still start — under
a new hold reason beside the four in `HoldReason`:

```rust
HoldReason::Workspace { identity: String, pool: u32, slots: u32, idle: u32,
                        maintaining: u32, overflow: onevcs::Bound, overflow_in_use: u32 }
// node-held entry: {"kind": "workspace", "identity": ..., "pool": ..., "slots": ..., "idle": ...,
//                   "maintaining": ..., "overflow": "unlimited"|N, "overflow_in_use": ...}
```

It sits **beside** the executor's host-wide `capacity()` (`slots_free`, load, memory) and
is never folded into it. A read that fails — an unregistered identity, an unreadable
file — holds nothing: the read is advisory and `open` is authoritative. A held node
raises a **non-blocking** surface of kind `workspace-wait`, source proposal, workstream
the node, on the cadence `release-wait` uses, saying the identity admits no more
sessions now, the numbers, that `onevcs pool status <identity>` names the holders, and
that nothing times it out. While any node is held this way the loop re-reads capacity on
a paced timer (a session another run closes is not an event this run sees) and on every
pass, and the hold clears the pass the identity admits it. `status` renders the hold
like the other holds; `holds_now` reports it; `HoldReason::of_payload` reads it back.
Within one pass, the loop decrements its own view of `admits` per dispatch it makes on
an identity, so two nodes of one identity in one pass do not both leave on one reading.

**2. The typed refusal returns the node to queued.** `onevcs::Error::PoolExhausted` at
session open is classified off the typed value in `vcs::session_refusal` — a head marker
on the `Error::Sibling` message, exactly as `SESSION_OPEN_CONFLICT` is composed and read
— and `vcs::session_open_exhausted(&Error)` reads it. In `engine::attempt`, an exhausted
refusal ends the attempt loop at once: no backoff, no boundary attempt spent, no
`no-agent-progress`, no `infrastructure-failure`. The dispatch thread reports it as a
message of its own (the way `SessionConflicted` travels), and the loop takes the node
out of `in_flight` **without a settlement**, journals the refusal against the node as a
pipeline record you name in the contract (a requeue carrying `reason:
workspace-exhausted` and the refusal's text is the suggested shape), holds it under
`workspace`, and re-evaluates it on the next pass. A run whose only remaining nodes are
held this way is not terminal and not `awaiting-planner`: it is waiting on the host.

**3. Node overrides pass through.** Two additive schema-3 plan-node fields, refused by
name below schema 3 the way `draft` is: `pool: Option<u32>` and `overflow:
Option<onevcs::Bound>` (`onepipeline.pool` / `onepipeline.overflow` in task metadata),
copied onto `SessionRequest.pool` / `.overflow` in `vcs::request_for` and carried by the
per-step request in `lifecycle.rs`. `onepipeline` adds no vocabulary of its own: the
types and their meanings are the sibling's (`--pool 0` still counts against
`overflow`; `--overflow unlimited` opts out of the cap).

**Docs**: reconcile `docs/contract.md` — the schema-3 field list and reserved metadata
keys, the hold reason and surface kind where the others are named, the launch/attach
paragraph's "nothing else able to move" against a workspace hold — and add the
divergence-log entry in the log's own shape recording what this build does beyond the
approved text; `tests/contract.rs` parses that log's source blocks, so give it one.
§14 of that log may note that the sibling's record-based reclamation and the pool's
never-reclaimed slots close the destruction it records; resolving it is its owner's.

## Why

The user's words: the pool should let "you trade off disc wear for not blocking work"
and "have a small overflow to prevent blocking work". A finite overflow means an
identity can be full; a scheduler that dispatched anyway would spend a node's whole
boundary budget on refusals and settle it `failed` for a host being busy, and one that
stalled silently would read as a hung run. So the read holds the node with a reason a
person can see, and the refusal that can still race the read costs the node nothing.
...
- The workspace requires the adopted `onevcs` release (and `onevcs-testing` beside it)
  ...
  check confirms.
- With a pooled identity whose resolved policy admits one session, two lifecycle nodes
  on it under `concurrency: 2`: the first dispatches, the second is held under
  `workspace` with the identity and the pool and overflow numbers in its `node-held`
  entry, a `workspace-wait` surface is raised
  non-blocking naming the identity and the numbers, `status` shows the hold, and when
  the first's session closes the second dispatches and completes — driven through the
  real linked `onevcs` under a scratch state root with a `workspaces.yml`.
- A node whose identity's capacity read fails is dispatched as before; a node of
  another identity behind a held node starts in the same pass; two nodes of one
  identity in one pass do not both leave on one reading.
- A session open refused `PoolExhausted` returns the node to queued: no boundary
  attempt is spent, no settlement is written, the refusal is journalled against the
  node in the recorded shape, the node is held under `workspace`, and it dispatches
  once the identity admits it — driven with a dispatch-env hook that opens a real
  session on the identity's last slot before the driver's own open.
- `pool` and `overflow` on a plan node reach the session request the linked sibling
  receives, and are refused by name on a plan declaring an earlier schema.
- `docs/contract.md` and `docs/contract-divergences.md` state the fields, the hold
  reason, the surface kind and the requeue record, and `tests/contract.rs` holds the
  entry's source blocks.
...
- This node adopts `published`: the engine held it until the `onevcs` crate was on
  crates.io, and the reference block rendered into this task names the version. Take the
  pins from there; the adoption instruction the producer declares is rendered with it.
- This plan is the planner's ruling on the contract for these surfaces; record the
  amendment and the divergence entry rather than reporting a conflict. A shared interface
  is never changed unilaterally once this plan runs: `HoldReason::Workspace`'s payload,
  the surface kind and the two node fields are what this host's adoption node documents
  — raise any departure with `"$ORCHESTRATOR_ASK_MANAGER"` and keep building.
...
- The dispatch-env hook runs before any session is opened for a dispatch and is a
  script the journey writes, so a hook that runs `onevcs session open` against the
  scratch state root is the outside opener that makes the refusal deterministic;
  `tests/e2e/dispatch_env_hook.rs` and `tests/e2e/real_vcs.rs` are the precedents.
- `tests/e2e/lifecycle.rs`'s session-conflict journey is the model for the refusal
  path; `src/release.rs`'s `WAIT_SURFACE_KIND` and its surface text are the model for
  `workspace-wait`; `holds_now` and `report_holds` are where holds are diffed and
  written.
...
{"onepipeline.adoption":"published","onepipeline.consumes":{"onevcs-pool-placement-2":"crate"},"onepipeline.id":"onepipeline-workspace-capacity","onepipeline.max_turns":240,"onepipeline.node":"onepipeline-workspace-capacity","onepipeline.persona":"engineer","onetaskgraph.item_kind":"task"}
-->
