...
**feat(config): adopt per-node graph overrides on this host**

*Project description:*
## What

Adopt the published onepipeline wheel carrying this plan's engine change by moving `config/onepipeline.version`, updating the installed dependency lock and any sibling CLI pins the wheel's SBOM requires. Take the release from this node's cross-repository reference and release note, not from a tag list. Move `config/onepipeline-ui.version` to the published `onepipeline-api-cli` wheel produced by `opui-relink-overrides`, which links that same engine, as `tests/test_linked_libraries.py` requires. Take its release from the dependency reference and release note; do not select an unrelated UI tag or declare a UI-engine divergence. If the matching release cannot be identified, surface the blocking release need to the manager. Do not add a host-side workaround.

Update `docs/onejudge-integration.md` to explain the working method in one place: use a small oneharness `extends` child file to change an identity/role without copying whole identity sections; point a node's `onepipeline.sets` at that file and, where needed, set the agent or judge model path on the graph; use planner-channel `set-node-sets` or `set-run-node-sets` to replace the ordered list for the next dispatch. State that a node list reaches all its agent steps, a custom `agent_graph` must accept its paths, a live turn and dag-scope observer retain their launch configuration, and the pr-author draft dispatch receives none. Replace the stale advice that suggests hand-copying whole identity sections. Show launch-wide file/environment/flag spellings and the accepted CLI > environment > file precedence. Keep paths relative to the declaring file according to oneharness `extends`.

These are the same producer contracts, not a new host API: node `sets` / task metadata `onepipeline.sets` default `[]`; `set-node-sets {id,sets}` and `set-run-node-sets {sets}` replace ordered lists; launch-config `node_sets` and `dag_sets` are arrays; `ONEPIPELINE_NODE_SETS` and `ONEPIPELINE_DAG_SETS` are JSON arrays; invalid graph paths are refused at plan load or live edit. Do not change these shared contracts unilaterally. Derive examples from the released onepipeline `docs/contract.md`, and keep the host docs and executable example tests reconciled with that source.

## Why

The user wants different agents, and agents in an already running run, to receive different oneharness configs and models without changing shared config files. The host must install the engine that implements that method and document how managers use it. The UI pin must agree with the installed engine so a browser adoption runs the same release.

## Acceptance criteria

- The committed `config/onepipeline.version` contains the exact nonempty version supplied when the published dependency hold releases, with the package declaration and lock updated to that version. `config/onepipeline-ui.version` contains the exact nonempty version supplied when the published UI dependency hold releases, linking the same engine, and any sibling CLI pins required by the wheel's bill of materials are updated. These files are the complete host adoption record; `tests/test_linked_libraries.py` and the affected pin/bootstrap tests later compare the committed content with the installed wheels and registry. Their outside comparison is not this criterion's bar.
- `docs/onejudge-integration.md` gives a complete, executable example using a short `extends` child, task metadata `onepipeline.sets`, the two channel edit payloads, and launch-wide flag/file/environment forms with their precedence. It explains next-dispatch scope, custom-graph refusal, and the unchanged observer and pr-author paths, without the old whole-identity copy advice. A host e2e journey drives the installed onepipeline CLI with the documented `extends` child and plan metadata, then submits both documented channel edits and observes the changed harness config or model on the affected next dispatch. It also exercises the documented launch-wide file, environment and flag spellings and their precedence.
- The affected host tests and lint over this diff are green on the finished tree. Every claim in the completion report about the change and its verification remains true of that final tree.

## Additional info

This is the host adoption node; its `published` dependencies and the host release override supply the onepipeline and onepipeline-api-cli wheel targets, so it declares no `consumes`. The host's `AGENTS.md` governs both the dispatch pin and the linked UI pin. Use the released contract as the reference for examples.

The interfaces in this task are shared contracts. If implementation reveals a needed change to a field, edit payload, or precedence rule, propose it over the planner channel and keep building against the agreed contract until the manager rules; do not change it unilaterally. The onepipeline source definitions and contract fixtures must make every restatement drift-visible.
...
{"onepipeline.adoption":"published","onepipeline.execution_checkout":"ai-orchestrator-isolated","onepipeline.id":"aio-adopt-overrides","onepipeline.persona":"engineer","onetaskgraph.item_kind":"task","orchestrator.plan-review":{"by":"review-plan","key":"e76bc1cb747cb2e8b7f1bba2089ef80515ea35695a6debe2203c21c2b73b97a5","reviewed_at":"2026-09-24T06:20:45.486817+00:00"}}
-->
