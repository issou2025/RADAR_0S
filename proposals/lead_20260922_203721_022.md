...
**test(sdk): let the framework's hang timeout bound the TypeScript SDK's real-binary journeys**

*Project description:*
## What

https://github.com/nickderobertis/onetaskgraph/issues/2279. `sdks/typescript/tests/generator.test.ts` overrides the per-test limit
to 30 seconds on its real-binary journeys (`}, 30_000);` at 100, 137, 185, 280 and 321), and
the copy journeys in `sdks/typescript/tests/client.test.ts` (`:291`, the document copy at
`:404`) run under Bun's default 5-second per-test limit with no override at all
(`sdks/typescript/project.json:71` runs plain `bun test`; there is no `bunfig.toml`). Their
wall time varies with compilation and host load, so these limits expire before anything is
actually stuck. Remove the 30-second overrides, and let one framework-level hang timeout —
set once for the suite (a `bunfig.toml` `[test] timeout`, or the test target's
`--timeout`), sized to terminate a genuinely stuck test — bound every real-binary journey,
including the copy journeys. Keep correctness in the subprocess assertions; encode no
expected performance per test.

## Why

Unrelated publications fail after long gates whenever a loaded host delays an
otherwise-correct subprocess, and the only workaround is rerunning the whole publication
gate on a quieter host.

## Acceptance criteria

- No test in the TypeScript SDK suite sets its own per-test time limit, and one suite-level
  hang timeout, stated once in configuration with a comment saying what it bounds, applies
  to every real-binary journey, the copy journeys included.
- A test through the real Bun test runner, under the suite's own configuration, proves two
  things: a successful subprocess delayed past the old 5-second limit completes and
  passes, and a genuinely stuck test is terminated by the shared hang timeout. It also
  proves the copy journeys run under that policy.
- The TypeScript SDK's test target is green over the finished tree, and so is the judged
  lint over this change's diff.
- The branch carries GitHub closing metadata associating it with issue #2279 of this repository and with no other issue; whether the issue actually closes is decided downstream when the change lands, and is not this dispatch's bar.
- Every claim this dispatch makes about the finished work is true of the tree as it
  finally stands.

## Additional info

onetaskgraph's `pre-push` hook runs `just gate` over the affected projects; its CI matrix covers ubuntu, macos and windows. Put `Closes #2279` in the body of the commit that resolves each ticket, so the landed change closes it; the issues live in this repository. onetaskgraph releases from conventional commits through release-plz; this is a test-only change nothing on this host adopts, so `test` is the honest type.
...
{"onepipeline.execution_checkout":"nickderobertis__onetaskgraph","onepipeline.id":"otg-ts-sdk-timeouts","onepipeline.max_turns":12,"onepipeline.node":"otg-ts-sdk-timeouts","onepipeline.persona":"engineer","onetaskgraph.delivers":["followups:I_kwDOUBBgxs8AAAABSNuPTA"],"onetaskgraph.item_kind":"task"}
-->
