...
**feat(recoverable): scope the listing to one identity with --repo**
...
Close `nickderobertis/ai-orchestrator` issue #1004 root cause 9: `onevcs recoverable` is
scoped only by working directory. `RecoverableArgs` (`crates/onevcs/src/cli.rs:338-348`)
carries `--all` and `--json` alone, and the scope comes from `resolve_here`
(`crates/onevcs/src/app.rs:1454-1473`) walking up the working directory, while
`publish-branch` takes a required `--repo <PATH>` (`cli.rs:282-284`) resolved through
`store::resolve_path` (`store.rs:510-519`).

**The contract** (pinned by `aio-adopt-releases` and consumed by `aio-recoverable-repo`, whose `just recoverable` forwards every
argument untouched to this verb; state it in `docs/inferred-surface.md`'s `recoverable` usage
block, which `crates/onevcs/tests/contract.rs`'s flag drift gate reads):

```
onevcs recoverable [--repo <PATH>] [--all] [--json]
```

- `--repo` accepts exactly what `publish-branch --repo` accepts — a registered checkout alias,
  a path inside a registered checkout, an identity key, or a normalized origin — resolved by
  the same resolution, so the two verbs can never disagree about what a value names.
- With `--repo`, the listing is scoped to that one identity whatever the working directory is,
  and the first human line is the single-identity line naming that identity; its wording no
  longer claims the command was run in the checkout when the scope came from `--repo`. With
  `--json`, the scope line goes to stderr as today, and `recover_command` and every other
  published field are unchanged.
- A `--repo` value that resolves to nothing exits non-zero naming the value, and lists
  nothing.
- Without `--repo`, behaviour is exactly today's: scoped by working directory inside a
  registered checkout, every identity elsewhere.
- `--repo` composes with `--all`, which keeps its current meaning.

## Why

Supervising the #855 run meant reading which verb a finished `onepipeline` branch needed from
the `ai-orchestrator` session, and the only way to scope `recoverable` was `cd` — from `/tmp`
it answered for 245 branches across every identity. The `just recoverable` wrapper, whose
re-rendering of each resume command in `just` form is what keeps a pasted command from opening
a change request with an empty description, cannot be reached from another checkout. The
operator wants one command, from where they already are, that answers for the repository they
name.

## Acceptance criteria

- `onevcs recoverable --repo <value>` lists exactly the preserved branches of the identity
  `<value>` resolves to, from a working directory outside every registered checkout and from
  inside a different identity's checkout, for each of the four spellings `publish-branch
  --repo` accepts — proven end to end against the real binary. `--repo <value> --all` lists, for
  that identity alone, exactly what `--all` lists when run inside that identity's checkout without
  `--repo`, also proven end to end.
- Its first human line names that identity and does not claim the command was run in that
  identity's checkout. With `--json`, that scope line is written to stderr, stdout carries the
  JSON document alone, and its fields are unchanged.
- An unresolvable `--repo` exits non-zero naming the value and lists nothing.
- Without `--repo` the listing and its first line are byte-for-byte what they are today, in
  both scoping cases, and the existing `recoverable` journeys stay green.
- `--repo` appears in the `recoverable` usage block the contract drift gate reads, and that
  gate passes.
- The tests covering the changed CLI and app code pass over the finished tree, and the judged
  lint over this node's diff leaves no finding unanswered.
- Every claim the dispatch makes about the finished work is true of the tree as it finally
  stands.

## Additional info

- Read first: `AGENTS.md`, `docs/contract.md` (how a CLI change is recorded, line 1014-1016),
  `docs/inferred-surface.md` (its `recoverable` usage block), `crates/onevcs/src/cli.rs`,
  `app.rs` (`recoverable` rendering at about 683-767, `resolve_here`), `vcs.rs:315-316`,
  `store.rs` `resolve` / `resolve_path`.
- Tests nearest this change: `crates/onevcs/tests/e2e/edges.rs:1665`
  (`recoverable_answers_for_the_repository_it_is_run_in`), `tests/e2e/landing.rs`,
  `tests/e2e/lifecycle.rs:2727`, `tests/contract.rs`'s flag drift gates, `tests/e2e/cli.rs`,
  and `scripts/smoke-published.sh` if it asserts flags. `just test-one <name>`;
  `just lint-llm-diff <base>`.
- The contract above is consumed by another node. If you find it wrong, surface a proposal to
  the manager over `$ORCHESTRATOR_ASK_MANAGER` and keep building against it.
- The title must stay a releasing type: this host adopts it through the CLI wheel's release.
...
{"onepipeline.id":"onevcs-recoverable-repo","onepipeline.max_turns":130,"onepipeline.persona":"engineer","onetaskgraph.item_kind":"task","orchestrator.plan-review":{"by":"review-plan","key":"3eaaf7cde31ccd035bb9a67228268d78c9241ca2dba8ca835834d45d3da57cbc","reviewed_at":"2026-09-13T17:32:12.576476+00:00"}}
-->
