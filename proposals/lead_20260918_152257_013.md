...
**fix(lint): load the Bash rules and the renamed project-graph plugin in llmlint.yml**

*Project description:*
## What

Two accepted tickets against this repository's root `llmlint.yml`, one node because a
split would buy nothing. One commit per ticket, and the plugin-path one first because
nothing validates until the configuration loads.

**1. The configuration pins a removed plugin path** —
https://github.com/nickderobertis/skilltest/issues/39, accepted at severity `high`.
`llmlint.yml` still loads
...
which the producer renamed to `project-graph.llmlint.yml`, so both the judged lint and
`llmlint validate` fail while loading configuration and no change can receive its lint
verdict. Replace the stale URL with
`https://raw.githubusercontent.com/nickderobertis/dero-skills/main/skills/bootstrap/create-repo/assets/llmlint/project-graph.llmlint.yml@1`,
and add a configuration-validation check that resolves every remote plugin URL. The
`llmlint` job of `.github/workflows/ci.yml` already runs `just lint-llm-validate
--diff-base origin/main` before the judge, scoped to the diff; what is missing is a
validation of the whole configuration that runs whatever the diff reaches — every plugin
URL resolved, every directive's rule known — so a plugin renamed upstream, or a directive
naming a rule no plugin defines, fails a check that always runs rather than one a
diff-empty run skips. Put it where this repository's deterministic checks live (the
workflow's lint job, and the `just` recipe a contributor runs), and say where. The ticket
names three other repositories carrying the same stale path; they are outside this node,
and the manager has them.

**2. A directive references an unloaded Bash rule** —
https://github.com/nickderobertis/skilltest/issues/38, accepted at severity `high`.
`scripts/setup-llmlint.sh`'s file-scoped directive suppresses `robust_shell`, but
`llmlint.yml` does not load the Bash language plugin that defines it, so once plugins
load, `llmlint validate` rejects the repository with `unknown rule "robust_shell"` before
any judged lint can run. Add
`https://raw.githubusercontent.com/nickderobertis/dero-skills/main/skills/bootstrap/create-repo/assets/llmlint/languages/bash.llmlint.yml@1`
to the plugin list and keep the existing directive, so the script is validated under the
rule it deliberately suppresses.

## Why

The user accepted both at severity `high` because together they mean no change to this
repository can get its judged lint verdict at all: the configuration fails to load, and
once it loads it fails validation on a rule nothing defines. A lint tier that cannot run
is a required check that either blocks every publication or, worse, is worked around.

## Acceptance criteria

- The root `llmlint.yml` loads every plugin it names, including the renamed project-graph
  plugin and the Bash language plugin, and `llmlint validate` over the tree exits clean.
- The directive in `scripts/setup-llmlint.sh` suppresses a rule the loaded plugins define,
  and the directive itself is unchanged.
- A contributor-facing recipe validates the whole configuration — every remote plugin
  resolved, every directive's rule known — independent of any diff, and the lint workflow
  runs that same recipe before it judges anything, whatever the diff reaches; the workflow
  parses.
- A test through that validation entry point shows a configuration naming a plugin URL
  that does not resolve, and one whose directive names a rule no loaded plugin defines,
  each refused with the offending item named, while the repository's own configuration
  passes it.
- The branch's history carries, for each of these tickets, a closing reference naming its
  issue (39, 38), so the change that carries the work is what closes it.
- The configuration's validity is proven by the real `llmlint validate` run over the
  finished tree rather than by inspection.
- The judged lint over this change's diff is green over the finished tree.
...
`just lint-llm-validate` runs the validation and `just lint-llm-diff <base>` the judged
tier; both need the plugins to resolve over the network, which this host has. Neither
runs this repository's whole gate, and nothing here asks you to; the required checks run
downstream on the change this branch becomes. A hosted workflow run cannot be exercised
here; the workflow parsing plus the local validation is what stands for it.

This repository's pull-request titles are held to conventional commits with a lowercase
subject; `fix(lint)` is acceptable. Nothing on this host adopts this node's release.
...
{"onepipeline.execution_checkout":"nickderobertis__skilltest","onepipeline.id":"st-llmlint-config","onepipeline.persona":"engineer","onetaskgraph.delivers":["followups:I_kwDOS2giJ88AAAABR5YDHw","followups:I_kwDOS2giJ88AAAABR5YDYw"],"onetaskgraph.item_kind":"task"}
-->
