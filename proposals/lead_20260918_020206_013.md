...
**fix(tests): stop e2e store commands inheriting ONETASKGRAPH_BIN**

*Project description:*
## What

Ticket: https://github.com/nickderobertis/onepipeline/issues/325 — *e2e store commands inherit
`ONETASKGRAPH_BIN`*.

**Root cause (from the ticket).** The e2e `World::store_cmd` (`tests/e2e/harness.rs:818-830` at basis
`59019824d1a753a5ff550c42ed3d2d481db2e545`) filters `ONETASKGRAPH_BIN` out of its explicit
environment map but does not remove the same key inherited by the child process, while
`src/taskgraph.rs:3841` names it (`STORE_BINARY_ENV`). onetaskgraph reads the inherited key as an
unknown `bin` configuration field, so every store-backed e2e journey fails before its assertion
when a developer uses onepipeline's documented binary override; the same journey passes with the
binary selected through `PATH`. Transcript: `onepipeline transcript delivers-plan
op-queued-delivers-2`.

**The accepted fix.** Call `env_remove(STORE_BINARY_ENV)` on the store command before applying the
filtered e2e environment, and add a journey that exports the override in the parent while proving
the selected store still runs.
...
Setting onepipeline's own documented `ONETASKGRAPH_BIN` override makes every store-backed e2e
journey fail, so a developer or worker who selects a store binary that way reads a whole suite as
broken.

## Acceptance criteria

- `World::store_cmd` removes `ONETASKGRAPH_BIN` (through the `STORE_BINARY_ENV` name, not a second
  literal) from the child's inherited environment.
- A journey sets `ONETASKGRAPH_BIN` in the test process's own environment and proves a
  store-backed command run through `World::store_cmd` still runs the selected store and succeeds.
...
- The checks that exercise this change — the e2e journey this change adds or touches, and the repository's judged lint over this change's diff — are green over the finished tree.
...
**Downstream.** `op-dispatch-env-hook` stacks on this branch because its journeys use the same e2e
harness; keep `World`'s public helpers' names and signatures as they are.
...
{"onepipeline.amendment":"Manager amendment (binding; where this and the task's operational notes disagree, this wins). The prior dispatch's work is committed on this branch (two commits ahead of the base it was cut from) and is on origin as https://github.com/nickderobertis/onepipeline/pull/336, whose merge path refused it on one check only: `cross (windows-latest)`, failing `views::host_never_renders_a_dispatch_of_a_run_that_was_stopped` (tests/e2e/harness.rs, `out_has`: the host view printed a `halted … UNPROVEN: the run's dispatch registry holds no entry for it` row where `no live dispatches` was expected) on both attempts, and the same test fails on onepipeline's main at 4ee96400 (workflow run 35282853659) — it is a Windows-only defect of main, not of this branch. The finished tree has these properties. It carries every commit of this branch, and carries origin/main at or past 79a6b9e (the merge of #353) through one merge commit, with no cherry-pick or rebase of the existing commits. It carries one further commit that makes `views::host_never_renders_a_dispatch_of_a_run_that_was_stopped` pass on Windows without weakening what it proves — the host view still never renders a dispatch of a run that was stopped, on every platform — and changes nothing outside what that failure needs; the commit's subject says what it fixes. The existing acceptance criteria hold unchanged on the merged tree: `World::store_cmd` removes `ONETASKGRAPH_BIN` through `STORE_BINARY_ENV`, and the journey exporting the override in the parent still proves the selected store runs. The checks that exercise the change — the e2e journeys this branch adds or touches, the views journey the further commit fixes, and the judged lint over the diff — are green on the finished tree. Publication updates PR #336 (the change request this branch already has) and opens no second change request. The report names the merge commit, the fix commit, the check results, and states as a fact that the Windows failure it fixes reproduces on main.","onepipeline.branch":"onevcs/s-e9514a5547fe","onepipeline.id":"op-325-2","onepipeline.max_turns":200,"onepipeline.persona":"engineer","onepipeline.resume":{"branch":"onevcs/s-e9514a5547fe"},"onetaskgraph.delivers":["followups:I_kwDOTx8aUc8AAAABRkqYww"],"onetaskgraph.item_kind":"task"}
-->
