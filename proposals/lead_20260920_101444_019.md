...
Adopt this workstream's cargo build contract in `onevcs` (github.com/nickderobertis/onevcs; checkout `onevcs` (registered, `~/projects/onevcs`)).

onevcs is a Cargo workspace of `crates/*` (`onevcs`, `onevcs-testing`), plus two crates outside it: `compat/` (a separate cargo project with its own `[workspace]` table and lockfile, holding a released `onevcs` as a dev-dependency) and `crates/onevcs/tests/fixtures/dual-consumer/` (a fixture `tests/contract.rs` copies into a temporary directory before building, so it never builds inside the clone).

**What exists today:** no `.cargo/config.toml`; `Cargo.toml` carries `[profile.release]` only, which stays untouched.

Crates outside the workspace, each of which the contract now reaches: `compat/`, `crates/onevcs/tests/fixtures/dual-consumer/`.
...
- `--target-dir target/compat` on `_crate-compat` and `_crate-lint` in the `justfile`, whose comment says the separate project's target directory sits under this one's so the two share a cache location and neither is a second thing to clean

Consolidate it: drop `--target-dir target/compat` from `_crate-compat` and `_crate-lint` in the `justfile`. The released `onevcs` the compat project depends on and the workspace's own crate carry different versions, so cargo keys their artifacts apart inside one target directory; nothing else the sub-directory protected remains. Drop the `.gitignore` comment that explained it along with the `/compat/target` entry.

**Builds that happen outside the clone or already name the contract's directory** (read them; none changes):
- `tests/contract.rs` (`a_consumer_of_both_crates_resolves_one_vcs_and_one_bus`) copies the `dual-consumer` fixture to a temporary directory and builds it there — outside the clone, unchanged
...
- `.gitignore` carries `/compat/target` with a comment saying it covers a bare `cargo` run inside `compat/`; under the contract a bare cargo run inside `compat/` resolves to `<clone>/target`, so that entry and its comment are dead and go
- `crates/onevcs/tests/e2e/pool.rs` and `maintain.rs` create and read `target/...` paths inside temporary trees they build themselves — unrelated to this clone's target directory
- `.github/workflows/ci.yml`, `published-smoke.yml`, `release.yml`
...
- Cargo's own metadata, read from the repository root, from `compat/`, from `crates/onevcs/tests/fixtures/dual-consumer/`, reports `target_directory` as the clone's own `<clone>/target` — the root of the worktree this dispatch runs in, not another checkout's — and the report quotes those outputs (the command is under `## Additional info`).
...
- No `CARGO_TARGET_DIR`, `--target-dir` or `build.target-dir` anywhere in the `justfile`, `project.json`, `nx.json`, workflows, scripts or tests names any target directory but `<clone>/target` itself for a build inside the clone (a build in a temporary directory outside the clone, and the directories the build tool or its own tooling creates under `<clone>/target` for a build it drives, are the only exceptions); every invocation that named one before is inventoried in the report with what replaced it, and nothing introduces a directory shared across worktrees or repositories. The behaviours the removed sub-directories protected, named under `## What`, still hold: the compat project's tests and clippy still run against the released `onevcs` it pins.
- `.gitignore` ignores the root `target` directory and carries no entry for a nested target directory the contract makes dead (`/compat/target` and its comment are removed).
- The checks that read the target directory, spawn a built binary or run under coverage are green over the finished tree (their commands are under `## Additional info`): the compat project's clippy and test recipes under the one target directory; a bare build invocation inside `compat/` resolving to the clone's `target`; the `contract` integration test of the `onevcs` crate, which builds the `dual-consumer` fixture in a temporary directory.
- Coverage collection through this repository's own coverage interface still works over the finished tree, reporting usable per-file line results under the line-table debuginfo: a scoped run through the workspace's coverage interface reporting per-file line results is green (its command is under `## Additional info`).
...
- cargo's metadata: `cargo metadata --format-version 1 --no-deps` (read `target_directory`) from the repository root and from `compat/` and `crates/onevcs/tests/fixtures/dual-consumer/`
...
- the compat project's clippy and test recipes under the one target directory: `just _crate-lint && just _crate-compat`
- a bare build invocation inside `compat/` resolving to the clone's `target`: `cd compat && cargo metadata --format-version 1 --no-deps`
- the `contract` integration test of the `onevcs` crate, which builds the `dual-consumer` fixture in a temporary directory: `cargo nextest run --locked -p onevcs --test contract`
- coverage: a scoped run through the workspace's coverage interface reporting per-file line results: `cargo llvm-cov nextest --workspace --locked -E 'kind(lib)' --summary-only`
...
{"onepipeline.id":"onevcs","onepipeline.persona":"engineer","onetaskgraph.item_kind":"task","orchestrator.plan-review":{"by":"planning-closeout","key":"e9c2a0daa6a48e4728cbebc1ef86f75ab34e5223261741b7e88ac01cd37d9acd","reviewed_at":"2026-09-20T09:43:09.729637+00:00"}}
-->
