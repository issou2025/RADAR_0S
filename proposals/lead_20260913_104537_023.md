...
**refactor(note): take the note contract and its inbox from onemessagebus, and register the frame protocol as schemas**

*Project description:*
## What

Make `nickderobertis/onejudge` consume `onemessagebus`: `crates/onejudge/src/note.rs` stops **declaring** the
note contract and re-exports it from `onemessagebus_agent::note`; `Notes` / `NoteInbox` become the profile's
`Sender<Note, Accepted>` / `Inbox<Note, Accepted>` over the core's in-process backend; and the JSON-lines frame
protocol `crates/onejudge/src/command.rs` speaks (`respond`, `user`, `supervisor`, `judge`, `assess` — protocol
v6, `docs/protocol.md`) is **registered as schemas** in the bus's registry, so `onemessagebus`'s `serve --codec
onejudge` and `onejudge`'s own `CommandProvider` are held to one declaration. No behaviour changes: every
delivery rule in `note.rs`'s module doc (reopen the worker's turn before the judge is consulted, re-take the judge's
decision, `JudgedWith`, `Undelivered` on a finished conversation) stays in `onejudge`, because it is the engine's
routing and not the inbox's.

Read first: `AGENTS.md`, `docs/protocol.md`, `docs/contract.md`, `crates/onejudge/src/{note.rs,command.rs,
provider.rs,engine.rs}`, `crates/onejudge/src/sdk_schema.rs`, and the released `onemessagebus`'s `docs/inbox.md`
and `docs/contract.md`. The contracts this node consumes are Contracts I and N stated in the `bus-inbox-notes` task
and Contract R in `bus-scaffold-wire-core`, restated in those documents; this node restates none and changes none.
**A shared interface is never changed unilaterally**: a departure this node wants is a proposal to the manager
over the ask seam, while the work continues against the agreed surface.

**What moves, and to what:**

- Every public item `onejudge::note` exports at 0.8.1 — `Addressee`, `Party`, `Criterion`, `CriterionRefused`,
  `NoteText`, `Note`, `NoteRefused`, `DeliveredNote`, `Accepted`, `Undelivered`, `Criteria`, `Notes`,
  `NoteInbox`, `supervisor_block` — becomes `pub use onemessagebus_agent::note::…` at the same path, so
  `oneagentgraph::note`'s and `onepipeline::note`'s re-exports keep resolving. `Notes::channel()`, `Notes::send`,
  `NoteInbox::delivered()` keep their signatures (the profile provides them over the core inbox).
- The engine's use of the inbox (`Engine::with_notes`, the take-and-answer at each turn boundary) moves onto
  `Inbox::take` / `Delivered::answer(Accepted::…)`; the disposition the engine answers with is the one it decides
  today, at the point it decides it.
- `command.rs`'s `Request` and response shapes gain `schemars` derivations under the existing `sdk-schema` feature
  and are registered with the bus registry as `agent.onejudge-frame.<op>@6` (one id per op, version = the
  protocol version); `sdk_schema::bundle()` gains them; `docs/protocol.md` says the schemas are the declaration
  and names the ids. The `onemessagebus` codec's fixed strings and these registrations are reconciled by a test
  here that generates each op's schema and compares it to the one the released `onemessagebus-agent` crate
  carries for that id (`onemessagebus_agent::codec::onejudge::schemas()`), so the two repositories cannot drift
  silently.
- `Cargo.toml`: `onemessagebus` and `onemessagebus-agent` as dependencies of `crates/onejudge`, resolved from the
  registry per the adoption instructions rendered into this task's references block. `schemars` is already a
  workspace dependency; the bus's `Message` derive needs it on the frame types under `sdk-schema`, and on `Note`
  it is the profile's.
- **`docs/contract.md`** wherever it declares a note shape: this task is the planner's ruling that those shapes are
  `onemessagebus-agent`'s, re-exported here; the fenced examples stay as copies marked as such and
  `tests/contract.rs` keeps driving them through the re-exports. Say so in the commit body, as the contract's own
  rule ("a proposal to the planner who owns that contract") asks.

**What does not move:** the routing, `Criteria::compose`'s use at both judging sites, the `Provider` trait, the
`CommandProvider` process protocol, `control.rs` (an address for `oneharness interrupt`, not a message).

### Working notes

- The bar is behaviour identity: the note e2e journeys in `crates/onejudge/tests/` (worker-live, judge-live,
  between-turns, finished-conversation, criterion binding) pass unchanged; the only diff a journey may show is the
  `judge_decisions`-style absence of nothing — i.e. none.
- Commit as you go on the branch the session opened.

## Why

The note seam is where the family already decided that one declaration re-exported everywhere beats three that
drift, and the user's ask is to generalize exactly that: the bus "plugs into oneagentgraph" and the stack is
"refactored out" onto it. `onejudge` owning the two-party conversation is unchanged; what changes is that the
note it accepts, and the frames it speaks, are declared where every other message in the stack is, so
`onemessagebus serve --codec onejudge` reads the frame `onejudge` writes by construction rather than by a Python
adapter measured against "onejudge 0.8.1 as it writes it".

## Acceptance criteria

- Every item `onejudge::note` exports at 0.8.1 still resolves at the same path over the finished tree as a
  re-export of `onemessagebus_agent::note`, and `crates/onejudge/src/note.rs` declares no note shape of its own;
  `Notes::channel`, `Notes::send` and `NoteInbox::delivered` keep their signatures.
- Every note journey in the e2e suite — a note during the worker's turn reopening it before the judge is consulted,
  one during the judge's turn re-taking the decision, one between turns queued into the next turn's transcript,
  one after completion answered `Undelivered`, and a bound criterion entering both judging sites — passes with
  the transcript, report and dispositions it produced at 0.8.1, captured from the unchanged tree.
- Each frame op the command protocol speaks has a registered schema `agent.onejudge-frame.<op>@6` emitted by
  `sdk_schema::bundle()`, and a test compares each against the schema the released `onemessagebus-agent` codec
  carries for that id and fails on a difference; the Python SDK's generated declarations carry them with the drift
  check green.
- `docs/protocol.md` names the schema ids as the declaration of the frames; `docs/contract.md`'s note shapes say
  they are `onemessagebus-agent`'s; every fenced block is driven by `tests/contract.rs`.
...
Release 0.8.1 is what this branch's base carries; every "at 0.8.1" above is measured off the unchanged tree in the
worktree before the change. `oneagentgraph` links this crate's release together with the bus, so this node's
release is the one `oneagentgraph-adopt-bus` waits on. Run the crate-scoped tests and the lint over your diff as
you iterate.
...
{"onepipeline.adoption":"published","onepipeline.consumes":{"bus-ask-validators-codec":"crate"},"onepipeline.id":"onejudge-adopt-bus","onepipeline.max_turns":170,"onepipeline.persona":"engineer","onetaskgraph.item_kind":"task"}
-->
