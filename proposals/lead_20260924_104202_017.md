...
**feat(deps): relink the read API for live graph overrides**

*Project description:*
## What

Relink the read API and bundled browser view to the published `crate:onepipeline` release carrying `op-graph-overrides`, using the version supplied by this node's release hold. Move the exact `onepipeline = "=..."` requirement in `Cargo.toml`, resolve `Cargo.lock` and any directly constrained linked siblings consistently, and update the engine-version fixtures including `tests/fixtures/healthz.json`. Follow this repository's release process: a releasable conventional commit changes packaged files, release-plz chooses the UI version, and `release-targets.toml` declares `pypi-cli` (`pypi:onepipeline-api-cli`) as the wheel the host installs. Do not hand-edit this repository's own version. Account for `scripts/semver-check.sh` if the new engine crosses the pre-1.0 public type boundary; the relink changes the types this crate exposes.

Keep the read API's `POST /api/v2/runs/{run}/channel/reply` contract: it forwards the reply envelope bytes to `onepipeline::verbs::reply` unchanged, and browser adoption uses the linked engine as the driver. Update the browser's shared model in `packages/dag-model/src/index.ts` to accept a plan node's optional ordered `sets: string[]` (default `[]`) and the reply-envelope version and commands supplied by the released engine: `{"op":"set-node-sets","id":string,"sets":string[]}` and `{"op":"set-run-node-sets","sets":string[]}`. These replace their respective ordered lists, including an empty list that clears one. `add`, `retry`, and `requeue` amendments must preserve the same node field. Add first-class browser composer controls and shortcuts for both edits alongside the existing editor: a node id plus an ordered list for `set-node-sets`, and an ordered run-wide list for `set-run-node-sets`. Composing an empty list must send `sets: []`, and manually entered envelopes remain accepted. Update this repository's authoritative `docs/contract.md` for the UI-owned browser controls and verbatim API relay, deriving the engine-owned fields from the released engine's `docs/contract.md` and source command/plan types; keep a drift test over the shared shape. Do not change that shared contract unilaterally: propose any needed departure over the planner channel and continue against the agreed one until the manager rules.

## Why

A run adopted from the browser is driven by the engine statically linked into `onepipeline-api`. The user needs a mid-run choice of oneharness config or model to survive that adoption and to be accepted and relayed by the browser's command form; a host with a newer standalone engine and an older read API would give different answers depending on who drives the run.

## Acceptance criteria

- `Cargo.toml` names the exact published engine crate version supplied by the dependency hold, `Cargo.lock` resolves that same engine and compatible linked siblings, and the engine-version fixtures agree with `onepipeline::VERSION`. The crate's packaged source and conventional commit are eligible for its automated `pypi:onepipeline-api-cli` release under `release-targets.toml`; that later registry publication is not this dispatch's bar.
- The read API relays both new edit envelopes byte for byte to the linked engine, and a browser adoption uses the linked engine to replay the journalled edits on the next dispatch. An end-to-end server journey exercises an accepted `set-node-sets` and `set-run-node-sets` edit across adoption, including a rejected edit that leaves effective lists unchanged; the test drives the real route and engine rather than a mocked handler.
- The browser's parsed plan preserves `sets` in displayed and composed `add`/`retry` nodes and `requeue` amendments, including an explicit empty array. The browser offers controls and shortcuts for editing a node's ordered sets list and the run-wide list; both send replacement edits, including explicit `sets: []` to clear a list. The existing editor still sends manually entered envelopes as typed. A real served-browser journey sends each command through the API to an adopted run and observes the changed next dispatch. The UI `docs/contract.md` describes those controls and the relay. Client model and browser tests exercise the shapes and failures, and a drift check reconciles the browser's command and node fields with the engine contract.
- The affected Rust and browser tests, a built-bundle browser journey, format checks, lint, and judged lint over this diff are green on the finished tree. The new controls fit at the repository's documented viewports, with visual review showing no layout regressions at those widths. Every claim in the completion report about the change and its verification remains true of that final tree.

## Additional info

This node consumes the published engine **crate**, not the wheel the host installs. The UI repository's `AGENTS.md` makes `docs/contract.md` authoritative for its own route, requires the server to use the linked SDK, and makes the packaged file set the release trigger. Its recent #91 relink moved the exact Cargo requirement, lock, linked siblings, fixtures, and adopt-route journey; #93 added a semver guard for that public dependency edge. The host's `config/onepipeline-ui.version` consumes this repository's `pypi-cli` target through its release override.

Use `just dag-ui-screens` to inspect the controls at the repository's documented viewports; those screenshots are review evidence, not a published artifact.

The engine plan node owns the wire and plan fields. Do not change them unilaterally; bring any necessary departure to the manager over the planner channel. Keep the browser's copies reconciled to the engine declaration or a drift check.
...
{"onepipeline.adoption":"published","onepipeline.consumes":{"op-graph-overrides":"crate"},"onepipeline.execution_checkout":"nickderobertis__onepipeline-ui","onepipeline.id":"opui-relink-overrides","onepipeline.persona":"engineer","onetaskgraph.item_kind":"task","orchestrator.plan-review":{"by":"review-plan","key":"79d1559ab4cb83505fbba3c6289b322b3790188ba35c725ce092d4616a094c91","reviewed_at":"2026-09-24T06:26:18.760232+00:00"}}
-->
