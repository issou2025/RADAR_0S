...
Adopt this workstream's cargo build contract in `notignored` (github.com/nickderobertis/notignored; checkout `notignored` (registered, `~/projects/notignored`)).

notignored is one root crate (a `[package]` with no `[workspace]` table) beside npm and Python SDK projects; no other `Cargo.toml` in the tree.
...
- `target/packaging-e2e` as `CARGO_TARGET_DIR` in `tests/e2e/packaging.rs` (`wheel_target_dir`), whose comment says a packaging build sharing `target/debug` would replace `target/debug/notignored` mid-run while sibling journeys spawn it

Consolidate it: drop the private `target/packaging-e2e` from `tests/e2e/packaging.rs` (`wheel_target_dir`) and keep the sibling journeys that spawn `target/debug/notignored` from racing the packaging build another way — the packaging build writing its binary to a temporary output path (`cargo build --artifact-dir` is unstable, so copy the built artifact out under a lock, or build the packaged binary once before the spawning journeys through a nextest test group) — stated in that module's comment and proven by the whole e2e tier passing as the gate runs it.
...
- `npm/notignored-sdk/test/support.mjs` and `python/notignored-sdk/tests/conftest.py` resolve `target/debug/notignored` at the clone root; `scripts/pr-comment-body.sh`, `scripts/screenshots.sh` and `scripts/demo-gif.py` read `target/release/notignored`
- `.github/workflows/notignored.yml` sets `CARGO_TARGET_DIR: ${{ github.workspace }}/target` for its `cargo install` step, and `tests/action_contract.rs` asserts that line — both name the directory the contract names, so neither changes
- `_crate-test` is `cargo llvm-cov nextest --fail-under-lines 95`; `.github/workflows/ci.yml`, `published-smoke.yml`, `release.yml`
...
- the `action_contract` integration test, which asserts the workflow's `CARGO_TARGET_DIR`: `cargo nextest run --locked --test action_contract`
- the whole e2e tier as the gate runs it, packaging journeys beside the journeys that spawn the binary: `just test-e2e`
- the Node SDK tests, readers of `target/debug/notignored`: `cargo build --locked && bash scripts/nx.sh run notignored-sdk-npm:test`
- coverage: a scoped run through the crate's coverage interface reporting per-file line results: `cargo llvm-cov nextest --locked -E 'not binary(e2e)' --summary-only`
...
{"onepipeline.id":"notignored","onepipeline.persona":"engineer","onetaskgraph.item_kind":"task","orchestrator.plan-review":{"by":"planning-closeout","key":"fc7e50021d042e33f67557de38c39fb1e364b1a2a53f25b684a8eb826a7510db","reviewed_at":"2026-09-20T09:43:09.596867+00:00"}}
-->
