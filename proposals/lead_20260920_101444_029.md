...
Adopt this workstream's cargo build contract in `llmlint` (github.com/nickderobertis/llmlint; checkout `nickderobertis__llmlint` (registered)).

llmlint is one root crate (a `[package]` with no `[workspace]` table); no other `Cargo.toml` in the tree.

**What exists today:** no `.cargo/config.toml`; `Cargo.toml` carries `[profile.release]` and a `[profile.profiling]` that inherits `release` with `debug = true` for `samply`/`cachegrind` — both stay untouched.
...
- `scripts/bench.sh`, `scripts/bench-instructions.sh`, `scripts/profile.sh`, `scripts/demo-gif.py` and `.github/workflows/bench.yml` read `target/release/llmlint`, `target/profiling/llmlint` and `target/bench` at the clone root
- `just test` is `cargo llvm-cov nextest --fail-under-lines 95`; `.github/workflows/ci.yml`, `live.yml`, `release.yml`, `win-color.yml`
...
- The checks that read the target directory, spawn a built binary or run under coverage are green over the finished tree (their commands are under `## Additional info`): the e2e tier, which spawns the built binary; a verbose release build showing the release profile unchanged (no `debuginfo=1`), since the `profiling` profile inherits it.
- Coverage collection through this repository's own coverage interface still works over the finished tree, reporting usable per-file line results under the line-table debuginfo: the test tier, which is the suite under coverage instrumentation with its floor is green (its command is under `## Additional info`).
...
- the e2e tier, which spawns the built binary: `just test-e2e`
- a verbose release build showing the release profile unchanged (no `debuginfo=1`), since the `profiling` profile inherits it: `cargo build --locked --release -v`
- coverage: the test tier, which is the suite under coverage instrumentation with its floor: `just test`
...
{"onepipeline.id":"llmlint","onepipeline.persona":"engineer","onetaskgraph.item_kind":"task","orchestrator.plan-review":{"by":"planning-closeout","key":"2c67844e0e61ec097a112599cf0fd84f4e8cf2ecd628005a7670339e78b5b15a","reviewed_at":"2026-09-20T09:43:09.584452+00:00"}}
-->
