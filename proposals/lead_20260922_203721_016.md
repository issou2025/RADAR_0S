...
**feat(harness): move the worker and supervisory tiers onto the current models**

*Project description:*
## What

Move the two model tiers this host's harness routing names onto their successors:
`claude-opus-5` to `claude-opus-5-5`, and `gpt-5.6-sol` to `gpt-6-sol`.

**The spellings are settled; do not re-derive them.** The planner established the Claude
one by turn against this host's own subscription: `claude-opus-5-5` is accepted, and
`claude-opus-5.5` is refused as a model that does not exist. The Codex one follows the
family spelling `gpt-6-astra` already in the tree. If a turn under either identifier comes
back refused as an unknown model, that is a blocking question for the planner — never a
near-spelling, and never some other model that does exist put in its place. A model
identifier a server will not run is not a loud failure: the candidate is classified and the
chain steps past it, so a whole identity can quietly stop being available while every file
still looks right.

**Where those two values live after the node before this one.** The refactor states the
six identities once, on the variants of `oneharness.identities.toml`, and leaves a role
file restating a `model` only where that role deliberately differs. Both values this node
moves are parent values: the four Claude variants resolve `claude-opus-5` from the parent,
and the two Codex variants resolve `gpt-5.6-sol` from it. So the routing half of this
change is six lines in one file. If the tree you find does not have that shape, follow the
tree rather than this paragraph, and say in your report where it differed.

**The two tiers that do not move, and why they are not oversights.** The judge side of the
worker conversation and the follow-up agent name `claude-sonnet-5` on purpose — a cheaper
supervisor keeps loop overhead down — and the design-document writer, the change-request
drafter and the plan reviewer name `gpt-6-astra` on purpose. Neither was asked for and
neither changes. A tree in which a Sonnet side came out on an Opus tier, or an Astra side
on Sol, has done something this node was not asked to do.

**Two places outside the routing files name a model identifier literally**, and a tier move
that leaves them behind leaves the suite describing a host that no longer exists:
`CONFIGURED_MODEL` in `tests/lost_turn_producer.py`, which is the model a controlled turn is
configured with in the lost-turn journey, and the `modelUsage` key in
`tests/e2e/test_quota_fallthrough_e2e.py`. Read what each one is standing for before moving
it. One of them may be naming a model for what it *stands for* in that journey — the model a
server would substitute, against which the configured one is compared — rather than
selecting a tier this host runs on, and such an occurrence is deliberately left where it is.

## Why

The planner's user asked for it in these words: *upgrade opus 5 to opus 5.5 and gpt 5.6 sol
to gpt 6 sol.* The harness should run the workers and the supervisory tiers on the current
models rather than on the ones that were current when these files were written.

It is also the first real use of what the rest of this plan built. Before the refactor this
tier move was forty-six lines spread across ten files, each one an opportunity to move nine
and forget the tenth; the point of stating the identities once is that a decision about a
tier is made once. This node is where that claim either holds or does not.

## Acceptance criteria

- In the finished tree, every Claude variant that selected `claude-opus-5` selects
  `claude-opus-5-5`, and every Codex variant that selected `gpt-5.6-sol` selects
  `gpt-6-sol`.
- No `model` key in any of this repository's harness routing files selects `claude-opus-5`
  or `gpt-5.6-sol` in the finished tree. A literal occurrence elsewhere may remain where it
  names a model for what it stands for in a journey rather than selecting a tier this host
  runs on, and the report names each occurrence it kept and what it stands for.
- Every side that deliberately names a different tier still names it: the worker
  conversation's judge side and the follow-up agent on `claude-sonnet-5`, and the
  design-document writer, the change-request drafter and the plan reviewer on `gpt-6-astra`.
- Nothing else about the routing moves: each role's identity order, its deadline, its
  history labels, its streaming and schema settings, and every credential mask and
  environment indirection are what they were before this change.
- What the real `oneharness` CLI resolves for each of the role configs matches the committed
  snapshot of the resolved configurations, so the snapshot moves in this change rather than
  drifting from it; the tests that read this repository's harness routing and its Claude
  identity wiring pass; and the judged lint over this change's own diff passes.
- A journey that drives a **real turn** through the pinned harness proves the changed
  selection reaches execution: the turn runs under the model the config names, and a turn a
  server would run under some other model, or refuse, is a failure of that journey rather
  than a candidate the chain steps quietly past. `tests/e2e/test_controlled_turn_model_e2e.py`
  is the journey that already takes this and it passes against the changed configuration;
  where it does not reach one of the two changed identifiers, the finished tree carries the
  coverage that does, driving the same real interface rather than a double of it.
- The completion report names every file this change touched and, for each, what moved in
  it; where the tree's shape differed from what this task describes, it says how; and every
  claim the report makes is true of the tree as it finally stands.
...
{"onepipeline.execution_checkout":"ai-orchestrator-isolated","onepipeline.id":"upgrade-the-model-tiers","onepipeline.max_turns":90,"onepipeline.node":"upgrade-the-model-tiers","onepipeline.persona":"engineer","onetaskgraph.item_kind":"task"}
-->
