...
**fix(deps): relink the onevcs and oneagentgraph releases carrying the JSON sweep reports**

*Project description:*
## What

Move this workspace's `onevcs` and `oneagentgraph` requirements (`Cargo.toml`'s
workspace dependencies) onto the newly published releases carrying each crate's
`sweep --format json` report — the two sweeper nodes this node depends on — and
re-resolve the lockfile so the build links them. Each dependency's release is named
for you in this task's cross-repository references and in the note this run delivers
when the release arrives; take the versions from there and from nowhere else.
...
for the fix in this stack before. **One copy, still**: `oneagentgraph` and `onevcs-testing` each
require `onevcs` at a minor version and this workspace deliberately holds one copy of
each crate so the types crossing those seams are one type; if a named release falls
outside what a sibling accepts (the new `oneagentgraph` outside what `onevcs-testing`
or this crate's `onevcs` window admits, or the reverse), raise it over `$ORCHESTRATOR_ASK_MANAGER` rather than admitting a second copy.
`scripts/linked-engines.sh` is the check that reconciles what this build links, and
`Cargo.toml`'s own comments on each pin record why every earlier bump was taken; add
this one's in the same voice.

No behaviour of this engine changes: neither release touches a seam this crate calls
(both add a `--format` to a CLI verb and a decimal age floor to one). If the relinked
crates surface a compile or test difference anyway, fix it here and say what it was.

## Why

The consumer host (`ai-orchestrator`) holds its `config/onevcs.version` and
`config/oneagentgraph.version` CLI pins **equal** to the releases its engine links,
read off the engine wheel's SBOM (`tests/test_linked_libraries.py` there), because a
host on which the two differ has two answers to "which onevcs is in force" and has
acted on the wrong one twice. So the JSON sweep reports reach that host only once an
engine release links them: this relink is what lets its adoption node move the three
pins together and delete its 920-line `sweep.sh`. The user's aim behind all of it:
*"code that works around an upstream problem is an anti-pattern here"* — and the
workaround cannot go until the upstream fix is in force.

## Acceptance criteria

- The workspace's `onevcs` and `oneagentgraph` requirements name the releases this
...
- The resolved dependency graph holds exactly one copy of `onevcs` and one of
  `oneagentgraph`, satisfying the requirements `oneagentgraph` and `onevcs-testing` place on
  `onevcs`, and the
  linked-engines reconciliation is green over the finished tree.
- `Cargo.toml`'s comment on each moved pin says which release it moved to and why, in
  the voice the existing pin comments use.
- The tests covering the seams this engine reaches into each relinked crate (the onevcs
  seam journeys and the oneagentgraph liveness journeys) are green over the finished
...
This is the third node on this repository in this plan; `op-supervision-verbs` and
`op-publication-verbs` have landed on the base your session was cut from. Iterate with
`scripts/linked-engines.sh`, the onevcs-seam and oneagentgraph-liveness journeys, and
this repository's lint over your diff; the full gate runs on the merge path after you
settle. If a named release cannot yet be fetched from the registry when you start, wait
for the index rather than pinning a git reference.
...
{"onepipeline.adoption":"published","onepipeline.consumes":{"oag-sweep-report":"crate","ov-sweep-report":"crate"},"onepipeline.execution_checkout":"nickderobertis__onepipeline","onepipeline.id":"op-relink-sweepers","onepipeline.persona":"engineer","onetaskgraph.item_kind":"task","orchestrator.plan-review":{"by":"planning-closeout","key":"529be7a09ab57f80f1028f7b492a6bc21b127804e79fcbdf5a724370963ac1a4","reviewed_at":"2026-09-20T19:30:11.722679+00:00"}}
-->
