...
**test(subprocess): give the real shell's handshake its own deadline before the 20ms request**

*Project description:*
## What

https://github.com/nickderobertis/onetaskgraph/issues/2278. `a_request_deadline_turns_a_silent_child_into_a_named_source_error`
(`crates/onetaskgraph-core/tests/subprocess.rs:373-416`) connects a real `/bin/sh` through
`connect_with_deadline(…, RequestDeadline::from_millis(20))`, and that 20ms applies to the
initialization handshake as well as to the silent request the test means to exercise, so
scheduler delay fails the handshake before the behaviour under test begins. Complete the
real child's handshake under a separate, generous deadline, then apply the 20ms deadline
only to the silent request whose timeout the test asserts.

The public API has one deadline for every exchange
(`src/subprocess/source.rs:142-150` → `Peer::spawn(program, args, deadline.duration())`).
Two routes: the test spawns the child itself and uses `over_with_request_deadline`
(`:228`), whose deadline starts after the handshake — which stops exercising `connect_*`;
or a connection constructor taking separate handshake and request deadlines, which is a
public API addition this ticket did not ask for. Prefer the route that keeps the test
driving the connect path users call; if that needs the API addition, ask first.

## Why

A loaded host fails this test before the behaviour it exists to check even starts, which
refuses unrelated pre-push gates; the only workaround is rerunning the whole gate on a
quieter host.

## Acceptance criteria

- The test completes the real shell's handshake under a deadline generous enough for a
  loaded host, and applies the 20ms deadline only to the silent request, still asserting
  that request times out with the named source error.
- The `onetaskgraph-core` subprocess tests, built with the `linear` feature they require,
  are green over the finished tree, and so is the judged lint over this change's diff.
- The branch carries GitHub closing metadata associating it with issue #2278 of this repository and with no other issue; whether the issue actually closes is decided downstream when the change lands, and is not this dispatch's bar.
- Every claim this dispatch makes about the finished work is true of the tree as it
  finally stands.

## Additional info

onetaskgraph's `pre-push` hook runs `just gate` over the affected projects; its CI matrix covers ubuntu, macos and windows. Put `Closes #2278` in the body of the commit that resolves each ticket, so the landed change closes it; the issues live in this repository. onetaskgraph releases from conventional commits through release-plz; this is a test-only change nothing on this host adopts, so `test` is the honest type.
...
{"onepipeline.execution_checkout":"nickderobertis__onetaskgraph","onepipeline.id":"otg-subprocess-handshake","onepipeline.max_turns":12,"onepipeline.node":"otg-subprocess-handshake","onepipeline.persona":"engineer","onetaskgraph.delivers":["followups:I_kwDOUBBgxs8AAAABSNuM3g"],"onetaskgraph.item_kind":"task"}
-->
