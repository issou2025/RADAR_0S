...
Adopt this workstream's cargo build contract in `github-secrets` (github.com/nickderobertis/github-secrets; checkout **no registered checkout on this host, and no rule in `config/onevcs.rules.yml`** — see the project record; its default branch is `master`).

github-secrets is one root crate (a `[package]` with no `[workspace]` table); no other `Cargo.toml` in the tree.

**What exists today:** no `.cargo/config.toml`; `Cargo.toml` carries `[profile.release]` and a `[profile.profiling]` inheriting `release` with `debug = true` for cachegrind — both stay untouched.
...
- `scripts/bench.sh` and `scripts/bench-instructions.sh` read `target/release/gh-secrets`, `target/profiling/gh-secrets` and `target/bench` at the clone root; `.github/workflows/bench.yml`
- no judged lint tier here (no `llmlint.yml`); `.github/workflows/ci.yml`, `pr-lint.yml`, `release.yml`
...
- The checks that read the target directory, spawn a built binary or run under coverage are green over the finished tree (their commands are under `## Additional info`): the unit and binary test tier; a verbose release build showing the release profile unchanged (no `debuginfo=1`).
- Coverage collection through this repository's own coverage interface still works over the finished tree, reporting usable per-file line results under the line-table debuginfo: where the tree at dispatch offers a coverage interface (a recipe, target or script running the coverage tool or an equivalent), a scoped run through it reports per-file line results; where it offers none — none was found when this plan was written — the criterion is not applicable and the report states that outcome.
- The lint over this branch's own diff is green: the format check and clippy (this repository has no judged lint tier).
...
- the unit and binary test tier: `just test`
- a verbose release build showing the release profile unchanged (no `debuginfo=1`): `cargo build --locked --release -v`
- the format check and clippy (this repository has no judged lint tier): `just format-check && just lint`
...
{"onepipeline.id":"github-secrets","onepipeline.persona":"engineer","onetaskgraph.item_kind":"task","orchestrator.plan-review":{"by":"planning-closeout","key":"0ff9fc1f6d2d3d869f720a7dc00a69da291e744d28cd289ac4b8f5d5b381b898","reviewed_at":"2026-09-20T09:43:09.571312+00:00"}}
-->
