...
**feat(timeline)!: serve a lane for every member a run's recorded graphs declare, with no member built in**

*Project description:*
## Planner amendment (attempt 2, continuing branch `onevcs/s-e91edf0caa7f` at `7d46daf`)

Where this section and anything below disagree, this section wins.

**What attempt 1 left, and the one thing its judge refused.** The branch you continue carries eight commits ending
in `7d46daf`: `agent_role` is served as the member name the run's recorded graph declarations name, `ROLE_MEMBERS`,
`AGENT_ROLES` and the closed `agentRoleSchema` are gone, the browser draws one lane per distinct `agent_role` in
payload order, timeline schema 10 and telemetry schema 17 carry the derived-lane rule in `docs/contract.md`, and the
server and browser e2e suites prove it over recorded runs declaring `sentinel`, `ticker` and `drafter`. Keep all of
that: this attempt changes one attribution and what documents and proves it, and rewrites no history. The judge
refused the tree for one reason, and the refusal is right: the judge's own conversation and the span it is reachable
through are attributed `agent_role: "judge"` from `JUDGE_PARTY` (`Party::Judge`, `src/payload.rs`), and
`docs/contract.md` names that as a second exception beside the engine's run-level lane. That is an agent-role word
built into `src/` and served as a lane, which the criteria below forbid; the task named one exception, the engine's
own lane, and the judge is not it.

**The ruling.** The judge is one party of the two-party member whose dispatch it supervised, and the run records it
only inside that member's report — so its `agent_role` is **that member's name**, read exactly as the agent side's
is (from the run's recorded declarations, or absent when they name no such member), and its `transport` stays
`judge`. That is the precedent `src/payload.rs` already sets for the lint tier: served under the semantic role of
whoever did the work, told apart by the transport word and by nothing else. `Party` remains what it is — the
transport's closed vocabulary (`agent`, `judge`, `llmlint`) — and is never the source of an `agent_role`.

**What must be true when you finish:**

- No `agent_role` anywhere in `src/` or `packages/dag-model/src/` originates from a word the crate or the client
  carries: the judge's conversation attribution, the judge span, and the run's session links for the judge side each
  carry the supervised member's name as `agent_role` (or none, where that member is served none), with
  `transport: judge`; a run of this host's shape serves its worker dispatch's judge conversation under `worker`, and
  a recorded run whose node graph declares `drafter` serves that dispatch's judge side under `drafter` — each proven
  by the server e2e suite.
- The browser draws the judge's span in the lane of the member it supervised, told apart from the agent side by its
  transport as the lint tier already is, and draws no lane labelled `judge` — proven by the browser e2e suite over
  the served fixture run the branch already carries, extended where it has to be.
- `docs/contract.md` states this attribution where it now states the opposite (the schema-14 paragraph on the judge's
  conversation and the timeline-10/telemetry-17 paragraph), naming the engine's run-level lane as the one lane that
  is not a member's; the timeline and telemetry schema versions stay at 10 and 17, because nothing this branch
  serves has yet reached a client.
- Every check that exercises what this attempt touched passes on the finished tree — the server e2e suite, the
  browser e2e suite, the contract test, and the judged lint over the diff against the merge base — and every claim
  in your report is true of the tree as it finally stands; the tree stays clean and committed; publication is the
  harness's.

## What

Stop building the members of one host's observer graph into the read API and the browser view. Today
`src/payload.rs` declares `AGENT_ROLES` and a `ROLE_MEMBERS` map (`monitor` served as `orchestrator`, `check-in` as
`check-in`), and `packages/dag-model` closes `agentRoleSchema` over those words, so a host whose observer members
carry other names gets no lane for them. Afterwards:

- A session's **`agent_role` is the member name the run recorded for it**, served when the run's recorded graph
  declarations — the observer graph's and the node graphs' declared members, as the run keeps them — name that
  member. No member or role word is built into the crate or the client: a host's `monitor` member is served as
  `monitor`, its `check-in` member as `check-in`, and a member named `sentinel` as `sentinel`.
- A session with no recorded member is served no `agent_role`, unless the run records a declaration that names its
  persona's word as a member — never through a built-in list of persona words.
- The run-level lane the engine's own driving is drawn in keeps whatever name this crate serves it under today,
  because it is the engine's lane rather than a member's.
- `agentRoleSchema` admits any member name the engine's member-name grammar admits, the browser draws one lane per
  role a payload serves in the order the payload serves them, and the timeline and telemetry schema versions move as
  this repository's contract rules require, in `src/contract.rs` and the client together.

This node stacks on `onepipeline-ui-adopt-open-channel`, whose branch you start from and which adopted the engine
release this reads.

Read first: `AGENTS.md` and `src/AGENTS.md`; `docs/contract.md` (the `agent_role` paragraph and the timeline schema
history) and `docs/dag-ui.md` (lanes); `src/payload.rs` (`AGENT_ROLES`, `ROLE_MEMBERS`, `agent_role`,
`event_agent_role`, `JUDGE_MEMBER`), `src/contract.rs`, `src/store.rs`; `packages/dag-model/src/index.ts`
(`agentRoleSchema`), `apps/dag-ui/src/features/timeline/`; `tests/support/fixture_run.rs`, `tests/e2e/server.rs`,
`apps/dag-ui-e2e/`; and from the linked `oneagentgraph` and `onepipeline` releases, where a run records the members
its graphs declared (`oneagentgraph`'s run record carries `declared_members`).

## Why

The user's goal is that the stack "stays agnostic to the actual structure of agent graphs": the engine and the bus
now name no observer member, and a viewer that maps `monitor` onto a built-in lane and closes its role vocabulary
over one host's member names has not reached that goal — a host that names its members differently gets sessions
with no lane, and the manager reading the timeline cannot see them. The user rejected deferring the viewer, and the
manager ruled that the lanes are derived from the run's own recorded graph declaration rather than built in.

## Acceptance criteria

- `src/` and `packages/dag-model/src/` build in no member or agent-role word: no `ROLE_MEMBERS`, no closed
  `AGENT_ROLES` or `agentRoleSchema` enumeration of member names, and no mapping of one member name onto another
  lane.
- Through the real `onepipeline-api` server over recorded runs whose declarations name members named by nothing
  built in — members declared by the observer graph (for example `sentinel` and `ticker`) **and** members declared
  by a node graph (for example `drafter`) — the timeline and run-detail payloads serve each of those members'
  sessions with that member's name as `agent_role`, and a recorded run of this host's shape serves its `monitor` and
  `check-in` sessions as `monitor` and `check-in` — proven by the server e2e suite.
- A session with no recorded member whose persona's word a recorded declaration names as a member is served that
  member's name as `agent_role`; a session whose recorded member is not among the run's recorded declarations, and a
  session with no recorded member whose persona no declaration names, are served with no `agent_role` — each proven
  by the server e2e suite.
- The browser view draws exactly one lane per distinct `agent_role` the served payload carries, in the order the
  payload serves them, labelled with that role's word, with no role dropped and none drawn twice, and draws the
  engine's run-level lane under the name it carries today — proven by the browser e2e suite over a served fixture run
  carrying at least two roles named by nothing built in, served in an order neither an alphabetical sort nor any
  built-in ordering would produce, beside the engine lane.
- The contract moves by the repository's rules: `docs/contract.md` states the derived-lane rule and names the
  timeline schema version and the telemetry schema version that introduced it, each advanced past the value the
  branch this node starts from carries, because `agent_role` is carried by timeline and telemetry payloads alike and
  a client that switched on the closed vocabulary misreads an open one; `src/contract.rs` and `packages/dag-model`
  carry those same two versions; and the contract tests (`tests/contract.rs` and the model package's tests) fail
  when the server's, the client's or the document's version of either schema disagrees.
- `agentRoleSchema` admits exactly the member-name grammar `oneagentgraph` declares — `config::is_member_name`, as
  `MemberName::parse` states it: letters, digits, hyphens and underscores — held by a model-package test that accepts
  names that grammar admits (including `sentinel`, `check-in` and a name with an underscore) and refuses names it
  does not (including the empty string, a name containing `/`, a name containing whitespace, and `..`), so replacing
  the schema with an unrestricted string fails that test.
- The checks that exercise this change are green over the finished tree: the crate's tests and e2e journeys, the
  model package's and browser view's tests and browser e2e suite, the type check, the lint and format check, and
  the judged lint over this branch's own diff.
...
- **Added by a revision of this plan, on the manager's ruling.** The manager ruled that the viewer's member-to-lane
  map is derived from the run's recorded graph declaration rather than built in, and that the viewer's work is split
  into two stacked nodes rather than deferring either half; this is the second.
- If a recorded run does not keep its graphs' declared members anywhere this crate can read through the linked
  releases, do not reintroduce a built-in list: ask the manager with `"$ORCHESTRATOR_ASK_MANAGER"` with what you
  found, and keep building what does not depend on the answer.
- **Criteria repaired after plan review, on the manager's rulings.** The review found the lane criteria too narrow:
  they covered observer-graph declarations only, left the persona fallback's positive case untested, proved one
  arbitrary lane rather than one lane per served role in payload order with the engine lane's name kept, and let
  the schema versions merely agree and `agentRoleSchema` become an unrestricted string. Each is now a criterion of
  its own with a check that fails on the violation; the member-name grammar is `oneagentgraph`'s, because the lanes
  are its members. The report-truth criterion now holds labelled evidence of an earlier or induced state to its
  label. Nothing else in this task changed.
- Lane styling in the browser is yours to decide within the criteria; say in your report what a reader sees for a
  run of this host's shape before and after.
- The breaking marker is deliberate: a served vocabulary a client switched on exhaustively becomes open.
- The checks above are this repository's recipes — `just test`, `just test-e2e`, `just typecheck`, `just lint`,
  `just fmt-check`, the browser e2e target `just --list` names, and `just lint-llm-diff`. Running the pieces
  separately is fine; the full `just gate` is not owed.
...
{"onepipeline.branch":"onevcs/s-e91edf0caa7f","onepipeline.id":"onepipeline-ui-member-lanes-2","onepipeline.max_turns":180,"onepipeline.persona":"engineer","onepipeline.resume":{"branch":"onevcs/s-e91edf0caa7f"},"onetaskgraph.item_kind":"task"}
-->
