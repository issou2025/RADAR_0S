...
**feat(deps): adopt the engine and onejudge releases that stack judges, and serve each judge's decision**

*Project description:*
## What

Adopt, in one change in `nickderobertis/onepipeline-ui`, the `onepipeline` release that links the judge-stacking
`oneagentgraph` and `onejudge` — all three named by the `## Cross-repository references` block appended to this
task — and serve each judge's decision where a settled member's transcript is read.

Read `AGENTS.md`, `src/AGENTS.md` ("The report a settled member left, which is what a transcript is": the stored
onejudge report is the transcript, linked rather than copied, and `read_report` refuses a report whose
`schema_version` exceeds the onejudge this crate links), `Cargo.toml`'s comments on the exact `onepipeline =`
pin and the unpinned siblings, `tests/packaging.rs` (it reconciles this crate's resolution of `oneagentgraph`,
`onejudge` and `onevcs` against the pinned engine's), `tests/fixtures/healthz.json`, `src/payload.rs`
(`conversation_document`, `read_report`, `judge_links`), and `docs/contract.md`.

**What to change.** Move `onepipeline` to the exact adopted release — from `=0.27.2`, past 0.28.x, to the release
`onepipeline-adopt-judges` produced — and `onejudge` and `oneagentgraph` to whatever that release resolves, so
`tests/packaging.rs` reconciles one copy of each; move `tests/fixtures/healthz.json` with it; bring every
statement this crate makes about the engine's store, telemetry document or event kinds to the adopted release
(read that release's `CHANGELOG.md` for what moved between 0.27.2 and it, and size the work honestly — the manager
ruled this pin moves here, in this node, whatever that jump costs, because a host running the new engine with a
read API linking the old onejudge renders no transcript for any new run). Then, in the conversation document, carry
the report's `judge_decisions` beside the supervisor turn each belongs to — a `judges` array on that turn's row,
each entry `{judge, kind, decision, reason}` — state it in `docs/contract.md` and the checked-in fixtures, and
render it minimally in the browser view (one line per judge under the supervisor turn). A journal `judge-decided`
event is read without refusal wherever this crate reads member events.

The contracts this node consumes are onejudge's Contract C (the report's `judge_decisions`, `schema_version` 12,
the `judge` label on telemetry records) and oneagentgraph's Contract E (`judge-decided`), stated in those releases'
`docs/judges.md` and `docs/contract.md`; this node restates none and changes none. A shared interface is never
changed unilaterally: a departure this node wants is a proposal to the manager over the ask seam.

### Working notes

The checks that exercise this change are `cargo test` narrowed to `tests/packaging.rs`, the contract tests over
`src/payload.rs` and the fixtures you touched, `cargo clippy`, the frontend's own test and lint targets for the
view you changed (`just --list` and `nx.json` name them), and `just lint-llm-diff origin/main`. `just bootstrap`
provisions the `onepipeline` CLI at the version the lock pins, so run it after moving the pin. Do not run the whole
gate; the merge path does. Commit on the branch the session opened.

## Why

The user reads a dispatch through this read API and its browser view, and asked that a supervisor be able to read
which judge said what. This crate refuses a stored report whose schema is newer than the onejudge it links, so the
moment the host runs the stacking engine, every new run's transcript goes blank in the view unless this crate has
moved with it — which is why the manager ruled the host's adoption waits on this node rather than following it later.

## Acceptance criteria

- `onepipeline` is pinned exactly at the release the cross-repository reference row for `onepipeline-adopt-judges` carries, `onejudge` and `oneagentgraph` resolve to what that release links, the packaging test and the `/healthz` fixture agree with the lock, and every statement this crate makes about the engine's documents holds at the adopted release.
- A stored onejudge report at `schema_version` 12 is read and served as a transcript, and one carrying `judge_decisions` exposes each supervisor turn's decisions as a `judges` array of `{judge, kind, decision, reason}` on that turn's row of the conversation document, stated in `docs/contract.md` and the checked-in fixtures and rendered in the browser view beneath that turn — proven by a journey that serves a run holding a fixture report carrying two judges' decisions through the real read API process, reads the conversation document back off the real transcript route, and finds both decisions on the row of the supervisor turn they belong to and on no other row; and by the frontend's own browser test rendering that transcript and finding one line per judge beneath that same turn.
- A journal carrying `judge-decided` events is read without refusal by every route that reads member events, proven by driving each of those routes on the real read API process against a run whose journal carries one and receiving a successful response that includes the event.
- The tests over the code this change touches, the workspace lint, the frontend checks that exercise the changed view, and the judged lint over this change's own diff are green over the finished tree.
...
{"onepipeline.adoption":"published","onepipeline.consumes":{"oneagentgraph-judge-list-3":"crate","onejudge-judge-panel":"crate","onejudge-llmlint-judge":"crate","onepipeline-adopt-judges":"crate"},"onepipeline.id":"onepipeline-ui-adopt-judges","onepipeline.max_turns":160,"onepipeline.persona":"engineer","onetaskgraph.item_kind":"task"}
-->
