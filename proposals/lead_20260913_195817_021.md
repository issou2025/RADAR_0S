...
**feat(deps): link the onejudge that names the artifacts a judge reads**

*Project description:*
## What

**Read against** `nickderobertis/oneagentgraph` `origin/main` at `7545a7c` (v0.4.0). Every path and line this task cites was checked against that tree; if the tree has moved when you start, re-read what moved rather than trusting a cited line number.

Carry `nickderobertis/ai-orchestrator` issue #1004 root cause 3's onejudge fix one step toward
a dispatch. oneagentgraph composes onejudge's loop in-process for a `kind: onejudge` member,
and requires `onejudge = { version = "0.10.0", features = ["cli"] }` (`Cargo.toml:286`); the
onejudge release that adds `user.artifacts` is a new minor version, which that requirement
excludes. Move the requirement to the onejudge release carrying `onejudge-named-artifacts`
and resolve the lock to it.

**The surface this carries unchanged** (stated in full in the `onejudge-named-artifacts` task,
declared once in onejudge's config type): `user.artifacts`, an optional list of file or
directory paths, relative ones resolved against the judge's evidence worktree; overrides
`--artifact <PATH>` and `ONEJUDGE_ARTIFACTS`; rendered into the judge-side prompts only when
non-empty. oneagentgraph adds no schema of its own for it: `persona::merge` copies every
`user.*` key over the base (`src/persona.rs:437-447`), `Persona::read` validates the delta with
onejudge's own `Config` (`:245-247`), and `judge.rs:897-915` parses the merged file with
onejudge's types and applies `Overrides::from_env`. So what this node owes is proving that
path end to end, and documenting the key where the persona format is documented.

Keep `oneharness-core` resolving to a single copy in the tree. Record in the change request
description whether this release's public API changed with the onejudge types it re-exposes,
since `onepipeline-link-fixes` has to move its oneagentgraph requirement to match.

## Why

The operator wants a design-document judge that is told where the document is, rather than
one that loops on a git question that can never see a gitignored file (#1004 root cause 3). The
judge runs inside the engine through this crate, so until this crate links the onejudge that
knows the key, a host configuring it gets a hard `deny_unknown_fields` error instead of a fix.

## Acceptance criteria

- `Cargo.toml`'s onejudge requirement and the `Cargo.lock` entry it resolves admit
  `user.artifacts` in onejudge's config type, with a single copy of `onejudge` and of `oneharness-core` in the tree.
- A `kind: onejudge` member whose persona delta carries `user.artifacts` is accepted by persona
  validation, and its judge's prompt names the artifacts; a member whose base config carries
  the key behaves the same; `ONEJUDGE_ARTIFACTS` in the member's environment reaches its judge
  — each proven end to end through `oneagentgraph run` with the harness doubled.
- A member that names no artifacts runs exactly as before, and the existing member and persona
  journeys stay green.
- `docs/persona-format.md` lists `user.artifacts` beside the other `user.*` keys.
- The tests covering the changed dependency and persona surface pass over the finished tree,
...
- Read first: `AGENTS.md`, `docs/persona-format.md`, `src/invoke.rs:318-378`,
  `src/persona.rs` (`read`, `merge`, `validate_base`), `src/judge.rs:897-915`, and the comment
  blocks above the onejudge and oneharness-core requirements in `Cargo.toml` (they record why
  each version was taken; add this raise to them in the same voice).
- The harness references block the engine appends names the onejudge release this node was
  released against; take the version from there.
- If moving onejudge forces other requirements to move (for example `oneharness-core`), that is
  part of this node; if it forces a change you cannot make without amending the surface above,
  surface a proposal to the manager over `$ORCHESTRATOR_ASK_MANAGER`.
- The title must stay a releasing type: `onepipeline-link-fixes` consumes this crate's release.
...
{"onepipeline.adoption":"published","onepipeline.consumes":{"onejudge-named-artifacts":"crate"},"onepipeline.id":"oneagentgraph-link-onejudge","onepipeline.max_turns":120,"onepipeline.persona":"engineer","onetaskgraph.item_kind":"task","orchestrator.plan-review":{"by":"review-plan","key":"ad12cf5ae598eed0ef39fbe9b09520ecd533f737c54e3bc6417acb86bc160daf","reviewed_at":"2026-09-13T17:11:53.127667+00:00"}}
-->
