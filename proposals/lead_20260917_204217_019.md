...
**fix(sdk): ship the py.typed marker the Python SDK's classifier promises**

*Project description:*
## What

Make the Python SDK at `python/onemessagebus-sdk` (package `onemessagebus`, `uv_build`,
`[tool.uv.build-backend] module-name = "onemessagebus"`, `module-root = "src"`) ship the
marker its manifest already promises — `Typing :: Typed` is among its classifiers today
while no `py.typed` exists in the tree — and hold the wheel to it in this repository's
own gate.

- **The marker.** An empty `src/onemessagebus/py.typed` beside `__init__.py` — outside
  `src/onemessagebus/_generated/` and `models.py`, which `scripts/generate.py` owns and
  rewrites. `scripts/pack.py` copies `src` whole into the publishable copy, so the
  marker travels with it; nothing else has to change for it to reach the wheel.
- **The classifier.** Already declared; keep it.
- **The wheel-held check.** `scripts/build` (the project's `build` target, in its
  `check`) already builds the publishable wheel through `just python-sdk-dist` — the
  same recipe `release.yml`'s `build-python-sdk` job runs — installs it into a fresh
  virtualenv and imports it. Extend that proof so it also fails when the built wheel
  lacks `onemessagebus/py.typed` or its `METADATA` lacks `Classifier: Typing :: Typed`,
  reading the wheel as the zip it is, with the same one-line failure-and-next-action
  shape the script's other refusals take; or put the same assertion in a test under
  `tests/` that builds the wheel through `just python-sdk-dist` into a temporary
  directory, whichever fits the repository's own shape better. Either way, the assertion
  is over the built wheel, never over the source tree.
- **Regeneration keeps it.** A test runs the real generator (`scripts/generate.py`, as
  `tests/test_scripts.py` already does) and shows `py.typed` still present afterwards.

Nothing about the SDK's runtime changes. Do not touch the CLI wheel (the root
`pyproject.toml` is a maturin `bin` wrapper with no importable module, exempt by the
invariant), the Node SDK, or the release workflow's publishing jobs.
...
this a problem across all our python sdks?" This SDK declares `Typing :: Typed` and ships
no marker, so the classifier is a promise the wheel does not keep: a consumer's mypy
would still read every name as `Any`. The user's direction: "i want to make sure we
handle this properly in all of them" — held to the built wheel, which is why the proof
here reads the wheel `build-python-sdk` builds.

## Acceptance criteria

- The publishable wheel the repository's Python SDK dist recipe produces — the one
  `release.yml`'s `build-python-sdk` job builds — contains `onemessagebus/py.typed` as an
  empty member, and its `METADATA` carries `Classifier: Typing :: Typed`.
- A check reached by the `onemessagebus-python-sdk` project's `check` (through its
  `build` target or its `test` target) asserts both facts on that built wheel's contents
  and fails when either is absent.
- The marker lives outside `src/onemessagebus/_generated/` and `models.py`, and a test
  that runs the real generator shows `py.typed` present in the package afterwards; the
  project's `lint` target, which runs `generate.py --check`, still passes.
- `scripts/pack.py`'s tests still pass, including the one that lists exactly
  `README.md`, `pyproject.toml` and `src` in the packed copy.
- This change publishes nothing and touches no release workflow or secret: the diff
  changes no file under `.github/workflows/`, and no criterion here rests on a release of
  this package.
- The checks that exercise this change are green over the finished tree: the
  `onemessagebus-python-sdk` project's `format-check`, `lint`, `typecheck`, `test` and
  `build` targets, and the judged lint over this change's diff.
...
`python/onemessagebus-sdk/AGENTS.md` is the package's own rules: the development
environment is `scripts/run`, not `uv run`; the tests drive the built binary
(`cargo build -p onemessagebus-cli`); versions are stamped by `pack.py`, never written.
Iterate with the one Nx target that reports something and take the repository's
diff-scoped judged lint for the last check. The cheapest evidence that the wheel check
can fail is to induce the absence once — remove the marker, run the check, quote the
failure, restore — and say in your report that the quoted run is the induced one. Commit
each piece as it works.

Publication is through the harness after you settle; the branch is verified by the
repository's own required checks downstream of you. `publish-python-sdk` failing on the
empty `PYPI_SDK_TOKEN` secret on every release (v0.5.0–v0.7.0) is a fact about the
release workflow's secrets, not about this branch, and is nothing you can or should fix
here: say it in your report. Finish the branch, commit everything, leave the tree clean,
and report.
...
{"onepipeline.execution_checkout":"nickderobertis__onemessagebus","onepipeline.id":"omb-sdk-py-typed","onepipeline.persona":"engineer","onetaskgraph.item_kind":"task"}
-->
