...
**fix(release): take the semver reading without a built browser view**

*Project description:*
## What

Make onepipeline-ui's release automation reach a verdict again on `main` after #79 (merge
commit 3259a643, "feat(serve): serve the browser bundle and the read API on one origin"),
so that the release carrying #79 can be cut.

### The gap

Since that merge the `Release-plz` workflow on `main` fails at its step *Read the public
surface against the baseline* (`just semver-check <baseline-root> v0.11.0`, run
35576409661 at 2026-09-21T08:13Z). `cargo semver-checks` builds the current crate's
rustdoc through a generated manifest with the crate's features enabled — its default
feature heuristic includes `bundled-ui`, which is not a default feature — and `build.rs`
refuses that build because `apps/dag-ui/dist` does not exist in that job:

    onepipeline-ui: the `bundled-ui` feature is on, but there is no built browser view at
    /home/runner/work/onepipeline-ui/onepipeline-ui/apps/dag-ui/dist
    ACTION: build it first — `just build` — ...
    error: failed to build rustdoc for crate onepipeline-ui v0.11.0
    ##[error]the reading exited 101 without a verdict; the release would otherwise be
    versioned as API compatible with nothing read

`scripts/semver-check.sh` correctly refuses to let a reading that did not happen stand as
"compatible", so no release PR is opened or updated: v0.11.0 stays the latest release,
and #79 and everything after it on `main` is unreleased. Downstream, this harness's own
node adopting #79 is held on that release with no timeout.

### What to change

The reading has to *happen* — reach exit 0 or 100 — on a checkout that holds no built
browser view, while keeping #79's contract: a binary built **with** `bundled-ui` and no
bundle is still refused at build time naming what is missing, and one built without the
feature still refuses `--ui` at run time and serves a view from `--ui-dist` only. Which of
these you do is yours to decide from the code, and the choice and its reason go in the
header comment of whichever file carries it:

- have the reading build the crate's surface without the bundle — for instance by having
  `scripts/semver-check.sh` name the feature set `cargo semver-checks` reads (its
  `--default-features` / `--only-explicitly-listed-features` / `--features` flags), if
  `bundled-ui` changes no public item; or
- have the release-plz job build the view before it takes the reading, the way
  `release.yml` already does for the prebuilt distributions; or
- another shape that satisfies the criteria below.

Do not weaken `build.rs`'s refusal into a warning, and do not make `scripts/semver-check.sh`
treat a build failure as a verdict — that script's header says why both are the failure
it exists to stop. If `bundled-ui` does gate a public item, the reading has to cover it,
which rules out the first shape.

### Where it is recorded and proven

`tests/e2e/semver_check.rs` drives `just semver-check`; extend it so the journey covers a
current tree that carries no `apps/dag-ui/dist`, and reaches a verdict. The workflow file
is CI configuration nothing here can run, so where the change is only there, the proof is
the script or recipe the workflow step calls, driven by that journey with the same inputs
the step hands it. Add the reason for the change to the comment beside the step or script
you touched, so the next reader knows why a reading without a bundle is the intended one.

Start the change request description with what only this dispatch knows: the failing run
above, which shape you chose and why the other was not taken.

## Why

The user asked for a scripts audit of ai-orchestrator that retires its workaround code,
and one retirement — deleting the local bun proxy that served the DAG view beside the
read API — waits on the onepipeline-ui release that serves the view itself (#79). That
release cannot be cut while the release gate cannot read the crate's surface, and the
manager's rule is that a release blocker is fixed at its source in the repository that
owns it, never routed around in the consumer: the consumer's node is held on the
published release exactly so that it adopts a real one.

## Acceptance criteria

- In a checkout of this crate at the finished tree with no `apps/dag-ui/dist` present,
  the public-surface reading the release workflow's *Read the public surface against the
  baseline* step takes, against the v0.11.0 baseline prepared as that workflow's *Use the
  latest release tag as the version baseline* step prepares it, ends in a verdict — the
  surface read as compatible or as broken — rather than the reading-not-taken failure
  quoted above.
- A build of the crate with the `bundled-ui` feature in a checkout with no
  `apps/dag-ui/dist` still fails at build time with the message naming the missing view,
  and a binary built without the feature still refuses `--ui` naming what is missing and
  serves a directory given as `--ui-dist`; the tests that hold each of these
  (`tests/packaging.rs`, `tests/e2e/ui.rs` and the `ui::`/`cli::` tests #79 added) pass
  unchanged in what they assert.
- `tests/e2e/semver_check.rs` drives the reading over a current tree without a built
  browser view and asserts it reaches a verdict, and that journey passes.
- The file that carries the change states in its header comment why a reading without a
  bundle is the intended one, naming the failure it fixes.
- The judged lint over this change's diff is green, and the focused tests named above pass.
- Every claim this dispatch makes about the finished work is true of the tree as it
  finally stands.

## Additional info

The reading the first criterion names is `just semver-check <baseline-root> v0.11.0`, and
the build the second names is `cargo build --features bundled-ui`; running their pieces
separately is fine.

The failing run is https://github.com/nickderobertis/onepipeline-ui/actions/runs/35576409661;
`gh run view -R nickderobertis/onepipeline-ui 35576409661 --log-failed` prints the step
log quoted above. The two prior reads in this repository's history of a reading that did
not happen are in the header of `scripts/semver-check.sh`. This repository publishes
through a change request whose required checks are its merge path; the Release-plz
workflow runs on `main` after the merge, and the manager reads its result there.
...
{"onepipeline.execution_checkout":"nickderobertis__onepipeline-ui","onepipeline.id":"ui-release-semver-reading","onepipeline.node":"ui-release-semver-reading","onepipeline.persona":"engineer","onetaskgraph.item_kind":"task"}
-->
