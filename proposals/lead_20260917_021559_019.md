...
**fix(local-md): fail a listing that meets a malformed record**

*Project description:*
## What

Ticket: https://github.com/nickderobertis/onetaskgraph/issues/1313 — *local-md list silently drops
malformed items*.

**Root cause (from the ticket).** The local-md list path discards every task or document whose parser
returns an error by applying `filter_map(... .ok())`
(`crates/onetaskgraph-local-md/src/lib.rs:746-759`, basis `9158e053c7a6bb88ad8874cc7ab24b6769b9757d`),
while `lib.rs:703-715` turns a malformed delivery reference into `SourceError::Malformed`. A task
whose `delivers` holds an empty or invalid task reference is omitted from `task list`, which can
return an empty `items` array with no errors while the file exists; `task show` on it reports the
malformed entry. Transcript: `onepipeline transcript delivers-plan op-queued-delivers`.

**The accepted fix.** Collect parse results as `Result<Vec<_>, SourceError>` so a malformed record
makes the list operation fail with its path and parse diagnostic instead of disappearing. Apply the
same rule to tasks, projects and documents, and add query tests proving malformed front matter and
malformed delivery references are surfaced. A multi-source query keeps the store's existing
partial-failure rule (a failed source fails the command unless `--allow-partial`).
...
This host's plans and follow-up drafts are local-md sources, and the engine reads plans through
this store. A listing that silently omits a malformed record hides work from managers and from the
engine until somebody knows which item is missing.

## Acceptance criteria

- Listing tasks, projects or documents from a local-md source containing a malformed record fails
  with a `SourceError` naming the record's path and its parse diagnostic, rather than returning the
  remaining items as a success.
- Query tests cover malformed front matter and a malformed `delivers` reference for tasks, and
  malformed records for projects and documents.
- A journey drives the real CLI's `task list` over a local-md root holding one malformed task and
  proves it exits non-zero naming that file.
- A test drives a query over two local-md sources together, one healthy and one holding a malformed
  record, and proves the command fails by default naming the malformed source's file, and that with
  `--allow-partial` it returns the healthy source's items while reporting the failed source.
...
- The checks that exercise this change — the local-md query tests, the CLI journey this change adds, and the repository's judged lint over this change's diff where it runs one — are green over the finished tree.
...
{"onepipeline.amendment":"Manager amendment (binding; where this and the task's operational notes disagree, this wins). The criterion that the repository's judged lint over this change's diff is green over the finished tree means: the finished tree carries one run of that judged diff lint taken after the last commit that changes anything the lint reads, that run reported no findings, and the report cites it. The operational note \"clear the findings it names, then stop — do not re-run hunting a clean sheet\" forbids re-running the lint over an unchanged tree to get a different verdict; it does not excuse a tree the lint never passed. A tree changed after a lint run that named findings owes one further run, and only a run naming something new owes another. The prior dispatch's work is committed on this branch: the collectors, the query tests, the CLI journey and the label/dependency coverage the lint asked for are already there; read the branch before adding anything. One criterion is added over the prior dispatch's task: the two-source partial-failure test, which proves the store's existing `--allow-partial` rule still holds now that a malformed record fails its source. What happened since that amendment was written, and what this dispatch owes on top of it. The branch published twice as https://github.com/nickderobertis/onetaskgraph/pull/1472, and both times the repository's required check `check (windows-latest)` refused it on the existing test `a_reader_never_sees_a_record_part_written` in `crates/onetaskgraph-local-md/tests/metadata.rs` (panics at lines 736 and 762): with the list no longer swallowing a read error, a reader racing the metadata writer's atomic replacement of the same file on Windows now observes a transient failure it used to hide. The third dispatch committed a candidate repair — `4d83d11` (synchronize atomic record replacement), `c7bf2c8` and `f6bbc5c` — and was then cut off by a failure of its judge's own harness turn before it published, so those three commits are on this branch and on no remote; nothing has judged them. Read them first. The finished tree has this property: a listing that races the writer's atomic replacement of a record on Windows sees either the old record or the new one and never an error, while a record that is stably malformed or escapes the root still fails the listing as the ticket accepts; the code's own comments say why the Windows path needs it, so a reader on any platform can follow the reasoning without running Windows. The prior repair `d1ee7ed` (retry transient resolution races) is on the branch too; the tree keeps one coherent mechanism rather than both stacked. The pull request's title still matches the node's title, and its body is finished by the drafter, so the report starts the description with what only this dispatch knows.","onepipeline.branch":"onevcs/s-77fc9a2e62e4","onepipeline.id":"otg-1313-3","onepipeline.max_turns":200,"onepipeline.persona":"engineer","onepipeline.resume":{"branch":"onevcs/s-77fc9a2e62e4"},"onetaskgraph.delivers":["followups:I_kwDOUBBgxs8AAAABRk8n9Q"],"onetaskgraph.item_kind":"task"}
-->
