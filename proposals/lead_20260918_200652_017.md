...
**fix(github-projects): preserve visible body bytes around the metadata slot and give sweep tests unique registrations**

*Project description:*
## What

Two accepted tickets against `crates/onetaskgraph-github-projects`, one in the plugin and
one in its test suite, folded into one node because neither is large enough to pay for a
dispatch of its own. One commit per ticket.

**1. GitHub metadata reads trim visible trailing whitespace** —
https://github.com/nickderobertis/onetaskgraph/issues/1835, accepted at severity `medium`.
`crates/onetaskgraph-github-projects/src/lib.rs`'s parser of a terminal metadata slot
applies `trim_end()` to all visible issue content before the slot, although the slot's
writer preserves the pre-existing bytes — so an issue body `"Design prose.\n"` gains a
slot without those bytes changing and reads back as `"Design prose."`, breaking the
narrow-write preservation the plugin promises. Change the slot parser and composer
together so the read removes only the canonical separator the composer inserts for the
slot and preserves every other visible-body byte, and add a round-trip test for a body
ending in a newline (and one ending in several) that writes metadata and reads the
content back byte for byte.

**2. Sweep-gate tests race on fixed process registrations** —
https://github.com/nickderobertis/onetaskgraph/issues/1838, accepted at severity `high`.
`crates/onetaskgraph-github-projects/tests/sweep_gate.rs` uses small fixed process numbers
in tests sharing the process-wide `RUNS.registry`, so parallel tests contend for one
registration: `a_process_id_reissued_to_a_new_run_costs_a_delay_and_never_a_deletion`
constructs `ended_run(5)` and expects `Registration::take` for process 5 to succeed, and
the recorded gate failed there while an immediate re-run passed. Give every sweep-gate
test a unique process registration allocated by a shared test helper, keeping an explicit
reissue inside the one test whose subject is PID reuse — that test still reissues, but a
number no sibling can hold.

## Why

The user accepted these against this host's own `plans` and `followups` boards: the first
means adding or updating a ticket's metadata — which every follow-up copy and every
settlement write-back does — changes the content the board reads back for any issue whose
prose ended in whitespace, and the second makes this repository's own gate refuse valid
publications one run in several until somebody re-runs it. The first is not fixed for this
host until the plan-store executable this checkout spawns carries it, which a sibling
node of this plan adopts.

## Acceptance criteria

- Writing a metadata slot onto an issue whose visible body ends in trailing whitespace of
  any kind — newlines, spaces, tabs, in any combination — and reading the task back
  through the source yields content byte-identical to the original visible body, and
  reading an issue that carries no slot is unchanged.
- The slot composer and parser share one statement of the separator the read removes, and
  a test fails when the two disagree.
- Every sweep-gate test registers a process number no sibling test can hold, allocated by
  one shared helper, and the PID-reuse test still exercises an explicit reissue of its own
  number.
- The branch's history carries, for each of these tickets, a closing reference naming its
  issue (1835, 1838), so the change that carries the work is what closes it.
- The byte preservation is proven through the plugin's real metadata-write and task-read
  entry points, as the engine calls them, over a recorded board fixture, rather than
  through the composer and parser called on their own.
- With every sweep-gate test running concurrently, no two tests hold one registration and
  none observes another's, so the suite's verdict does not depend on scheduling.
- The tests that cover the crate this change touches are green over the finished tree,
...
Iterate with this repository's diff-scoped judged lint and the github-projects crate's
tests rather than the whole gate; the required checks run downstream on the change this
branch becomes. The plugin's live journeys need a board credential this dispatch does not
carry; the round trip is provable through the plugin's own composer and parser and its
recorded-fixture tests under `crates/onetaskgraph-github-projects/tests/fixtures`, so say
which tier proved it.

This repository's releases are cut from conventional commits, and this host's plan-store
pin adopts the release that carries the first fix, so a commit type that cuts no release
would leave that adoption waiting. `fix` is the type for both.
...
{"onepipeline.branch":"onevcs/s-0ddd93c7fe4f","onepipeline.execution_checkout":"nickderobertis__onetaskgraph","onepipeline.id":"otg-github-projects-3","onepipeline.persona":"engineer","onepipeline.resume":{"branch":"onevcs/s-0ddd93c7fe4f"},"onetaskgraph.delivers":["followups:I_kwDOUBBgxs8AAAABRy2YVw","followups:I_kwDOUBBgxs8AAAABRy2y1w"],"onetaskgraph.item_kind":"task"}
-->
