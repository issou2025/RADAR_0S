...
**fix(linear): tell a missing label apart from a duplicated one**

*Project description:*
## What

Ticket: https://github.com/nickderobertis/onetaskgraph/issues/1309 — *Linear label resolution
conflates zero and duplicate matches*.

**Root cause (from the ticket).** Linear label resolution emits one uniqueness refusal for both
zero matches and multiple matches: `crates/onetaskgraph-linear/src/lib.rs:910-918` (basis
`9158e053c7a6bb88ad8874cc7ab24b6769b9757d`) uses the same message for every non-singleton result.
Index lag right after a label is created is therefore indistinguishable from duplicate data; a live
write immediately after creating a unique label reported only that it could not resolve the label
uniquely. Transcript: `onepipeline transcript delivers-plan otg-delivers-3`.

**The accepted fix.** Report zero and multiple matches distinctly, each naming the label and the
match count (and, for duplicates, the matching ids), and make the zero-match path eligible for
bounded settling where writes depend on newly created labels: the live journey's
`settle::settled_label` waits out a zero-match refusal within its bound, while a duplicate-match
refusal is returned at once, because waiting cannot fix duplicated data.
...
Operators cannot choose the right recovery when "not indexed yet" and "duplicated" read the same,
and the live lane's label wait cannot tell a lagging index from bad data.

## Acceptance criteria

- The Linear plugin's label resolution refuses zero matches and multiple matches with distinct
  messages, each naming the label and the count, proven by the plugin's loopback tests
  (`tests/plugin.rs`) for both cases.
- The settling helper waits out a zero-match refusal until the label resolves or its bound is
  spent, and returns a duplicate-match refusal immediately, proven in `tests/settle_gate.rs` against
  a loopback label filter that answers late, never, and twice.
...
- The checks that exercise this change — `tests/plugin.rs`, `tests/settle_gate.rs`, a build of the live test target, and the repository's judged lint over this change's diff where it runs one — are green over the finished tree.
...
**Stacked** on `otg-1308`; `otg-1311` stacks on this branch.
...
{"onepipeline.id":"otg-1309","onepipeline.max_turns":200,"onepipeline.persona":"engineer","onetaskgraph.delivers":["followups:I_kwDOUBBgxs8AAAABRkMxgQ"],"onetaskgraph.item_kind":"task","orchestrator.plan-review":{"by":"planning-closeout","key":"af7fdd4bb75d8f6e75e7b6a74c6f7170a26d8467a151a7c093f44c31e5cfbe90","reviewed_at":"2026-09-16T15:22:01.100351+00:00"}}
-->
