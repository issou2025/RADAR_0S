...
**feat(status): answer a reference that names no known work with a typed error**

*Project description:*
## What

`release_status` answers a reference that names no work this host knows with
`Error::Invalid` whose prose reason reads "… names no work this host knows"
(`crates/onevcs/src/status.rs:2273` and `:2279`), so a caller that must react to exactly
that case — onepipeline's release asker falling back from a squash commit to the change
request it settled (https://github.com/nickderobertis/onepipeline/issues/420) — can only
match prose.

**The contract, fixed for this run** (this plan's `op-relink-onevcs` links it and
`op-release-fallback` matches it): `onevcs::Error` gains
`UnresolvableReference { reference: String, reason: String }`, returned exactly where the
reason today reads "names no work this host knows" — at both sites, carrying the same
reason text — and nowhere else. The CLI reports it with the exit code `Error::Invalid` has
today (2), so no command's output or exit status changes. `docs/contract.md` states the
variant and when it is returned, and `crates/onevcs/tests/contract.rs` holds it.

This is the seam and its test and nothing more; if it turns out to need more than that,
ask over `$ORCHESTRATOR_ASK_MANAGER` before widening it.

## Why

The user accepted onepipeline #420 so that a published dependency is released without an
operator restating a settle. The fallback that does that has to fire exactly when onevcs
cannot resolve a reference and never on an I/O or host error; without a typed answer the
only way to tell those apart is matching error prose, which breaks silently the day the
wording changes.

## Acceptance criteria

- `release_status` over a reference naming no work the host knows returns
  `Error::UnresolvableReference` carrying the reference and the reason, at every site that
  answered that case with `Error::Invalid` before, and no other error is reported as it.
- The CLI's output and exit status for that case are unchanged, proven by a journey driving
  the real `onevcs release status` over an unknown reference.
- A library test drives `release_status` over an unknown commit and an unknown
  change-request URL and asserts the typed variant.
- `docs/contract.md` states the variant and `tests/contract.rs` holds it.
- The tests covering what this change touches — the release-status e2e journeys,
  `tests/contract.rs` and the unit tests in `status.rs` — are green over the finished tree,
  and so is the judged lint over this change's diff.
- The branch's history names https://github.com/nickderobertis/onepipeline/issues/420 as
  the consumer, without closing it.
- Every claim this dispatch makes about the finished work is true of the tree as it
  finally stands.

## Additional info

onevcs has no local hooks; its merge path is the CI in `.github/workflows/ci.yml`. `just test-one <name>` and `just test-quick` are the narrow runners. `docs/contract.md` states the public surface and `crates/onevcs/tests/contract.rs` holds it: a change to a public type is stated in both. onevcs releases from conventional commits through release-plz with `semver_check` on, and this plan's onepipeline relink waits for the release carrying this node. If `Error` is not `#[non_exhaustive]`, a new variant is a breaking change and the semver check will say so: use `feat!` then, `feat` otherwise.
...
{"onepipeline.execution_checkout":"nickderobertis__onevcs","onepipeline.id":"ov-unresolvable-reference","onepipeline.max_turns":12,"onepipeline.node":"ov-unresolvable-reference","onepipeline.persona":"engineer","onetaskgraph.item_kind":"task"}
-->
