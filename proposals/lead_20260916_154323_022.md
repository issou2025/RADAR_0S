...
**fix(tests): stop the sandbox's Nx daemon and retry llmlint cache teardown**

*Project description:*
## What

Ticket: https://github.com/nickderobertis/onepipeline/issues/321 — *llmlint cache sandbox teardown
races late writers*.

**Root cause (from the ticket).** `npm/test/llmlint-cache.test.mjs` removes a sandbox checkout once
(`removeSandbox`, `rmSync` at lines 340-353 at basis `32c8316ad950975863b3a48c9a32df679b60bc14`)
while background tooling — the sandbox's Nx daemon — can still create `.nx` files inside it, and it
deliberately performs no bounded retry. Different judged-tier subtests have failed in `t.after`
with `ENOTEMPTY` while `.nx` files carried mtimes later than the sandbox. Transcript: `onepipeline
transcript delivers-plan op-queued-delivers-2` (run on this host; readable only where that run
directory exists).

**The accepted fix.** Disable or stop the sandbox's Nx daemon before teardown (for example run the
sandbox's Nx with `NX_DAEMON=false`, or stop its daemon before removal), and use a short bounded
retry of the removal that keeps the existing diagnostics (`removalDiagnosis`: what was left and
what was running) on exhaustion and rethrows the last error. The doc comment on `removeSandbox`
currently argues against retrying; rewrite it to state the new rule and why the daemon stop
addresses the late writer.
...
An already-passing subtest can fail its teardown with `ENOTEMPTY`, refusing onepipeline's gate
for unrelated changes. Every plan that changes onepipeline publishes through that gate, so the
flake costs this host re-dispatches and manual re-runs.

## Acceptance criteria

- The sandbox's Nx daemon is disabled or stopped before its sandbox is removed.
- Sandbox removal retries a bounded number of times with a short delay; a test drives the real
  removal helper against a sandbox that a writer keeps populating briefly after removal starts and
  proves it succeeds once the writer stops.
- A test drives the helper against a sandbox that never empties and proves it gives up within its
  bound, writes the diagnosis of what was left, and rethrows the removal's error.
...
- The checks that exercise this change — the npm test file this change touches, and the repository's judged lint over this change's diff — are green over the finished tree.
...
{"onepipeline.id":"op-321","onepipeline.max_turns":200,"onepipeline.persona":"engineer","onetaskgraph.delivers":["followups:I_kwDOTx8aUc8AAAABRkM7tQ"],"onetaskgraph.item_kind":"task","orchestrator.plan-review":{"by":"planning-closeout","key":"fcdbe9dea6ae1ae6eb01b2c5f844f0ab071132a2bb1a1f2b908926124814f2ad","reviewed_at":"2026-09-16T15:22:01.068950+00:00"}}
-->
