...
**fix(local-md): atomic status writes, listing stable across a replacement, and a project-scoped walk**

*Project description:*
## What

Three accepted tickets against `crates/onetaskgraph-local-md/src/lib.rs`, which is why
they run in one node; the second and third both change the traversal in `paths`, and the
second's own regression is the listing the first's concurrency test performs. One commit
per ticket, in this order.

**1. Status and delivery writes are non-atomic** —
https://github.com/nickderobertis/onetaskgraph/issues/1836, accepted at severity `high`.
`set_task_status` and `set_delivered_by` both go through `rewrite_front_entry`, whose last
step is a plain `fs::write`, while metadata writes persist through the staging-file-and-
rename replacement (`replace_atomically`), so a concurrent reader can see a truncated or
half-written task whenever status or delivery changes. Make `rewrite_front_entry` persist
through that same atomic replacement helper, and apply the metadata concurrency regression
pattern (`crates/onetaskgraph-local-md/tests/metadata.rs`'s concurrent listing during
repeated replacements) to both the status and the delivered-by writes.

**2. Listing races the atomic replacement on Windows** —
https://github.com/nickderobertis/onetaskgraph/issues/1837, accepted at severity `medium`.
`paths`'s `visit` canonicalises every listed entry during traversal and compares that
transient result with the source's earlier canonical root; while a replacement is renamed
over the same path, Windows can answer a spelling for that instant that fails the lexical
containment check (`a.md escapes configured root` on a job whose two displayed paths named
the same extended-length temporary root), so a concurrent write makes `query_tasks` fail
spuriously and makes the regression from piece 1 flaky. Make the containment check stable
across an in-place rename — compare resolved file identity, or a consistently normalised
root-relative path, rather than a canonicalisation taken mid-rename — and add a stress
regression that lists during repeated replacements, gated to run on Windows where the
race is observable and still exercising the same code on other platforms.

**3. A project-scoped task list walks every project's records and reports a benign
vanish as malformed** — https://github.com/nickderobertis/onetaskgraph/issues/1992,
accepted at severity `high`. `query_tasks` (through `parse_work_records`, which calls
`paths` unconditionally) walks the whole `tasks/` tree whatever the query's `project`
filter says, applying the predicate only in memory afterwards, because a task's project is
a front-matter key rather than a directory boundary; and `visit` maps any file that
vanishes between enumeration and `canonicalize` to `SourceError::Malformed` — a deliberate
change from silently hiding malformed records, but one that cannot tell "malformed" from
"deleted by another process in an unrelated project". So a project-scoped `task list`, and
`task copy` and everything built on its dry run, fail on an unrelated non-task file
anywhere under the whole tree, naming a file the caller never asked about; this host's
own follow-up ticket copies hit it three times in one run. Two changes: a vanish
(`ENOENT`) met mid-walk is skipped rather than reported, with `Malformed` reserved for a
record that was actually read and failed to parse; and a query naming a `project` reads no
more than it must to answer — read the front matter to filter before treating anything
about a record as fatal, or keep an index keyed by project, whichever the plugin's own
conventions and its per-file `Location` reporting support (decide from the code and say
which in your report). A record under another project that fails to parse must not fail
a query scoped to a project it is not in, and a record in the scoped project that fails to
parse still must.

## Why

The user accepted all three against this host's own stores: the `authoring` root every
planner writes into, the `drafts` root every follow-up ticket lives in, and the `plans`
copies. The first means a reader can observe a half-written task on any status change —
the settlement write-back every run performs. The second makes the very regression that
proves the first flaky on the required Windows leg. The third stopped a run from copying
any of its verified tickets to the followups board because one unrelated file elsewhere
in the drafts tree did not parse, with no workaround short of a code fix. None of them is
fixed for this host until the plan-store executable this checkout spawns carries them,
which a sibling node of this plan adopts.

## Acceptance criteria

- A status write and a delivered-by write each replace the task file atomically, and a
  reader listing or reading during a stream of such writes never observes a truncated or
  partly written record.
- A listing performed during repeated in-place replacements of a record succeeds, its
  containment check compares resolved identity or a consistently normalised root-relative
  path rather than a canonicalisation taken mid-rename, and a record that genuinely
  resolves outside the configured root is still refused — proven on this host, with the
  Windows stress regression committed for the downstream leg, whose result is not this
  node's bar.
- A query naming a project decides a record's membership from its front matter and reads
  no further into a record that is not in that project: a test with a record whose front
  matter names another project and whose remainder cannot be parsed proves the scoped query
  succeeds, returns exactly the scoped project's records, and reports nothing about that
  record — which only holds if the plugin stopped at its front matter or answered from an
  index keyed by project — while a record whose front matter names the scoped project and
  fails to parse still fails the query as malformed, naming that file; membership is never
  inferred from where a record sits on disk.
- A file that vanishes between enumeration and resolution is skipped rather than reported
  as malformed, in every query.
- `docs/local-md.md` states what a project-scoped query reads and how a vanished file and a
  malformed record are each treated.
- The branch's history carries, for each of these tickets, a closing reference naming its
  issue (1836, 1837, 1992), so the change that carries the work is what closes it.
- Every behaviour this change adds or alters is proven by a test or journey driving the
  real plugin and, for the scoped query, the real CLI — including the concurrent-writer
  and mid-walk-vanish cases under real concurrency — rather than by inspection, and no
  test substitutes a double for the layer under test.
- The tests that cover the crate and the documents this change touches are green over the
  finished tree, and so is the judged lint over this change's diff.
- Every claim this dispatch makes about the finished work is true of the tree as it finally
  stands.

## Additional info

One commit per ticket is the shape to aim for, so each squash headline and each closing
reference read cleanly, but it is guidance rather than a bar: what the finished branch owes
is the closing reference for every issue.

Iterate with this repository's diff-scoped judged lint and the local-md crate's tests plus
the CLI journeys that read a local-md store rather than the whole gate; the required
checks run downstream on the change this branch becomes. The Windows leg cannot be run
here — what you can prove locally is the identity-based containment check and the stress
regression running green on this platform; say so in your report.

This repository's releases are cut from conventional commits, and this host's plan-store
pin adopts the release that carries these fixes, so a commit type that cuts no release
would leave that adoption waiting. `fix` is the type.

`scripts/bootstrap-workspace.sh` today installs the pinned `release-plz` into the global
cargo bin directory when the host's differs; that is onetaskgraph #1990's defect, resolved
by a sibling node of this plan, and not yours to touch here.

Board items on the follow-ups board are not yours to move — its credential is masked from
every dispatch. The `Closes` lines are the whole of what this node does about the issues.
...
{"onepipeline.branch":"onevcs/s-2957d1ece972","onepipeline.execution_checkout":"nickderobertis__onetaskgraph","onepipeline.id":"otg-local-md-2","onepipeline.persona":"engineer","onepipeline.resume":{"branch":"onevcs/s-2957d1ece972"},"onepipeline.settlement":{"branch":"onevcs/s-2957d1ece972","detail":"onevcs: push rejected: the publishing push of \"onevcs/s-2957d1ece972\" was rejected by the merge path: error: failed to push some refs to 'https://github.com/nickderobertis/onetaskgraph.git'. Its whole output is artifact a-b9f85d6257b5 — `onevcs artifact cat a-b9f85d6257b5`, and preserved at /home/whoop/.onevcs/workspaces/github.com-nickderobertis-onetaskgraph-9c94c67fcf19/runs/s-2957d1ece972/gate-logs/onevcs-s-2957d1ece972/gate-0004.log. It said:\n[…earlier output omitted; the whole of it is where this refusal says…]\nsrc/onetaskgraph_sdk/_generated/status_category.py                           11      0   100%\nsrc/onetaskgraph_sdk/_generated/status_options_report.py                     33      0   100%\nsrc/onetaskgraph_sdk/_generated/task_detail.py                              117      0   100%\nsrc/onetaskgraph_sdk/_generated/task_ref.py                                   5      0   100%\nsrc/onetaskgraph_sdk/_generated/task_status_set.py                           61      0   100%\nsrc/onetaskgraph_sdk/client.py                                               68      0   100%\n-------------------------------------------------------------------------------------------------------\nTOTAL                                                                      1491      1    99%\n\\u{001b}[32mRequired test coverage of 95% reached. Total coverage: 99.93%\n\\u{001b}[0m\\u{001b}[32m\\u{001b}[32m\\u{001b}[1m30 passed\\u{001b}[0m\\u{001b}[32m in 282.92s (0:04:42)\\u{001b}[0m\\u{001b}[0m\n\n\n\n NX   Running target coverage for 6 projects and 2 tasks they depend on failed\n\nFailed tasks:\n\n- sdk-typescript:coverage\n\n  Run duration:      10m 39s\n  Cache:             0/7 hit (0%)\n  Critical path:     6m 7s (1 task)\n  Recoverable time:  4m 10s (39% of the run)\n\n  Recommendations:\n    - Increase parallelism to recover up to 4m 10s → https://nx.dev/docs/concepts/ci-concepts/parallelization-distribution?utm_source=nx-cli&utm_medium=cli&utm_campaign=performance-report&utm_content=parallelization.\n    - Drastically reduce your run duration by sharing a cache across your team and CI → https://nx.dev/ci/features/remote-cache?utm_source=nx-cli&utm_medium=cli&utm_campaign=performance-report&utm_content=remote-cache.\nerror: recipe `coverage` failed on line 89 with exit code 1\npre-push: the gate failed — see the output above.\npre-push: fix it and push again. Bypassing this hook pushes work nothing has\npre-push: checked, and the required checks on main will refuse it anyway.\nerror: failed to push some refs to 'https://github.com/nickderobertis/onetaskgraph.git'","outcome":"push-rejected","status":"failed"},"onetaskgraph.delivers":["followups:I_kwDOUBBgxs8AAAABRy2dWQ","followups:I_kwDOUBBgxs8AAAABRy2ijQ","followups:I_kwDOUBBgxs8AAAABR7a_rA"],"onetaskgraph.item_kind":"task"}
-->
