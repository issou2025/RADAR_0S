...
Adopt this workstream's cargo build contract in `oneagentgraph` (github.com/nickderobertis/oneagentgraph; checkout `oneagentgraph` (registered, `~/projects/oneagentgraph`)).

oneagentgraph is a Cargo workspace declared with `members = ["."]` — the root crate alone; no crate outside the workspace.
...
- `npm/test/launcher.test.mjs` resolves `target/debug/oneagentgraph` at the clone root; `scripts/npm-build.mjs` takes the binary path as an argument
- `_crate-test` runs `cargo llvm-cov nextest --all-features` over the crate; `.github/workflows/ci.yml`, `published-smoke.yml`, `release.yml`
...
- The checks that read the target directory, spawn a built binary or run under coverage are green over the finished tree (their commands are under `## Additional info`): the Node launcher tests, readers of `target/debug/oneagentgraph`; one cheap integration-test target compiled under the new profile.
...
- the Node launcher tests, readers of `target/debug/oneagentgraph`: `cargo build --locked --all-features && bash scripts/nx.sh run oneagentgraph-npm:test`
- one cheap integration-test target compiled under the new profile: `cargo nextest run --locked --all-features --test release_declaration`
- coverage: a scoped run through the crate's coverage interface reporting per-file line results: `cargo llvm-cov nextest --locked --all-features -E 'not binary(e2e)' --summary-only`
...
{"onepipeline.id":"oneagentgraph","onepipeline.persona":"engineer","onetaskgraph.item_kind":"task","orchestrator.plan-review":{"by":"planning-closeout","key":"d420ee51c8c838b244ee0ae5f2bb6abd49784854bbc0dd5e39787245a8a2ce2b","reviewed_at":"2026-09-20T09:43:09.608683+00:00"}}
-->
