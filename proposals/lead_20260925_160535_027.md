...
**feat(stop-guard): consult host-declared verdict sources beside unwatched, and never fail open**

*Project description:*
## What

Give `onepipeline stop-guard` a way for the host that registers it to name additional
verdict sources the verb consults on every stop, so a harness whose "may this turn end"
question is wider than `unwatched` is answered by this verb rather than by a wrapper the
host maintains beside it.

Today the verb answers one verdict over `unwatched` — whether the session owes a run
nothing is watching. Its whole surface is `--session`, `--continuation` and `--format`,
and there is no flag, payload field, or configuration through which a caller can add a
second question to the same verdict. `--help` states the scope in its first line: "Whether
a session's turn may end: one verdict over `unwatched`, for a harness's stop hook".

A host whose stop condition includes something only that host can compute therefore has
three roads, and each one loses a property this verb already holds:

- **A wrapper between the harness and this verb.** The host then owns verdict composition
  it cannot test against this verb's contract, re-implements the `--format` renderings, and
  re-derives `--continuation` de-duplication. Each is a thing this verb already does
  correctly, now forked into a caller that learns about every contract change by breaking.
- **A second hook entry beside this one.** The harness runs the two independently, so their
  verdicts cannot be combined into one answer, cannot be de-duplicated against each other
  across a continuation, and reach the user as two unrelated messages.
- **Dropping the extra question**, which is what actually happens, and leaves the condition
  unenforced while looking enforced.

The caller driving this is the `ai-orchestrator` harness, whose own stop condition is two
questions: a run nothing is watching, which this verb answers, and a branch preserved on
its origin but never published, which only that host can compute because the answer
depends on its own repository registry, its session labels and its acknowledgement records.
It has a verb of its own that answers the second (`just unfinished`). What it has no
supported way to do is get one verdict out of both.

## Why

The person who asked for this wanted a stop guard that will not let a turn end while work
the session owns is unfinished, and for them "unfinished" has always meant both halves: a
run nothing is watching, and a branch that was preserved and never published. The second
half is not hypothetical on their host. `just recoverable` there reports **15 preserved
unpublished branches** for a single repository identity right now, several of them from
sessions that ended believing they were done. Work that is preserved but unpublished is
invisible to every view that asks "is anything still running", because nothing is running:
the session ended, the branch sits on its origin, and no merge path has ever ruled on it.

They accepted a narrower deliverable on their own host rather than have that host grow a
wrapper around this verb, on the explicit grounds that a fill made in a consumer is never
promoted back and never deleted. This node is the other half of that decision: the
question belongs to the verb that owns the stop verdict, so the fix lands here, is
released, and the consumer adopts it and carries no composition code of its own.

## Acceptance criteria

- A caller registering this verb can declare one or more **additional verdict sources** the
  verb consults on every stop, and the declaration travels with the registration the
  harness already holds rather than requiring a configuration file the harness has no way
  to name. A stop with none declared is the verb as it behaves today.
- Each declared source is asked about **the same session the verb itself is answering
  about** — the one the payload names under `--format claude-code` and `codex`, not the one
  the environment names — and is handed that session in the shape this verb already
  publishes for its own input, so a source is drivable by hand with the same bytes the verb
  sends it.
- A source answers in **the vocabulary this verb already renders** — a block carrying a
  reason, a warning carrying a message, or nothing owed. No new verdict word is introduced,
  and a source's answer needs no knowledge of which `--format` the caller asked for.
- The verdict the harness receives is the **combination** of the verb's own answer and every
  source's, with the strongest winning and every blocking reason present in that one answer
  rather than only the first. All three `--format` renderings carry a combined verdict, and
  each renders it in the shape that format already documents.
- `--continuation` holds **per source**: a stop following a block answers nothing owed for
  the report it last blocked this session on, source by source, so a declared source cannot
  hold a turn in a loop it can never leave. A journey drives a block, then the continuation
  after it, and reaches an ended turn.
- **A source that cannot be consulted never silently passes.** A source that is missing,
  exits non-zero, times out, writes nothing, or answers outside the vocabulary is reported
  in the verdict the harness receives, naming the source and what went wrong, and the
  finished tree states which of block or warn that is and why that choice is the safe one.
  A guard that fails open while looking answered is the failure this criterion exists to
  refuse, and no code path reaches a `none` verdict by discarding a source's error.
- Every ending above is proven by a journey driving **the real verb** with real payloads on
  standard input over a real registry — the verb's own contract, not a unit call on an inner
  function — and each `--format` is driven at least once, including the session-targeting
  ending.
- The verb's own `--help` states the new surface and the failure rule, and the
  repository's documentation of the stop verdict says what a declared source is asked, what
  it may answer, how answers combine, and what happens when one cannot be consulted. No
  dated claim is added without whatever that repository requires to keep a date honest.
- The checks that exercise this change are green over the finished tree — the tests over the
  stop-guard surface and the crates this touches, and the lint over this diff — and the
  dispatch's report states what was changed, what was run over the finished tree, and the
  evidence for each claim. Nothing is weakened, skipped, ignored, marked or made
  conditional to reach green.
- Registering the verb in the consuming harness, and that harness's own view of
  preserved-but-unpublished branches, are **outside this node**: it delivers the capability
  and its proof in this repository, and the consumer adopts the release separately.

- At settlement the branch's tip commits the whole of this dispatch's work: no tracked
  file of the worktree differs from what that tip commits, and nothing is left staged or
  unstaged. Every check the report cites as green was run over content identical to what
  that tip commits, and the report establishes that identity for each one — by naming the
  commit the check ran over, or by stating that nothing the check reads changed between
  the run and the commit. The judged lint over this branch's diff against its own base is
  among the checks the report accounts for that way.
- The report accounts for the ten commits this branch already carries: what of that work
  the finished tree retains, what this dispatch added or corrected, and anything it redid
  or replaced, with the reason in each case.

## Additional info

**You are continuing a branch, not starting one.** Your session opens on
`onevcs/s-b3bc036bbb1a`, which already carries ten commits from this node's previous
dispatch — `5998466 feat(stop-guard): let a host declare further verdict sources with
--source` through `4d3f2ae fix(stop-guard): refuse a source whose input was not delivered
in full` — touching `src/stopguard.rs`, `src/cli.rs`, `src/driver.rs`,
`tests/e2e/stop_guard.rs`, `docs/stop-guard.md` and `docs/contract-divergences.md`, 1369
insertions in total. `onevcs` reports its provenance complete. Read that work first and
build on it.

**Why the previous dispatch did not settle done**, in its judge's own words: *"The
transcript shows edits after the reported commit, with no evidence that those final
changes were committed or that diff lint was rerun on them."* Its acceptance criteria were
otherwise met. So the gap is between what it verified and what it committed, and the
clauses above are what close it.

**Its worktree has been reaped**, so any edit it made after `4d3f2ae` is gone and cannot be
recovered — do not go looking for it. Establish what the tree still needs by reading the
tree and running the checks, not by reconstructing what the predecessor was doing.

Iterate with the checks that exercise this change — the `stop_guard` e2e journeys,
`cargo clippy --all-targets --locked -- -D warnings`, and the judged lint over your diff —
rather than this repository's whole gate; the complete bar runs downstream on the push that
publishes this branch.

Commit as you go, and make the last thing you do before reporting a check over the
committed tree rather than an edit on top of it.
...
{"onepipeline.branch":"onevcs/s-b3bc036bbb1a","onepipeline.execution_checkout":"nickderobertis__onepipeline","onepipeline.id":"op-stop-guard-consults-host","onepipeline.node":"op-stop-guard-consults-host-2","onepipeline.persona":"engineer","onepipeline.resume":{"branch":"onevcs/s-b3bc036bbb1a"},"onepipeline.supersedes":["op-stop-guard-consults-host"],"onetaskgraph.item_kind":"task"}
-->
