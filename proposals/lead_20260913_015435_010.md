...
**test(smoke): describe the scratch repository as the kept, reused one the smoke depends on**

*Project description:*
## What

Change the one sentence `ensure_repo` in `tests/smoke/main.rs` writes as the description of
the smoke's scratch repository, so that it says what is true of that repository instead of
inviting its deletion. Issue https://github.com/nickderobertis/ai-orchestrator/issues/855 §2
is the specification; this node delivers fix 1 of its "Accepted fix" and nothing wider.

Today the `gh repo create` call there writes:

> Scratch repository for onepipeline's real-everything smoke. Nothing here is kept.

The function's own doc comment says the opposite of that last sentence — the repository is
*"created rather than required … and never deleted, by this journey or any other"* — and the
whole file depends on it persisting: every run probes `DEFAULT_SMOKE_REPO`
(`nickderobertis/onepipeline-smoke`), reuses it when it exists, and there is no delete path
anywhere. Two people have now renamed it aside believing they were retiring leaked scratch,
and neither rename retired anything, because a renamed GitHub repository keeps redirecting
from its old name and the probe succeeds on it.

The new description must say, in one or two sentences a person reading the repository's
GitHub page can act on:

- the repository is **intentional**, and **reused by every smoke run**;
- **deleting or renaming it is not a cleanup** (a rename leaves the old name redirecting, so
  the smoke goes on writing to it);
- **`tests/smoke/main.rs` in `nickderobertis/onepipeline` owns it** — the file to read before
  touching it.

GitHub caps a repository description at 350 characters; keep it under that. Word the
sentence so it is true whatever the repository is called, since the human node of this plan
pastes the same wording onto the existing repository after restoring its name. Change no
behaviour of `ensure_repo`: the issue's "deliberately not fixed" item — refusing when the
resolved `nameWithOwner` differs from the requested slug — stays not fixed, and the probe,
the create call's other flags, and the readme wait stay as they are.

The credentialled smoke (`just smoke-real`, the `smoke` job of `ci.yml`) exercises
`ensure_repo` on the existing repository and never reaches the create call, so it cannot
prove the new sentence is what a created repository receives — and it is the merge path's
to run, not yours. What proves it inside this dispatch is a test that runs in the offline
tier (the smoke binary is excluded from that tier by name, so the test lives where that
tier runs — a sibling test binary reaching the smoke module through `#[path]`, the way the
harness's own does, or whatever that tier already provides): it puts a `gh` stand-in on
`PATH` that answers the probe as absent and records its invocations, calls `ensure_repo`
on a throwaway slug, and asserts on the `repo create` invocation it recorded.

## Why

Twice an operator has spent time removing something load-bearing and believed they had
succeeded; the second occurrence also produced a wrong follow-up entry, "leaked scratch,
safe to delete", which is the more durable cost — the artifact teaches every reader the
wrong thing about itself. The function that creates the repository writes the sentence that
gets it deleted; changing that sentence is the fix. The user asked for what remains of #855
to be dispatched, and every one of its four entries remains.

## Acceptance criteria

- The description string `ensure_repo` passes to `gh repo create` says that the repository
  is intentional and reused by every smoke run, that deleting or renaming it is not a
  cleanup, and that `tests/smoke/main.rs` owns it; the sentence "Nothing here is kept" is gone;
  it is under 350 characters; and it does not name the repository's own slug, so the same
  wording is true after a rename.
- `ensure_repo`'s behaviour is unchanged: the same probe, the same create flags apart from
  the description, the same readme wait, and no check of the resolved `nameWithOwner`.
- A test that runs in the offline tier drives the real `ensure_repo` with a `gh` stand-in on
  `PATH` that answers the `repo view` probe as absent and records every invocation, and
  asserts that the recorded `repo create` invocation carries the description the first
  criterion describes and the same other flags as before; the stand-in never reaches
  GitHub.
- Nothing else in the tree changes beyond the sentence, the comment beside it if that
  comment restated the old sentence, that test, and what wiring the offline tier needs to
  run it.
- The checks that exercise this change are green over the finished tree: that test, the
  smoke test binary compiling and linting clean under the offline tier's clippy over all
  targets, and the judged lint over the node's diff reporting nothing unaddressed — with
  nothing skipped and no suppression added to make any of them pass.
- Every claim the dispatch makes about the finished work is true of the tree as it finally
  stands.

## Additional info for this node

Read this repository's `AGENTS.md` and `tests/AGENTS.md` first; each is binding here. The
credentialled tier is `just smoke-real`; it needs a GitHub credential this dispatch is not
handed, refuses without one, and is not yours to run — the change request's own CI runs it.
Commit on the branch the session opened, under a `test(smoke): …` subject: this is test
content, in force by merging, and cuts no release.
...
{"onepipeline.id":"op-smoke-description","onepipeline.persona":"engineer","onetaskgraph.item_kind":"task"}
-->
