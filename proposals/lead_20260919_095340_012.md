...
**fix(build): make the network destinations optional core features and pin sibling crates exactly**

*Project description:*
## What

Two accepted tickets about this workspace's manifests, folded into one node because both
edit the workspace `Cargo.toml` and the core crate's manifest. One commit per ticket.

**1. Core unconditionally links the network destination plugins** —
https://github.com/nickderobertis/onetaskgraph/issues/1426, accepted at severity `medium`.
`crates/onetaskgraph-core/Cargo.toml` declares `onetaskgraph-github-projects` and
`onetaskgraph-linear` as unconditional dependencies while also owning the generic
subprocess serve loop, so a local subprocess-plugin host that wants only `local-md`
compiles the whole HTTP and TLS stack — a local-md-only host measured 61 extra packages
and roughly 110 seconds on a Windows build. Make the GitHub Projects and Linear
integrations fully optional cargo features of the core crate with no default feature
enabling them, gating `registry.rs`'s registration of each kind and everything that names
them, so a configuration naming a plugin this build did not compile is refused by name
with the feature to enable; then update each consumer inside this workspace to enable
exactly the destinations it uses — the `onetaskgraph` CLI crate enables both so the
binary this host installs changes nothing, and the reference subprocess host and every
test host enable what they exercise. Check what `cargo tree` reaches from a core-only,
local-md-only dependent afterwards, and record the before and after in your report.

**2. Lock-step workspace crates publish incompatible caret dependencies** —
https://github.com/nickderobertis/onetaskgraph/issues/1428, accepted at severity `high`.
The published crates require their siblings with caret requirements although exhaustive
enum matches and struct construction make patch releases lock-step incompatible:
`onetaskgraph-local-md 0.2.30` resolves `onetaskgraph-plugin-api 0.2.32` and fails to
compile on an uncovered `StatusCategory::Queued`, with no lockfile error. Change every
internal workspace dependency between these pre-1.0 crates to an exact `=<workspace
version>` requirement, and add a publish-time check that resolves each published crate
against the newest registry graph — a script the `publish-crates` job runs before
`cargo publish`, provable locally against a registry answer the test supplies — so a
sibling that would resolve to a different version fails the publication rather than a
downstream build. Confirm that `release-plz`, which rewrites these versions on every
release, keeps the `=` requirements in step (`scripts/check-real-release-preparation.sh`
drives the real pinned tool through a release decision without contacting GitHub, and
`scripts/check-distribution-contract.sh` holds the manifests to the release policy); if
the tool cannot, that is a fork to raise over `$ORCHESTRATOR_ASK_MANAGER` rather than a
reason to keep the carets.

## Why

The user accepted both because each turns a valid onetaskgraph release into somebody
else's broken build: a consumer that wants only the local store pays for two network
stacks it never runs and pushes cross-platform gates past their bounds, and a consumer
that takes one sibling at a patch behind the others compiles nothing, with no lockfile
error to point at the cause — this host's engine pins every onetaskgraph crate to one
exact release today as the workaround.

## Acceptance criteria

- A dependent that takes `onetaskgraph-core` with its default features links neither
  `onetaskgraph-github-projects` nor `onetaskgraph-linear` nor the HTTP and TLS stack
  behind them, and a configuration naming one of those plugins in such a build is refused
  naming the feature that enables it.
- The `onetaskgraph` binary this host installs still registers both destinations, and
  every workspace consumer enables exactly the destinations it uses.
- Every internal dependency between the published workspace crates is an exact
  requirement on the workspace version, and the release tooling's rewrite of those
  versions keeps them exact and in step.
- The publish job refuses to publish a crate whose siblings would resolve to a different
  version on the registry, and that refusal is driven by a test against a supplied
  registry answer.
- The branch's history carries, for each of these tickets, a closing reference naming its
  issue (1426, 1428), so the change that carries the work is what closes it.
- Every behaviour this change adds is proven by tests driving the real manifests, the real
  feature resolution and the real release-preparation script rather than by
  inspection, and no test substitutes a double for the layer under test.
- The tests and checks that cover the manifests, workflow and scripts this change touches
  are green over the finished tree, and so is the judged lint over this change's diff.
...
Iterate with this repository's diff-scoped judged lint, `cargo tree` over the feature
combinations, the distribution-contract and release-preparation scripts, and the crate
tests the feature gates touch rather than the whole gate; the required checks run
downstream on the change this branch becomes. Publishing itself cannot be exercised here;
the publish-time check is provable against a registry answer the test supplies, and the
`scripts/crate-publication-status.sh` reader shows how this repository already reads
crates.io.

A sibling node of this plan (`otg-release-tooling`) changes `.github/workflows/release.yml`
and `scripts/bootstrap-workspace.sh` after this node lands; keep your edits to the release
workflow to the one publish-time check step so that node has a stable base.
...
by the sibling node, and not yours to touch here.
...
{"onepipeline.branch":"onevcs/s-51d31f666bf9","onepipeline.execution_checkout":"nickderobertis__onetaskgraph","onepipeline.id":"otg-manifests-2","onepipeline.persona":"engineer","onepipeline.resume":{"branch":"onevcs/s-51d31f666bf9"},"onetaskgraph.delivers":["followups:I_kwDOUBBgxs8AAAABRp2OAg","followups:I_kwDOUBBgxs8AAAABRp28iQ"],"onetaskgraph.item_kind":"task"}
-->
