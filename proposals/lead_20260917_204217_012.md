...
**test(printobserver): close three races in the CLI tier's test support**

*Project description:*
## What

Three independent races in the `printobserver` crate's tests (and one twin of the second
in the `printobserver-server` crate's test support), each a small change to the lines its
cause needs, landed as **three commits on one branch** — each commit's subject naming the
test and the cause, so the squash headline can list all three — and proven together under
the load that broke them.

### 1. The stand-in's recording is read before it is written

`crates/printobserver/tests/service.rs::a_started_server_hands_every_turn_the_directory_the_sign_in_wrote`
panicked on **`main`'s own run** 35111698102 (365896e, job 104846763271) and on PR #57's run
35161387386 with `the stand-in recorded no directory:` — and nothing after the colon. The
recording was empty, not missing. The stand-in `support/harness.rs::stand_in` writes its
supervision-turn record as
...
so the shell creates and truncates `turn-seen` *before* the group runs, while
`service.rs::eventually` polls `read_to_string` on that path every 200 ms and returns the
first text it can read. Under load the poll lands between the open and the echoes.

Fix in the stand-in script: write the record to a temporary name in the same directory and
rename it into place, so the path a reader polls exists only once it is whole (`mv` within one
directory is atomic on every platform this suite runs on). The temporary file is created under
the script's own `umask 077`, so the `turn_file_owner` mode assertion at `service.rs` (the
`(TURN_SEEN, "turn_file_owner", "600")` row) keeps holding. Do the same for `sign-in-seen` if
you judge the symmetry worth a line — its readers wait for the process to exit, so it is not
racing today — and say which you did. Nothing in `service.rs` needs to change; PR #58
rewrites much of that file and PR #57 adds five lines to it, so leaving it alone is what makes
this rebase-friendly. `harness.rs` is untouched by both branches.

### 2. "Nothing listens here" stops being true

`crates/printobserver/tests/cli.rs::the_environment_names_the_server_and_wins_over_the_file`
failed on PR #58's run 35159303712 (job 105006165020, `cli.rs:287`) with exit code 7
(`Refused`: the supervisor answered something) where 3 (`Unreachable`) was expected. The
helper it uses —
...
— binds an ephemeral port, drops the listener, and hands the address out. Under the parallel
suite, which binds ephemeral listeners constantly (every world's server and proxy, every
stand-in), another process took that port before the command connected, and the command met a
served answer. The same release-then-connect shape is at
`crates/printobserver/tests/sign_in.rs::nothing_listening`,
`crates/printobserver/tests/journeys/running.rs::against_nothing` (the `nowhere` block), and
`crates/printobserver-server/tests/support/probes.rs::unreachable_address`. Audit for any
other, in Rust and Python, before you finish (`tools/obico-env/tests/harness.py::free_port`
is the same shape in the other direction — a port for a server to bind next — and is out of
this node's scope; name it if you find more like it).

Fix: an address "nothing listens on" has to be one nothing *can come to* listen on for as
long as the test uses it. Two forms satisfy that; pick one and use it at every site:

- **Held, not listening**: bind the port and keep the socket alive for the test's lifetime
  without ever calling `listen` — a connect to a bound, non-listening TCP port is refused by
  the kernel, and while the socket lives no other process can bind the port. `std` cannot
  bind without listening; the `socket2` crate can, and it is already in `Cargo.lock` through
  `tokio`/`mio`, so it is a `dev-dependencies` line for each crate and no new resolution.
  The helper then returns a guard the test holds.
- **Unbindable by anyone here**: a low privileged port on loopback that no unprivileged
  process can bind and no daemon on this host or a hosted runner serves — the repository
  already writes `http://127.0.0.1:1` for exactly this meaning in
  `crates/printobserver-sdk/tests/integration.rs` and the server crate's `reconciling.rs`.
  `cli.rs`'s test needs two distinct such addresses (the file names one, the environment
  another).

Whichever you take, add a test that shows the property — for the held form, that binding
the address fails with *address in use* and a connect to it is refused while the guard
lives; for the fixed-port form, that an unprivileged bind is refused and a connect is refused
— and cite the old helper's address being taken by a following `bind(127.0.0.1:0)` as the
induced failure it replaces, if you reproduce it.

### 3. The second supervisor is killed before it has printed

`crates/printobserver/tests/journeys/redaction.rs::a_second_server` (PR #57's run
35197078698, job 105122809364, aarch64, `redaction.rs:377`: `the second supervisor did not
start, so this says nothing about what it prints:` followed by nothing) spawns a second
`printobserver server`, sleeps a fixed 800 ms, kills it, and asserts the collected output
contains `is serving on`. A coverage-instrumented server under load had not reached that line
in 800 ms.

Fix: wait for the event, not for a duration — read the server's standard error line by line
until the `is serving on` line appears (the pattern is `service.rs::serving_on`, with a bound
that fails naming what was printed rather than hanging), then kill it and gather the rest of
both streams. `printed` must still be **everything** the server said, because
`no_run_of_the_walk_prints_the_credential` searches it: what you read while waiting is part of
it. The assertion on `is serving on` stays. PR #57's only edit to `redaction.rs` is the
Unix-only mode assertion above this function; keep clear of it.

## Why

The user's words: "let's make sure to fix flaky tests too — they're super expensive when
the gate is so long." The stand-in race has already turned **`main` itself red** on a tree
that was right (365896e), and each of these costs a two-hour multi-platform round when it
fires; none of them says anything about the program. Closing them is what makes a red gate
mean the tree is wrong.

## Acceptance criteria

- The stand-in's supervision-turn record at `$dir/turn-seen` is never observable in a
  partial state: it is written to a temporary name in the same directory and renamed into
  place, and a reader that finds the path finds the whole record. The file's mode stays
  `0600` (the `turn_file_owner` row in `service.rs` still holds).
- That fix is shown to be one by an induced failure, cited as such: with a deliberate pause
  inserted between the stand-in creating the record and writing its lines, the unchanged
  reader reads an empty record and `a_started_server_hands_every_turn_the_directory_the_sign_in_wrote`
  fails with `the stand-in recorded no directory:`; with the renamed write and the same
  pause, it passes. The pause is not in the finished tree.
- Every helper that hands out "an address nothing listens on" — `cli.rs::nothing_listening`,
  `sign_in.rs::nothing_listening`, `journeys/running.rs::against_nothing`,
  `printobserver-server/tests/support/probes.rs::unreachable_address`, and any other the
  audit finds — hands out an address no other process can come to listen on for as long as
  the test uses it (held without listening, or unbindable by an unprivileged process), in one
  form used at every site, and a test proves that property against the real socket API.
- `redaction.rs::a_second_server` waits for the server's own `is serving on` line, bounded by
  a deadline that fails naming what was printed, before stopping the server; what the
  journey searches for the credential is still everything the server printed on both
  streams, including what was read while waiting.
- No test's coverage is removed or narrowed, no test is marked ignored, and no assertion is wrapped in a retry that could mask a failure; no tolerance is widened; `service.rs` is not
  edited, and `redaction.rs` is edited only in `a_second_server`.
- The three fixes hold under the concurrent load that broke them: the finished tree's
  `service`, `cli`, `sign_in` and `journeys` test binaries of the `printobserver` crate and the
  `printobserver-server` crate's tests are shown passing repeatedly on this Linux host while
  the rest of the repository's parallel test tier runs beside them — the same contention the
  gate applies, which is what makes ephemeral ports scarce, spawns slow and polls land early —
  with the number of runs and the commit they ran on stated, and no run among them failing.
- The tests over the code this node touches are green over the finished tree (the four
  `printobserver` test binaries above and the `printobserver-server` crate's tests, run as
  the gate runs them, under the coverage-instrumented runner), both crates' linter over their
  test targets reports no warning, the formatter's check is clean, and the judged lint over this
  change's own diff leaves no finding unanswered — each named with what it reported.
...
**Reading order.** `crates/printobserver/tests/support/harness.rs` whole (the stand-in
script and the `FORKING` lock), `service.rs` around `eventually`, `serving_on` and the test
named above; `cli.rs` lines 40–55 and the failing test at 264–292, and `answering_with` at
~403 (that one keeps its listener and is fine); `sign_in.rs::nothing_listening`;
`journeys/running.rs::against_nothing`; `printobserver-server/tests/support/probes.rs` and
its one caller in `journeys/configuration.rs`; `journeys/redaction.rs::a_second_server` and
`no_run_of_the_walk_prints_the_credential`. The three failure logs:
`gh api --allow-escape-sequences repos/nickderobertis/printobserver/actions/jobs/<job>/logs`
for jobs 104846763271, 105006165020 and 105122809364.

**Running the binaries.** `cargo nextest run -p printobserver --features test-fixtures
--test service` (likewise `--test cli`, `--test sign_in`, `--test journeys`), and
`cargo nextest run -p printobserver-server`; the gate's forms are in each crate's
`project.json` (`cargo llvm-cov nextest … --no-report`). `strace` must be on the host — it is.

**Three commits, one per cause.** Land each fix as its own commit whose subject names the
test and the cause (the manager wants the squash headline to be able to list all three); a
later correction may add a commit, that is fine — what matters is that a reader can find
each cause's change by its subject.

**Proving under load.** Before the change, the same loop is a fair way to see each race on
this host; a reproduction is welcome evidence, and for the stand-in the induced-pause
demonstration in the criteria is the required one.

**Producing the load, and how many runs convince.** Background one full `just test` with a
sentinel and log named for this node and run the named tests beside it while it runs; repeat
for each run you count. This workstream's manager reads the count against these numbers: at
least three separate runs beside the full parallel tier, five preferred, and twenty to thirty
consecutive loops of a single binary for the cases a looping binary reproduces; list each
run's commit and outcome in the report. A `just test` here takes on the order of ten to
fifteen minutes; keep every invocation inside the turn that started it.

**Iterate with** the binaries you touched, `cargo clippy -p printobserver -p
printobserver-server --all-targets --features test-fixtures --locked -- -D warnings`,
`cargo fmt -- --check`, and the judged lint over your diff. The whole gate runs downstream on
the merge path; nothing here asks you to have run it first.

**A shared interface is never changed unilaterally.** The stand-in's recording format
(`key=value` lines, the file names in `harness.rs`) and the exit-code vocabulary the CLI tests
assert are fixed for this run; a departure you want is a proposal to your manager over
`$ORCHESTRATOR_ASK_MANAGER`, and you keep building against the agreed surface while you wait.
...
{"onepipeline.execution_checkout":"nickderobertis__printobserver-isolated","onepipeline.id":"test-support-races","onepipeline.max_turns":30,"onepipeline.persona":"engineer","onetaskgraph.item_kind":"task","orchestrator.plan-review":{"by":"review-plan","key":"c186d9966d7c4296e171bca8010dde128e980fdaf75625440a44b2a505b0e7c1","reviewed_at":"2026-09-17T17:54:17.681152+00:00"}}
-->
