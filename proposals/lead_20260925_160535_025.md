...
**feat(deps): adopt the releases that give this host human-readable branch names**

*Project description:*
## What

Move this host's pins onto the releases that carry human-readable branch names, and leave
a future maintainer the two sentences they will need.

**The pins.** `config/onepipeline.version`, `config/onepipeline-ui.version`,
`config/onevcs.version` and `config/onetaskgraph.version` move to the releases this node's
four dependencies produced. That is not four independent edits:
`tests/test_linked_libraries.py::test_the_cli_pin_names_the_release_the_engine_linked`
holds **every** sibling CLI pin equal to what the engine's own SBOM says that release
linked, so moving `config/onepipeline.version` will also move
`config/oneagentgraph.version`, `config/onejudge.version` and
`config/onemessagebus.version` to whatever the new engine resolved, whether or not
anything in this plan touched those libraries. Read the SBOM, not the tags; read
`AGENTS.md`'s "Which pin governs a dispatch" before any of it.

**The prose.** `AGENTS.md` gains the terse note a future maintainer needs and cannot
recover from the tree: **where the branch prefix is set on this host, and that the rest of
a node's branch name comes from onepipeline's template** — this host configuring no
template and taking the default. Keep it to that; this file holds judgment, not a copy of
either tool's help.

**The proof.** Pins, an SBOM reconciliation and a prose gate can all be green while the
branch a run actually cuts is unchanged, so this node adds the journey that says
otherwise: a `tests/e2e/` journey that launches a real one-node `local-md` plan through
the real `just orchestrate` against a **scratch** identity and a **scratch**
`ONEVCS_HOME` with a prefix configured on it, and reads the branch out of the run's own
journal. `tests/e2e/test_worker_start_directory_e2e.py` is the model for all of it — what
is real, what stands in, how the scratch identity is seeded, and how the `session-opened`
event is read — and `tests/e2e/scratch_identity.py` is the helper that seeds the
identity. Follow that shape rather than inventing a second one.

**Not the prefix itself.** Setting `nick/` is host state — a file under `$ONEVCS_HOME`,
outside every checkout — and no dispatch can write it: your isolated worktree is not the
host. The manager performs it after this lands, and the project record records it. Your
deliverable is the note saying where it goes, not the value.

Read `AGENTS.md` first — "Which pin governs a dispatch", "Sequencing a node behind a
release", "The tools this harness configures", and the **self-dispatch rule**, which is
why this node works in a worktree cut from the isolated safety clone and never in the
shared canonical checkout.
...
Everything upstream of this node is a capability sitting in a registry. This is the node
that makes the user's own branches read that way: until these pins move, every branch this
host cuts is still `ai-orchestrator/backend-engineer/20e03b22-14eb600a83`, and the work
the other four nodes did is invisible to the person who asked for it.

## Acceptance criteria

1. `config/onepipeline.version`, `config/onepipeline-ui.version`, `config/onevcs.version`
   and `config/onetaskgraph.version` each name the release its dependency produced, as the
   references block composed into this task names them — no version literal is stated in
   these criteria, because the release is not known until the hold that gates this node
   releases.
2. Every sibling CLI pin is reconciled against what the adopted engine's SBOM says it
   linked, so `tests/test_linked_libraries.py` is green over the finished tree — including
   the pins for libraries this plan never touched, if the new engine moved them.
3. A journey in this repository's own `tests/e2e/` tier proves that a node dispatched
   through the real `just orchestrate` path is cut a **human-readable, prefixed** branch
   under the adopted releases. It drives for real everything this repository's launch
   journeys already drive — the real recipe, the real `scripts/onepipeline.sh`, the real
   adopted `onepipeline` driver, the real adopted `onevcs` registry and its worktrees, and
   the real graphs in `graphs/` — standing in for the paid model alone, the way
   `tests/e2e/test_worker_start_directory_e2e.py` does, and it runs against a scratch
   identity and a scratch `ONEVCS_HOME` and never this host's own. On that scratch host a
   prefix is configured the way the adopted `onevcs` says a host configures one, and the
   plan it launches is a one-node plan from a `local-md` source, whose tasks carry no
   `key`. The branch the run's own record names for that node — the `session-opened` the
   journal carries — is the configured prefix followed by the plan's name and the node's
   id, and carries neither a session token nor the repository-and-persona name this host's
   branches carry today. So a journey that passed while the pins were still where they are
   now could not exist.
4. `AGENTS.md` states, in this repository's own voice and terse enough for a maintainer
   who has no context: **where on this host the branch prefix is set** — the file, not the
   value — **and that the rest of a node's branch name is rendered by onepipeline from a
   template, which this host does not configure because it takes the default**. It states
   no version number that `tests/test_linked_libraries.py` would then have to reconcile
   against a pin, and no date that `tests/test_dated_claims.py` would have to re-take.
5. The prefix this host will use is **not** set by this change: no value of it is
   written into this repository's tracked configuration, its `.env`, or anywhere else in
   the tree, and nothing in the tree configures a branch prefix for this host. The scratch
   prefix criterion 3's journey configures on its own scratch host is that journey's
   fixture and is not this host's prefix.
6. Every prose claim in this repository that names a linked library's version still names
   the version the adopted engine links, so
   `tests/test_linked_libraries.py::test_claims_about_a_linked_library_name_the_linked_version`
   is green.
7. Every test that exercises what this change touches is green over the finished tree —
   the pin and prose reconciliation modules above, the engine-source drift gate
   `tests/test_engine_contracts.py`, the journey criterion 3 adds, and whatever else in
   `tests/` reads a file you edited — and the judged lint over this branch's own diff
   against its merge base is clean. The repository's complete bar is the merge path's to run, not this dispatch's.
8. Every claim the dispatch's own report makes about the finished work — what it ran, what
   it saw, which pins moved and to what, and on which commit — is true of the tree as it
   finally stands. The report names, for each pin that moved and was not one of the four
   above, why the SBOM required it.

## Additional info

- This node adopts `published` on all four dependencies and names **no `consumes`**, so
  each resolves the `default_target` this host's `config/onevcs.releases.yml` declares for
  it — the wheel this host actually installs. `orchestrator/host_installs.py` is the table
  those artifacts are held to; nothing there changes unless a wheel's *name* did.
- **The `onevcs` pin moves for the whole host at once.** That library keeps one registry
  under `$ONEVCS_HOME` and migrates it on the first contact of any verb, a read included,
  after which the previous release fails on every verb — including the copy a live
  dispatch of an older engine links. Read `AGENTS.md`'s paragraph on it, confirm against
  the adopted release's changelog whether this adoption migrates that schema, and **say so
  in your report** either way: the manager needs to know before re-provisioning checkouts.
- This host's pins lag the published releases by several minors already, so this adoption
  crosses releases that have nothing to do with branch names. Read the intervening
  changelogs and report what else came with them, rather than moving numbers.
- **Contracts 1 to 4 of the project record `human-readable-branches` bind this run**, and
  none of them is yours to change. A departure you want is a proposal over
  `"$ORCHESTRATOR_ASK_MANAGER"`, made while you keep building against them.
- The adoption instructions you follow are each producer's own, rendered into your task's
  references block. This task writes no pinning instruction of its own; do not go looking
  for a commit to pin.
- **The adopted tags are already in the registered checkouts.**
  `tests/test_engine_contracts.py` reads each engine's source at `v<pin>` out of the
  checkout `config/onevcs.checkouts` registers for it, and nothing here fetches one. The
  manager fetches all four before this node is dispatched — the project record lists it
  among the manager actions this plan depends on — so an `invalid object name 'v<pin>'`
  from that gate is a host condition rather than a release that does not exist: raise it
  over `"$ORCHESTRATOR_ASK_MANAGER"` and keep building, rather than working around the
  gate or fetching into a checkout you do not own.
- The recipes that matter here are the pytest modules by path under `uv run pytest`, and
  `just lint-llm-diff <base>`. `just smoke` spends a real harness turn and is deliberately
  outside the gate; do not reach for it.
...
{"onepipeline.adoption":"published","onepipeline.execution_checkout":"ai-orchestrator-isolated","onepipeline.id":"ai-orchestrator-adopt-branch-naming","onepipeline.max_turns":200,"onepipeline.persona":"engineer","onetaskgraph.item_kind":"task","orchestrator.plan-review":{"by":"review-plan","key":"81a44fea11961226113fb859ef6e7ef5bbfbb87351484c59c442dfa5da388ac0","reviewed_at":"2026-09-25T13:53:52.262747+00:00"}}
-->
