...
**fix(deps): link the oneagentgraph and onejudge releases that stack judges, and relay which judge decided what**

*Project description:*
## What

Adopt, in one change in `nickderobertis/onepipeline`, the `oneagentgraph` release that stacks judges and the
`onejudge` release it links — both named by the `## Cross-repository references` block appended to this task —
so the engine every dispatch on this host runs through resolves one turn engine again, and prove that the new
`judge-decided` event a two-party member publishes reaches a run's journal.

Read `AGENTS.md`, the `[workspace.dependencies]` comment block in `Cargo.toml` (it states why `oneagentgraph` and
`onejudge` move together or `onejudge` resolves twice), `src/report.rs` (it reads `onejudge::NamedVerdict` off a
settled member and coerces between the two libraries' types), `src/agentgraph.rs`'s linked-library tests,
`crates/testfakes` (the `fake-oneagentgraph` double), `graphs/node-scope.yaml`, and `tests/e2e/turns.rs`.

**What to change.** Move both requirements to the adopted releases, refresh the lock so `just engines-current` is
green (one copy of each engine), and rewrite the comment block so it describes the releases now linked: a judge
side that is a list, an `llmlint` kind, `Observation::JudgeDecided` relayed by oneagentgraph as `judge-decided`, and
onejudge report schema 12. `graphs/node-scope.yaml` keeps its single harness judge — nothing here stacks a judge;
that is the operator's graph to change later. Where this crate holds a closed set of member event kinds it relays or
renders, `judge-decided` joins it; where the relay is total, prove that with the double rather than assuming it.
Where `src/report.rs` reads a settled member's report, the schema-12 report still reads (it is additive) — hold
that with the test that already drives a settled report through it.

The contracts this node consumes are onejudge's Contracts A–D and oneagentgraph's Contract E, stated in those
releases' `docs/judges.md` and `docs/contract.md`; this node restates none and changes none. A shared interface is
never changed unilaterally: a departure this node wants is a proposal to the manager over the ask seam.

### Working notes

The checks that exercise this change are `just engines-current`, `cargo test` narrowed to `src/report.rs`,
`src/agentgraph.rs` and the e2e binaries that relay member events (`tests/e2e/turns.rs`, the dispatch journeys),
`cargo clippy`, and `just lint-llm-diff origin/main`. Do not run the whole gate; the merge path does. Commit on the
branch the session opened.

## Why

The engine this crate publishes is what every dispatch on the user's host runs — `config/onepipeline.version`
decides what a dispatched node links, never the CLI pins beside it — so a onejudge that stacks judges reaches a
worker there only through this release. The user asked for every consumer to adopt in the crate graph's order,
and this is the link that turns the two libraries' releases into one engine a host can install.

## Acceptance criteria

- The `oneagentgraph` and `onejudge` requirements name the releases the cross-repository reference rows for `oneagentgraph-judge-list` and `onejudge-llmlint-judge` (the later of the two onejudge rows) carry, the committed `Cargo.lock` resolves each of `onejudge`, `oneagentgraph` and `oneharness-core` exactly once and at the version those two releases link, and the repository's own check over its engine pins agrees with the lock for those crates.
- A `judge-decided` event published by a two-party member reaches the run's journal with its payload intact, proven by driving the `fake-oneagentgraph` double to publish one through a real dispatch; where this crate keeps a closed set of member event kinds, the kind is in it.
- A settled member's onejudge report at schema 12 carrying `judge_decisions` is read by `src/report.rs` with the verdicts it reads today intact, proven by the test that drives a settled report through it.
- The comment block beside the pins describes the releases now linked and why they move together, and no longer narrates the previous pair as current; `graphs/node-scope.yaml` is unchanged.
...
{"onepipeline.adoption":"published","onepipeline.consumes":{"oneagentgraph-judge-list-3":"crate","onejudge-judge-panel":"crate","onejudge-llmlint-judge":"crate"},"onepipeline.id":"onepipeline-adopt-judges","onepipeline.max_turns":110,"onepipeline.persona":"engineer","onetaskgraph.item_kind":"task"}
-->
