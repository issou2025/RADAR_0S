...
**feat(deps): link the oneharness-core that writes a per-run pointer line for every harness run**

*Project description:*
## What

Move `onejudge`'s `oneharness-core` requirement to the release `oh-history-pointer`
produced, and nothing beyond what that move forces.

This crate links `oneharness-core` for the wire contract of the boundary it drives
(`RunReport`, `RunResult`, `FailureKind`, `RunStreamEnvelope`) and today requires
`0.14.0`. The release this task's dependency references name — a version, never a
placeholder, never one you went looking for — is two breaking minors past that (0.15.0's
`--format` on every JSON-stdout verb, 0.16.0's fallback and text defaults) plus the pointer
feature, so the move is a **requirement** edit in the workspace manifest (pre-1.0, `^0.14`
admits no 0.17.x) with the lock refreshed to resolve exactly one copy of the crate, and
whatever those releases' breaking changes force here: a `RunRequest` field the double
(`crates/onejudge/src/bin/fake_oneharness.rs`) or a provider now has to state, a default
that moved, a report field the crate's own types gained. Read each release's CHANGELOG
entry and the crate's `docs`; take what compiles and behaves as before, and change nothing
this crate does not have to. The `oneharness` CLI floor `AGENTS.md` / `README.md` state
for the spawned binary moves only if the new core's report cannot be read from the CLI
the floor names today — say which in the change.

The one addition beyond the relink: a floor test beside the crate's existing
linked-library assertions holding that the linked core exposes
`oneharness_core::io::history::read_pointers` and
`oneharness_core::domain::history::HistoryPointer`, so the requirement cannot slide back
below the release that carries them without this suite saying so. No pointer-file
behaviour is added to onejudge: a two-party member's turns spawn the `oneharness` CLI on
PATH, which writes the pointer line under its own configuration, and this crate passes
nothing new on the argv.

## Why

The user asked that "general visibility into all agents launched ... be done
automatically by onepipeline via oneharness + metadata and work regardless of the agent
structure used in the repo". The mechanism is a pointer line `oneharness-core` writes at
the start of every harness run, and it reaches a dispatch only through the core the
engine links — and `onepipeline`, `onejudge` and `oneagentgraph` hold `oneharness-core` to
one resolved copy (`onepipeline`'s `just engines-current` refuses a lock that splits it;
`oneagentgraph` requires `onejudge` exactly). So the engine cannot take the release until
this crate and then `oneagentgraph` require it. The move is also overdue on its own
account: core main sits two breaking minors ahead of every consumer, and the fixes those
releases carry (oneharness#1297, #1308) reach a dispatched turn only through this same
relink.

## Acceptance criteria

- The workspace manifest's `oneharness-core` requirement names the released version this
  task's dependency references give for `oh-history-pointer` — a version, never a
  placeholder — with its comment saying what this raise brings and why the requirement
  rather than the lock moved, and `Cargo.lock` resolves exactly one `oneharness-core`.
- Beyond the manifest, the lock, the floor test the next criterion asks for and the prose
  the one after it asks for, every change is one the new core's breaking changes forced,
  named as such in the completion report against the CHANGELOG entry that forced it; no
  provider, CLI or report behaviour changes otherwise, shown by the existing suites
  (`crates/onejudge/tests/e2e.rs`, `cli.rs`, `contract.rs`, `notes.rs`,
  `judge_seat_frames.rs`) green unmodified except where a forced change made an assertion
  false, each such edit named in the report.
- A test holds the linked `oneharness-core` to expose `io::history::read_pointers` and
  ...
  carries them.
- `AGENTS.md`, `README.md` and `CHANGELOG.md`'s unreleased section record this raise where
  they record the core and CLI floors today.
- These checks are green over the finished tree, run separately or together as suits the
  work: the crate's unit tests, the integration and e2e suites named above, the Python
  SDK check where the generated schema moved, rustfmt and clippy under the gate's denied
  warnings, the doc build, the MSRV build, and the judged lint over this change's diff,
...
nothing to them. If the released core cannot be linked as stated — a breaking change you
cannot absorb without changing what a judge or a provider does — put it to your manager
over `$ORCHESTRATOR_ASK_MANAGER` as a proposal and keep building against the agreed
release while you wait.

**Pins and versions.** This task names no version, deliberately: the references block the
harness appends names the release and the landing commit for `oh-history-pointer`, and the
note it sends when the release exists names the version. Do not go and find or pin a
commit yourself; `oneharness-core` is a published registry version, never a git rev.

**Where things are.** `Cargo.toml`'s `[workspace.dependencies]` comment block records
every core floor and why each was taken — write this one in that register. The last relink
(`fix(deps): relink oneharness-core for the server-overloaded fallback`, #94) is the
pattern: manifest, lock, `AGENTS.md`, `README.md`, `crates/onejudge/src/cli/mod.rs`
where the CLI floor is spelled. `crates/onejudge/src/oneharness/turn.rs` composes the
`oneharness run` argv (it already passes `--history`, `--history-name` and `--format
json`); `crates/onejudge/src/bin/fake_oneharness.rs` is the double that writes reports at
the linked core's types. `just --list`: `just test-fast`, `just test-e2e`, `just lint`,
`just format-check`, `just doc`, `just msrv`, `just lint-llm-diff <base>`. `just check` is
the pre-push bar and is not asked of this dispatch.
...
{"onepipeline.adoption":"published","onepipeline.consumes":{"oh-history-pointer":"core"},"onepipeline.id":"oj-relink-core","onepipeline.max_turns":140,"onepipeline.node":"oj-relink-core","onepipeline.persona":"engineer","onetaskgraph.item_kind":"task"}
-->
