...
**fix(github-projects): state why unknown ships disabled and never close it**

*Project description:*
## What

**Read against** `nickderobertis/onetaskgraph` `origin/main` at `0c2e2a6`, the commit issue #1004 read. Every path and line this task cites was checked against that tree; if the tree has moved when you start, re-read what moved rather than trusting a cited line number.

Close the onetaskgraph half of `nickderobertis/ai-orchestrator` issue #1004 root cause 8: the
`github-projects` source ships `StatusCategory::Unknown => StatusTarget::Disabled`
(`crates/onetaskgraph-github-projects/src/lib.rs`, `shipped_default`) while `local-md` keeps an
unknown word by name (`crates/onetaskgraph-local-md/src/lib.rs:553-560`, `:1244-1267`), and
nothing states why the two differ.

**Decision: keep `unknown` disabled by default and state why, rather than align the defaults.**
The two stores hold status differently. `local-md` writes the word itself, so an open-ended word
round-trips. `github-projects` writes a board option or the issue's closed state
(`column_for`, about `:2706-2760`), never creates an option (`:71-73`), and refuses a missing one.
So a shipped column default for `unknown` would refuse on every board lacking that option and
would fold every unknown word into one option. And a closed state for `unknown` is not merely a
poor default but wrong: it reads back as `done` or `cancelled` (`:2673-2678`), so the item's
category changes on every copy and a copy never settles. Yet the refusal text today offers
exactly that ("or to a closed state", about `:2784`).

**The contract** (with onepipeline's words and this host's board mapping; restated in
`aio-board-unknown-status`):

- The shipped default for `unknown` stays disabled.
- `status_mapping.unknown` set to a board option name is accepted. Every word classified
  `unknown` is written to that option, and reads back as the `unknown` category under that
  option's name.
- `status_mapping.unknown` set to a closed state (`{closed: completed}` or
  `{closed: not-planned}`) is refused when the source's configuration is read. The refusal names
  the category a copy would read it back as.
- The refusal for a disabled `unknown` names only a board option as the remedy, and says every
  unknown word lands on that one option.
- Every other category's default, target and refusal is unchanged.

State the difference between the two sources where each is documented: the crate docs and the
`status_mapping` field doc of `github-projects` (about `:69-78`, `:1145-1150`) and
`docs/local-md.md` (about `:65-77`).

## Why

In the run that closed #855, every write-back to the operator's GitHub Projects board was refused
from the first failed node onward, and onepipeline had designed its status words believing a store
keeps an unknown word by name — true of `local-md` and false here. The operator wants the two
sources' behaviour stated where a consumer reads it. They also want the one remedy the refusal
offers to actually be a remedy, instead of a configuration that never settles.

## Acceptance criteria

- With no `status_mapping.unknown`, writing a status classified `unknown` is still refused, and
  the refusal names a board option as the remedy, says every word classified `unknown` is then
  written to that one option, and never offers a closed state.
- With `status_mapping.unknown` naming an option the board has, statuses named `failed`,
  `provider-failed`, `parked` and `skipped` are each written to that option and read back as the
  `unknown` category under its name.
- A `status_mapping.unknown` naming a closed state is refused when the configuration is read,
  and the refusal says why by naming the category a copy would read that item back as: `done` for
  `{closed: completed}` and `cancelled` for `{closed: not-planned}`, each refused and named.
- Each of these is proven end to end through the compiled `onetaskgraph` binary against the
  repository's loopback board.
- Every other category's default mapping and refusal behaves as before, and the existing
  status-mapping tests stay green.
- The `github-projects` crate docs, its `status_mapping` field doc, and `docs/local-md.md` state
  how the two sources treat an unknown status word and why they differ.
- The tests covering the changed source code pass over the finished tree, and the judged lint over
  this node's diff leaves no finding unanswered.
- Every claim the dispatch makes about the finished work is true of the tree as it finally stands.

## Additional info

- Read first: `AGENTS.md`, `crates/AGENTS.md`, `docs/local-md.md`,
  `crates/onetaskgraph-plugin-api/src/work.rs:264-294`, and in
  `crates/onetaskgraph-github-projects/src/lib.rs` the crate docs, the `status_mapping` field,
  `shipped_default`, `StatusMapping::resolve`, `column_for`, the read-back near `:2662-2697`, and
  the refusals near `:2774-2788`.
- Tests nearest this change: `crates/onetaskgraph-github-projects/tests/plugin.rs` (the Draft
  refusal near `:3951`, the missing-option case near `:3969`, the closed-state mapping near
  `:4078`, `:5371`), `crates/onetaskgraph/tests/e2e/fixtures.rs` (the loopback board). Tests live
  under `tests/`, never in `#[cfg(test)]`. `just lint-llm-diff <base>` is the judged tier.
- The contract above is shared with other nodes. If you find it wrong, surface a proposal to the
  manager over `$ORCHESTRATOR_ASK_MANAGER` and keep building against it.
- The title must stay `fix`: the refusal text ships in the CLI wheel this host adopts.
...
{"onepipeline.id":"onetaskgraph-unknown-status","onepipeline.max_turns":120,"onepipeline.persona":"engineer","onetaskgraph.item_kind":"task","orchestrator.plan-review":{"by":"review-plan","key":"688c25c37dbdd6c23b247766cbe2adffdb6b11cb4e27aba24e505acb982f1173","reviewed_at":"2026-09-13T17:31:34.693544+00:00"}}
-->
