...
**feat(windows): run printobserver as a Windows service**
...
## Amendment (planner, continuation after the driver's death, 2026-09-19 00:45Z)

Where this amendment disagrees with the task, with any earlier amendment, with the planner evidence there, or with any carried note, this amendment wins; everything else there stands.

- The change request for this work is pull request #70 (https://github.com/nickderobertis/printobserver/pull/70), open for review, on branch `onevcs/s-a4e132b44479`; the session this dispatch opens in carries that branch at 547590f or later. The hosted round on 098feda (ci run 35391320473) reported every platform cell green — the build-and-test cell and the printer integration cell on linux-x86_64, linux-aarch64, windows-x86_64 and windows-aarch64, every probe, the title check and the suppressions check — and only the judged-lint check red; the three commits above 098feda (710accb, 066e7cc, 547590f) are the answer to that refusal and nothing else, they have never reached the origin, and the finished tree carries them, with the branch's diff against its base carrying no finding under the repository's judged lint rules. No further hosted round is required before settling: the lifecycle's publication after settlement is what pushes the branch and re-runs the merge path on the final tree, and a lifecycle worker runs no non-draft publish of its own.
- The report names the 098feda round per platform, names each commit above it with what it changes, and states, for each of those commits, which check exercises it and what that check reported on the final tree.

### Operational notes (continuation)

The dispatch before this one was condemned by its heartbeat rule four minutes after it opened, at 00:28Z on 2026-09-19, with nothing wrong in its tree: the host had restarted moments before, two other dispatches and another run's driver were starting at the same moment, and the load average stood above 50. Run one local suite at a time, in the foreground, and never several concurrently.
...
{"onepipeline.branch":"onevcs/s-a4e132b44479","onepipeline.execution_checkout":"nickderobertis__printobserver-isolated","onepipeline.id":"windows-service-3","onepipeline.persona":"engineer","onetaskgraph.item_kind":"task"}
-->
