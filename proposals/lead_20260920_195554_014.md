...
**feat(serve): serve the browser bundle and the read API on one origin**

*Project description:*
## What

Let `onepipeline-api serve` serve the browser view beside the read API, so a host that
installs one artifact can open the DAG Observatory with one command.

### The gap

`onepipeline-api serve --runs-root DIR --bind HOST:PORT` serves the read API and
nothing else. The view is published as the npm package `onepipeline-ui`, a built
`dist/` "to be served statically" that asks for `/api/v2/…` and `/healthz` relative to
whatever origin served it and declares no API host. So a browser needs the two behind
one origin, and the read API sends no CORS headers — which is right — so a consumer
has to write a same-origin proxy. `ai-orchestrator` did (`scripts/dag-ui-server.js`, a
bun static server proxying those two prefixes to this API, plus the recipe that
installs the npm bundle for it). That proxy is the workaround this change retires.

### `onepipeline-api serve --ui`

- `--ui` (off by default, so every existing caller sees the API alone) makes the
  same server also serve the built browser bundle **of this same release** — the one
  `npm-build.mjs ui` publishes as `npm:onepipeline-ui` — at `/`, with the API under the
  prefixes the bundle already asks for (`/api/v2/…`, `/healthz`) unchanged, and every
  path the bundle's router owns falling back to its `index.html` so a deep link opens.
  A request for a path under the API prefixes is never answered by the bundle, and a
  request for the bundle is never answered by the API.
- **The bundle ships inside the artifact a consumer installs**: the `onepipeline-api`
  binary the crate, the PyPI wheels and the npm launcher all wrap. The release
  workflow already builds the bundle in the same run; embed the built `dist/` in the
  binary at build time so no host installs npm packages to use `--ui`, and so the view
  a host opens is stamped with the release of the API serving it — the pairing
  `release-targets.toml` already says one release makes. How the build reaches the
  bundle (a build script that requires the bundle to be present, or a feature gate)
  is yours to choose; a binary built without the bundle must refuse `--ui` naming
  what is missing, never serve an empty page.
- `--ui-dist <DIR>` may override the embedded bundle with a directory on disk, for
  developing the view against a real runs root, and is the only way to serve a bundle
  of another version.
- The address is unchanged: `--bind` names one `HOST:PORT` for both.

### Where it is recorded and proven

State the flag, the path split and the embedding in `docs/contract.md` where this
repository records its serving contract, and in the README's install section beside
`onepipeline-api serve --runs-root ./runs`. Prove it with journeys driving the
compiled binary: with `--ui`, `GET /` answers the bundle's `index.html`, a bundle asset
answers with its bytes and content type, a router-owned deep link answers
`index.html`, `GET /healthz` and `GET /api/v2/runs` answer exactly what they answer
without `--ui`; without `--ui`, `GET /` answers what it answers today. The e2e
project that already drives a real browser over the served view (`apps/dag-ui-e2e`)
gains one run that opens the view through `--ui` alone, with no separate static
server. `tests/packaging.rs` reconciles the artifacts the release publishes; hold the
embedded bundle to it so a release that stopped building the bundle before the binary
fails there rather than shipping an `--ui` that refuses.

## Why

The user's direction for `ai-orchestrator` is that it be *"primarily config ... very
light or no wrapper on CLI tools"*, and `docs/dag-ui.md` there explains a 127-line
bun proxy plus install recipe whose whole job is putting this API and its own view
behind one origin — *"onepipeline-ui publishes the built bundle alone"*. The view and
the API are one product released together; serving them together is this
repository's to do, after which the consumer's `just dag-ui` is one line.

## Acceptance criteria

- `onepipeline-api serve --runs-root DIR --bind ADDR --ui` serves the read API under
  `/api/v2/…` and `/healthz` exactly as it does without `--ui`, and serves the built
  browser bundle of the same release at `/` with its assets and with router-owned deep
  links answering `index.html`; without `--ui` the server's responses are unchanged
  from the adopted release.
- The bundle the binary serves is embedded in the one binary that the crate, the
  wheels and the npm-published launcher all wrap, so `--ui` works on a host that installed only
  that binary; a binary built without the bundle refuses `--ui` naming what is missing.
- `--ui-dist DIR` serves the bundle at `DIR` in place of the embedded one.
- `docs/contract.md` and the README state the flag, the path split and the embedding,
  and `tests/packaging.rs` reconciles the embedded bundle against the release
  configuration that builds it.
- The behaviours above are proven by journeys driving the compiled binary over HTTP,
  and the browser e2e project opens the view through `--ui` alone; no double stands in
  for the server or the bundle.
- The tests covering the server, the CLI and packaging are green over the finished
  tree, and so is the judged lint over this change's diff.
- Every claim this dispatch makes about the finished work is true of the tree as it
  finally stands.

## Additional info

The consumer's proxy is the specification of the path split; read
`~/ai-orchestrator/scripts/dag-ui-server.js` and `~/ai-orchestrator/docs/dag-ui.md`
(read-only; that checkout is another manager's publication checkout). The consumer
will run `onepipeline-api serve --runs-root "$ONEPIPELINE_RUNS_DIR" --bind <addr> --ui`
from its `justfile` and nothing else, so the flag name and the "same release, no npm
install" property are what it relies on; a departure you want is a proposal over
`$ORCHESTRATOR_ASK_MANAGER`, not an edit. The adoption on that host waits for this
repository's `pypi-cli` target (`pypi:onepipeline-api-cli`), so the wheel is the
artifact that has to carry the bundle.

Iterate with the server and packaging tests and this repository's lint over your
...
{"onepipeline.execution_checkout":"nickderobertis__onepipeline-ui","onepipeline.id":"ui-serve-bundle","onepipeline.persona":"engineer","onetaskgraph.item_kind":"task","orchestrator.plan-review":{"by":"planning-closeout","key":"6d894083c61e0a12460fdfad931a53e89aee1935ab4eaf971746fe35806a4229","reviewed_at":"2026-09-20T19:30:11.837064+00:00"}}
-->
