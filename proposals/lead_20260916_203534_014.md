...
**fix(tests): give each writeback test its own scratch directory**

*Project description:*
## What

Ticket: https://github.com/nickderobertis/onepipeline/issues/323 — *parallel writeback tests share
process-scoped scratch paths*.

**Root cause (from the ticket).** The writeback unit tests' scratch helper in `src/writeback.rs`
(`fn scratch(name)`, lines 3547-3554 at basis `32c8316ad950975863b3a48c9a32df679b60bc14`) derives a
path from the caller's name and the process id, removes it, then recreates it, so two parallel
invocations of the same helper name remove and recreate one directory.
`a_refused_snapshot_published_again_is_not_queued_and_a_different_one_is` failed once at
`create_dir_all` and passed unchanged on rerun. Transcript: `onepipeline transcript delivers-plan
op-queued-delivers`.

**The accepted fix.** Use a unique temporary directory per test invocation (a guard type that owns
the directory and removes it when dropped — the `tempfile` crate if the repository already carries
it as a dev-dependency, otherwise a per-invocation unique suffix under the system temp directory),
and let its owner remove it after use. Every caller of the helper moves to it.
...
Parallel unit tests fail nondeterministically while creating their scratch directory, which
refuses onepipeline's gate for changes unrelated to write-back and costs re-runs on every plan
that changes the engine.

## Acceptance criteria

- Every writeback test's scratch directory is unique to that invocation and removed by its owner
  when the test ends; no scratch path is derived from the caller's name and process id alone.
- A test proves two concurrent invocations with the same name receive distinct directories, and
  that one invocation's removal does not remove the other's.
...
- The checks that exercise this change — the `writeback` unit tests, and the repository's judged lint over this change's diff — are green over the finished tree.
...
{"onepipeline.amendment":"Manager amendment (binding; where this and the task's operational notes disagree, this wins). The interface this node changes is the writeback tests' scratch-directory helper in `src/writeback.rs`, and the test that drives it is a `writeback` unit test: the criterion \"proven end to end by a test or journey that drives the real interface named above\" is satisfied by a unit test in that module that calls the real helper and proves two concurrent invocations with the same name receive distinct directories and that one invocation's removal leaves the other's in place. No separate e2e journey, CLI run, or launch is required or wanted for this node.","onepipeline.branch":"onevcs/s-e8ed43e1adb2","onepipeline.id":"op-323-2","onepipeline.max_turns":200,"onepipeline.persona":"engineer","onepipeline.resume":{"branch":"onevcs/s-e8ed43e1adb2"},"onetaskgraph.delivers":["followups:I_kwDOTx8aUc8AAAABRkNGAg"],"onetaskgraph.item_kind":"task"}
-->
