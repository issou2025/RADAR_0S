...
**feat(session): name a node's branch from a template over its task and plan**

*Project description:*
## What

Make the branch a node's session cuts render from a template, so a run's branches read
`ENG-123/aio-adopt-op` instead of a hash. Three pieces.

**Carry the node's task record onto the node.** The loaded `Node` carries no field of the
task it came from — its `title` is the *change request's* title, and `TaskItem` in
`src/taskgraph.rs` reads `title`, `project`, `content`, `metadata`, `repositories` and
`delivers` and not the task's native id. The template needs the task's `key` (the field
the released `onetaskgraph` now reports), its native id, its title and its `delivers`, so
`taskgraph.rs` has to read them and the node has to carry them.

**Render.** A setting holding a **Jinja** template — `minijinja`, which this crate already
depends on at `2.24` for producers' release `adoption_instructions`, so nothing new is
added — resolved in this crate's own existing order: `--branch-template TEMPLATE` on
`start` beats `ONEPIPELINE_BRANCH_TEMPLATE` in the environment, which beats the launch
config's `branch_template`, which beats the shipped default. `branch_template` is a
launch-config **`schema_version: 10`** key (this build writes 9 today — see
`src/filter.rs`'s `LAUNCH_CONFIG_SCHEMA_VERSION` and the paragraph above it), refused by
name in a document declaring an earlier one. The resolved template is retained in the
launch record and replayed by `adopt`, the way `bus_config` and `maintenance_config` are.

**Hand the rendered name to `onevcs` as a name to cut.** The released `onevcs` this node
consumes carries a `SessionRequest` field distinct from `branch` meaning *a name I want
cut fresh*, and does the sanitizing, the prefixing and the collision suffix itself. This
crate renders and nothing else: no sanitization, no prefix, no suffix, no existence check.
`vcs::request_for` sets the new field **only where it sets no `branch`** — which is
already the one place that answers "is this a fresh cut", since a `resume`, a pinned
`retry`, and the publication-retry re-dispatch all arrive with `branch` pinned by the
reconciler.

**Contract 3 and Contract 4 of the project record `human-readable-branches` are the
agreement, and they are the source rather than this paragraph**: the division of labour
with `onevcs`, the exact variable namespace, the one literal the default template is, that
the template applies only when a branch is cut, and what happens when a template does not
parse or does not render. Read them before writing anything.

`docs/contract.md` is the approved contract, not edited to match code. **The user has ruled
that this surface be added, and the manager has relayed that ruling and authorized this
node to record it**: extend that document with the flag, the key, the environment
variable, the variable namespace, the default template literal, and the refusals — and
extend `tests/contract.rs` so the new fenced blocks are driven through the public types,
because a contract type added without that is unproven. Anything you could not compile
exactly as the contract states goes to `docs/contract-divergences.md` as a proposal.

Read `AGENTS.md` first — "The contract is the source of truth", "Invariants" — and
`src/taskgraph.rs`'s module header for why the store is driven rather than linked.
...
This is the node that decides what a branch **says**. `onevcs` supplies the namespace in
front and the rules that make a name a valid, unique ref; the ticket handle and the piece
of the plan are things only the engine knows, because only the engine holds the node
beside the task it came from. Until it renders them, a person reading `git branch` on a
repository several runs have touched cannot tell which branch is which work.

## Acceptance criteria

1. With **no template configured anywhere**, a lifecycle node whose task carries a `key`
   opens its session on a branch named `<task key>/<node id>`, and one whose task carries
   no `key` opens on `<plan name>/<node id>`. Both proven by running a real node to the
   point its session exists and reading the branch the session actually reports — not by
   asserting on a rendered string in isolation.
2. A **custom template** set by `--branch-template` names the branch instead, and it can
   reach each of `task.key`, `task.id`, `task.title`, `task.delivers`, `plan.name`,
   `plan.id`, `node.id` and `run` — every one proven to render the value it names.
3. The three layers resolve in the order Contract 4 states: the flag beats the
   environment, the environment beats the launch config's `branch_template`, and that
   beats the shipped default. `branch_template` in a document declaring a launch-config
   schema version below 10 is refused **by name**, the way every other versioned key is.
4. The resolved template is retained in the launch record and replayed by `adopt`, so a
   run adopted by a process whose environment names a different template goes on cutting
   branches from the template it launched with. Proven by adopting a run.
5. A node dispatched onto an **existing branch keeps it**, with no render attempted:
   proven for a node the reconciler pinned — a `resume`, or a `retry` naming a branch —
   whose branch is the pinned one and not a rendered one.
6. A **collision** takes the suffix rather than continuing the existing branch: with the
   name the default template renders already carried by a branch, the node's session opens
   on the suffixed name and the existing branch is untouched. This is `onevcs`'s behaviour
   and the criterion is that this crate reaches it — that the rendered name arrives as *a
   name to cut* and never as `branch`.
7. A **name needing sanitization** — a key, title or plan name holding characters a ref
   name may not hold — still produces a session on a valid ref, proven through a real
   branch cut.
8. A template that **does not parse** is refused at the launch, naming the flag or the key
   it came from, and **no run is minted**. A template that fails to render at a node, or
   renders empty, settles that node under the existing outcome word
   `infrastructure-failure` carrying the template's own error and naming the node, and
   **no branch is cut** — never a silent fall back to another name. No new outcome word
   and no new event kind are introduced.
9. A node whose task has no `key` because its source has none — a `local-md` plan, which
   is what this repository's own journeys run against — is the case criterion 1's second
   half is proven on, driving the real released `onetaskgraph` binary rather than a
   stand-in. The `key` case may be driven through the `ONETASKGRAPH_BIN` seam and
   `crates/testfakes`' `fake-onetaskgraph`, since no source with a key can be reached
   without a credential this dispatch does not hold.
10. Every test that exercises what this change touches is green over the finished tree —
    the contract test, the taskgraph tests, `tests/onevcs_seam.rs`, and the e2e journeys
    that open a session — and the judged lint over this branch's own diff against its
    merge base is clean. The repository's complete bar is the merge path's to run, not
    this dispatch's.
...
- **Contracts 3 and 4 are fixed for this run.** A departure you want — a different
  variable namespace, a different default, a different failure word — is a **proposal**:
  raise it over `"$ORCHESTRATOR_ASK_MANAGER"` and keep building against them while you
  wait. The node downstream of you and this host's own adoption both read the same words.
- This node adopts `published` on both dependencies and **consumes each one's `crate`
  target**, because that is what a Rust consumer links and what this repository's own
  checks `cargo install`. The adoption instructions you follow are the producers' own,
  rendered into your task's references block; this task writes no pinning instruction of
  its own and you should not go looking for a commit to pin.
- `onevcs` and `onevcs-testing` release on two cadences but from one workflow run, and
  this manifest requires an exact pair — see `[workspace.dependencies]`'s note. A `onevcs`
  minor forces an `onevcs-testing` release that requires it, so move both and let
  `just engines-current` tell you whether the lock and the requirements agree.
- The `onetaskgraph` release also moves `justfile`'s `onetaskgraph-version` and the
  exact-pinned `onetaskgraph-core` / `-local-md` family in the lock, which
  `taskgraph::tests::every_onetaskgraph_crate_in_the_lock_is_the_release_the_checks_install`
  holds together. Move them as one.
- `vcs::request_for` is `pub` and takes a node alone; it will need the plan and run context
  to render. Its three callers are in `src/engine.rs` and `src/lifecycle.rs`.
- Run the tests over what you touched and the lint over your diff as you iterate. `just
  smoke-real` needs a credential and is outside both tiers; do not reach for it.
...
{"onepipeline.adoption":"published","onepipeline.consumes":{"onetaskgraph-task-key":"crate","onevcs-branch-prefix":"crate"},"onepipeline.id":"onepipeline-branch-template","onepipeline.max_turns":200,"onepipeline.persona":"engineer","onetaskgraph.item_kind":"task","orchestrator.plan-review":{"by":"review-plan","key":"b639a26bd1e408c1be40f82aff1222d5fd083654dd95fe4ef32d56182d8777a6","reviewed_at":"2026-09-25T13:37:32.569710+00:00"}}
-->
