...
**fix(hooks): give run-end hook idempotency a retry epoch**

*Project description:*
## What

Resolve https://github.com/nickderobertis/onepipeline/issues/295, accepted at severity
`high`: a run whose failure hook fired can never fire its success hook, so a run recovered
from failure completes with its success-only automation skipped.

Run-end hook idempotency is keyed to the presence of **any** prior `run-hook-fired` event.
`src/hooks.rs:401-424` refuses every later firing once a marker exists and
`src/hooks.rs:427-432` searches for any such event without considering the hook kind or a
retry epoch; `docs/contract.md:45` codifies the same once-per-run behaviour outright,
including the case of a failed run later requeued and completed. Run `follow-up-proposal-plan`
on this host is the incident: its failure hook fired after a node failed, the manager
retried the node, the replacement completed, an adopted driver settled the run `complete`,
and only `hooks/failure.log` exists — the success hook that dispatches follow-up
verification never ran.

**The fix the ticket settles on, after a round of the user's own questions.** Invalidate the
prior firing marker when an accepted graph edit makes the run **live again**, taking the
most recent such edit as a simple idempotency epoch. That is simpler than defining and
storing terminal episodes and it gets the important retry case right: failure, accepted
retry, then failure again fires a second failure hook for the new attempt.

Two shapes the ticket rules out, and the task carries both as constraints:

- **Do not key by hook kind alone.** It would allow failure then success, but failure,
  retry, failure would fire no second failure hook, losing a real terminal notification.
- **Do not invalidate on every accepted command indiscriminately.** A `note`, or any other
  command that does not reopen the run, would duplicate a hook for the same ending. Only an
  edit that makes the run live again is an epoch.

The once-per-run sentence in `docs/contract.md` is the contract this changes, so it is part
of the deliverable rather than a follow-on: state the epoch rule there, in the place the
once-per-run rule is stated today, so there is one source for it and the tests below
reconcile against it.

Cover it with end-to-end journeys for failure then retry then success, and for failure then
retry then failure, and with a concurrent-adoption check proving each epoch fires exactly
once — the marker's check and append already sit under the handover gate a driver releases
the run under, and that property has to survive the change.

## Why

The user accepted this at `high` severity, and the automation it skips is this host's own:
the success hook is what launches the follow-up verification run, so every run recovered
from a failure quietly loses the verification of every follow-up it drafted. A manager has
to notice the missing hook — from the absence of a file — and invoke the automation by hand
each time, which is exactly the supervision this run-end machinery exists to remove.

## Acceptance criteria

- A run whose failure hook fired, whose node was retried through an accepted edit, and which
  then completes, fires its success hook.
- A run whose failure hook fired, whose node was retried through an accepted edit, and which
  then fails again, fires a second failure hook for that new attempt.
- An accepted command that does not make the run live again — a note among them — leaves the
  prior firing marker standing, so no hook fires twice for one ending.
- Two drivers judging one run under the same epoch fire exactly one hook between them.
- `docs/contract.md` states the epoch rule where it states the once-per-run rule today, and
  states it once.
- Each of the four behaviours above is proven by an end-to-end journey driving the real
  driver, the real journal and a real hook command rather than by inspection, and no journey
  substitutes a double for the layer under test.
- The commit body carries a `Closes` line naming
  https://github.com/nickderobertis/onepipeline/issues/295.
- The tests that cover the sources and the document this change touches are green over the
...
hooks — this repository's `test-quick` tier and a filtered end-to-end selection are the
narrow choices — rather than its whole gate. The repository's own required checks run
downstream on the change this branch becomes; nothing here asks you to have run them first.

A sibling node in this plan changes `src/edits.rs` and the same contract document for a
different reason, and it starts from this node's work rather than beside it, so what you
land here is the base it builds on. Keep your change to what this ticket needs.
...
{"onepipeline.execution_checkout":"nickderobertis__onepipeline","onepipeline.id":"op-hook-epoch","onepipeline.persona":"engineer","onetaskgraph.item_kind":"task"}
-->
