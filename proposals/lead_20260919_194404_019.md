...
**feat(pool): maintain idle slots one at a time, recording when each was last maintained**

*Project description:*
## What

Add the one imperative maintenance verb to the pool the previous node landed:
`onevcs pool maintain [REPO] [--older-than SPAN] [--json]`, and its library seam
`pool_maintain`. It runs the identity's `maintain.command` in each **idle** slot's
worktree under the identity's `timeout`, claiming the slot with the maintaining record
the previous node declared in `slot.json` so an `open` will not take it, **at most one
slot per identity at a time** so maintenance never removes more than one slot from
service, and records on the slot the fact that it ran. `onevcs` holds **no schedule**
and keeps no state beyond the slot record: *when* to maintain is the caller's — a
driver's idle branch, a cron, or a person — and the verb is idempotent and cheap when
nothing qualifies, so all three can call it and never run a slot twice.

Read first: the previous node's landing (its `docs/contract.md` amendment, `slot.json`,
the idle test in `workspace.rs`), `processes.rs` (`ProcessStart`, how this crate bounds
a child and ends its process tree on timeout — every git command is bounded the same
way), `lock.rs`, `stream.rs` / the artifact store (`onevcs artifact cat`), `sweep.rs`'s
report shape (this verb's human report follows it: what it did, what it kept and why).
...
`MaintenanceOutcome` is the previous node's type. Exit codes: `0` when nothing ran or
every command succeeded; `1` when any ran command failed or timed out (the report says
which); `2` for an invalid request (a `REPO` nothing resolves, a malformed `SPAN`).

**How one identity is maintained.** Take the identity's maintenance lock (an exclusive,
non-blocking `lock::` take keyed on the identity, the way run-root occupancy is keyed) —
held by another process, the outcome is `Claimed` and nothing else is touched. Shed
surplus idle slots exactly as `open` does. Then for each slot in number order: skip one
in use (an open record names it), one whose claim names a live process, one not due,
one broken; otherwise **claim it** — write `maintaining` with this process's `pid`,
`started` and `since` — run `command` with the slot's worktree as working directory, no
shell, this process's environment, stdout and stderr captured as one artifact, bounded
by `timeout` with the process tree ended on expiry; then write `last_maintained` (now)
and `last_outcome` (`succeeded` / `failed {exit}` / `timed-out`), clear the claim, and
only then move to the next slot. A crash mid-command leaves a claim naming a dead
process, which the previous node's idle test already reads as void. `last_maintained`
records the **attempt**: a failing command is not retried until `older_than` has
elapsed again, and `pool status` shows `last_outcome` beside it.

**Interlock with `open`**: an open that finds every idle slot claimed falls through to
create or overflow rather than waiting — that is the previous node's placement order,
and this verb must keep it true by never claiming more than one slot per identity.

**Docs**: `docs/contract.md`'s amendments section (the verb, the types, the exit codes,
the one-slot rule), `docs/inferred-surface.md`, `README.md`'s command list, `DESIGN.md`
(why `onevcs` holds no schedule; why the attempt is what is recorded).
...
than dag run times". So `onevcs` owns *what* maintenance is per identity and the fact of
when it last ran; the schedule lives with the caller. A warm slot that is never swept
grows without bound, which would turn the disk-wear fix into a disk-space problem.

## Acceptance criteria

- `pool_maintain` and `onevcs pool maintain` maintain idle slots: on a pooled identity
  with two idle slots and a `maintain` command, one call runs the command in each
  slot's worktree in turn with the slot claimed for exactly the duration of its run,
  records `last_maintained` and `last_outcome` on each, and exits 0; a second call
  with `--older-than` longer than the elapsed time runs nothing and reports every slot
  `NotDue`; a slot never maintained is always due.
- A slot an open session holds is skipped and reported `InUse`; a slot claimed by a
  live process is skipped; a claim left by a process that is gone is void and the slot
  is maintained; while one slot is claimed, a `session open` on that identity takes
  another idle slot or falls through to create or overflow rather than waiting.
- A second `pool maintain` on an identity another is maintaining reports `Claimed`
  with the holder's pid and touches nothing, so two callers whose ownership attempts
  on one identity overlap never run a slot twice; and a call whose `--older-than`
  makes the attempt the first completed not due, arriving after the first released
  the identity, reports that slot `NotDue` and runs nothing on it. Two overlapping
  `Scope::All` calls that reach one identity at different times — the second taking
  the identity lock after the first released it, with no `older_than` or with one
  that has already elapsed since the completed attempt — maintain that identity's
  slots again, correctly: the lock prevents simultaneous maintenance, and a slot due
  by the caller's own threshold is maintained rather than deduplicated.
- A command that exits non-zero records `failed` with its exit code, one that runs
  past `timeout` is ended with its process tree and records `timed-out`, the verb
  exits 1 in both cases, the captured output is an artifact the report names, and
  neither is run again before `older_than` elapses.
- An identity with no `maintain` command, or no slots, is reported so and costs no
  process; `Scope::All` covers every registered identity; an unresolvable repository
  or a malformed span exits 2 naming it.
- `docs/contract.md`'s amendments, `docs/inferred-surface.md`, `README.md` and
  `DESIGN.md` state the verb, its types and its rules, and `tests/contract.rs` holds
  the amendment's source blocks to the types.
- The checks that exercise this change are green over the finished tree: the crate's
  tests and end-to-end journeys (the new ones driving the compiled binary through a
  run, a not-due skip, an in-use skip, a claim, a failure and a timeout), the format
  check, and the judged lint over this branch's own diff.
...
- This plan is the planner's ruling on the contract for these surfaces; record the
  amendment rather than reporting a conflict. A shared interface is never changed
  unilaterally once this plan runs: the slot record shape and the claim marker are the
  previous node's declaration and `onepipeline` is built against them — fill them, do
  not reshape them; raise any departure with `"$ORCHESTRATOR_ASK_MANAGER"` and keep
  building against what is stated.
- The recipes: `just test`, `just test-one <journey>`, `just fmt-check`, `just
  lint-llm-diff`. The full `just gate` is not owed.
- A realistic maintain command for the journeys is a script that writes a marker file
  into the worktree and sleeps on request, so the claim window, the timeout and the
  interlock with `open` are all observable from the compiled binary.
- The next consumer runs this verb from a driver thread with `Scope::Repo` per identity
  and `--older-than` set to that identity's schedule; keep the identity-level outcomes
  cheap, because a driver asks about every registered identity on each idle tick.
...
{"onepipeline.id":"onevcs-pool-maintain","onepipeline.max_turns":200,"onepipeline.node":"onevcs-pool-maintain","onepipeline.persona":"engineer","onetaskgraph.item_kind":"task"}
-->
