...
**feat(evidence): name the artifacts a judge reads directly, gitignored ones included**

*Project description:*
## What

**Read against** `nickderobertis/onejudge` `origin/main` at `ad9cc13`, the commit issue #1004 read. Every path and line this task cites was checked against that tree; if the tree has moved when you start, re-read what moved rather than trusting a cited line number.

Close the onejudge half of `nickderobertis/ai-orchestrator` issue #1004 root cause 3: the
judge's only mediated evidence is `{"tool":"git_status"}` (`git status --porcelain=v1`) and
`{"tool":"git_diff"}` (`crates/onejudge/src/provider.rs:826-877`), read-only mode is a tool
allowlist with no shell, and a product written under a gitignored path is invisible to both.
A design-document judge asked `git_status` six turns running and exhausted the retry limit.

Give a caller a way to name the artifacts a judge should read directly.

**Decision, departing from both of the issue's suggestions.** Not `evidence.history_files`:
onejudge fills it itself from harness telemetry and it means "exact producer-returned history
paths" (`engine.rs:1071-1078`, `provider.rs:29`, `:960`), so a caller cannot set it and reusing
it would change what it means. Not `git status --ignored`: in the default untracked mode it
collapses an ignored directory to one `!! .plans/` line and names no file inside it, while
`-uall` would expand `target/` and `node_modules/` and list ignored secrets such as `.env`.

**The contract** (carried by `oneagentgraph-link-onejudge` and `onepipeline-link-fixes`,
consumed by `aio-design-doc-judge-artifacts`, which points the design-document judge at the document;
`crates/onejudge/src/cli/config.rs`'s config type is the one declaration, and the generated
schema, `starter.yaml`, the Python SDK types and `docs/cli.md` are derived from or reconciled
with it):

- **Config key `user.artifacts`**: an optional list of strings, default empty. Each entry is a
  path to a file or a directory; an absolute path is used as written and a relative one is
  resolved against the judge's evidence worktree (the directory `git_status` runs in). It is
  accepted wherever `user.*` keys are accepted today, so a persona delta's `user` map carries it
  through a composer that merges `user.*` keys.
- **Override**: `--artifact <PATH>` (repeatable) on the CLI and `ONEJUDGE_ARTIFACTS` in the
  environment (entries separated by the platform path-list separator, as `PATH` is), which
  replace the configured list with the same precedence `--persona` / `ONEJUDGE_PERSONA` have
  over `user.persona`. A composer that cannot template a per-run config uses the environment.
- **Rendering**: when the list is non-empty, every judge-side prompt that carries the evidence
  contract (supervisor, judge and assessment) gains a section after it that lists each entry's
  resolved path; says these artifacts may be untracked or gitignored, that `git_status` and
  `git_diff` do not show them, and that they are read with the file-reading tools; marks an
  entry that does not exist as not existing, rather than failing; and for a directory lists the
  files beneath it, newest-modified first, bounded to at most 50 entries, with one line stating
  how many were omitted. The listing is re-read on every judge-side turn. When the list is
  empty, every prompt's bytes are identical to today's.
- **Protocol**: the `CommandProvider` evidence payload carries the resolved entries as
  `artifacts`, omitted when empty, recorded under a protocol version bump in
  `docs/protocol.md`.
- **Unchanged**: `EVIDENCE_TOOL_RETRY_LIMIT` stays `4`, `git_status` and `git_diff` run exactly
  the commands they run today, and read-only mode's tool allowlist is untouched. The loud
  failure after repeated identical requests is correct behaviour; the defect is that the
  channel could not see its subject.

## Why

In the run that closed #855, a whole design-document dispatch was lost: its judge asked the one
question it was allowed to ask, six times, about a file that question could never see, and
the rerun asked it four more times before reaching a verdict — same prompt, same member, so
the loop is reproducible. The operator wants a judge that is told where the work it is judging
lives. A retry limit that stops a looping judge is the correct signal and stays as it is.

## Acceptance criteria

- When the artifacts key of a config names a gitignored file and a gitignored directory, every
  judge-side prompt that carries the evidence contract — supervisor, judge and assessment — names
  both resolved paths; says they may be untracked or gitignored, that `git_status` and `git_diff`
  do not show them, and that they are read with the file-reading tools; and, for the directory,
  lists its files newest-modified first under the stated bound, with an omission line when the
  bound is exceeded. The listing is re-read on every judge-side turn: when the directory's contents
  change between two judge-side turns, the later turn's prompts list the changed contents. Each is
  proven end to end over the fake harness with a real repository worktree whose `.gitignore`
  covers both.
- `--artifact` and `ONEJUDGE_ARTIFACTS` each replace the configured list, with the same
  precedence as the existing persona override, proven end to end.
- A named path that does not exist is rendered as not existing and the run does not fail.
- With no artifacts named, every judge-side prompt is byte-identical to today's, and the golden
  prompt files and their equality assertions pass unchanged.
- A `CommandProvider` judge receives `artifacts` in its evidence payload when entries are named
  and no such member when none are, and `docs/protocol.md` records the field under a version
  bump, with its snippet test passing.
- `EVIDENCE_TOOL_RETRY_LIMIT` is still `4`, the commands `git_status` and `git_diff` run
  are unchanged, and read-only mode's tool allowlist is unchanged — no shell or other tool is
  added to it, held by a test that fails when the allowlist differs from today's.
- The config schema, `starter.yaml`, the generated Python SDK types and `docs/cli.md` name
  `user.artifacts` and its overrides, and the checks that reconcile each with the config type
  pass.
- The tests covering the changed config, provider, engine and protocol code pass over the
...
- Read first: `AGENTS.md`, `tests/AGENTS.md`, `docs/protocol.md` (around lines 20-23 and
  167-172), `docs/cli.md`, `crates/onejudge/src/provider.rs` (`EvidenceContext`,
  `evidence_prompt`, `resolve_evidence_request`), `crates/onejudge/src/engine.rs`
  (`history_files` and its uses), `crates/onejudge/src/oneharness/mod.rs` (the three evidence
  loops), `crates/onejudge/src/cli/config.rs` (`UserConfig`, `Overrides`),
  `crates/onejudge/src/command.rs` (`EvidencePayload`), `crates/onejudge/src/bin/fake_harness.rs`.
- One test binary: `cargo nextest run --features fake-provider,cli,sdk-schema --test e2e`;
  `just python-sdk-generate` regenerates the SDK types; `just lint-llm-diff <base>` is the
  judged tier. The merge path runs the full gate.
- This is a feature and will release as a new minor version; `oneagentgraph-link-onejudge`
  adopts it. The contract above is consumed by other nodes. If you find it wrong, surface a
...
{"onepipeline.id":"onejudge-named-artifacts","onepipeline.max_turns":200,"onepipeline.persona":"engineer","onetaskgraph.item_kind":"task","orchestrator.plan-review":{"by":"review-plan","key":"89f3e2f2955e8c4e6c570292df04386c888f8928cd1045339f33dc4a275ecc27","reviewed_at":"2026-09-13T17:30:00.411235+00:00"}}
-->
