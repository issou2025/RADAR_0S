...
**feat(channel)!: name no observer member in the engine: edit-applied, resettable check-in, the detailed profile**

*Project description:*
## What

Finish making the engine agnostic to the structure of the agent graphs a host runs: after the previous node
(`onepipeline-open-kinds-retire-serve`, whose branch you start from) retired `channel serve`, opened the
surface-kind vocabulary and opened `Author`, the engine still names the members of one host's observer graph in
four places. Remove each, with the behaviour it carries expressed generically:

1. **The `monitor-edit` surface kind becomes `edit-applied`.** It is raised exactly when the previous node raises
   it — an edit by any author other than the planner was applied — with `source` the applying author's word (so a
   host whose author is named `monitor` keeps writing `source: monitor`, byte for byte), `blocking: false`, and a
   message naming that author (`<author> applied an edit: <command>`). A `finding` still raises none. No field is
   added to the stored surface shape.
2. **`CHECK_IN_MEMBER` is gone.** Consuming a surface through `next` restarts the clock of exactly the members the
   run's recorded observer graph declares `resettable` in their `schedule` — the graph document is the
   configuration that names the pacemaker, and the engine names no member. A scheduled member that is not
   `resettable` keeps its cadence, and a run whose observer graph declares no resettable member, or has no observer
   graph, resets nothing and reports no failure for it.
3. **The shipped read-time filter profile `monitor` is renamed `detailed`**, with no alias: `next --filter
   detailed` and `monitor --filter detailed` read the whole merged stream, and the launch configuration's
   `filters.profiles` overrides it by that name. `planner` is unchanged and stays the default.
4. **No other `monitor` or `pacemaker` word remains** in `src/` or `docs/`: identifiers, comments, help text,
   messages and documentation describe "the observer graph", "a scheduled member", "an author other than the
   planner", "the check-in clock" and so on. The shipped example graphs and personas under `graphs/` and `personas/`
   stay as examples, and the engine does not read them at runtime.

**The `onepipeline monitor` view verb stays** — it means "watch a run" and names no member — and so do its
`MonitorArgs`, `monitor_of`-style identifiers and the prose naming that verb; the word check allows exactly those.

**Nothing persisted is renamed by the sweep.** Recorded keys such as the launch record's `heartbeat_interval` keep
their spelling, recorded journals and checkpoints naming `monitor-edit`, an author `monitor` or a `monitor` profile
override still read, and only the three words this node's contract names (the kind, the reset rule's member, the
profile) change what a new run writes or accepts.

Read first: `AGENTS.md`; `docs/contract.md` (the channel paragraph, the refusal examples, read-time profiles, the
shipped-content paragraph), `docs/contract-divergences.md`; `src/engine.rs` (`monitor_edit`, `finding_surface`),
`src/driver.rs` (`next` and its reset, the offline `reply` path), `src/agentgraph.rs` (`CHECK_IN_MEMBER`,
`reset_timer`, `recorded_graph_run`), `src/filter.rs` (`MONITOR_PROFILE`, `shipped_profile`, the launch-config
goldens), `src/cli.rs`, `src/views.rs`, `src/watch.rs`, `src/ledger.rs`, `src/edits.rs`, `src/projection.rs`,
`src/payload.rs`; the existing check-in reset journey and the words test the previous node added; and from the
linked `oneagentgraph` release, `run::signal`, `Record::declared_members` and how a `resettable` schedule honours a
reset.

## Why

The user's constraint for this plan is that `onepipeline` "stays agnostic to the actual structure of agent graphs"
and "should only expose the tools to set up agents for the whole lifecycle" — it "may not know what a monitor, a
pacemaker, or a judge seat is". The first revision of the plan deferred these names to a follow-up; the user read
the design and said the deferred items "should be included". An engine that resets a member called `check-in` by
name, raises a kind called `monitor-edit` and ships a profile called `monitor` still knows one host's graph, and a
host whose observer members are named anything else gets no check-in reset and a vocabulary describing somebody
else's arrangement.

## Acceptance criteria

- The engine raises no `monitor-edit` surface: an edit applied for an author other than the planner raises one
  non-blocking `edit-applied` surface whose `source` is that author's word and whose message names that author, a
  `finding` raises none, and a recorded journal carrying a `monitor-edit` surface still folds and renders — proven
  through the compiled binary in the e2e suite with a launch whose `--bus-config` declares an author named by
  nothing built in, both through the driving loop and through a `reply` applied while nothing is driving the run.
- No member name decides a check-in reset: `src/` has no `CHECK_IN_MEMBER` or other constant naming an observer
  member, and e2e journeys through the compiled binary over real launches show all of: (a) for an observer graph
  whose members are named by nothing built in and which declares **several** `resettable` scheduled members beside a
  scheduled member that is not `resettable`, consuming a surface through `next` restarts the clock of **every**
  resettable member, each reset observed on its own member, and leaves the non-resettable member on its cadence;
  (b) for an observer graph declaring no resettable member, consuming a surface delivers it with no reset failure
  reported; (c) for a launch with no observer graph at all (`--dag-graph off`), surfaces are queued and consumed
  through `next` without error and with no reset failure reported. The existing check-in reset journey passes
  re-taken against this rule.
- The shipped profiles are `planner` and `detailed`: `next --filter detailed` and `monitor --filter detailed`
  deliver the whole merged stream, `--filter monitor` is refused as naming no profile unless the launch
  configuration defines one, a launch configuration's `filters.profiles.detailed` overrides the shipped profile,
  and every recorded launch-configuration golden still loads — proven by the filter tests and an e2e journey through
  the compiled binary.
- No `monitor` or `pacemaker` word names a member in the engine: across `src/` and `docs/` (test fixtures,
  recordings under `tests/`, `CHANGELOG.md` and `docs/contract-divergences.md` exempt), the whole words `monitor`
  and `pacemaker` (case-insensitive) occur only where they name the `onepipeline monitor` view verb. The words test
  the previous node added is widened to this, names the view verb as its only allowance by name, and fails naming
  the file and line of any other occurrence.
- Nothing in `src/` reads or embeds this repository's shipped `graphs/dag-scope.yaml` or any file under
  `personas/`, and `docs/contract.md` describes those files as examples a host may copy.
- `docs/contract.md` states the `edit-applied` kind, the resettable reset rule and the `detailed` profile, and
  `docs/contract-divergences.md` records the rename of `monitor-edit`, the removal of the member-named reset and the
  rename of the shipped profile, each with what a recorded run still reads.
- The checks that exercise this change are green over the finished tree: the crate's tests and e2e journeys, the
  format check, and the judged lint over this branch's own diff.
- Every claim the dispatch makes about the finished work — what it changed, what it verified, and the evidence it
  ...
  later edit, and every claim about how the finished tree behaves, and every check result cited for it, holds of the
  finished tree. Evidence the dispatch explicitly labels as describing an earlier state or a deliberately induced
  failure is held to that label — it must be true of the state it names — and not to reproduction on the finished
  tree.

## Planner amendment (attempt 4, continuing branch `onevcs/s-20fb23113816`, PR #350)

Where this section and the operational notes below disagree, this section wins.

**Where the branch stands.** The branch carries the node's work plus three later commits: `918314c`
(`fix(agentgraph): deliver a graph's last burst before ending its relay`, which cleared the `gate` refusal),
`3ae951c` (`fix(engine): keep a dispatch's registry entry until its node is settled`, which cleared the
`cross (windows-latest)` refusal, with its e2e `host_proves_stale_the_dispatch_a_running_node_has_already_ended`
in `tests/e2e/views.rs`), `c62fcbd` (docs), and `c77f8be`, a merge of `origin/main` (which brought only the
`v0.35.0` release chore). At `918314c` every required check that reached a verdict was green, `smoke` included. At
`c77f8be` the required check `smoke` failed:
https://github.com/nickderobertis/onepipeline/actions/runs/35286719909/job/105420626464 — the journey
`a_lifecycle_node_opens_a_real_pull_request_merges_it_and_the_base_advances` in `tests/smoke/main.rs` (real git,
real `onevcs`, real `oneagentgraph`, real GitHub; only the model turn is the `fake-oneharness` stand-in, whose
`harness.work` script writes one file into the worker's worktree) settled its one node `failed` as `empty-branch`:
"compared against origin/main: onepipeline-smoke/30863-op-30863-0 carries nothing it does not. This dispatch
committed nothing to it". The sibling recorded `session-opened, fetch, session-opened, session-closed` and nothing
published. That journey has failed on no other branch and not on `main` in the last two days, so the regression came
in with the three commits above, and `3ae951c` is the only one that changes what the engine does with a dispatch
after its stream drains. You cannot run the smoke tier here — it needs the scratch repository's token — so read the
job log and reproduce the shape locally.

**What must be true of the finished tree, in addition to every criterion below:**

- A lifecycle publication reads what the worker turn left: an e2e journey through the compiled binary with the
  smoke's shape — a lifecycle node whose worker turn leaves an uncommitted file in the worktree the dispatch was
  given, and whose publication is read after the dispatch's stream has drained — publishes that file's change,
  and the journey fails on the tree at `c77f8be` with the merge path's own words (`empty-branch` / "committed
  nothing") and passes on the finished tree. If the suite's existing lifecycle journeys already cover that shape
  and pass at `c77f8be`, the journey you add is the one that does not, and your report says what the fakes were
  blind to. Whether the hosted `smoke` check then passes is the merge path's verdict, outside this bar.
- One of two finished-tree behaviours over the dispatch registry, and your report says which: `host` proves an
  ended dispatch's registry entry stale while its node is not yet settled and a `stop` landing in that window
  leaves no `UNPROVEN` row, with `host_proves_stale_the_dispatch_a_running_node_has_already_ended` passing; or
  the tree carries none of `3ae951c`'s registry-lifetime change and neither that journey nor `EndedDispatches`,
  with the reason the two properties could not coexist stated in your report. A tree keeping part of that change
  is neither.
- `tests/smoke/main.rs` and the assertion of `views::host_never_renders_a_dispatch_of_a_run_that_was_stopped`
  are unchanged.
- The description you leave for closeout states, in one paragraph, what the smoke refusal was, what caused it,
  and what the finished tree does instead, with the job link above.

The publication budget is fresh for this attempt; the harness publishes, as the operational notes say.

## Additional info

- **Added by a revision of this plan, on the user's ruling.** The plan first deferred these names to a follow-up;
  the user rejected that. The manager ruled over the ask seam: the kind is `edit-applied` with source the author's
  word and no new stored field; the reset follows the graph's own `resettable` flag with no new configuration key
  anywhere; the profile is `detailed` with no alias. It is a second engine node rather than part of
  `onepipeline-open-kinds-retire-serve` because that node already carries the retirement, the open kinds, the open
  author and the adoption of four crates, and the open author had to stay there because adopting the bus without a
  built-in `monitor` author does not compile otherwise.
- **The reset needs no new sibling API.** `oneagentgraph`'s run record already lists `declared_members`, and
  `run::signal` with `Signal::Reset` is honoured only by a schedule that declared itself `resettable`, so signalling
  every declared member of the recorded observer graph run is one way to meet the rule; confirm that an ignored
  reset leaves a non-resettable member's hold untouched before relying on it. If the linked release cannot deliver
  the rule without naming a member, ask the manager rather than adding a fallback name.
- **Contracts K and A are unchanged here.** This node renames one engine-raised kind and one shipped profile; the
  open kind vocabulary and the configured authors stay exactly as the previous node and the bus state them. A
  departure you want goes to `"$ORCHESTRATOR_ASK_MANAGER"` as a proposal while you keep building against the agreed
  surface.
- The default node graph path (`graphs/node-scope.yaml`, resolved against the launch directory) and the
  `worker`/`judge` roles of node dispatch are not this node's subject.
- `ai-orchestrator` re-points its `--filter monitor` call sites and its `monitor-edit` prose in its own node; do not
  go looking for them here.
- **Criteria repaired after plan review, on the manager's rulings.** (1) The reset criterion proved one resettable
  member and a graph with none, which an engine resetting only the first resettable member, or failing without an
  observer graph, could pass; it now requires several resettable members each reset on one consumed surface, and a
  launch with no observer graph that consumes surfaces without error. (2) The report-truth criterion's last clause
  read as excluding evidence the appendix permits — a run from before a change, or a failure induced on purpose — so
  it now holds claims about the finished tree to the finished tree and labelled evidence to its label. Nothing else
  in this task changed.
- The title's breaking marker is deliberate: a shipped profile name and an engine-raised kind change.
- The checks above are this repository's recipes: `just test` (or `just test-e2e` with a filter while iterating),
  `just fmt-check` and `just lint-llm-diff`. Running the pieces separately is fine; the full `just gate` is not owed.
...
{"onepipeline.branch":"onevcs/s-20fb23113816","onepipeline.id":"onepipeline-no-member-names-2","onepipeline.max_turns":220,"onepipeline.persona":"engineer","onepipeline.resume":{"branch":"onevcs/s-20fb23113816"},"onetaskgraph.item_kind":"task"}
-->
