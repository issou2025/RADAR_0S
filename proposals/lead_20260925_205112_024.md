...
**fix(tests): make the Windows cross leg deterministic**

*Project description:*
## What

Make onepipeline's `cross (windows-latest)` CI leg pass reliably by fixing the causes of its intermittent test failures, in the product code or the tests, whichever is actually wrong.

About a third of recent runs of that leg failed, each on a different test and each time on a test unrelated to the change under review. Known failures:
- **shutdown pid reuse:** `tests/e2e/shutdown.rs` `status_and_results_read_the_latest_shutdown_and_name_an_unreadable_one` failed in job 107983536824 of run 36107561280. The first `shutdown --force` killed the fake agent, then Windows reissued its pid 7588 within seconds. The second `shutdown --force` resolved the registry claim as `Claim::Reissued` and came back `Teardown::IdentityDeclined` (`src/driver.rs` `roots_to_stop`, `src/verbs.rs` near line 1156), so it exited 2 instead of 0.
- **loopcost:** `tests/e2e/loopcost.rs:454` `another_runs_ledger_is_read_on_its_own_interval_and_not_on_the_loops` failed on job 107997168596.
- **Others seen in recent runs:**
  - `tests/e2e/lifecycle.rs:7277`
  - `src/sys.rs:2784`
  - `tests/e2e/maintenance.rs:432`
  - `src/ledger.rs:4230`
  - `tests/note/main.rs:1346`, on main in job 107320554481
  - `tests/e2e/driver.rs` `a_run_belongs_to_the_session_that_launched_it`, job 107833608155
- **checkpoint access denied:** `tests/e2e/listing.rs` `a_host_sized_runs_root_lists_in_seconds_and_ten_times_the_journal_bytes_barely_moves_it` failed in job 108016861167. A reader of `checkpoint.json` got `Access is denied. (os error 5)` while the file was being atomically replaced.

Diagnose each failure from its own recorded job log before changing anything. `gh run view --job <id> --log-failed` reads a job's log. For an entry named without a job, `gh run list --workflow ci.yml` finds the run it failed in.

## Why

The user wants the per-node and mid-run graph-override change (pull request #495) merged. It is green on every required check except this leg, which failed twice in a row on unrelated tests. The user chose to make the leg stop being a lottery rather than re-run it until it passes.

## Acceptance criteria

- Every failure listed under `## What` is accounted for in the completion report. The report gives the test, the job it failed in, and its diagnosed cause, and says whether this change fixes that cause or why the failure is not this repository's to fix (for example a GitHub runner outage), with the evidence.
- The shutdown pid-reuse failure is fixed. When a dispatch process the same shutdown or an earlier one on this run already killed has its pid reissued, `onepipeline shutdown` and `stop` read it as gone rather than refusing. A pid reissued for a process this run never killed is still declined as before. E2e journeys prove both cases on Linux through the real `onepipeline shutdown` and `onepipeline stop` commands, checking each command's exit status and output. Each journey makes the recorded identity disagree with the live pid, so it does not depend on pid reuse actually happening.
- Each other fixed cause is fixed where it arises: a real race or wrong assumption in product code is corrected there, and a test that depends on timing, pid values, path separators or the scheduler holds by construction. No test is skipped, ignored, or marked Windows-only to make the leg pass, and no timeout or retry count is raised as the whole fix.
- The checkpoint access-denied failure is fixed where it arises: a reader of an atomically published file tolerates the replacement window instead of failing. A regression test drives a reader concurrently with repeated replacements of the same file and fails without the fix.
- Every correction to product code has a test or e2e journey through the real affected command or public interface. That test recreates the diagnosed trigger on Linux (for example a delayed scheduler, a reissued pid, a slow or contended ledger read, or a Windows-style path), fails on the tree without the correction, and passes with it. The completion report names that test for each correction.
- The tests this change touches, and the product tests over the code it changes, pass on this host. The format check, the lint and the judged lint over this diff are green on the finished tree.
- A dispatch line in a run's record of the processes its teardowns ended (`ended.jsonl`) that was written by a build of this branch before 46dd46c, and so carries no `pid`, needs no compatibility read. Such a line is declined as it is on the tree at 9a26dfd, meaning a stranger on that pid is left running, and is never read as matching a dispatch.
- Every claim in the completion report is true of the tree as it finally stands.

## Additional info

This host runs Linux, so the Windows leg itself runs only on the merge path when this branch is published. Use the recorded CI logs as the evidence of each Windows failure.

This node's previous two dispatches did the work. It is committed on the branch this dispatch continues, ending at 9a26dfd (30 commits over main), and that tree passed the touched tests, the format check, the lint and the judged lint. The last dispatch's reviewer failed it only because it wanted `ended.jsonl` dispatch lines without `pid` to be read, and the criterion above settles that question. Read the branch and its last report, confirm that each criterion holds on the tree as it stands, and change only what is still missing. If nothing is missing, re-run the checks this change touches and report.

Pull request #495 (branch `onevcs/s-83fb6bbb9c2c`) is another node's work. Do not change it or depend on it.
...
{"onepipeline.branch":"onevcs/s-321c212e472a","onepipeline.execution_checkout":"nickderobertis__onepipeline","onepipeline.id":"op-windows-flakes","onepipeline.node":"op-windows-flakes-3","onepipeline.persona":"engineer","onepipeline.resume":{"branch":"onevcs/s-321c212e472a"},"onepipeline.supersedes":["op-windows-flakes","op-windows-flakes-2"],"onetaskgraph.item_kind":"task"}
-->
