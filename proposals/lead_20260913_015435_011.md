...
**feat(graph)!: let a two-party member stack several judges, and publish which judge decided what**

*Project description:*
## What

Adopt the `onejudge` release that stacks judges — named by the `## Cross-repository references` block appended to
this task, with the producer's adoption instruction beside it — in `nickderobertis/oneagentgraph`, and let a
`kind: onejudge` member's `judge:` be a **list** of sides so a graph can stack them.

Read `AGENTS.md`, `docs/contract.md`, `docs/persona-format.md`, `src/config.rs` (`AgentSide`, `JudgeSide`,
`JudgeHarness`, `JudgeCommand`), `src/invoke.rs` (`provider_block`, how each side's oneharness config is written
into the member's scratch), `src/judge.rs` (`plan`, `ingest`, the `Observation` match) and `src/event.rs` first;
then the producer's `docs/judges.md` at the adopted release, which states the config shape, the panel's verdict
rules and the per-judge surfaces you are composing onto. The three contracts below are restated from that release
and from the plan; they are read-only here.

**What to build.**

1. Move the `onejudge` requirement in `Cargo.toml` to the adopted release and refresh the lock so the graph resolves
   one `onejudge` and one `oneharness-core`; rewrite the comment block beside the pin so it describes the release
   now linked (a list of judges, an llmlint kind, `Observation::JudgeDecided`, report schema 12) rather than
   narrating the previous pair.
2. The graph surface per Contract E: `judge:` accepts one side or a list, normalized to one representation with one
   composition path; the `Llmlint` variant of `JudgeSide` with its fields; `provider_block` composing `kind:
   oneharness` for a single harness side (unchanged) and `kind: split` + `judges:` otherwise, one scratch file per
   harness side, the llmlint `config` anchored to an absolute path and never copied; labels passed through.
   `oneagentgraph graph validate` (or whatever the validating verb is named) refuses a list entry that is none of the
   three shapes with a message naming the entry.
3. `ingest` handles `Observation::JudgeDecided` by publishing the new `judge-decided` event kind with the stated
   payload, and `docs/contract.md`'s event-kind list gains it; the fake doubles this repository drives a two-party
   member through carry the new observation so the relay is proven rather than assumed.
4. Every place this repository documents the judge side as one harness or one command — `docs/contract.md`,
   `docs/persona-format.md`'s refused-field table (whose wording may name `judges`), `README.md`, `src/AGENTS.md`
   — describes the list.

### Contract E — a `kind: onejudge` member's `judge:` may be a list (oneagentgraph states it; onepipeline and this host restate it from here)
...
members:
  worker:
    ...
    judge:                                            # one side, exactly as today — OR a list of sides
      - oneharness_config: ./oneharness.judge.toml    # a harness side: `oneharness_config`, optional `model`, optional `label`
      - kind: llmlint                                 # an llmlint side: optional `config`, `bin`, `diff_base`, `args`, `label`
        config: ./llmlint.yml
        diff_base: origin/main
      - command: [my-judge, --flag]                   # a command side: `command`, optional `label`
```

- A single mapping is the one-element shorthand for the list, and the two are normalized into one representation
  (`Vec<JudgeSide>`) with one composition path. `JudgeSide` gains a third variant, `Llmlint`, recognized by its
  required `kind: llmlint` field; the harness and command variants keep their spelling, so every graph on this host
  and its `members.worker.judge.oneharness_config=REF` overrides stay valid.
- Composition into onejudge's `provider` block: a list holding exactly one harness side composes `kind: oneharness`
  carrying both sides, exactly as today; any other list composes `kind: split` with `skill: {kind: oneharness,
  bin, stream, control}` and `judges: […]` in list order — a harness side as `{kind: oneharness, bin, judge_config:
  <member scratch>/judge-<label>.toml, label}` with that side's resolved config written to its own file, an llmlint
  side as `{kind: llmlint, config: <absolute path>, bin, diff_base, args, label}` with `config` resolved against
  the graph document's directory the way every other path in a graph is and handed over as an absolute path
  (never copied into scratch, because an llmlint config resolves its own plugins relative to itself), and a
  command side as `{kind: command, command, label}`. Labels pass through verbatim; an absent label is left for
  onejudge to default.
- A persona still may not carry `provider`; `judges` reach onejudge only from the graph.
- The event stream gains one kind, `judge-decided`, published from `onejudge::Observation::JudgeDecided` with
  payload `{turn, judge, kind, decision, reason}` and the member's usual labels, listed in oneagentgraph's
  `docs/contract.md` beside `turn-message`.


Contracts A–D are onejudge's, stated in the adopted release's `docs/judges.md`; this node restates the parts it
composes onto (Contract A's `judges:` and `kind: llmlint` fields, Contract C's `Observation::JudgeDecided`) and
changes none of them. A shared interface is never changed unilaterally: a departure this node wants is a proposal
to the manager over the ask seam, built against the agreed surface meanwhile.

### Working notes

The checks that exercise this change are `cargo test` narrowed to the config, invoke, judge and persona modules and
the e2e binaries that drive a two-party member (`just --list` names the fast tier), `cargo clippy`, and `just
lint-llm-diff origin/main`. Do not run the whole gate; the merge path does. Commit on the branch the session opened.
The title is the publication subject and is never re-derived; the `!` declares the public `JudgeSide` change, and a
semver finding against this crate's API is a finding to report, not a reason to retitle.

## Why

Every worker on this host runs through a `kind: onejudge` member of this crate, whose `judge:` is today one
harness side or one command side, so until a graph can name a list here the user's stack — an LLM reviewer, an
llmlint run and a repository's own scripts on one worker at once — cannot be reached from any dispatch. The user
asked for exactly that stack, and for the consumer chain to adopt it in order: this crate is the first link after
onejudge, and onepipeline cannot move its own pins until this one links the new release.

## Acceptance criteria

- The `onejudge` requirement names the release the cross-repository reference row for `onejudge-llmlint-judge` carries (the later of the two onejudge rows, whose release carries the panel and the llmlint kind both), and the committed `Cargo.lock` resolves exactly one `onejudge` and one `oneharness-core`, which the inventory test proves.
- The graph surface Contract E in this task's `## What` fixes holds over the finished tree: a `kind: onejudge` member's `judge:` accepts one side or a list, normalized to one representation with one composition path; the single spelling and this host's `members.worker.judge.oneharness_config=REF` override compose the same `kind: oneharness` provider block they compose at the release before this change, proven by a test comparing the composed block; and a list of two or more sides — or a single llmlint or command side — composes `kind: split` with `judges:` in list order, each harness side with its own scratch config file and each llmlint side with its `config` as an absolute path, proven by driving a real two-party member through the repository's deterministic doubles and reading the onejudge config it was launched with.
- A list entry that is none of the three side shapes, and a harness or llmlint side naming a config that cannot be resolved, are refused with a message naming the entry.
- `Observation::JudgeDecided` reaches the event stream as a `judge-decided` event carrying `turn`, `judge`, `kind`, `decision` and `reason`, proven by driving a member whose double produces one; `docs/contract.md` lists the kind beside the other member event kinds.
- Every document in this repository that describes the judge side describes the list shape, and the comment block beside the `onejudge` pin describes the release now linked rather than the previous one.
- The tests over the code this change touches, the workspace lint, and the judged lint over this change's own diff are green over the finished tree.
- Every claim the dispatch reports about the finished work — what it verified and the evidence for it — is true of the tree as it finally stands.

## Amendment (manager ruling; where anything below or in `## Acceptance criteria` disagrees with this, this wins)

- Contract E's "labels pass through verbatim" means a judge entry's `label` reaches onejudge unchanged whenever onejudge would accept it — onejudge's own label alphabet: letters, digits, hyphens and underscores — and a label outside that alphabet is refused at graph validation, naming the member and the entry, because a harness judge's label names the config file this crate writes for it. That refusal is the pass-through Contract E requires, not a departure from it: no label onejudge accepts is altered, and no label onejudge accepts is refused.
- The preserved branch this dispatch continues already holds the finished work and its review; read it before changing anything, and do not redo work the branch carries.

- The preserved branch's tip already carries a judged-lint pass taken after its last commit (the previous dispatch's diff-scoped judged lint ran after commit `067b1aa` and reported 0 failed). The completion report names, beside each verification claim, the commit hash of the tree that check was taken over.
...
{"onepipeline.adoption":"published","onepipeline.branch":"onevcs/s-e2a4c040e99e","onepipeline.consumes":{"onejudge-judge-panel":"crate","onejudge-llmlint-judge":"crate"},"onepipeline.id":"oneagentgraph-judge-list-3","onepipeline.max_turns":160,"onepipeline.persona":"engineer","onepipeline.resume":{"branch":"onevcs/s-e2a4c040e99e"},"onetaskgraph.item_kind":"task"}
-->
