...
**fix(github-projects): converge the live journey's wait for a board item it just created**

*Project description:*
## What

`onetaskgraph`'s `pre-push` gate reaches `scripts:distribution-test` →
`scripts/check-fixture-discrimination.sh` → `cargo test -p onetaskgraph-github-projects
--test live`, and that journey fails reproducibly at
`crates/onetaskgraph-github-projects/tests/journey/mod.rs:2684`:

    GitHub live capability journey failed: the board never reported the task this run
    created (I_kwDOUBBgxs8AAAABSya5fA)

Three publications died there minutes apart, each on a different created issue. **Nothing
about any of those changes is involved** — this journey runs on every push to this
repository, so every change to it is blocked until this is fixed. Correct that, in
`crates/onetaskgraph-github-projects`, as a general correction.

### What planning already established — start here rather than rediscovering it

- **`await_on_board`** (`tests/journey/mod.rs:1729`) creates an issue, adds it to the
  nominated board, then polls a **freshly built** source thirty times over thirty seconds
  for the board's items narrowed to the run's title prefix. The item never appears.
- **Building the source afresh is the whole point of the wait, and it defeats a
  compensation the source already has.** `GitHubProjectsSource::created` (`src/lib.rs:1699`)
  exists precisely because *"an issue added to a board with `addProjectV2ItemById` is
  routinely absent from the very next read of that board"*, and a board read is completed
  from it. A source built afresh remembers nothing, so this wait asks GitHub's own board
  connection and nothing else — which is what it is for.
- **A board read can silently drop an item.** `GitHubProjectsSource::resolve`
  (`src/lib.rs:3092`) answers `None` for an item whose `content` is null, under a comment
  reading *"an item whose content the token cannot see has nothing to report at all"*. A
  content the token cannot see is one reading of a null; a projection GitHub has not
  finished is another, and the two are indistinguishable at that line. Whether that is what
  is happening here is yours to establish — it is the strongest candidate planning found,
  not a conclusion handed to you.
- **Paging is ruled out, not merely unconfirmed.** Read under the projects credential
  during planning: board number 1, titled "onetaskgraph live", holds **23 items**, and the
  read asks for 100 in one page.
- **The node cost is not a symptom.** The roughly 9,847 nodes per read is the designed cost
  of the requested page size — GitHub costs a document by its `first:` values, not by its
  rows — and would be identical against a one-item board. Do not spend the dispatch on it.
- **It is the board connection, not the issue search.** The failing session's own cost
  report separates them: 33 requests *reading the board* against 3 *searching this board's
  issues*. The eventually-consistent issue search is not what this wait is failing on.
- What is left to explain is why `addProjectV2ItemById` reports success while
  `ProjectV2.items` does not list the item for thirty seconds — or whether the item **is**
  in the response and is dropped during resolution.

### You cannot run the live journey, and here is what you drive instead

Every `oneharness.*.toml` on this host masks `GH_PROJECTS_TOKEN` from the dispatch chain,
so `--test live` **skips** in this dispatch. A skip is not a pass and must never be reported
as one. The credentialed gate at publication is where that journey actually runs, and that
is the design rather than a hole in this task.

What you *can* drive is the same journey. `tests/plugin.rs` builds the plugin through
`SourcePlugin::build` and answers every request over a **real loopback socket** from a
fixture board that keeps board state the way GitHub does, and at its foot it calls
`journey::against` and `journey::run` — the very same journey code, `await_on_board`
included, with no credential at all. That file's own opening says why the journey is shared
rather than spelled twice: *"Two spellings of one journey would measure two journeys."*
Keep it that way. The fixture board is where a new behaviour belongs; the journey is not to
grow a second, offline-only path.

### What this repository will hold you to

- **The correction is general.** Not a widened wait dressed as a fix, not a skipped or
  conditional check, not a change to the gate's composition to route around it, and never a
  push that bypasses a hook. Those were ruled out before this task was written.
- **The journey must still prove what it exists to prove**: that GitHub's own board
  connection reports the item, read through a source that did not do the writing. A
  correction that lets a source's memory of what it just created answer this wait has
  deleted the test rather than fixed it.
- `distribution-check` and `distribution-test` sit outside the `check` fan-out on purpose;
  `just check` and `just gate` are this repository's own tiers and its AGENTS.md is the
  authority on which is which. Read it before you choose what to run.
- This repository keeps durable notes per area and treats a `CLAUDE.md` as a symlink to an
  `AGENTS.md`. A mechanism this subtle that arrives with no explanation will be undone by
  the next person who reads that line and believes its comment.

### If the honest answer is that nothing here is wrong

It is possible that the source and the journey are both correct and GitHub's board
connection has simply become slower than the wait allows. If your evidence says that, **do
not widen the wait and do not decide it alone** — that outcome is refused by this task's
own constraints, so it is a fork for the manager. Ask, over the channel, saying what you
found and what you propose.

## Why

Nobody can merge anything to `onetaskgraph` until this is fixed. That is worse than the
project this task belongs to: a whole repository of the stack is frozen, its own release
train included, and the visual-docs branch waiting behind it is only the first thing to
notice. The operator asked for screenshots across six repositories and four of them landed;
this is the one blocker whose cost is not the README at all.

## Acceptance criteria

1. The finished tree carries a correction in `crates/onetaskgraph-github-projects` that
   makes a read of the board report an item this run's own writes put there, and states —
   where this repository keeps that kind of reasoning — which mechanism it answers and what
   the evidence for that mechanism was.
2. The correction is general rather than an accommodation: no wait in the journey is longer
   than it was, no check is skipped or made conditional on anything, the composition of this
   repository's gate is unchanged, and nothing in the finished tree causes a push to bypass
   a hook.
3. The journey still proves what it exists to prove. Its wait is answered by a read of
   GitHub's own board connection through a source that did not perform the writing, so a
   source's own memory of what it has itself created cannot satisfy it, and the finished
   tree shows that property still holding.
4. The failure is reproduced through this crate's real interface without a credential:
   driven over the loopback fixture board configured to model the behaviour the correction
   answers, the whole journey reaches the refusal the credentialed run reached — the board
   never reporting the item that run created — when the correction is taken out of the
   source, and converges with it in place. Neither that removal nor any temporary narrowing
   of what the journey asks is left in the finished tree.
5. The behaviour the fixture board gained is the board's and not the journey's: the
   credentialed target and the offline target drive one body of journey code, so neither can
   pass on behaviour the other never exercises.
6. This repository's own credential-free test tiers are green over the finished tree, and
   the judged lint over this branch's own diff against its merge base is clean.
7. The dispatch's report states plainly that the live journey did not run in this dispatch
   and why — the credential it needs is masked from every dispatch on this host — and names
   what was driven in its place. Neither the report nor anything in the finished tree claims
   the live journey passed.
8. Every claim the dispatch makes about the finished work — what it ran, what it saw, and on
   which commit — is true of the tree as it finally stands.

## Additional info

**Read this plan's own project record, `authoring:stack-visual-docs-recovery`, first.** Its
Contract 2 says what this node hands the node that follows it, and why the correction lands
on `main` on a branch of its own rather than riding the visual-docs branch.

**A credentialed observation is available to you, and you are meant to use it.** The
manager holds `GH_PROJECTS_TOKEN` and has agreed to answer a **read-only** GraphQL
observation over `$ORCHESTRATOR_ASK_MANAGER` with the output as it came back — what
`ProjectV2.items` returns for that board, each item's `content`, whatever narrowing your
question needs. Stand on your own evidence and the offline drive first, and ask only where
one observation decides a fork you cannot decide from the code. If the fork genuinely needs
an add-then-read cycle against that board rather than a read, ask for that **explicitly**
and say what you need observed: that board is the live test lane, and the manager will make
a deliberate probe on it knowingly and clean up after it rather than have one arrive as a
side effect. Batch your questions.

Where a fix in this crate turns out to need something a library this crate depends on does
not offer, that gap is surfaced rather than filled here — blocking over the ask channel if
this work cannot proceed without it, and as a follow-up draft if it can.

The failing gate log this task quotes is preserved at
...
— three of them, one per attempt. Read them for the session cost report and the exact
refusal rather than taking this task's summary of them on trust.

Commit as you go, on the branch your session opened; nothing here names a branch to work
on, and there is none to cut.
...
{"onepipeline.id":"journey-fix","onepipeline.max_turns":320,"onepipeline.persona":"engineer","onetaskgraph.item_kind":"task","orchestrator.plan-review":{"by":"planning-closeout","key":"37360d088bf96b853598782e4c51ee66e7178a5379b6222c5083b59900c20bbd","reviewed_at":"2026-09-23T16:45:01.430449+00:00"}}
-->
