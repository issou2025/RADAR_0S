...
**fix(deps): relink the fixed onevcs and oneharness-core, settle a host-prerequisite refusal without redispatch**

*Project description:*
## What

Two halves on one branch, in this order, because the second needs the first.

**1. Relink the two crates this plan fixed.** Move this workspace's `onevcs` requirement
(`Cargo.toml`'s workspace dependency, `0.24.2` today) onto the newly published `onevcs`
release carrying the local-direct closing lines, the late-landing baseline backfill and
the host-prerequisite failure kind (https://github.com/nickderobertis/onevcs/issues/154,
https://github.com/nickderobertis/onevcs/issues/158, and the onevcs half of
https://github.com/nickderobertis/onepipeline/issues/360), and this workspace's
`oneharness-core` requirement (`0.14.0` today) onto the newly published `oneharness-core`
release carrying the clean-exit rate-limit fall-through and the variant bin override
(https://github.com/nickderobertis/oneharness/issues/1297,
https://github.com/nickderobertis/oneharness/issues/1308). Each dependency's release is
named for you in this task's cross-repository references and in the note this run
delivers when the release arrives; take the versions from there and from nowhere else.
**The fix is the resolution, not the requirement**: a widened requirement with the
lockfile untouched changes nothing about what this build links, and has been mistaken
for the fix in this stack before. **One copy, still**: `oneagentgraph` and `onejudge`
each require `oneharness-core` and `onevcs` at a minor version, and this workspace
deliberately holds one copy of each so the types crossing those seams are one type; if a
named release falls outside what those siblings accept, raise it over
`$ORCHESTRATOR_ASK_MANAGER` rather than admitting a second copy. `scripts/linked-engines.sh`
is the check that reconciles what this build links, and `Cargo.toml`'s own comments on
each pin record why every earlier bump was taken.

**2. Route the new failure kind** — https://github.com/nickderobertis/onepipeline/issues/360,
accepted at severity `high`. `src/lifecycle.rs`'s attempt loop sends every
`Attempt::Preserving` publication failure back through the same bounded worker
redispatch, so a refusal no worker edit can repair — the onetaskgraph publication refused
for a release-plz version on the host — spent the node's whole budget without changing
the condition. The relinked `onevcs` now classifies a refused push whose merge-path
output carries the host-prerequisite marker as `FailureKind::HostPrerequisite`, its reason
carrying the hook's remediation. `src/vcs.rs::failure_of` matches the kinds arm by arm on
purpose so a new kind is a routing decision made here: map `HostPrerequisite` to a failure
that is neither preserving nor unread — a failed settlement under the existing outcome
word `infrastructure-failure`, carrying the preserved branch, its head, and the gate's
remediation in the detail, with no redispatch and no new outcome word or event kind.
`ChecksFailed`, `ChecksUnsettled`, `PushRejected` and `SyncConflict` keep the bounded
redispatch exactly as today, and a refusal carrying no marker is `PushRejected` as today.
Bring `docs/contract.md`'s statement of the failure vocabulary and its routing into
agreement, and the outcome vocabulary a consumer holds this engine to.

## Why

The user accepted #360 at severity `high`: a host missing a tool is not something a worker
can fix by editing the tree, and re-dispatching a worker onto the same branch for each
refusal burned a whole retry budget and hours of a run while the fix was one install on
the host. The relink is what makes every one of this plan's onevcs and oneharness fixes
reach a dispatch on this host at all: a dispatch publishes through the `onevcs` this
engine links and falls through identities through the `oneharness-core` it links, so a
fix landed in either repository is in force nowhere until this workspace resolves to it
and this host's engine pin moves.

## Acceptance criteria

- The workspace's onevcs requirement names onevcs 0.25.0 (with onevcs-testing 0.6.6), resolved
  from crates.io and never a git or path dependency, and the lockfile resolves onevcs to
  exactly that release. The oneharness-core requirement is left on the line this workspace's
  resolved oneagentgraph and onejudge require, by the manager's explicit deferral of that
  relink, and the report and the description the worker leaves for the change request
  state that deferral, naming oneharness issues 1297 and 1308 as not yet in force.
- The resolved dependency graph holds exactly one copy of `onevcs` and one of
  `oneharness-core`, satisfying the requirements `oneagentgraph` and `onejudge` place on
  them, and the linked-engines reconciliation is green over the finished tree.
- A lifecycle publication whose merge path refuses the push with the host-prerequisite
  marker settles the node failed under `infrastructure-failure` with the preserved branch,
  its head and the remediation in its detail, and dispatches no further worker attempt.
- A refused push carrying no marker, a red required check, an unsettled check and a sync
  conflict each still take the bounded worker-redispatch path exactly as before.
- The contract document's failure-kind routing and this engine's outcome vocabulary agree
  with the finished code.
- The branch's history carries a closing reference naming
  https://github.com/nickderobertis/onepipeline/issues/360, so the change that carries the
  routing closes it; names the onevcs issues the relink puts in force (154, 158) without
  closing them; and names oneharness issues 1297 and 1308 only as deferred — not yet in
  force in this engine — never as adopted.
- The routing is proven by an end-to-end lifecycle journey driving the real binary against
  a merge path whose hook prints the marker, rather than by inspection, and no test
  substitutes a double for the layer under test.
- The tests that cover the modules this change touches are green over the finished tree,
  and so is the judged lint over this change's diff.
- Every claim this dispatch makes about the finished work is true of the tree as it finally
  stands.

## Manager ruling

Where the operational notes below or a rendered references block disagree with the criteria above, the criteria win. The releases were pinned by the manager on 2026-09-19 after the run's `published` hold could not read the host's release acknowledgements: onevcs crate 0.25.0 (crates.io 2026-09-18T20:13Z; tag v0.25.0 carries landing c2731b39, PR onevcs#163) is the onevcs release to relink; oneharness-core 0.16.1 exists (tag oneharness-core-v0.16.1 carries landing d554384d, PR oneharness#1319) but is deferred out of this node for the reason the criterion states. Read a references block's branch names as the landings those releases carry, never as what to depend on.

## Additional info

A relink commit and a routing commit is the shape to aim for, so each reads cleanly, but it
is guidance rather than a bar: what the finished branch owes is the closing reference for
#360 and the attribution of the relink.

Iterate with this repository's diff-scoped judged lint, the linked-engines check, and the
`lifecycle` and `real_vcs` e2e modules rather than the whole gate; the required checks run
downstream on the change this branch becomes. `tests/e2e/hook.sh` is the pre-push hook the
lifecycle journeys write into their own merge path, and a journey whose hook prints the
marker is the natural proof.

The marker's spelling is onevcs's to state, in its `docs/contract.md` and as a named
constant in its crate; read it from the relinked crate rather than restating it here, and
if the relinked crate does not expose one, ask over `$ORCHESTRATOR_ASK_MANAGER` rather than
inventing a spelling.

Do not write a version into this task's own prose or into a commit message beyond what the
manifest and lockfile carry: the versions this node works against are the ones the
references and the release note state.

This repository's releases are cut from conventional commits, and this host's engine pin
adopts the release that carries this node, so a commit type that cuts no release would
leave the whole chain waiting. `fix` is the type.
...
{"onepipeline.adoption":"fast","onepipeline.branch":"onevcs/s-9c88fa646a57","onepipeline.consumes":{"oh-core-4":"core","ov-publish":"crate"},"onepipeline.execution_checkout":"nickderobertis__onepipeline","onepipeline.id":"op-relink-and-route-2","onepipeline.persona":"engineer","onepipeline.resume":{"branch":"onevcs/s-9c88fa646a57"},"onetaskgraph.delivers":["followups:I_kwDOTx8aUc8AAAABR5zlNw"],"onetaskgraph.item_kind":"task"}
-->
