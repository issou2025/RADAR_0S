...
**fix(settle): give a stated landing one authoritative reading and release evidence**

*Project description:*
## Resuming a preserved branch

One earlier dispatch of this node worked this branch; you continue it in your own worktree
rather than starting over, and the branch already carries the whole change in five commits
on top of the base at `727a2ba`: `ff789d7` (the typed stated landing, its projection to
results, status and the write-back, and release attribution at settlement), `c9bc9a1` (each
settled-landing journey under its own ignore-block), `a2b1425` (a release acknowledged before
a refused envelope stands harmlessly), `20a82c2` (the settle advice resolves its targets
through the release watch's own resolution) and `7856b28` (the judged findings on the
journeys closed, the settle op's `StatedAt` shape, and the fold's supersession arm explained).
On that tree the earlier dispatch observed, after `7856b28` was committed: the unit tests of
the touched modules green, the contract tests green, clippy and rustfmt clean, the adoption
and live-edit journeys green (79 passed), and the judged lint over the diff against the base
green (14 passed, 0 failed, 1 ignored). Three rulings the manager gave that dispatch stand:
the `release` field on `settle` rides reply-envelope v3 with divergence entry 57 recording
why the number did not move; a stated landing is the node's landing on its own evidence tier
and every surface presents it without re-reading the superseded branch; and the no-baseline
message goes to the reply's stderr beside a receipt line that stays byte-for-byte what it
was.

That dispatch was refused by its judge on one point only: its last substantive report
preceded its last commit, so the judge could not tie the lint result it cited to the tree as
it finally stood, even though the lint run in question was taken after that commit. What the
branch still owes is a dispatch whose report is about the tree as it finally stands, with the
evidence for each claim taken on that tree and named as such. Read the branch first; nothing
about it is to be reworked unless reading it shows one of the claims above untrue, in which
case repair that and say so. Any check you cite is one you ran on the final tree, or one you
name as taken before a commit you show is inert for it.

## What

Resolve https://github.com/nickderobertis/onepipeline/issues/296, accepted at severity
`high`: a landing supplied to `settle` has split semantics. It is stored as
`LandingFromEvidence` (`src/edits.rs:1687-1703,1784-1811`), release correlation prefers it
(`src/release.rs:27-31,273-292`) but reports an absent baseline as unrecoverable without an
acknowledgement, and the views ignore it entirely: `src/views.rs:573-624,630-642,2870-2878`
base the answer on the projected landing plus branch and repository, re-read the superseded
dispatch branch, and render `NOT landed` for work that merged. This repository's own
`docs/contract-divergences.md:3307-3318` already records the gap and its proposal.

What the incident cost, and it is the user's own run: in `follow-ups-lifecycle` the work
really did reach its base and really was carried by `onetaskgraph-cli` 0.2.31, but the
manager stated the landing after the merge, so no landing-time probe snapshot exists. The
dependent stayed `not-answered` indefinitely while `onepipeline results` rendered
`NOT landed`, and it took an independent verification plus `onevcs release acknowledge` to
free it.

**Two halves are in scope, and one is explicitly not.**

**(a) One typed, authoritative interpretation of a stated landing, projected to every
consumer.** Views, `results` and the status write-back must surface the stated landing and
its evidence tier instead of contradicting it with a fresh read of the superseded branch.
Cover both spellings the op accepts — a squash-merge commit object name and a change
request's URL. The existing divergence entry is the record of the gap; closing it is part of
this work, so the document and the behaviour agree when you are done.

**(b) Explicit release attribution at settlement.** Extend manual settlement to accept
target and version evidence and persist it with the acknowledgement semantics
`onevcs::acknowledge_release` already provides — it is a public library function of the
pinned `onevcs`, taking a reference, a target name, a version and a supersede flag, so no
change to that sibling is needed. When no version evidence is supplied and none is
derivable, `settle --landing` must say **immediately** that the landing has no release
baseline, and must print the exact acknowledgement command with the reference and the
target already filled in from what the engine knows, so a manager can paste it — rather
than leaving an unexplained indefinite hold. Do not fabricate a landing-time baseline from
commit dates or from the probe's current answer: neither proves what was public at landing,
and a generic target need not expose publication history or tie versions to commits.

**Out of scope, deliberately.** The ticket also floats a target-specific historical
recovery, for targets that do expose publication history tied to versions. That is a change
to `onevcs`'s own probe contract, which would add a fifth repository to this plan, a release
wait on it, and a relink of the engine. The manager has ruled it out of this node; a
follow-up records it. Do not implement it here, and do not widen the probe contract to make
room for it.

## Why

The user accepted this at `high` severity after three rounds of their own questions on the
issue, which is a measure of how confusing the split behaviour is to read. Today a manually
settled landing leaves two of this harness's own surfaces contradicting each other — the
run record says the work landed, `results` says it did not — and a dependent waiting on the
release can wait for ever with nothing saying why. The manager's remedy is to verify the
release by hand, acknowledge it, and remember not to trust the results view: three manual
steps and a piece of lore, for work that already merged.

## Acceptance criteria

- A stated landing has one typed interpretation, and views, the results view and the status
  write-back all present that interpretation rather than a fresh read of the superseded
  dispatch branch.
- A node settled at a squash-merge commit object name, and a node settled at a change
  request's URL, each read as landed in those surfaces, with the evidence tier that answer
  rests on shown beside it.
- Manual settlement accepts a release target and a version as evidence, and persists that
  attribution with the acknowledgement semantics the sibling library already provides, so a
  dependent held on that attribution proceeds.
- Settling at a landing with no release baseline and no supplied version evidence answers
  immediately in a message that says the landing has no release baseline and carries a
  ready-to-paste acknowledgement command with the reference and the target filled in.
- That immediate answer happens at the settlement rather than being discovered later from a
  dependent still waiting.
- No baseline is inferred from a commit date or from a probe's current answer anywhere in
  this change.
- The probe contract is unchanged, and no target-specific historical recovery is added.
- `docs/contract-divergences.md`'s entry recording this gap is brought into agreement with
  the behaviour this change lands, so the document and the code state one answer.
- Every behaviour above is proven by an end-to-end journey driving the real settle op, the
  real projection and the real views over a real run rather than by inspection, and no
  journey substitutes a double for the layer under test.
- The finished change records that it resolves
  https://github.com/nickderobertis/onepipeline/issues/296, so the ticket is traceable to
  the change that resolves it.
- The tests that cover the sources and the documents this change touches are green over the
  finished tree, and so is the judged lint over this change's diff, and the report names, for
  each result it cites, the commit the tree stood at when it was taken.
- Every claim this dispatch makes about the finished work is true of the tree as it finally
  stands.

## Additional info

Iterate with `just lint-llm-diff <base>` and the narrowest test selection that exercises
the settle op, the projection and the views rather than this repository's whole gate. The
repository's own required checks run downstream on the change this branch becomes; nothing
here asks you to have run them first.

This is the largest node in the plan, and it is one node because both halves change
`src/edits.rs`, `src/release.rs`, `src/projection.rs` and `src/views.rs` together: a split
would stack two dispatches on the same files and buy no parallelism. Commit each coherent
piece as it works rather than holding one commit to the end.

This node starts from the work of the sibling node that gives run-end hooks a retry epoch,
which also edits `src/edits.rs` and `docs/contract.md`. That work is on the base you start
from.

The interpretation in (a) and the message in (b) are the agreements other nodes and this
host read; they are frozen for this run. If you find you cannot satisfy one as stated, raise
it over `$ORCHESTRATOR_ASK_MANAGER` and keep building against the stated one while you wait
— a shared interface here is never changed unilaterally.

This repository's releases are cut from conventional commits, and this host adopts the
release that carries this fix, so a commit type that cuts no release would leave it
unreleased and the adoption waiting indefinitely.

Board items on the follow-ups board are not yours to move — its credential is masked from
every dispatch. Recording the issue in the change is the whole of what this node does about
it.
...
{"onepipeline.branch":"onevcs/s-c39ad9b54519","onepipeline.execution_checkout":"nickderobertis__onepipeline","onepipeline.id":"op-stated-landing-r2","onepipeline.max_turns":4,"onepipeline.persona":"engineer","onepipeline.resume":{"branch":"onevcs/s-c39ad9b54519"},"onetaskgraph.item_kind":"task"}
-->
