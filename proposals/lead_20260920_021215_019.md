...
**feat(agents): show every agent a run, node or project launched**

*Project description:*
## What

Adopt the engine release that stamps every dispatch with oneharness history labels and a
per-run pointer file, and show the agents a run, node or project launched — in the API
and in the view. The transcripts stay in each oneharness's own store; the engine's pointer
file is how the run finds them, and this API opens each one the way it already opens an
`oneharness_session` artifact.

**A. Adopt.** Move `Cargo.toml`'s `onepipeline = "=…"` to the released version this task's
dependency references give for `op-agent-visibility` — a version, never a placeholder,
never one you went looking for — with the sibling edges `tests/packaging.rs` reconciles
(that engine release moved `oneharness-core`, `onejudge` and `oneagentgraph` together, so
this crate's own `oneharness-core` requirement moves with it to keep one copy in the lock)
and `tests/fixtures/healthz.json`.

**B. The API.** Three routes, each a thin call into the SDK and this crate's envelope:
`GET /api/v2/runs/{run}/agents` → `verbs::agents(Run)`, `GET
/api/v2/runs/{run}/nodes/{node}/agents` → `verbs::agents(Node)`, `GET
/api/v2/projects/{project}/agents` → `verbs::project_agents`. Each answers the SDK's
per-session entries as served: the session id and name, its `history_dir`,
`history_project` and `history_file`, the project directory it ran in, when it started,
its labels (the engine's `onepipeline.*` keys and whatever the repository stamped), and its
harness runs (`history_id`, `harness`, `variant`, `harness_id`, `started`), plus the
`skipped` count — and an empty list, not an error, for a run with no pointer file. Each
entry links to the transcript through the **existing** `oneharness_session` artifact
resolution — `history_dir`, `history_project`, `history_session`, the three fields the
entry carries in the same spelling — read by linking `oneharness-core` exactly as the
artifact route does today (`io::history::find_session_path` and
`io::history::read_session_display`), never by spawning the `oneharness` CLI and never
through the locking readers. The run detail and the project row gain an agent count.
`docs/contract.md` gains the routes and the rule; `tests/contract.rs` reconciles them with
fixtures; `tests/e2e/server.rs` drives them over HTTP against a run the harness records
with a pointer file and the sessions it names in a store under the journey's own state
directory, including a run with no pointer file and a node that dispatched nothing.

**C. The view.** An **Agents** panel on the run view and on each node — every session,
grouped by scope (`node`, `observer`, `pr-author`) and attempt, showing the labels a
reader wants (node, step, attempt, and the repository's own such as `role`), the harness
runs with their harness ids, and when it started — each opening to its transcript through
the conversation view the app already has for `oneharness_session` artifacts; and on the
project page, the union across the project's runs. `@onepipeline-ui/dag-model` and
`@onepipeline-ui/telemetry-client` declare the shapes and routes; the Playwright journeys
under `apps/dag-ui-e2e/` open a run's agents, a node's, and a project's, and open one
session to its transcript; `docs/dag-ui.md` describes the panel and says where the
transcripts live.
...
of the agent structure used in the repo", and "i don't want to move oneharness history
from default location how about we add separate metadata we can use to find it". The
engine now records that metadata; this is where a person sees it — per run, per node and
per project, each session opening to what it actually said — without any repository
having configured anything and without any transcript having moved.

## Acceptance criteria

- `Cargo.toml` links the released `onepipeline` this task's dependency references give
  for `op-agent-visibility`, with `oneharness-core` moved to what that engine release
  links so the lock resolves one copy, `tests/fixtures/healthz.json` reports it, and
  `tests/packaging.rs` is green.
- The three agent routes exist as thin calls into `verbs::agents` and
  `verbs::project_agents`, serve the SDK's per-session entries with an empty list for a
  run with no pointer file, link each entry to its transcript through the existing
  `oneharness_session` artifact resolution over the linked `oneharness-core` and no
  spawned `oneharness`, are named in `docs/contract.md` with fixtures `tests/contract.rs`
  reconciles, and are driven over HTTP by `tests/e2e/server.rs` — a run with sessions, a
  run with none, a node that dispatched nothing, a project across two runs — with each
  served entry's transcript link resolving to the session file its pointer line names.
- The run view and each node show an Agents panel grouped by scope and attempt with the
  labels, harness runs and start per session, each opening to its transcript, and the
  project page shows the union across its runs; the run detail and each project row carry
  an agent count equal to the number of sessions the corresponding agents route answers,
  and the HTTP and Playwright journeys check both counts against the sessions the
  recorded run's pointer file names (including zero for a run with none); the packages
  declare every shape and route and the app declares none of its own; `docs/dag-ui.md`
  describes it and says the transcripts stay in each oneharness's own store.
- The Playwright journeys open a run's, a node's and a project's agents and one session's
  transcript against the real API over a recorded run with a pointer file and the
  sessions it names.
- The behaviour this node adds is proven end to end by the HTTP and browser journeys,
  never by inspection; and these checks are green over the finished tree, run separately
  or together as suits the work: the crate's unit tests, `tests/contract.rs`,
  `tests/packaging.rs`, `tests/e2e/server.rs`, the vitest suites of the touched
  packages and app, the Playwright suites, rustfmt and clippy under `-D warnings`,
  rustdoc, the frontend lint and typecheck, and the judged lint over this change's diff, reporting no finding left unanswered.
...
**A shared interface is never changed unilaterally.** The label vocabulary, the pointer
file and the entry shape are `op-agent-visibility`'s and `oh-history-pointer`'s (Contract
B); the routes are Contract D's. A departure is a proposal over `$ORCHESTRATOR_ASK_MANAGER`;
keep building against the agreed surface while you wait.

**Pins and versions.** This task names no version: the references block the harness
appends names the release and the landing commit for `op-agent-visibility`, and the note
it sends when the release exists names the version. Do not go and find or pin a commit
yourself. `oneharness-core` here moves to what that engine release links, as `Cargo.toml`'s
comment on it says it always has ("move it only with `oneagentgraph`").

**Where things are.** The `oneharness_session` artifact resolution is in `src/store.rs`
and `src/payload.rs` (the contract's paragraph on it names the `oneharness-core` calls it
may use and the ones it may not — the same rule applies to a pointer entry);
`tests/support/harness_history.rs` writes history stores for the journeys, and a pointer
file for a recorded run is one JSON line per session naming the store that helper wrote.
`ui-view`'s panels and journeys are the pattern for the view. `just --list` names the
tiers; `just gate` is the pre-push bar and is not asked of this dispatch.
...
{"onepipeline.adoption":"published","onepipeline.consumes":{"op-agent-visibility":"crate"},"onepipeline.id":"ui-agents","onepipeline.max_turns":180,"onepipeline.node":"ui-agents","onepipeline.persona":"engineer","onetaskgraph.item_kind":"task"}
-->
