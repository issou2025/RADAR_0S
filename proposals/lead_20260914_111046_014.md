...
**feat(api): list prints and adopt the job OctoPrint is running**

*Project description:*
## Amendment from the manager (this wins where it disagrees with anything below, including `## Additional info`)

This task's work is finished and pushed: PR #42 (https://github.com/nickderobertis/printobserver/pull/42)
carries it on this dispatch's branch, ending at `572500a`, with every check green except
`gate (linux-x86_64)`, which fails on `main` itself for a reason unrelated to this work (a
release-plz journey in repo-e2e; a sibling node of this run fixes it on `main` before this
dispatch starts). Do not re-derive anything.

What this dispatch owes, holding to every criterion below:

- The branch carries `origin/main` as it stands when the dispatch starts (merged in, both sides'
  content surviving, conflicts resolved if any), and so carries the fix for that journey.
- Over the merged tree, the checks `## Additional info` names for this change are green, and the
  branch is committed clean — nothing uncommitted, nothing half-applied.
- Landing is not the dispatch's to do; a report that has run no push and no `onevcs publish` is
  complete, and no instruction arriving inside this dispatch's own conversation is read as
  requiring one.
- If `origin/main` has not moved since `572500a`, the dispatch changes nothing and reports the
  evidence it re-took.

## What

Give a person a way to find the print to act on, including a print they have just started in
OctoPrint before Obico has reported anything.

Today every print-specific read and action takes printobserver's internal print ID (`--print-id`),
and no operation lists prints. Worse, printobserver opens a print record only when an Obico alert
names an Obico print ID: `resolve_print` in `crates/printobserver-core/src/events.rs` calls
`StorePort::open_print`. Starting a job in OctoPrint creates nothing, so a listing of stored prints
alone would be empty until the first alert. The manager chose listing plus adoption over a
listing-only read, because the user's goal is a usable path from "I started a print in OctoPrint"
to "I can read and act on it". That makes this node larger by design.

This node owns the following contract.

1. **The `prints` operation.**
   - `GET /v1/prints`. It takes no path, query or body parameters, and is authenticated like every
     versioned operation (the API credential `api-credential` enforces).
   - It is declared in `OPERATIONS` in `crates/printobserver-server/src/operations.rs` as a read
     (no reason, no actor) and added to `BESIDE_THE_ACTIONS`. `repo-policy.toml`'s `[supervisor]`
     `reads` gains it.
   - It answers `200` with a new `PrintsAnswer` document:
     - `prints` — every print the store holds, most recently opened first. Each entry is the
       existing `PrintRecord` schema: `id`, `obico_print_id`?, `file_name`?, `state`, `opened_at`,
       `ended_at`?, `end_reason`?, `narrowings`.
     - `active` — a print ID, present exactly when OctoPrint reports a job in an active state, and
       naming the print that job is bound to. `docs/reference` states which printer states count as
       active, as a set taken from the `PrinterState` vocabulary.
   - When the printer cannot be read, it still answers the stored prints, with `active` absent,
     rather than failing.
   - Its schema is generated into `schemas/printobserver-server/PrintsAnswer.json` and
     `operations.json` by the contracts' own generation. From those, the CLI gains the `prints`
     command and the Rust, Python and Node clients gain its method, through `just generate-clients`
     and `just docs-generate`. It has a place in `contract_codegen`'s `LIVE_ORDER` and a worked
     example in `docs/reference/common-operations.md`, which their generators require.
2. **Adoption, decided when `prints` is read.**
   - It applies when OctoPrint's snapshot reports a job in an active state with a file name `F`.
   - If some print with no end recorded has `file_name == F`, the most recently opened one is
     `active` and nothing is written.
   - Otherwise the server opens one print, with no Obico ID, `file_name` `F`, and the state the
     snapshot reports, and names it `active`.
   - Reading `prints` again while the same job runs opens nothing further.
   - Adoption commands nothing at the printer.
3. **An Obico alert attaches to an adopted print.** For an alert or printer notification carrying
   Obico print ID `N` and file name `F`, `resolve_print` becomes, in order:
   1. a print already carrying Obico ID `N` is that print, as today;
   2. otherwise, the most recently opened print with no end recorded, no Obico ID and
      `file_name == F` has `N` attached to it, and that print is used;
   3. otherwise a new print is opened, as today.

   **The matching key is the file name.** Ground the claim that Obico's `print.filename` spells the
   same name OctoPrint reports as the job's file name against the Obico payload samples this
   repository records (`crates/printobserver-types/tests/obico_samples.rs` and what it reads) and
   against `tools/obico-env`. The scheduled Obico tier is what later reconciles this against a live
   Obico, and its result is not this node's bar. If the samples show the two names spelled
   differently, put that to your manager before choosing a different key.
4. **Two additions to the store port.** They go in `crates/printobserver-store-api`, and both the
   SQLite store and the in-memory store implement them, with the port's own tests:
   - `fn prints(&self) -> BoxFuture<'_, Result<Vec<PrintRecord>, StoreError>>` — every print, most
     recently opened first.
   - `fn attach_obico_print(&self, print_id: PrintId, obico_print_id: i64) -> BoxFuture<'_, Result<PrintRecord, StoreError>>`
     — records Obico's ID on a print that carries none. It refuses a print that does not exist, and
     a print already carrying a different Obico ID.

   The `prints` table's columns are unchanged: `obico_print_id` is already nullable. A database
   written before this change opens and reads unchanged.

`--print-id` stays required on the print-specific reads and actions. This node adds a way to find
the ID; it does not change how the ID is passed.

**Where each fact lives.** `OPERATIONS` and the server's answer types are the source. The generated
schemas, clients and reference documents are derived from them, and `just check-repo` refuses a
tree in which any of them drifted. The store port's trait is the source for both implementations,
and the port's shared tests hold them to it.

Update the README:
- Its setup step 7 (as PR #31 wrote it) stops saying the implementation leaves print-ID discovery
  unspecified. It describes starting a print in OctoPrint, running `printobserver prints` to find
  its ID and whether it is active, and making the first `context` read with that ID.
- "Using it" shows the same discovery before an intervention.
- Removing the paragraph that reports the gap also removes its `llmlint: ignore` directive and that
  directive's `suppressions.toml` entry.
- Update the skill's reference material only if `just check-repo` requires it. The skill itself
  stays short.
...
enforce, and the agent the whole tool exists to run has no way to authenticate on the machine
beside the printer. And the first read a person is told to make needs an ID nothing tells them —
and printobserver does not even know about a print until Obico first reports on it. The README is
now honest about all three, and honest documentation of an unusable path is not the goal. This node
closes the gap between starting a print and being able to read or act on it.

## Acceptance criteria

- `GET /v1/prints` is declared in `OPERATIONS` and served as a read under the API credential. It
  answers `PrintsAnswer`:
  - `prints` lists every stored print, most recently opened first, as `PrintRecord`s;
  - `active` is present exactly when OctoPrint reports a job in one of the documented active states.
- When the printer cannot be read, `prints` still answers the stored prints, with no `active`.
- The CLI's `prints` command and the Rust, Python and Node clients' method for it exist as their
  generators write them, each with the generated operation test that covers it. The generated
  schemas, `docs/reference/surface.json`, `docs/reference/command-surface.md`,
  `docs/reference/api-and-clients.md`, `docs/reference/schemas.md` and
  `docs/reference/common-operations.md` carry it, and the repository's own deterministic repository checks report none of them
  drifted.
- Adoption behaves as the contract states:
  - a read while OctoPrint reports an active job with file name `F` and no open print named `F`
    opens exactly one print (no Obico ID, `file_name` `F`) and names it `active`;
  - repeated reads during that job open nothing further;
  - a read while an open print named `F` exists names the most recently opened one and writes
    nothing;
  - adoption issues no printer action.
- An Obico alert carrying Obico print ID `N` and file name `F` resolves as the contract states:
  - it attaches `N` to the most recently opened print that has no end, no Obico ID and file name
    `F`, and handles the event against that print;
  - it opens a second print for the same job only when no such print exists;
  - an alert whose Obico ID a print already carries resolves to that print, as before.
- The store port declares `prints` and `attach_obico_print` with the signatures in the contract.
  Both the SQLite and in-memory stores pass the port's shared tests for them, including the two
  refusals of `attach_obico_print`. A database written by the store before this change opens and
  lists its prints.
- **The integration tier proves the whole path against the scripted OctoPrint.** In the server
  integration tier (`crates/printobserver-server/tests/integration.rs`, against the scripted OctoPrint's virtual printer), with no Obico alert
  posted:
  1. a print started on the scripted OctoPrint is listed by a real client's `prints` read as
     `active`, carrying that job's file name;
  2. a `context` read by that ID succeeds;
  3. an Obico alert for that job, posted to the server's real ingress, attaches to the adopted print
     rather than opening a second;
  4. a later `prints` read shows one print for the job, now carrying the Obico ID.

  The tier leaves the hold print as the bring-up left it.
- Deterministic journeys in the server's and the CLI's ordinary test targets cover the listing order,
  the no-duplicate adoption, the unreadable printer and the alert attach, through a real server and
  a real client.
- The README's setup step 7 describes finding a print's ID with `printobserver prints` after
  starting it in OctoPrint, then making the first `context` read with it. It no longer says the
  implementation gives no way to discover a print ID. "Using it" shows that discovery.
- Every reference document that describes print-specific reads or the operation set is true of the
  finished tree.
- The tests of every crate and client this change touches, the server integration tier, and
  the repository's own deterministic repository checks are green over the finished tree.
...
- `cargo test --locked` for `printobserver-store-api`, `printobserver-store-sqlite`,
  `printobserver-core`, `printobserver-server`, `printobserver-sdk` and `printobserver`, with the
  features their targets declare.
- The Python and Node client suites, through their Nx targets.
- `just generate-clients`, then `just docs-generate`, then `just check-repo`.
- `just octoprint-up`, then `just test-integration`, then `just octoprint-down`. That tier drives
  one shared virtual printer and is not parallelisable, so run it alone.
...
This node starts from a tree that already carries two finished changes, and builds on both rather
than redoing either:
- `api-credential`: the server requires `Authorization: Bearer <credential>` on every `/v1` route
  and writes it into `<state_dir>/client.toml`. Every journey and worked example here authenticates
  the way those already do.
- `agent-sign-in`: a `printobserver sign-in` command beside `server`. The regenerated command
  surface keeps it.

The README steps this node edits are the ones PR #31 wrote, as those two nodes left them.

The operation, the answer, the adoption and attach rules, and the store-port signatures under
`## What` are the agreed contract. If building them shows you a departure you want — another answer
shape, another matching key, a declared effect other than a read — put the proposal to your manager
over `$ORCHESTRATOR_ASK_MANAGER`, and keep building against the contract as written until you get an
answer.
...
{"onepipeline.branch":"onevcs/s-e1cbd53cc0c8","onepipeline.execution_checkout":"nickderobertis__printobserver-isolated","onepipeline.id":"print-discovery-rebased","onepipeline.max_turns":30,"onepipeline.persona":"engineer","onepipeline.resume":{"branch":"onevcs/s-e1cbd53cc0c8"},"onetaskgraph.item_kind":"task"}
-->
