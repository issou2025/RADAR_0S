...
**fix(linear): page the live journey's document reads to exhaustion**

*Project description:*
## What

Ticket: https://github.com/nickderobertis/onetaskgraph/issues/1311 — *Linear document journey reads
only the workspace's first page*.

**Root cause (from the ticket).** The live journey's document helper issues one `PageRequest` capped
at `MAX_PAGE_SIZE` and filters that one response locally
(`crates/onetaskgraph-linear/tests/settle/mod.rs:147-166`, basis
`9158e053c7a6bb88ad8874cc7ab24b6769b9757d`), so the unfiltered and orphan checks in `tests/live.rs`
do not search the whole workspace. Once more than a page of earlier documents precede them, the
journey cannot see the two documents the current run created. Transcript: `onepipeline transcript
delivers-plan otg-delivers-3`.

**The accepted fix.** Page until the listing is exhausted, accumulating only this run's document ids,
under a bounded request and time budget, with diagnostics that distinguish **budget exhaustion**
(stated as such, with the pages read) from **a missing document** (naming the ids not found after a
complete listing).
...
Otherwise healthy live journeys fail once the scratch workspace holds more than one page of older
documents, which blocks onetaskgraph releases this host waits on.

## Acceptance criteria

- The live journey's unfiltered and orphan document reads follow the listing's cursor to its end,
  keeping only this run's ids, bounded by a stated request and time budget.
- `tests/settle_gate.rs` proves, through the real plugin against a loopback workspace serving the
  run's documents after more than one page of other documents, that the reads find them; that a
  document absent from a complete listing is reported as missing by id; and that a listing longer
  than the budget is reported as budget exhaustion, distinctly from a missing document, and that
  the exhaustion report states the number of pages read.
- The live test target compiles.
...
- The checks that exercise this change — `tests/settle_gate.rs`, a build of the live test target, and the repository's judged lint over this change's diff where it runs one — are green over the finished tree.
...
**Proving a live-lane fix offline.** The Linear live journey (`tests/live.rs`) needs a Linear
workspace credential this dispatch does not have, and running it is not part of this task. The
crate already proves the live journey's reads offline: `crates/onetaskgraph-linear/tests/settle/mod.rs`
holds the helpers `tests/live.rs` reads through, and `tests/settle_gate.rs` drives those same
helpers through the real plugin over real HTTP against a loopback workspace whose index lags. Put
the logic your fix needs into those helpers and prove it there; `tests/live.rs` then calls the
helpers, and compiling it (the live test target built without running) is the proof that the live
journey uses them.
...
{"onepipeline.amendment":"Manager amendment (binding; where this and the task's operational notes disagree, this wins). The criterion that the repository's judged lint over this change's diff is green over the finished tree means: the finished tree carries one run of that judged diff lint taken after the last commit that changes anything the lint reads, that run reported no findings, and the report cites it. The operational note \"clear the findings it names, then stop — do not re-run hunting a clean sheet\" forbids re-running the lint over an unchanged tree to get a different verdict; it does not excuse a tree the lint never passed. A tree changed after a lint run that named findings owes one further run, and only a run naming something new owes another. The prior dispatch's work is committed on this branch as `88b19b1` (paging to exhaustion, native-id ownership, page-count and elapsed-time budgets, the budget-exhausted versus missing-id diagnostics, and the loopback tests including the label-filtered regression the lint asked for); the judge failed it only because the lint was never run again after that fix. Read the branch before adding anything; what is owed is the run of the judged diff lint over the tree as it stands, any finding it names cleared, and the report citing that run.","onepipeline.branch":"onevcs/s-2f3061d6a271","onepipeline.id":"otg-1311-2","onepipeline.max_turns":200,"onepipeline.persona":"engineer","onepipeline.resume":{"branch":"onevcs/s-2f3061d6a271"},"onetaskgraph.delivers":["followups:I_kwDOUBBgxs8AAAABRkqUKA"],"onetaskgraph.item_kind":"task"}
-->
