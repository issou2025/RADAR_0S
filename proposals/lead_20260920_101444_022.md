...
Adopt this workstream's cargo build contract in `onepipeline-ui` (github.com/nickderobertis/onepipeline-ui; checkout `onepipeline-ui` (registered, `~/projects/onepipeline-ui`)).

onepipeline-ui is one root crate (a `[package]` with no `[workspace]` table) beside TypeScript packages; no other `Cargo.toml` in the tree.
...
- `scripts/ensure-baseline-api.sh` builds a baseline commit's tree in a temporary directory with `unset CARGO_TARGET_DIR` and `--target-dir "$work/target"` — outside the clone and explicit, unchanged
...
- `apps/dag-ui-e2e/fixtures/serve-fixture.mjs` resolves the built API server from `CARGO_TARGET_DIR` when set and cargo's default otherwise; `apps/dag-ui-e2e/dag-ui.spec.ts` drives both cases (an empty `CARGO_TARGET_DIR`, a custom one) and `dag-ui:build-api-server` builds into the default
- `tests/e2e/ensure_baseline.rs` drives the baseline script with a fake cargo that honours `--target-dir`
- `justfile` comments under `_crate-test*` explain why `RUSTFLAGS` is not set per recipe (it would rebuild the shared `target/debug`) — still true
...
- The checks that read the target directory, spawn a built binary or run under coverage are green over the finished tree (their commands are under `## Additional info`): the API-server build target and the `dag-ui-e2e` journeys that resolve the built server through cargo's target directory (the `CARGO_TARGET_DIR` cases); the `ensure_baseline` e2e tests, which drive the baseline script's explicit `--target-dir`.
- Coverage collection through this repository's own coverage interface still works over the finished tree, reporting usable per-file line results under the line-table debuginfo: the crate's test tier under coverage instrumentation with its floor is green (its command is under `## Additional info`).
...
- the API-server build target and the `dag-ui-e2e` journeys that resolve the built server through cargo's target directory (the `CARGO_TARGET_DIR` cases): `bash scripts/nx.sh run dag-ui:build-api-server && bash scripts/nx.sh run dag-ui-e2e:test -- --grep CARGO_TARGET_DIR (or the whole dag-ui-e2e:test target if the runner takes no narrowing)`
- the `ensure_baseline` e2e tests, which drive the baseline script's explicit `--target-dir`: `cargo nextest run --locked -E 'binary(e2e) and test(/ensure_baseline/)'`
- coverage: the crate's test tier under coverage instrumentation with its floor: `just _crate-test`
...
{"onepipeline.id":"onepipeline-ui","onepipeline.persona":"engineer","onetaskgraph.item_kind":"task","orchestrator.plan-review":{"by":"planning-closeout","key":"b22dfda24fc1f262c4a949ec157954cd4266b7337fa74b8e4672f78c246bcb3e","reviewed_at":"2026-09-20T09:43:09.666800+00:00"}}
-->
