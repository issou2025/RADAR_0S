...
**fix(deps): relink the harness core, onejudge, oneagentgraph and onevcs**

*Project description:*
## What

Move onepipeline onto the releases carrying four fixes of this plan, so the engine this host
installs links them:

- `oneharness-core` carrying `oh-1295` (Codex `server_overloaded` classified as `FailureKind`
  `server_overloaded`, retried a configurable number of times on the same candidate, then fallen
  through with reason `server-overloaded`);
- `onejudge` carrying `oj-relink` and `oneagentgraph` carrying `oag-relink` (both relinked onto
  that core);
- `onevcs` carrying `ovcs-150` (release status's recovery command names the resolvable reference
  it was asked with, not an unresolvable landing commit), with `onevcs-testing` at the release cut
  beside it.

Raise the `[workspace.dependencies]` requirements, move `Cargo.lock` so each resolves exactly one
copy at those releases, adapt the engine to whatever those releases changed in their public APIs,
and update the comments beside each requirement that say what its floor is for. `just
engines-current` is the recipe that holds requirement and lock together, and it is the narrow
check for this change, not the repository's whole gate.

**A journey through the linked classifier.** `5901982` (*drive a Claude login refusal through the
linked classifier*, #319) is the precedent: add the same kind of journey for a Codex
`server_overloaded` refusal, proving the linked core's classification reaches the engine's
settlement — a node whose every candidate ends `server_overloaded` settles with `cause`
`server_overloaded` as the producer spells it.

**The producers' contracts (restated; `oh-1295` and `ovcs-150` own them).** `server_overloaded` /
`server-overloaded` as above; onevcs's release status carries the reference it resolved and prints
it in `onevcs release acknowledge <REFERENCE> --target <NAME> --version <VERSION>`.
...
A fix in a library the engine links is in force on this host only once the engine release this
host installs links it (AGENTS.md, "Which pin governs a dispatch"). Two accepted tickets are fixed
in such libraries: oneharness#1295 (an overloaded Codex identity failing a node instead of falling
through) and onevcs#150 (a release-hold recovery command that refuses its own argument). This
node is what puts both into the engine.

## Acceptance criteria

- `Cargo.toml` requires, and `Cargo.lock` resolves exactly one copy each of, the `oneharness-core`,
  `onejudge`, `oneagentgraph` and `onevcs` releases carrying `oh-1295`, `oj-relink`, `oag-relink` and
  `ovcs-150`, with `onevcs-testing` at the release cut beside that onevcs, and the repository's check
  that holds each requirement and its lock resolution together (named under Additional info) is
  green.
- A journey drives a Codex `server_overloaded` refusal through the linked classifier and the
  node's settlement carries `cause` `server_overloaded`.
- The engine's existing journeys over what these releases changed — the report shape
  `tests/e2e/turns.rs` reads, and the onevcs seam `tests/onevcs_seam.rs` drives — are green.
...
**The requirement-and-lock check** is `just engines-current`. Running it and the test modules
separately, in any order, is fine.

**Parallel with `op-dispatch-env-hook`.** That node adds the dispatch environment hook in the same
repository on its own branch; do not implement or wait for it. `aio-adopt-engine` adopts the
onepipeline release carrying both.
...
{"onepipeline.adoption":"published","onepipeline.consumes":{"oag-relink":"crate","oh-1295":"core","oj-relink":"crate","ovcs-150":"testing-crate"},"onepipeline.id":"op-relink","onepipeline.max_turns":200,"onepipeline.persona":"engineer","onetaskgraph.item_kind":"task","orchestrator.plan-review":{"by":"planning-closeout","key":"c0d3368b68e415070865ed3d02614529954374a7fb6c064a6c679e3320520ae2","reviewed_at":"2026-09-16T15:22:01.089936+00:00"}}
-->
