...
**feat(windows): bring the gate and the printer tier up on Windows**

*Project description:*
## What

Make this repository's own suites true on Windows. On both `windows-x86_64` and
`windows-aarch64`: the gate runs and passes, the printer integration tier drives a real
OctoPrint and its real virtual printer, and the three artifact-client jobs build and prove the
three clients. Those cells come out of `platform-exclusions` and into the jobs that own them.

This is the node that pays for goal 7 — path, filesystem, process and environment handling
that is actually portable — and it pays for it by running the suites on Windows and fixing what
they report, which is what the bring-up record left by the node you depend on already begins to
say. What is **not** in this node: the three end-user install routes and the Windows service.
Both stay recorded as exclusions, each entry's reason naming the node of this plan that removes
it, and both entries stay `install path: no`.

What the tree already shows, so you can plan rather than discover it, none of it a substitute
for what the runners actually say:

- **The Rust crates.** The server takes its shutdown signal through `tokio::signal::unix`, which
  does not exist on Windows; the console-control equivalents are what a Windows process is
  stopped by. The command-line crate reads a signal number off an exit status through a Unix
  extension. The default configuration path, and the state and home directories the installer
  writes, are `/etc` and `/var/lib` literals; Windows has its own answer and so does macOS, and
  one module giving the per-platform default is the shape that stops a third copy appearing.
  Tests reach for Unix file modes, symlinks and a Linux-only tracer.
- **The Python tools.** They call `os.uname`, `os.geteuid`, `os.kill`, `os.getpgid` and
  `os.killpg`, and they look for a virtual environment's programs under `bin`, which is
  `Scripts` on Windows. The artifact build looks for the built program at a path with no
  executable suffix.
- **The recipes** run under `bash`, which the Windows runners have; whether every program a
  recipe reaches for is there is something only a run says.

Fix each where the failure is, and give each a test that drives the real thing that broke — the
compiled program as a subprocess, the committed script, the recipe a user runs — rather than an
internal helper standing in for it. A fix whose only evidence is that the tree still compiles for
the target is not what this node owes. Each such test is owed in two places, and the acceptance
criteria keep them apart: it is **wired into the cell for the platform the defect was on**, which
is a property of the committed tree you can read here, and it **runs and passes on the platform
this dispatch runs on**, the other platform's branch reached through a fixture — a Windows path
answer, a `Scripts` program directory, an executable suffix — which is the proof you can actually
produce. What the Windows cells then report belongs to the merge path, not to you.

Where a fix needs a platform-specific branch, keep it behind a compile-time or run-time
platform condition so that no other platform's build carries it, and keep the Linux behaviour
byte-identical: nothing in this plan weakens a guarantee Linux has today.

## Why

The user's ask was that Windows be supported first class, fully tested and documented, and
their objection to what they had was precise: a platform that probably works but is untested is
one they cannot recommend to anyone. Windows is where that is least true — nothing here has
ever compiled for it, let alone run a supervision journey on it — and the machine beside a 3D
printer is as often a Windows PC as anything else.

It is also the node the rest of the Windows work stands on. A Windows service and three install
routes written over crates that still take a POSIX signal and tools that still ask for
`os.uname` would each rediscover the same defects separately; fixing them once, under the tiers
that catch them, is what lets the two nodes after this one be about what they are named after.

## Acceptance criteria

- The `platform-exclusions` block names `windows-x86_64` and `windows-aarch64` only for the
  install-route, registry-proof and artifact-route jobs and for the service, each entry's reason
  naming the node of this plan that removes it; every other platform-dependent cell of theirs is
  carried by the job that owns it.
- Both Windows entries still answer `install path: no` on this node's finished tree, so nothing
  here claims an end-user install path that the two nodes after this one have not yet delivered.
- The gate job's matrix carries both Windows platforms, the exclusions block names neither of
  them for the gate, and the repository checks report no finding over the finished tree, which is
  what says the list, the exclusions and every committed matrix agree.
- Every defect the bring-up record attributes to a Windows cell is answered in the finished tree
  — fixed, with the test that proves it, or carried with a stated reason that is not that the
  work was left undone.
- The worker's draft is published carrying the evidence: its description links the workflow run
  for each Windows cell this node brings up — the gate, the printer integration tier and the
  three artifact-client jobs, on both Windows platforms — and says what each run reported,
  including where one is still red. Whether the merge path then passes those cells is the merge
  path's to say and is not this node's bar; a cell reported honestly as red satisfies this
  criterion where a missing link does not.
- The printer integration tier is covered on both Windows platforms, and the three things that
  asks are separate.
  **Committed coverage:** the integration job's matrix carries `windows-x86_64` and
  `windows-aarch64`, `platform-exclusions` and `virtual-printer-exclusions` name neither of them
  on the finished tree, and the job's committed steps on each cell are this repository's own
  bring-up, tier and bring-down recipes against a real OctoPrint and its real bundled virtual
  printer — no stand-in, no step that cannot fail, no condition that skips the cell — with the
  committed octoprint-env tests that cover the Windows answers wired into those cells and neither
  skipped nor allowed to fail there.
  **Dispatch-controlled proof:** every octoprint-env change this node makes is exercised on the
  platform this dispatch runs on, by tests that drive the committed script itself and reach its
  Windows branch through a fixture rather than through an internal helper standing in for the
  script, and every Linux test this change touches passes there.
  **The merge path's verdict:** whether those Windows cells pass is the merge path's to say and
  is outside this node's bar.
- The three artifact-client jobs are covered on both Windows platforms, and the three things
  that asks are separate.
  **Committed coverage:** each of the three jobs' matrices carries `windows-x86_64` and
  `windows-aarch64`, `platform-exclusions` names neither of them for any of the three, and the
  committed steps of each of those cells build the Rust, Python or Node client from the committed
  tree, install it into an environment holding no copy of these sources, and drive it against a
  real supervisor — no stand-in, no step that cannot fail, no condition that skips the cell.
  **Dispatch-controlled proof:** the client journeys those steps invoke are committed and
  executable, each drives the built client rather than an internal helper standing in for it, and
  each passes on the platform this dispatch runs on.
  **The merge path's verdict:** whether the Windows cells pass is the merge path's to say and is
  outside this node's bar.
- The server's shutdown is taken through the mechanism its own platform stops a process by,
  behind a platform condition so no other target's build carries it, and the proof of that is
  separate from the verdict on it.
  **Committed coverage:** a committed test drives a real server process to the Windows shutdown
  and asserts it shut down cleanly rather than was killed; it is wired into both Windows gate
  cells and is neither skipped nor allowed to fail there.
  **Dispatch-controlled proof:** the same test drives a real server process to the Unix shutdown
  and passes on the platform this dispatch runs on, and the graceful sequence both mechanisms
  enter is the one sequence rather than two — exercised there rather than only compiled for the
  Windows target.
  **The merge path's verdict:** what the Windows cells report about it is the merge path's to say
  and is outside this node's bar.
- The program's configuration path and its state and home directories come from one module that
  gives the per-platform answer; the Linux answers are the ones this tree gives today, byte for
  byte, and a test reads each platform's answer through that module.
- Every place that named the built program without an executable suffix, looked for a virtual
  environment's programs under `bin`, or reached for `os.uname`, `os.geteuid`, `os.kill`,
  `os.getpgid` or `os.killpg` either works on Windows or is behind a platform condition with a
  Windows answer beside it.
  **Committed coverage:** each such site is covered by a committed test that the Windows gate
  cells carry, neither skipped nor allowed to fail there.
  **Dispatch-controlled proof:** each site's Windows answer is exercised on the platform this
  dispatch runs on by a test that reaches that branch through a fixture — a Windows path answer,
  a `Scripts` program directory, an `.exe` suffix — and asserts the answer the Windows branch
  gives, so no site is proven only by compiling; and the Linux answer at each site is unchanged
  and still proven there.
  **The merge path's verdict:** what the Windows cells report about those tests is the merge
  path's to say and is outside this node's bar.
- Every portability defect this node fixes carries a test that drives the real thing that broke
  — the compiled program as a subprocess, the committed script, or the recipe a user runs —
  rather than an internal helper standing in for it, and no fix here is evidenced by a change
  that only compiles for the target.
  **Committed coverage:** that test is wired into the cell for the platform the defect was on and
  is neither skipped nor allowed to fail there.
  **Dispatch-controlled proof:** where the fix's own answer can be reached on the platform this
  dispatch runs on — a path, a device or program name, an executable suffix, a directory, a
  platform-conditional branch of a Python module or of a committed script — the test reaches it
  there through a fixture and passes. Where it cannot, because the code is behind a compile-time
  condition that keeps it out of this platform's build or because it needs a native facility
  this platform does not have, what stands here instead is the platform-independent half the two
  branches share, driven for real on this platform rather than asserted about: the graceful
  shutdown sequence both stop mechanisms enter, the descriptor a path answer is read from, the
  decision a conditional makes before it reaches the facility. Every defect falls on one side or
  the other and the worker's report says which, so a fix proven only by compiling is visible as
  one rather than folded into the merge path's verdict.
  **The merge path's verdict:** what the Windows cells report about any of those tests is the
  merge path's to say and is outside this node's bar.
- No Linux guarantee weakens: both Linux platforms are still carried by every job that carried
  them, `linux-aarch64` is still in the list, the real-printer smoke test still refuses to run
  on an unmet precondition — proven on the platform this dispatch runs on — and no check moves
  from required to optional.
- The required-checks record names every status context the committed workflows now report for a
  required job — the gate context **and** the printer integration context of both Windows
  platforms included, because the integration tier is a required check on every platform — and
  no context it named before is missing.
...
- The first attempt ended when its provider refused it for quota, on no judgment of its work; that work is what this dispatch continues, and your worktree opens on it: the Windows cells in the gate, integration and artifact-client matrices, the LF checkout attributes, the per-platform configuration/state/home module, the console-control shutdown, the OctoPrint environment on Windows, the Windows-target build of every crate, the tracer probe, and the draft pull request #57 (https://github.com/nickderobertis/printobserver/pull/57) that publishes them. The finished tree carries every capability that work delivered, and no second change request for this work exists.
- Every platform-exclusions reason that names the node removing it names it by node id: windows-service for the service-registration cells, windows-install-routes for the release artifacts job's Windows cells and for the three install path: no reasons.
- The command-line journeys tier's assertion that no process of the invocation's tree connected to any endpoint but the configured supervisor is kept on Windows, and the three things that asks are separate.
  Committed coverage: the tracer is selected by target, the Linux observation is the one the base carries — strace over the whole tree, every connect recorded — with nothing it saw before left unseen, traced keeps its signature and every caller, only a Windows backend is added because the macOS node adds the macOS one under the same selection in parallel, and the Windows backend's observation is made on the invocation's own process tree and records each connection at the moment a process of the tree opens it, to any address, whether or not anything listens there — never attribution at endpoints the tier itself owns and never a sample of a connection table at an instant; the tracer's own documentation states per platform what its observation cannot see; and where a platform's hosted runner offers no such observation the tier refuses there with the reason, as the module already does where its tracer is absent, rather than reporting green on a narrower observation.
  Dispatch-controlled proof: the tier and its tainting proof pass on the platform this dispatch runs on through the unchanged Linux path, and the Windows backend's reading of its platform's record into the set of connected endpoints is exercised on this platform by a test that hands it a captured record through a fixture and reads the endpoints back.
  The merge path's verdict: what the Windows cells report about the tier is the merge path's to say and is outside this node's bar.
- The per-platform configuration, state and home module gives Windows a configuration file at C:\ProgramData\printobserver\config.toml, a state directory at C:\ProgramData\printobserver\state and a home at C:\Program Files\printobserver, gives macOS an explicit arm whose answer is byte-for-byte today's Linux answer and is documented in the module as provisional and owned by macos-first-class, and gives Linux today's answer byte for byte.
- The release-artifacts tests that prove an end-user install route (the wheel-tag, per-platform npm package and script-route tarball tests, the checksum-per-platform test, the registry proofs and the route-installs-a-program-that-runs journeys) skip on a platform exactly when the platform table's install-path answer for it is no, read from that table and never from a platform name, with a reason naming windows-install-routes as the node that removes the skip; a test that is not a route proof, the supervisor-world tests in test_taking.py among them, is ported rather than skipped; and the description states which tests skip on the Windows cells and why.
...
- The draft's description is true of the tree as it finally stands: it links the workflow run for each Windows cell this node brings up — the gate, the printer integration tier and the three artifact-client jobs, on both Windows platforms — and says what each reported, red included.

## Additional info

**The change request may be published early**, as a draft you hold, and you will need it: the
only place these tiers run is a GitHub runner, and the pull-request trigger is what starts them.
Commit, publish the draft, read the Windows cells back, fix, push onto the same draft. Never
mark it ready — the lifecycle does that after you settle.

**Read the bring-up record first.** The node you depend on ran this repository's deterministic
tiers on all four new runners and quoted what each one said. It is the list this node is written
against, and starting from it rather than from a cold run is most of what makes this node
finishable.

**This is the largest node in the plan and it has a raised turn budget.** Work in committed
pieces: a tier at a time, each commit carrying the fix and the test that proves it, rather than
one commit at the end. What is uncommitted when your last turn ends is what nothing recovers.

**The contract node's platform table is frozen** — identifiers, runners and Rust targets come
from it. And nothing in this plan admits an exclusion for a Windows gate, integration or
artifact-client cell on the finished tree: if one of them turns out not to be bringable up at
all on `windows-11-arm` — a toolchain or an interpreter that image genuinely cannot carry is the
thing to watch — that is a question for your manager now, not an entry you write and a reason
you invent.

**Two of this repository's own rules will bite if you forget them.** A suppression is permitted
only where the rule is genuinely misapplied at that site and only with an entry in
`suppressions.toml` naming the rule, the file, the site and a reason, added in the same change —
and a platform-specific branch is a place a reviewer will look for one. And the allowlist is
derived from what the recipes and graph targets invoke and must name exactly that set, so a
recipe that starts calling a new program takes its allowlist entry in the same change.
...
{"onepipeline.branch":"onevcs/s-02aab8fbdadf","onepipeline.execution_checkout":"nickderobertis__printobserver-isolated","onepipeline.id":"windows-core-2","onepipeline.persona":"engineer","onepipeline.resume":{"branch":"onevcs/s-02aab8fbdadf"},"onetaskgraph.item_kind":"task"}
-->
