...
**fix(sdk): ship the py.typed marker in the onetaskgraph-sdk wheel**

*Project description:*
## What

Make the Python SDK at `sdks/python` (package `onetaskgraph_sdk`, `hatchling`,
`[tool.hatch.build.targets.wheel] packages = ["src/onetaskgraph_sdk"]`) a typed package
under the invariant below, and hold its wheel to it in this repository's own gate.

- **The marker.** An empty `src/onetaskgraph_sdk/py.typed` beside `__init__.py` —
  outside `src/onetaskgraph_sdk/_generated/`, which `sdks/python/generate.py` owns and
  rewrites. The generator neither emits nor removes it.
- **The classifier.** `Typing :: Typed` in `sdks/python/pyproject.toml`'s
  `[project].classifiers` (the manifest declares none today; add the list).
- **The wheel-held check.** A test in `sdks/python/tests` builds the wheel with
  `uv build` into a temporary directory — `release.yml`'s `build-distributions` job
  builds it with `uv build sdks/python`, and `tests/test_artifact.py` already builds one
  with `uv build --wheel` — and asserts that `onetaskgraph_sdk/py.typed` is a member of
  the wheel and that the wheel's `METADATA` carries `Classifier: Typing :: Typed`. Read
  the wheel as the zip it is; do not assert on the source tree.
- **Regeneration keeps it.** A test runs the real generator (`generate.py` against the
  built binary, the way `test_artifact.py` and the `generate-check` target already
  reach it) and shows `py.typed` still present afterwards.

Nothing about the SDK's runtime, its models or its client changes. Do not touch the
TypeScript SDK, the CLI wheel (`pyproject.toml` at the root is a maturin `bin` wrapper
with no importable module, exempt by the invariant) or the release workflow's publishing
jobs.
...
The user found `# type: ignore[import-untyped]  # Package omits py.typed.` over
ai-orchestrator's `onetaskgraph_sdk` import — the SDK that repository had just adopted
so that its plan-store code would be typed — and asked "why do i see type ignore on
python sdk import? is this a problem across all our python sdks?" Measured against the
latest wheel on PyPI, `onetaskgraph-sdk` 0.2.35 ships no `py.typed`, so mypy reads every
name it exports (`Client`, `CopyReport`, the metadata-set calls) as `Any` and checks
nothing ai-orchestrator does with them. The user's direction: "i want to make sure we
handle this properly in all of them", and the fix that counts is the one the built wheel
is held to, not a file in the tree — which is why the check here reads the wheel.

The release this change cuts is what ai-orchestrator's `aio-adopt-typed-sdk` node waits
for before it removes the ignores.

## Acceptance criteria

- The wheel `uv build sdks/python` produces contains `onetaskgraph_sdk/py.typed`, and its
  `METADATA` carries `Classifier: Typing :: Typed`.
- A test under `sdks/python/tests`, reached by the `sdk-python` project's `test` target,
  builds that wheel into a temporary directory and asserts both facts on the wheel's
  contents, and it fails when either is absent.
- The marker lives outside `src/onetaskgraph_sdk/_generated/`, and a test that runs the
  real generator shows `py.typed` present in the package afterwards.
- `generate-check` still passes: the generated surface is unchanged.
- The checks that exercise this change are green over the finished tree: the
  `sdk-python` project's `format-check`, `lint`, `typecheck`, `generate-check`, `test`
  and `coverage` targets, and the judged lint over this change's diff.
...
`sdks/AGENTS.md` and the root `AGENTS.md` are the repository's own rules; the SDK's
tests run through `uv run --frozen` in `sdks/python` and the `test_artifact.py` journey
already does the `cargo build` the generator needs. Iterate with the one Nx target that
reports something (`./scripts/nx.sh run sdk-python:<target>`), and take the repository's
diff-scoped judged lint for the last check. Commit each piece as it works. The cheapest
evidence that the wheel test can fail is to induce the absence once — remove the marker,
run the test, quote the failure, restore — and say in your report that the quoted run is
the induced one.

The title is a `fix`: `release-plz.toml`'s `release_commits` cuts a release only for
`feat`, `fix` and `perf`, and the release is what carries this marker to PyPI — the SDK
wheel publishes from the same `release.yml` run as the CLI wheel. Do not bump any
version or edit any manifest version by hand; the release pipeline writes them.
...
{"onepipeline.execution_checkout":"nickderobertis__onetaskgraph","onepipeline.id":"otg-sdk-py-typed","onepipeline.persona":"engineer","onetaskgraph.item_kind":"task"}
-->
