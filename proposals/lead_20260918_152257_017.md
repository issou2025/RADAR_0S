...
**fix(sdk): write generated Python modules as UTF-8 on every platform**

*Project description:*
## What

Resolve https://github.com/nickderobertis/onetaskgraph/issues/1989, accepted at severity
`high`: `sdks/python/generate.py` invokes `datamodel-codegen` without an output encoding
and then reads the generated module back as UTF-8, so a Windows regeneration writes the
non-ASCII schema descriptions in the platform code page and the Windows gate reports the
21 generated modules that carry one as stale — or fails to decode them.

Pass `--encoding utf-8` to every `datamodel-codegen` invocation the generator makes, and
add a generation-check case whose schema description carries a non-ASCII character, run
under a non-UTF-8 default encoding the way Windows would (a subprocess with
`PYTHONUTF8=0` and a legacy code page, or the closest a Linux host can stage — say which in
your report), proving the module written and the module compared are byte-identical. The
Windows leg itself cannot be run here; the staged case is what stands for it.

## Why

The user accepted this at severity `high` because the SDK's generation check is part of
the gate on the required Windows lane: with the platform code page in play it reports
every module with a non-ASCII description as drifted, so a valid change is refused on
Windows and regeneration is only safe on Linux — a lane that exists to prove the SDK
cross-platform proving instead that it is not.

## Acceptance criteria

- Every generated Python module is written in UTF-8 whatever the platform's default
  encoding, and the generation check compares byte-identical output on a host whose
  default encoding is not UTF-8.
- A schema description carrying a non-ASCII character round-trips through generation and
  the check without drift or a decode failure.
- The branch's history carries a closing reference naming
  https://github.com/nickderobertis/onetaskgraph/issues/1989, so the change that carries the work closes it.
- The behaviour is proven by a test driving the real generator under a staged non-UTF-8
  default encoding rather than by inspection, and no test substitutes a double for the
  layer under test.
- The tests and checks that cover the SDK generator are green over the finished tree, and
  so is the judged lint over this change's diff.
- Every claim this dispatch makes about the finished work is true of the tree as it finally
  stands.

## Additional info

Iterate with this repository's diff-scoped judged lint and the Python SDK project's
`generate-check` and test targets (`sdks/python/project.json` names them) rather than the
whole gate; the required checks run downstream on the change this branch becomes.

This repository's releases are cut from conventional commits; `fix` is the type. Nothing
on this host adopts this node's release directly.
...
{"onepipeline.execution_checkout":"nickderobertis__onetaskgraph","onepipeline.id":"otg-python-generator","onepipeline.persona":"engineer","onetaskgraph.delivers":["followups:I_kwDOUBBgxs8AAAABR5X_Fg"],"onetaskgraph.item_kind":"task"}
-->
