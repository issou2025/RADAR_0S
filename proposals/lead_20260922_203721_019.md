...
**feat(api): expose every command as a typed library operation, starting with registered identities**

*Project description:*
## What

https://github.com/nickderobertis/onevcs/issues/174. `crates/onevcs/src/lib.rs:82` keeps `mod store;` private, so the
migration-aware loader (`store::load()`, `store.rs:170`) is unreachable and a library
consumer that wants the registered identities has to spawn `onevcs repos` and parse its
display output — which onepipeline does today (`src/maintenance.rs:353-375`).
`onevcs::run` is the binary's dispatcher: it returns an exit code and writes display output,
not a value.

Make the command layer a thin formatter over public typed operations:

- Add **`pub fn registered_identities() -> onevcs::Result<Vec<String>>`**, backed by the
  migration-aware store loader, returning each registered identity's normalized origin
  (`host/owner/name`) exactly as `onevcs repos` prints it on its unindented lines, in the
  order it prints them. **This signature is fixed for the run**: this plan's
  `op-relink-onevcs` replaces its subprocess with it.
- Inventory every `cli::Command` variant (twenty-one top-level, about thirty-two leaves).
  About twenty already have a typed public entry point; about twelve have none —
  `register`, `repos`, `resolve` (partly through `Vcs::resolve_identity`), `publish-branch`,
  `recover`, `recoverable` (partly), `import`, `integrate`, `sync`, `sweep`, `events` (only
  `EventStream` is public), `artifact cat`, `rules check`. Expose each remaining private
  operation as a typed value-returning public function and have its command handler in
  `app.rs` call that function and only format.
- Add a **parity test** that fails whenever a `cli::Command` variant has no corresponding
  typed library operation.
- Keep `onevcs::run` as the exit-code-and-rendering adapter over those operations; every
  command's output and exit status stay what they are.

State the new public surface in `docs/contract.md` and hold it in
`crates/onevcs/tests/contract.rs` or `tests/e2e/library.rs`.

## Why

Every library consumer — onepipeline first — pays a process spawn and couples itself to
CLI prose to learn something the library already knows, and one prose change in a
display line breaks it silently. The parity test is what keeps the next command from
reopening the gap.

## Acceptance criteria

- `onevcs::registered_identities()` exists with the signature above and returns exactly
  the identities `onevcs repos` lists, in its order, over a migrated and an unmigrated
  registry — proven through the library in `tests/e2e/library.rs` against the CLI's own
  output.
- Every `cli::Command` variant has a public typed value-returning library operation, its
  command handler calls that operation and only formats, and a parity test fails when a
  variant lacks one.
- Every command's output and exit status are unchanged, proven by the existing e2e suite
  over the commands whose handlers moved.
- `docs/contract.md` states the new public operations and the contract tests hold them.
- The tests covering what this change touches — `tests/e2e/library.rs`,
  `tests/e2e/registry.rs`, `tests/contract.rs`, the new parity test, and the e2e modules of
  every command whose handler changed — are green over the finished tree, and so is the
  judged lint over this change's diff.
- The branch carries GitHub closing metadata associating it with issue #174 of this repository and with no other issue; whether the issue actually closes is decided downstream when the change lands, and is not this dispatch's bar.
- Every claim this dispatch makes about the finished work is true of the tree as it
  finally stands.

## Additional info

onevcs has no local hooks; its merge path is the CI in `.github/workflows/ci.yml`. `just test-one <name>` and `just test-quick` are the narrow runners. `docs/contract.md` states the public surface and `crates/onevcs/tests/contract.rs` holds it: a change to a public type is stated in both. This node starts from `ov-maintenance-types`' landing (which sits on `ov-workspace-scan`'s), so the workspace-listing errors and the new slot and command types are already in the tree; wrap existing internals rather than rewriting them. `ov-unresolvable-reference` runs beside this node in the error and release-status seams; leave those to it. `registered_identities()` first, committed on its own, then the inventory. Put `Closes #174` in the body of the commit that resolves each ticket, so the landed change closes it; the issues live in this repository. onevcs releases from conventional commits through release-plz, and this plan's onepipeline relink waits for the release carrying this node; this adds public API without breaking any, so `feat` is the type.
...
{"onepipeline.execution_checkout":"nickderobertis__onevcs","onepipeline.id":"ov-typed-operations","onepipeline.max_turns":32,"onepipeline.node":"ov-typed-operations","onepipeline.persona":"engineer","onetaskgraph.delivers":["followups:I_kwDOTx3GGM8AAAABSPGpmQ"],"onetaskgraph.item_kind":"task"}
-->
