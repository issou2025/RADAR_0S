...
**test(oneharness): read the checked-in assessment schema only under its lock**

*Project description:*
## What

Make every test in `crates/printobserver-oneharness/tests/supervision/` that reads the
checked-in `schemas/printobserver-supervisor-api/AgentAssessment.json` do so under the shared
lock the module already provides, and make a reader that does not hold it impossible to add
back silently.

### The cause

`assessment_schema.rs::the_answer_is_constrained_by_the_generated_assessment_schema` **rewrites
the checked-in artifact on disk** (`HeldArtifact::replace`, a plain `fs::write`, i.e. truncate
then write) under the exclusive `schema_lock()`, drives a turn, and puts it back. The support
module's documentation says every journey that reads the artifact takes `schema_read_lock()`
so "neither ever sees the other's half-done tree" — but the readers that only *build a
configuration* from it were never counted as readers. `support.rs::config` → `schema(path)` →
`AssessmentSchema::at(path)` parses the file at construction, and:

- `failures.rs::a_ledger_that_does_not_agree_with_itself_is_refused` and
  `failures.rs::a_recorded_turn_naming_no_session_is_refused` build one through
  `failures.rs::answering(&fixture)` and take no lock. Each has failed a Linux gate with
  `a journey names a schema to constrain an answer: …/AgentAssessment.json does not constrain
  an answer: EOF while parsing a value at line 1 column 0` — the file empty between the
  writer's truncate and its write: **`main`** runs 34417113934 (8b80d7b, through
  `test_gate_strictness`'s nested gate) and 34427684065 (140d8c3), and PR #57's run
  35214921093 (job 105181241224).
- The same shape, not yet seen failing, is in every other test that reaches
  `generated_assessment_schema()` without a lock. A source scan finds at least:
  `failures.rs` — `a_configuration_naming_files_that_are_not_there_is_refused`,
  `a_template_that_does_not_declare_each_slot_once_is_refused`,
  `a_ledger_written_under_another_shape_is_refused`, `a_ledger_of_another_print_is_refused`;
  `configuration.rs::a_schema_that_constrains_no_answer_is_refused_where_it_is_named`; and the
  helpers `closing.rs::answering`, `failures.rs::answering`, `reports.rs::one_real_report`,
  whose callers may or may not hold it. Audit the whole directory rather than trusting this
  list.

nextest runs every test of the binary in its own process, in parallel, so the writer and
any unlocked reader race on every run; `nx run-many` adds the `printobserver-supervisor-api`
schemas suite (which takes the same lock) beside them.

### The fix

Every read of the artifact happens while the reading test holds `schema_read_lock()` (the
writer holds `schema_lock()` and is the one exception). Then make the rule structural rather
than remembered: the cleanest form is for `support::config` (or `support::schema`) to take a
reference to the held lock — both lock functions already return the same `File` — so a test
cannot build a configuration from the checked-in artifact without naming a lock it holds,
and the writer passes its exclusive one. A test over the suite's own sources that fails on a
`generated_assessment_schema()` reader with no lock in scope is the acceptable alternative if
the signature change is judged too wide. `repo-policy.toml`'s `schema_lock_holders` names the
two files that lock; it does not need to change unless you move the lock functions, and it
should not.

Both live branches leave this crate's tests alone, so the change is free of rebase concerns.

## Why

The user's words: "let's make sure to fix flaky tests too — they're super expensive when
the gate is so long." This race has turned `main` red twice in a week on trees that were
right, once through the nested gate that `test_gate_strictness` runs — where it costs a
whole e2e tier, not one test — and the documentation already says what the rule is; the
suite just does not keep it.

## Acceptance criteria

- Every test in `crates/printobserver-oneharness/tests/supervision/` that reads the checked-in
  assessment artifact — directly, or by building a configuration that names it — holds the
  shared lock for the whole of that read; the assessment-schema journey holds the exclusive
  lock as it does today; no reader can be added that names the artifact without holding a
  lock, because either the support function that reads it requires a held lock by its
  signature, or a test over the suite's sources fails on such a reader (state which).
- The two tests named above, and every other reader the audit found, are listed in the
  report with what changed for each.
- The writer still proves what it proves: the artifact is changed on disk, the port refuses
  what it accepted before, and the artifact is restored byte for byte.
- No test's coverage is removed or narrowed, no test is marked ignored, and no assertion is wrapped in a retry that could mask a failure; `repo-policy.toml` is unchanged unless the lock
  functions moved, and they did not.
- The readers and the writer no longer race under the cross-process contention the gate
  applies — the writer journey rewriting the artifact under its exclusive lock while every
  other test of the binary runs in its own process beside it, and the
  `printobserver-supervisor-api` schemas suite reading the same tree at the same time: under
  that contention the finished tree's supervision suite is shown passing repeatedly on this
  Linux host, with the number of runs and the commit they ran on stated and no run among them
  failing; and the report shows the vulnerable interleaving was exercised, by citing the
  unlocked readers of the tree before the change failing with `EOF while parsing a value`
  under that same contention — or, where this host will not produce it, the three hosted
  logs named below as that prior-state evidence.
- The tests over the code this node touches are green over the finished tree (the
  `printobserver-oneharness` suite and the `printobserver-supervisor-api` `schemas` test, as
  the gate runs them), the crate's linter over its test targets with the `test-responder`
  feature reports no warning, the formatter's check is clean, and the judged lint over
  this change's own diff leaves no finding unanswered — each named with what it reported.
...
**Reading order.** `tests/supervision/support.rs` (lines 60–130: the artifact path, the two
lock functions and their documentation, `schema`, `config`), `assessment_schema.rs` whole
(`HeldArtifact`, `drive`), `failures.rs` (`answering`, the two failing tests at ~402 and ~473,
and the tests that already take `let _schemas = schema_read_lock();`), then every other file
in the directory. `crates/printobserver-supervisor-api/tests/schemas.rs::schema_lock` is the
other holder; `repo-policy.toml` `[supervisor] schema_lock_holders` names both files.

**Seeing the race here.** Thirty-plus runs of `cargo nextest run -p printobserver-oneharness
--features test-responder` before the change, with the supervisor-api schemas test looping
beside it, is a fair reproduction on this host; a reproduction is welcome evidence but is not
required — three hosted logs already name it (`gh api --allow-escape-sequences
repos/nickderobertis/printobserver/actions/jobs/105181241224/logs`, and jobs 102684328700,
102716290930 for the two `main` runs, where the panic sits inside the nested gate's output).

**Proving under load.** The pairing — the crate's suite looping while the
`printobserver-supervisor-api` schemas suite loops beside it — is what exercises the
reader/writer interleaving directly; the counted runs beside the full tier add the gate's
own load on top. If the race will not show itself here before the change even under the
loop, say so and cite the three hosted logs as the prior-state evidence instead.
...
**Iterate with** the crate's suite, `cargo clippy -p printobserver-oneharness --all-targets
--features test-responder --locked -- -D warnings`, `cargo fmt -p printobserver-oneharness --
--check`, and the judged lint over your diff. The whole gate runs downstream on the merge
path; nothing here asks you to have run it first.

**A shared interface is never changed unilaterally.** The lock file's name and location
(`target/printobserver-schemas.lock`, from `repo-policy.toml`) and the exclusive/shared split
between writer and readers are fixed for this run; a departure you want is a proposal to your
manager over `$ORCHESTRATOR_ASK_MANAGER`, and you keep building against the agreed surface
while you wait.
...
{"onepipeline.execution_checkout":"nickderobertis__printobserver-isolated","onepipeline.id":"assessment-schema-read-lock","onepipeline.persona":"engineer","onetaskgraph.item_kind":"task","orchestrator.plan-review":{"by":"review-plan","key":"bed7e8304b74bbb5dbeb3763d7576714669884d2d021188424820154ca84afc8","reviewed_at":"2026-09-17T17:53:02.290715+00:00"}}
-->
