...
**fix(taskgraph): retire the revision pin for a released onetaskgraph floor**
...
Root cause 6 of `nickderobertis/ai-orchestrator` issue #1004: a revision pin that was a floor
has outlived the ambiguity it existed for. Read the issue's entry for the evidence.

`src/taskgraph.rs:59` `CHECKED_MINIMUM = 0.1.0` and `:81` `FIRST_REVISION` exist because the
reserved metadata map landed between onetaskgraph 0.1.0 and the next release, and
`_ensure-onetaskgraph` (`justfile:76-95`, reading `onetaskgraph-rev` at `:39`) installs that
revision — so CI runs a onetaskgraph reporting 0.1.0 while every host runs a 0.2.x release. The
revision is four commits before onetaskgraph `v0.2.0`, so every release from `0.2.0` carries the
map and `>= 0.2.0` separates the two products exactly.

- Raise `CHECKED_MINIMUM` to `0.2.0` and delete `FIRST_REVISION`.
- Make `_ensure-onetaskgraph` install a released onetaskgraph version, stated in one place that
  the install and a test both read (so the two cannot drift apart). Choose the release the
  engine's own store tests pass against, and record why beside the stated version in the voice
  of the comment that explains the revision today.
- Resolve divergence entry 44 ("The minimum `onetaskgraph` this build needs is not a released
  version") in place, as its own text proposes. Add no new numbered entry: sibling nodes of this
  plan edit that file in parallel.

## Why

The operator is closing every root cause of issue #1004 so that none of them has to be
repaired by hand again. In the run that closed #855 a test passed locally against onetaskgraph
0.2.28 and would have failed in CI against the revision reporting 0.1.0. The operator wants CI
and every host to run released products, so a green CI says something about what a host runs.

## Acceptance criteria

- `CHECKED_MINIMUM` is `0.2.0` and `FIRST_REVISION` no longer exists.
- `_ensure-onetaskgraph` installs a released onetaskgraph version named in one place, and a test
  fails if that installed version falls below `CHECKED_MINIMUM` or if the recipe stops reading
  the one place it is named. The engine's store operations succeed end to end against the released
  onetaskgraph executable that `_ensure-onetaskgraph` itself installed, with the recipe driven for
  real rather than its version read: the engine's store journeys pass against that executable, and
  it reports the version named in that one place.
- The engine's refusal of a onetaskgraph below the minimum stays proven end to end.
- Divergence entry 44 is marked resolved and says how.
- The tests covering the changed Rust module and the justfile recipe pass over the finished tree,
  and the judged lint over this node's diff leaves no finding unanswered.
...
- Read first: `AGENTS.md`, `tests/AGENTS.md`, `src/taskgraph.rs`, `justfile` (`:36-42` and
  `:76-95`), `docs/contract-divergences.md` entry 44 (near line 1940), and the way a resolved
  entry is marked elsewhere in that file.
- Tests nearest these changes: `tests/e2e/store.rs:2848-2890` (the minimum refusal), and
  `src/taskgraph.rs`'s unit tests near `:1153-1242`, including
  `the_revision_the_checks_install_is_read_out_of_this_file`, which this change replaces. The
  judged tier: `just lint-llm-diff <base>`. The merge path runs the full gate.
- Siblings in this repository run in parallel and touch other files: `onepipeline-monitor-cursor`
  (`src/cli.rs`, `src/views.rs`, `src/watch.rs`, divergence entry 58),
  `onepipeline-writeback-divergence` (divergence entry 50), `onepipeline-teardown-diagnosis`
  (`npm/test/llmlint-cache.test.mjs`) and `onepipeline-link-fixes` (`Cargo.toml`, `Cargo.lock`).
- Nothing here changes a surface another node of this plan consumes; no node of this plan waits
  on the release carrying it, because what it fixes is in force for CI by merging and this host
  already runs a onetaskgraph above the new minimum.
...
{"onepipeline.id":"onepipeline-retire-revision-pin","onepipeline.max_turns":90,"onepipeline.persona":"engineer","onetaskgraph.item_kind":"task","orchestrator.plan-review":{"by":"review-plan","key":"ba401d1cfc82562b18e7c4441c26b3d6af5863dce4edfa3a75d7f0ac1a419ece","reviewed_at":"2026-09-13T17:30:57.670877+00:00"}}
-->
