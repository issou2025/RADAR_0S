...
**feat(deps): link the oneharness-core that writes a per-run pointer line and the onejudge that links it**

*Project description:*
## What

Move `oneagentgraph`'s `oneharness-core` and `onejudge` requirements together to the
releases `oh-history-pointer` and `oj-relink-core` produced, and nothing beyond what those
moves force.

This crate runs a `kind: oneharness` member's turn as an in-process
`oneharness_core::io::run::run_supervised` call (`src/harness.rs`) and drives a `kind:
onejudge` member through the `onejudge` library, and today requires `oneharness-core
0.14.0` and `onejudge 0.13.1`. The two releases this task's dependency references name —
versions, never placeholders, never ones you went looking for — are edited **together**
as requirements in `Cargo.toml` (pre-1.0, `^0.14` admits no 0.17.x; `oneagentgraph`
requires the `onejudge` that links the same core, or the graph carries two turn engines,
which `tests/inventory.rs` refuses), with the lock refreshed to resolve exactly one copy of
each. Absorb what the core's breaking minors between 0.14 and the release force here
(0.15.0's `--format`, 0.16.0's fallback and text defaults, the pointer release's own
break — a `RunRequest` field, a default that moved) and what the `onejudge` release
forces, reading each CHANGELOG entry; take what compiles and behaves as before, and change
nothing this crate does not have to. `src/harness.rs`'s `RunRequest` literal spreads
`..RunRequest::default()`, so the new field is taken by default: **do not set
`history_pointer_file` here** — the environment is how a consumer names the pointer file
and the core reads it there, which is the property the engine relies on.

Two additions beyond the relink, both proofs of what the relink buys rather than
behaviour of this crate's own. First, a floor test beside the crate's linked-library
assertions (`tests/inventory.rs`, or the module that holds the core floors) holding that
the linked core exposes `oneharness_core::io::history::read_pointers` and
`oneharness_core::domain::history::HistoryPointer`, so the requirement cannot slide back.
Second, a journey in `tests/e2e/` (`session.rs`'s pattern, over `support.rs`'s
`single_sided_graph`) that runs a `kind: oneharness` member with `ONEHARNESS_HISTORY=1`
and `ONEHARNESS_HISTORY_POINTER_FILE=<a file under the journey's own directory>` in the
graph's environment and nothing else configured, and reads that file back through
`read_pointers`: one line, whose `history_dir` / `history_project` / `history_session`
resolve through `io::history::find_session_path` to the session the member's own
`oneharness-session` event names and whose `history_id` is that event's — the property
`onepipeline` relies on for every in-process turn. No pointer-file behaviour is added to
this crate: the core writes the line from the environment it inherits.

## Why

The user asked that "general visibility into all agents launched ... work regardless of
the agent structure used in the repo (ai-orchestrator is just one way onepipeline can be
set up)". On that host the observer graph's monitor and the drafting graph's author are
exactly the members whose turns are this crate's in-process library call — no `oneharness`
binary is spawned for them — so the pointer line `oneharness-core` now writes at the start
of every harness run reaches those turns only through the core this crate links, which is
the one the engine links. The move is overdue on its own account too: core main sits two
breaking minors ahead of every consumer, and `onepipeline` PR #394 deferred the relink for
want of exactly this pair of moves.

## Acceptance criteria

- `Cargo.toml`'s `oneharness-core` and `onejudge` requirements name the released versions
  this task's dependency references give for `oh-history-pointer` and `oj-relink-core` —
  versions, never placeholders — with the manifest's comment block saying what each raise
  brings and why both moved together, and `Cargo.lock` resolves exactly one `oneharness-core` and
  one `onejudge` (`tests/inventory.rs` green).
- Beyond the manifest, the lock, the floor test and the pointer journey the next two
  criteria ask for and the prose the one after asks for, every change is one a breaking
  change of those releases forced, named as such in the completion report against the
  CHANGELOG entry that forced it; `src/harness.rs` sets no `history_pointer_file` of its
  own; and the existing journeys under `tests/e2e/` are green unmodified except where a
  forced change made an assertion false, each such edit named in the report.
- A journey drives a `kind: oneharness` member through the real graph with
  `ONEHARNESS_HISTORY=1` and `ONEHARNESS_HISTORY_POINTER_FILE` in its environment and
  nothing else configured, and the named file holds one line, read through
  `read_pointers`, whose `history_id` is the member's `oneharness-session` event's and
  whose `history_dir` / `history_project` / `history_session` resolve through
  `find_session_path` to the session file that event points at.
- A test holds the linked `oneharness-core` to expose `io::history::read_pointers` and
  `domain::history::HistoryPointer`, failing on a lock resolved below the release that
  carries them.
- `AGENTS.md`, `README.md` and the CHANGELOG's unreleased section record this raise where
  they record the core and onejudge floors today.
- These checks are green over the finished tree, run separately or together as suits the
  work: the crate's unit tests, `tests/inventory.rs`, the e2e suites under `tests/e2e/`
  with the new journey, rustfmt
  and clippy under the gate's denied warnings, the doc build, and the judged lint over
  this change's diff, reporting no finding left unanswered.
...
**A shared interface is never changed unilaterally.** The pointer line and the reader are
`oh-history-pointer`'s (Contract B in the project record); this node links them and adds
nothing to them. If a release cannot be linked as stated, put it to your manager over
`$ORCHESTRATOR_ASK_MANAGER` as a proposal and keep building against the agreed releases
while you wait.

**Pins and versions.** This task names no version, deliberately: the references block the
harness appends names the release and the landing commit for `oh-history-pointer` and for
`oj-relink-core`, and the note it sends when each release exists names the version. Do
not go and find or pin a commit yourself; both are published registry versions, never a
git rev.

**Where things are.** `Cargo.toml`'s dependency comment block (the numbered "raises" of
`onejudge` and the `oneharness-core` block beneath) is the register to write this in; the
last relink (`fix(deps): relink oneharness-core and onejudge for the overload fallback`,
#124) is the pattern. `src/harness.rs` is the library-path turn; `src/judge.rs` the
two-party member and where a `history_file` pointer is taken apart into
`history_dir`/`history_project`/`history_session` for the `oneharness-session` event —
the vocabulary the pointer line reuses, so nothing here changes. `just --list` names the
tiers (`just test`, the e2e recipes, `just lint`, `just fmt-check`, `just lint-llm-diff
<base>`); the pre-push gate is not asked of this dispatch.
...
{"onepipeline.adoption":"published","onepipeline.consumes":{"oh-history-pointer":"core","oj-relink-core":"crate"},"onepipeline.id":"oag-relink-core","onepipeline.max_turns":160,"onepipeline.node":"oag-relink-core","onepipeline.persona":"engineer","onetaskgraph.item_kind":"task"}
-->
