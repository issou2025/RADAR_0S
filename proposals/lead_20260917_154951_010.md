...
**fix(github-projects): write issue state and Status together**

*Project description:*
## What

Change the `github-projects` plugin's task-status projection so every write keeps the issue
state and the board's `Status` single-select consistent. Evolve the existing status target
model, its write ordering, documentation, fixture coverage, and shared real-GitHub journey
as one coherent change.

### Contract (fixed for this plan; the ai-orchestrator consumer is written against it)

The authoritative declaration and implementation live in `StatusMapping` and its resolved
target in `crates/onetaskgraph-github-projects/src/lib.rs`. Keep reads and writes deriving
from that one model; the plugin tests and shared live journey are the drift gate between
the resolved mapping, GraphQL mutations, and observed read-back.

- `done` writes the board option named by `status_mapping.done` **and** closes the issue as
  `COMPLETED`; `cancelled` writes the option named by `status_mapping.cancelled` **and**
  closes it as `NOT_PLANNED`.
- The shipped mappings are exactly `done: Done` and `cancelled: Cancelled`.
- Every open category (`backlog`, `todo`, `draft`, `queued`, `in-progress`, `unknown`) writes
  its mapped Status option and reopens the issue first when it is closed. A draft item has
  no issue state to reopen. Disabled categories retain the plugin's existing refusal.
- Resolve and validate the Status option before performing either half of a terminal write.
  If the mapped option is absent, refuse naming it exactly as the plugin already refuses any
  missing mapped option. Do not close the issue, do not change Status, and do not fall back
  to closure alone.
- Reads preserve the existing precedence: for a closed issue, `COMPLETED` reads `done` and
  `NOT_PLANNED` reads `cancelled` regardless of Status; the Status option may remain the
  displayed name. For an open issue, Status decides the category.
- A status write changes only the addressed item. It never scans, backfills, or mutates
  another board item. Hand moves are reconciled only when the store later writes that item.

Update every onetaskgraph document that describes terminal GitHub Projects projection to
state the two-part write and read precedence. Extend the shared journey driven by
`tests/live.rs` so the credentialed lane writes and reads back both `done` and `cancelled`,
observing both their board option and their exact issue close reason, then removes its own
artifacts as it does today. The loopback drive must exercise the identical journey.

Do not change the shared contract unilaterally. If implementation evidence suggests a
different surface, ask the manager to amend the plan and keep building against this one
until answered.

## Why

The user's follow-up board has closed cards still displaying `Todo`, `In Progress`, or
`Proposal`. Views can hide the mismatch, but a person opening a card still sees a Status
that contradicts the task state. A store write should leave both GitHub representations
saying the same thing, while direct issue closure by a person remains a decision reads
honor.

## Acceptance criteria

- Writing `done` to an addressed issue selects the configured Status option and closes it
  as completed; writing `cancelled` selects its configured option and closes it as not
  planned. Shipped configuration uses `Done` and `Cancelled` respectively.
- A mapped terminal option missing from the board refuses the write by that option's name
  before either Status or issue state changes; no terminal category falls back to closing
  without Status.
- Writing each supported open category to a closed issue reopens that issue and selects its
  mapped Status option; disabled-category and draft-item behavior remains well-defined and
  covered.
- Reads of contradictory data follow the agreed rule: close reason decides `done` versus
  `cancelled` for closed issues, while Status decides among open issues. Existing boards
  therefore keep their current read semantics.
- A write affects only the addressed item. Tests prove that unrelated fixture items receive
  no mutation, and no bulk migration or reconciliation path is added.
- Onetaskgraph's user-facing GitHub Projects documentation states the two-part terminal
  write, the reopen rule, read precedence, shipped option names, and missing-option refusal.
- The real-interface shared GitHub Projects journey exercised by the live lane writes and
  reads back both terminal categories, verifies the exact Status option and issue close
  reason for each, and leaves no residue; its loopback drive exercises the same journey.
- Focused deterministic tests for the github-projects crate and the repository's diff-scoped
  judged lint are green on the finished tree, with any live-lane credential absence handled
  only by the lane's existing required/skip contract.
- Every claim the dispatch makes about the finished work, its checks, and their evidence is
  true of the tree as it finally stands.

## Additional info

The repository's release automation publishes the `pypi` CLI wheel after this lifecycle
change lands; publication and release are not worker actions.
...
{"onepipeline.branch":"onevcs/s-b48d948d016d","onepipeline.execution_checkout":"nickderobertis__onetaskgraph","onepipeline.id":"otg-github-closed-status-2","onepipeline.persona":"engineer","onepipeline.resume":{"branch":"onevcs/s-b48d948d016d"},"onetaskgraph.item_kind":"task"}
-->
