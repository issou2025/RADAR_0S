...
Adopt this workstream's cargo build contract in `screencomp` (github.com/nickderobertis/screencomp; checkout `nickderobertis__screencomp` (registered)).

screencomp is one root crate (a `[package]` with no `[workspace]` table); no other `Cargo.toml` in the tree.

**What exists today:** no `.cargo/config.toml`; `Cargo.toml` carries `[profile.release]`, a `[profile.profiling]` inheriting `release` with `debug = true` for `samply`, and the `[lints]` tables — all untouched.
...
- `scripts/profile.sh`, `scripts/bench*.sh` and `scripts/dist*.sh` read `target/release/screencomp`, `target/profiling/screencomp` and `target/bench` at the clone root; `lefthook.yml` runs the hooks
- `just test-cov` is `cargo llvm-cov nextest --fail-under-lines`; `.github/workflows/ci.yml`, `docker.yml`, `release.yml`, `bench.yml`
...
- coverage: the coverage tier with its floor: `just test-cov`
...
{"onepipeline.id":"screencomp","onepipeline.persona":"engineer","onetaskgraph.item_kind":"task","orchestrator.plan-review":{"by":"planning-closeout","key":"4ae962d11b115f9f22bc5ae7545d3974d1fc43f57e0ba52348a2f9f45c81f4ac","reviewed_at":"2026-09-20T09:43:09.752676+00:00"}}
-->
