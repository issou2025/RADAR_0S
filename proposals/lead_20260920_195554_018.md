...
**feat(lifecycle): publish-branch and repo-recover verbs that draft the body the run path drafts**

*Project description:*
## What

Two verbs that put this engine's change-request drafter in front of the linked
onevcs's out-of-band publication verbs, so a branch landed by hand gets the same
description a branch landed by a run does — from one drafter rather than two.

### The gap

A lifecycle node's closeout drafts its change request's body by running the graph
`--pr-author-graph` names, in the node's worktree, on a composed task
(`src/lifecycle.rs`: `DRAFTING_TASK` and `drafting_task`), and reads the body out of the
graph's retained member reports (`src/report.rs::drafted`, the three `Undrafted`
endings). `onevcs publish-branch` and `onevcs recover` — the two verbs an operator
lands a branch with outside a run — take `--body` / `--body-file` and nothing that
drafts one. The consumer host re-implemented the drafter out of band in 820 lines of
shell (`ai-orchestrator/scripts/draft-pr-body.sh` and `land-branch.sh`), reading this
engine's opening sentence and ending vocabulary out of the binary to stay in step
(divergence entry 69 records that arrangement). Two drafters reconciled by convention
is the defect; the drafter is this engine's, so the out-of-band entry point is too.
...
- **Passthrough first.** `<BRANCH>`, `--repo`, and every argument this engine does not
  name (`--title`, `--policy`, `--body`, `--body-file`, and whatever the linked onevcs
  verb accepts) reach the linked onevcs `publish-branch` / `recover` unchanged and in
  order, through the library the engine already publishes through, never a spawned
  `onevcs`. The verb's own report, verdict and exit status are the onevcs verb's; a
  refused argument is onevcs's refusal, not a second one composed here.
- **`--repo` takes a registered alias or a path**, resolved through the linked onevcs's
  registry the way the lifecycle resolves one, because the drafter needs a directory
  and the verb accepts an alias.
- **Drafting.** Unless `--no-draft` is given, the caller passed `--body` or
  `--body-file`, or the identity's resolved workflow (from the linked onevcs's rules,
  never from `resolve`'s registration field) is `local-direct` — which opens no change
  request, so there is no description for a body to be — the verb drafts: it runs the
  graph `--pr-author-graph` names (required when drafting happens; refused at exit 2
  when drafting would happen and none is named) in a **temporary detached worktree of
  the branch** cut from the named checkout and removed on every exit path (the
  checkout itself is left byte-identical, its worktree list included), on the task the
  lifecycle composes with one substitution: where the run path appends the node's
  rendered task, this appends what the branch says about itself — its name, its base,
  and the full commit messages of `<base>..<branch>` — and says that these are a record
  of what was done rather than why, so a thin `## Why` is the correct outcome; there is
  no `## Change request` section and no `## Worker transcript` section, because there
  is no session and no run. The body is the first retained member result that both
  conformed to the schema and is non-empty, exactly as the run path reads one, and is
  passed to the onevcs verb as its body.
- **A draft that cannot run never blocks the landing.** Whichever of the three
  `Undrafted` endings the graph reaches — could not run or did not succeed, every
  answer refused by the schema, an answer with nothing in it — the ending is written to
  stderr in the run path's own words (with `oneagentgraph`'s classification of each
  member death, where it died), and the onevcs verb runs anyway with no body. There is
  no retry of the engine's own; the graph's schema retry budget is the only one.
- **The turn is spent before the push**, as the consumer's implementation accepts and
  its `docs/repo-lifecycle.md` explains: a branch the merge path then refuses has paid
  for a body nothing used.

### Where it is recorded and proven

Record both verbs in `docs/contract-divergences.md` in the register's entry shape, and
fold entry 69's "the consumer host reads each literal out of this binary" arrangement
into it as retired by this change — after this the consumer reads nothing out of the
binary for drafting. Prove them with journeys driving the compiled binary against a
real onevcs-registered local repository and the real drafting graph with a
deterministic member (the doubles this repository already uses for a drafting
dispatch): a `local-direct` identity lands with no drafting turn spent; a
change-opening identity lands with the drafted body; `--no-draft` and a caller's
`--body-file` each land with no turn spent; a graph that dies lands with no body and
the death classified on stderr; and every argument the caller passed is seen by the
onevcs verb unchanged.

## Why

The user's direction for ai-orchestrator is that it be *"very light or no wrapper on
CLI tools"*; the largest wrapper left after the dead ones go is this pair, and its own
header names the defect: *"a second drafter beside them would be a second contract
nothing reconciles"*. It exists because `onevcs publish-branch` / `recover` open a
change request with whatever body they are handed — none, from an operator naming a
branch — and the drafter that knows how to write one is in this engine's run path,
reachable only from a run. The manager ruled that the drafter is this engine's (it owns
the graph, the schema reading and the task composition; onevcs knows nothing of agent
graphs), so the out-of-band verbs are this engine's too, and `just publish-branch` /
`just repo-recover` in the consumer become passthroughs governed by its
`config/onepipeline.version`.

## Acceptance criteria

- `onepipeline publish-branch` and `onepipeline repo-recover` exist; each forwards the
  branch, `--repo` and every argument this engine does not name to the linked onevcs
  `publish-branch` / `recover` unchanged and in order, and answers with that verb's
  report and exit status; `--repo` accepts a registered alias as well as a path.
- Against a change-opening identity with `--pr-author-graph` named and no body
  supplied, the verb drafts the body through that graph in a temporary detached
  worktree of the branch, removed afterwards with the named checkout's worktree list
  and working tree unchanged, and the landed change request carries that body.
- Against a `local-direct` identity, with `--no-draft`, or with a caller-supplied
  `--body` / `--body-file`, no drafting turn is spent and the landing proceeds with the
  caller's body or none.
- When the drafting graph dies, refuses every answer, or answers with nothing, the
  landing proceeds with no body and stderr names which of the three endings it was,
  with `oneagentgraph`'s classification of each member death where one died.
- Both verbs are recorded in `docs/contract-divergences.md` in the register's entry
  shape, with entry 69's out-of-band arrangement marked retired by this change, and
  each behaviour above is proven by a journey driving the compiled binary against a
  real onevcs-registered repository, with no double for the layer under test.
- The tests covering the modules this change touches (`lifecycle`, `report`, `vcs`,
  `cli`) are green over the finished tree, and so is the judged lint over this change's
  diff.
...
The consumer's implementation is the specification of what the out-of-band task looks
like and of the three endings' reporting; read `~/ai-orchestrator/scripts/draft-pr-body.sh`
and `~/ai-orchestrator/scripts/land-branch.sh` (read-only; that checkout is another
manager's publication checkout). The consumer will call these as
`onepipeline publish-branch "$@" --pr-author-graph graphs/pr-author.yaml` from its
`justfile`, so the flag names and the passthrough rule above are what it relies on; a
departure you want is a proposal over `$ORCHESTRATOR_ASK_MANAGER`, not an edit.

This branch stacks on `op-supervision-verbs` (three other new verbs in `src/cli.rs`);
that node has landed on the base your session was cut from, so build beside its
additions rather than around them. Iterate with the affected test modules and this repository's lint over your
diff; the full gate runs on the merge path after you settle.
...
{"onepipeline.execution_checkout":"nickderobertis__onepipeline","onepipeline.id":"op-publication-verbs","onepipeline.persona":"engineer","onetaskgraph.item_kind":"task","orchestrator.plan-review":{"by":"planning-closeout","key":"9148cf06033782f209cb3cd979017e27012b6cd5095516d578556ea2473ea52f","reviewed_at":"2026-09-20T19:30:11.685303+00:00"}}
-->
