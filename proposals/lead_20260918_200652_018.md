...
**fix(views): read an adopted run as live, label superseded hooks, and report release holds**

*Project description:*
## What

Three accepted tickets about what `status`, `watch` and `results` tell a supervisor, all of
them in `src/views.rs` and the projection it renders, so they run in one node. One commit
per ticket.

**1. Adoption retains the prior driver's stopped state** —
https://github.com/nickderobertis/onepipeline/issues/328, accepted at severity `high`.
`src/projection.rs` folds `run-stopped` into `RunState.stop`, but its `driver-adopted` arm
does not clear it, and `src/views.rs::liveness` passes `state.stop_recorded()` as decisive
evidence that the driver the rewritten launch record names is dead. After a stopped run is
adopted, `status` prints `DRIVER DEAD` over a live replacement driver and `watch` exits
`nothing-driving` at once. Clear `RunState.stop` when folding `DriverAdopted`, so the stop
stays historical journal evidence but no longer speaks for the replacement driver, and
add an end-to-end journey that stops a live run, adopts it, and proves `status` reports the
adopted driver active and `watch` stays armed until a genuine terminal condition. Read
`src/unwatched.rs`'s use of the summary's `stop_recorded` before you change what the
field means: a run's settlement decision reads it too, and an adopted run is not settled.

**2. `results` renders hook records without retry epochs** —
https://github.com/nickderobertis/onepipeline/issues/343, accepted at severity `medium`.
`src/hooks.rs::results_lines` renders every historical `run-hook-fired` record in journal
order with no epoch and no recovery marker, and `src/views.rs` appends those lines beneath
the current graph, so a run recovered by a `retry` shows its first epoch's failure hook
and its instructions as if they described the run now. Associate each rendered hook
record with the edit-defined retry epoch hook idempotency already uses (`hooks.rs`'s
module documentation says what an epoch is), and label a hook from an earlier epoch as
superseded by the recovery edit that ended that epoch — naming the edit — so the current
epoch's hook, or the absence of one, reads as the run's present state.

**3. `status` ignores release-adoption holds** —
https://github.com/nickderobertis/onepipeline/issues/344, accepted at severity `medium`.
`src/views.rs::waiting_on` explains a ready node only by repository workspace occupancy,
while the driver's `src/release.rs` computes published-adoption holds separately, so a
node held on a release reads `ready — queued for dispatch` while recurring `release-wait`
events say it is waiting for a PyPI release. The view is a separate process and cannot
read the driver's memory, so the hold reaches it the way every other hold does: the
`node-held` record the driver journals with `reasons: [{kind: release, awaiting: [...]}]`,
which `src/projection.rs` already folds into `RunState.holds`. Render a ready node's
release hold from that state before considering workspace occupancy or the generic queued
reason — the releases awaited and how long it has waited — and keep the other two answers
exactly as they are for a node no release holds. Do not change how `src/release.rs`
decides a hold; onepipeline #366 does that in a sibling node of this plan, and a view
that reads the journal needs nothing from it.

## Why

The user accepted all three because each misleads the person supervising a run on this
host. The first is the worst: after a routine stop-and-adopt, every view says the driver is
dead and the watch that is supposed to guard the run exits, so a manager either babysits
the journal by hand or believes a live run is wedged. The second presents a superseded
failure hook's instructions as current after the run recovered. The third reports a node
deliberately held on a release as queued for dispatch and sends an operator hunting for a
scheduler fault that does not exist.

## Acceptance criteria

- After a run is stopped and then adopted, `status` reports the adopted driver as active
  and `watch` remains armed until the run reaches a genuine terminal condition, while a
  run that was stopped and never adopted still reads as stopped.
- A run's settlement decision (the unwatched reader's) still treats a stopped, unadopted
  run as settled and an adopted run as live.
- `results` labels every hook record from an earlier retry epoch as superseded, naming the
  recovery edit that ended that epoch, and presents the current epoch's hook record — or
  that none fired in it — as the run's present state.
- A ready node held on a release reads in `status` as held, naming the releases it awaits
  and how long it has waited, before any workspace or queued reason; a ready node no
  release holds reads exactly as it does today.
- The branch's history carries, for each of these tickets, a closing reference naming its
  issue (328, 343, 344), so the change that carries the work is what closes it.
- Every behaviour this change adds or alters is proven by an end-to-end journey driving the
  real binary — a stop and an adoption, a retry across a hook epoch, a node held on a
  release — rather than by inspection, and no test substitutes a double for the layer under
  test.
...
Iterate with this repository's diff-scoped judged lint and the `views`, `driver`,
`run_end_hooks` and `holds` e2e modules that cover what you touched rather than the whole
gate; the repository's required checks run downstream on the change this branch becomes.
`tests/e2e/harness.rs` is the `World` every journey builds on, and `tests/e2e/holds.rs`
already drives a node held on a release through a release probe double.

This repository's releases are cut from conventional commits, and this host's engine pin
adopts the release that carries these fixes, so a commit type that cuts no release would
...
{"onepipeline.branch":"onevcs/s-0a1c96558163","onepipeline.execution_checkout":"nickderobertis__onepipeline","onepipeline.id":"op-views-2","onepipeline.persona":"engineer","onepipeline.resume":{"branch":"onevcs/s-0a1c96558163"},"onetaskgraph.delivers":["followups:I_kwDOTx8aUc8AAAABRmhL_A","followups:I_kwDOTx8aUc8AAAABRy2oDw","followups:I_kwDOTx8aUc8AAAABRy2tSA"],"onetaskgraph.item_kind":"task"}
-->
