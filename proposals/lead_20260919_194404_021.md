...
**feat(driver): maintain idle pool slots from the idle branch on a persistent every-duration schedule**

*Project description:*
## What

Adopt the `onevcs` release carrying `pool_maintain` (this task's references block names
it; `onevcs-testing` moves beside it as before) and give the driver a **persistent
maintenance schedule** that is a pure function of a host file and the sibling's recorded
`last_maintained` — so `onepipeline` keeps **no state**, late is fine, never twice, two
drivers on one host are safe by the same fact, and between runs nothing runs and
nothing needs to.

**The schedule file**, handed to every launch the way `--bus-config` is:
`onepipeline start PROJECT --maintenance-config FILE`, and the launch config's
`maintenance_config` key — keys of launch-config `schema_version: 9`, refused by name
in a document declaring an earlier one — with the precedence every launch-config field
has: the flag beats the config, a blank value is this launch saying it has none.
Retained in the launch record as its parsed document (as `bus_config` is), omitted when
absent so an older record still reads, replayed by `adopt`, which takes no flag. It is
external input, refused **before a run is minted** naming the key: an unknown key, a
`version` other than 1, a rule naming no match field, a malformed span.

```yaml
version: 1
default:
  every: 7d                      # a onevcs `Span`: digits then s|m|h|d, nothing else
rules:                           # first match wins; `match` is the onevcs `RuleMatch`
  - match: {host: github.com, owner: nickderobertis, name: onevcs}
    every: 3d
```

`every` is the **only** key a rule or the default carries: no `at`, no cron shape. The
span is parsed through the linked `onevcs::Span` and the match is `onevcs::RuleMatch`
resolved through `onevcs::first_matching`, so the grammar and the match vocabulary have
one implementation each and this crate restates neither.

**The idle branch.** A pass is idle when it dispatched nothing, nothing became ready,
the run is below its own concurrency ceiling, and the local executor reports
`slots_free > 0`. On an idle pass, when the launch named a schedule, no maintenance
thread of this driver is running, and the driver's in-memory pacing interval (a
constant, minutes; not persisted) has elapsed since it last started one, the driver
starts **one** thread that, for every identity in `onevcs::registry::load()?.identities`
in sorted order, resolves `every` (rule, else default) and calls
`onevcs::pool_maintain(Scope::Repo(identity), Some(every))`. That is the whole of
"due": a slot is due when `last_maintained < now − every`, and the sibling decides it.
A second idle tick while the thread runs does nothing. The thread is bounded by
construction — every command runs under the identity's own `timeout` — and a driver
closing out joins it. The thread hands its `MaintainReport`s back over the loop's
channel, and the loop journals **one** pipeline record, `pool-maintenance`, carrying
per identity and slot what ran and how it ended, only when some slot ran or some
identity was `Claimed` or failed; nothing is written when every identity answered
`NoMaintainCommand`, `NoSlots` or `NotDue`. `status` names a maintenance in progress
while the thread is live; `results` names the last record. A launch naming no schedule
runs no maintenance, spawns no thread, and behaves exactly as before.

**Docs**: `docs/contract.md`'s driver contract (the flag and key, the schema bump), the
launch-config example, the journal kind where this crate's kinds are enumerated, and
the divergence-log entry in the log's own shape; `tests/contract.rs` parses its source
blocks, so give it one.

## Why

The user's words: "it should be a persistent schedule and recorded time last done not
every x time within a run as maintenance intervals will usually be longer than dag run
times"; "it's weird for onevcs to have cron config if it's not doing anything with
cron". A per-run timer would sweep on every run of a busy host and never on a quiet one;
a schedule whose only state is the sibling's recorded fact runs when it is due, from
whichever driver is idle, and never twice. The driver's idle capacity is the cheapest
time to pay for it.

## Acceptance criteria

- The workspace requires the adopted `onevcs` release (and `onevcs-testing` beside it)
  and `Cargo.lock` resolves exactly one version of each, which the engines-current
  check confirms.
- `--maintenance-config` and the launch config's `maintenance_config` are read with
  the flag beating the key and a blank value meaning none; a document with an unknown
  key, a `version` other than 1, a rule naming no match field or a malformed span is
  refused before a run is minted, naming the key; the parsed document is retained in
  the launch record, omitted when absent, and replayed by `adopt`; and the
  schema-version boundary holds functionally through the real `start` over real
  launch documents: a launch config declaring `schema_version: 9` admits
  `maintenance_config`, and a launch config carrying that key under every earlier
  schema version the loader still reads is refused before a run is minted, naming
  `maintenance_config` and the version that admits it.
- The schedule is persistent rather than per run, proven on a scratch state root
  holding a pooled identity with an idle slot and a `maintain` command that writes a
  marker into the worktree, driven through the real linked `onevcs`: under a
  schedule with a short `every`, an idle driver runs the maintenance, the marker
  appears, the sibling's `pool status` shows `last_maintained`, the run's journal
  carries one `pool-maintenance` record with that slot's outcome, and a further idle
  tick inside `every` runs nothing and writes nothing; a fresh driver launched
  against that state root inside `every` of the recorded `last_maintained` runs
  nothing and writes nothing, and one launched after `every` has elapsed maintains
  the slot again; two drivers idle at once on one state root maintain the slot once
  between them, the other observing `Claimed` or `NotDue` and writing no second
  record of a run; and maintenance is withheld — no thread started — while any one
  of the idle-capacity conditions fails: the pass dispatched something or something
  became ready, the run is at its own concurrency ceiling, or the local executor
  reports no free slot; and the status and results surfaces render what ran, read
  through the real commands over that run — `status` names a maintenance in progress
  while the thread is live and nothing about maintenance when none is, and `results`
  names the last `pool-maintenance` record with its per-identity and per-slot
  outcomes.
- A launch naming no schedule produces no maintenance thread, no process and no
  `pool-maintenance` record and behaves exactly as before; under a schedule, an
  identity whose resolved policy names no `maintain` command is skipped inside the
  one maintenance thread as `NoMaintainCommand` with no process spawned and no record
  written for it, and a tick on which every identity answered `NoMaintainCommand`,
  `NoSlots` or `NotDue` writes no record at all; a driver never runs two maintenance
  threads at once; a driver closing out while one runs waits for it; and successive
  thread starts respect the in-memory pacing interval — proven functionally by a
  journey in which the first maintenance thread finishes at once (every identity
  `NotDue`) and the idle passes that follow inside the interval start no second
  thread and sweep the registry no second time, observed through the driver's own
  journal or the sibling's call record rather than a timer read.
- A rule's `every` beats the default for the identity it matches, resolved through the
  linked sibling's matcher and span grammar rather than a copy of either.
- `docs/contract.md` and `docs/contract-divergences.md` state the flag, the key, the
  schema bump, the file's shape and the journal kind, and `tests/contract.rs` holds
  the entry's source blocks.
- Every behaviour of this crate that its existing tests and journeys cover is still
  covered and still passes with the same expected values.
- The checks that exercise this change are green over the finished tree: the crate's
  tests and end-to-end journeys touched or added here, the format check, the
  engines-current check, and the judged lint over this branch's own diff.
- Every claim the dispatch makes about the finished work is true of the tree as it
  finally stands.

## Additional info

- This node adopts `published`: the engine held it until the `onevcs` crate carrying
  `pool_maintain` was on crates.io, and the reference block rendered into this task
  names the version; take the pins from there.
- This plan is the planner's ruling on the contract for these surfaces; record the
  amendment and the divergence entry rather than reporting a conflict. A shared
  interface is never changed unilaterally once this plan runs: the flag, the key, the
  file's shape and the journal kind are what this host's adoption node documents and
  its launch wrapper names — raise any departure with `"$ORCHESTRATOR_ASK_MANAGER"`
  and keep building.
- The recipes: `just test`, `just test-e2e <filter>`, `just fmt-check`, `just
  engines-current`, `just lint-llm-diff`. The full `just gate` is not owed.
- Models in the tree: `src/release.rs`'s `Watch` for a sibling asked on a thread of its
  own; the `--dispatch-env-hook` / `dispatch_env_hook` pair in `cli.rs`, `driver.rs`
  and `ledger.rs` for a flag-plus-key launch field with a schema bump; the reconcile
  loop's `next` array in `engine.rs` for a paced read; `tests/e2e/real_vcs.rs` and
  `tests/e2e/dispatch_env_hook.rs` for journeys against the real sibling.
- The ai-orchestrator host will name this flag from `scripts/onepipeline.sh` on every
  `start` once its engine pin lists it in `start --help`, so the flag's `--help` line
  is part of what that host reads.
...
{"onepipeline.adoption":"published","onepipeline.consumes":{"onevcs-pool-maintain":"crate"},"onepipeline.id":"onepipeline-maintenance-schedule","onepipeline.max_turns":220,"onepipeline.node":"onepipeline-maintenance-schedule","onepipeline.persona":"engineer","onetaskgraph.item_kind":"task"}
-->
