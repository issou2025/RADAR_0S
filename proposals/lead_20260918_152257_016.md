...
**fix(release): scope release-plz to this repository and build both macOS architectures in one release job**

*Project description:*
## What

Two accepted tickets about this repository's release tooling and workflows, plus the one
line the manager ruled this repository emits for a sibling repository's fix. One commit
per ticket. This node starts from the manifests node's work (exact sibling requirements
and a publish-time check step in the release workflow), which is on the base you start
from; build on it.

**1. Bootstrap replaces the host-global `release-plz` binary** —
https://github.com/nickderobertis/onetaskgraph/issues/1990, accepted at severity `high`.
`scripts/bootstrap-workspace.sh` reads this repository's release-plz pin off
`.github/workflows/release-plz.yml` and, when the `release-plz` on `PATH` differs,
installs the pinned version into the caller's global cargo bin directory — while other
registered repositories on the same host pin other versions of the same global command
(printobserver 0.3.167, onepipeline 0.3.158 against this repository's 0.3.160), so
bootstrapping or publishing one repository invalidates another's gate until a person
replaces the tool again. Install the pinned release-plz into a repository-scoped tool
location and make every recipe, hook and script of this repository that runs release-plz
invoke that exact scoped binary (`scripts/check-real-release-preparation.sh` and its
callers among them), leaving the host-global `release-plz` untouched and never consulted.
Choose the location so that a fresh worktree or a publication's scratch tree of this
repository finds an already-provisioned binary of that version without reinstalling —
a version-namespaced directory under the user's cache home, keyed by the tool and its
version, is the shape that survives worktrees; a directory inside the checkout is not —
and say which you chose and why in your report. **The host-prerequisite marker:** when
the scoped binary cannot be provisioned or does not answer with the pinned version at
the point this repository's `pre-push` gate needs it, the failing check prints exactly
one line `onevcs: host-prerequisite: <what is missing and how to install it>` to its
output before failing — the marker onevcs's `docs/contract.md` states for a merge-path
hook refusing on a missing host tool, which a sibling node of this plan (`ov-publish`)
defines, and which onepipeline's router then settles without re-dispatching a worker
(https://github.com/nickderobertis/onepipeline/issues/360; name that URL in this commit's
body, not a `Closes` line). Emit it only for the missing or wrong-version tool, never for
a check that reads the candidate tree.

**2. macOS release work is duplicated across sequential stages** —
https://github.com/nickderobertis/onetaskgraph/issues/1991, accepted at severity `high`.
`.github/workflows/release.yml` schedules two macOS native-asset jobs and then, only after
that whole matrix completes, two more macOS wheel jobs, while `.github/workflows/ci.yml`
starts two macOS jobs again on every push to `main` — all competing for one scarce
account-level macOS runner pool, so a merge-to-PyPI took about 6.5 hours and a manager
cancelled valid runs by hand to free capacity. Build both macOS architectures' native
assets and wheels in one macOS release job (cross-compiling the second target on the one
runner, as `maturin` and `cargo` both support), keeping every artifact name the later
jobs and the npm carriers expect; and suppress the main-push macOS CI jobs when the same
commit already passed the pull-request checks, without changing the required check
names a pull request reports.

## Why

The user accepted both at severity `high` because both stall this host's publications on
somebody else's tool state: three repositories fighting over one global binary means a
valid tree is refused until a person reinstalls the right version, and duplicated macOS
work queues every merge and release behind itself for hours. The marker line is what lets
the engine on this host recognise the first as a host problem rather than spending a
worker's retry budget on it.

## Acceptance criteria

- Bootstrapping or publishing this repository leaves the host-global `release-plz` binary
  and version untouched, and every release-plz invocation of this repository runs the
  pinned version from the repository-scoped location.
- A fresh worktree of this repository on a host that already holds the pinned version
  finds it without reinstalling, and a host that does not provisions it into the scoped
  location on bootstrap.
- When the scoped release-plz is missing or at the wrong version at the point the pre-push
  gate needs it, the gate's output carries exactly one host-prerequisite marker line naming
  the tool, the version and the install command, and no tree-sensitive check of this
  repository ever prints that marker.
- The release workflow builds both macOS architectures' native assets and wheels in one
  macOS job, every artifact name and carrier the downstream jobs consume is unchanged, and
  the workflow parses and passes this repository's own workflow checks.
- A push to `main` of a commit whose pull-request checks already passed schedules no macOS
  CI job, a push without that record still does, and the check names a pull request
  reports are unchanged.
- The branch's history carries a closing reference naming each of these issues (1990,
  1991), so the change that carries the work closes them, and names
  https://github.com/nickderobertis/onepipeline/issues/360 as the work the marker line
  carries, without closing it.
- Every behaviour this change adds is proven by tests driving the real scripts and hooks —
  the scoped provisioning against a stand-in tool location, the marker on a missing tool,
  the workflow's shape — rather than by inspection, and no test substitutes a double for
  the layer under test.
- The tests and checks that cover the scripts, hooks and workflows this change touches are
  green over the finished tree, and so is the judged lint over this change's diff.
- Every claim this dispatch makes about the finished work is true of the tree as it finally
  stands.

## Additional info

One commit per piece is the shape to aim for, so each squash headline and each closing
reference read cleanly, but it is guidance rather than a bar: what the finished branch owes
is the closing reference for every issue and the attribution of the third piece.

Iterate with this repository's diff-scoped judged lint and the scripts' own checks
(`scripts/check-pre-push-provisioning.sh`, `scripts/check-real-release-preparation.sh`,
`scripts/check-distribution-contract.sh`, the workflow checks under `scripts/`) rather than
the whole gate; the required checks run downstream on the change this branch becomes. A
hosted workflow run cannot be exercised here: what stands for it is the workflow parsing,
this repository's own workflow checks passing, and the job-level condition proven by a
test over the workflow document. Say so in your report.

The marker's spelling quoted above is the agreement fixed for this plan: the `ov-publish`
node states the same line in onevcs's `docs/contract.md`, and this node runs in parallel
with it rather than after it, so take the spelling from this task. If you find a reason
it cannot be that line, ask over `$ORCHESTRATOR_ASK_MANAGER` rather than choosing another;
a shared interface in this plan is never changed unilaterally.

This repository's releases are cut from conventional commits; `fix` is the type for both.
Nothing on this host adopts this node's release directly.
...
{"onepipeline.execution_checkout":"nickderobertis__onetaskgraph","onepipeline.id":"otg-release-tooling","onepipeline.persona":"engineer","onetaskgraph.delivers":["followups:I_kwDOUBBgxs8AAAABR5zfgA","followups:I_kwDOUBBgxs8AAAABR5zshA"],"onetaskgraph.item_kind":"task"}
-->
