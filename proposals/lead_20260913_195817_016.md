...
**test(npm): make the llmlint cache teardown diagnose a failed removal**
...
Root cause 10 of `nickderobertis/ai-orchestrator` issue #1004: a test teardown fails the job with
no diagnosis, and its stated cause is wrong. Read the issue's entry for the CI failure.

`npm/test/llmlint-cache.test.mjs` `workspace()` (about `:321-325`) removes its sandbox in
`t.after` with `rmSync(sandbox, { recursive: true, force: true })`, and it failed CI with
`ENOTEMPTY`. The recorded subprocess-race cause was reproduced against and ruled out, so the
cause is unknown. When that removal throws, the teardown prints the entries still under the
sandbox, each with its mtime, and the current process table, then rethrows the original error.

**No retry of any kind:** no `maxRetries`, no loop, no second attempt. A retry would most likely
hide the next occurrence, which is the one thing that would say what is actually happening. This
is the issue's own instruction, and this task keeps it.

## Why

The operator is closing every root cause of issue #1004 so that none of them has to be
repaired by hand again. A CI job failed with nothing in its output that could say why, and the
cause recorded for it was ruled out. The operator wants the next occurrence to diagnose itself
rather than to be retried out of sight.

## Acceptance criteria

- When the llmlint cache test's sandbox removal throws, its teardown prints the entries still
  under the sandbox with their mtimes and the current process table, and rethrows the original
  error unchanged.
- A test induces a real removal failure — a real filesystem call that throws, not a stubbed
  `rmSync` — and observes both the diagnosis and the rethrown error.
- Nothing in the teardown retries: no `maxRetries`, no loop, no second removal attempt.
- A removal that succeeds prints nothing new, and the existing llmlint cache tests pass over the
  finished tree.
- The judged lint over this node's diff leaves no finding unanswered.
...
- Read first: `AGENTS.md`, `tests/AGENTS.md`, `npm/test/llmlint-cache.test.mjs`, and
  `biome.json` for the JavaScript style this file is held to.
- Run `node --test npm/test/llmlint-cache.test.mjs` and `just lint-llm-diff <base>`. The merge
  path runs the full gate.
- A permission-based failure does not throw for a process running as root, so do not rely on one
  alone to induce the failure; structuring the teardown so the diagnosing path is reachable with
  a path a real call fails on is one way.
- Siblings in this repository run in parallel and touch other files: `onepipeline-monitor-cursor`,
  `onepipeline-retire-revision-pin`, `onepipeline-writeback-divergence` and
  `onepipeline-link-fixes`.
- Nothing here changes a surface another node of this plan consumes, and no node waits on a
  release carrying it: a test change is in force by merging.
...
{"onepipeline.id":"onepipeline-teardown-diagnosis","onepipeline.max_turns":70,"onepipeline.persona":"engineer","onetaskgraph.item_kind":"task","orchestrator.plan-review":{"by":"review-plan","key":"981b51f74bc321e1156ea2eedf0150a84306a6e67873eddd1307a1c4cdeafff3","reviewed_at":"2026-09-13T17:13:56.519748+00:00"}}
-->
