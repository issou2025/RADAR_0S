...
**feat(release): read a cancelled check as no verdict, acknowledge lost baselines**

*Project description:*
## What

**Read against** `nickderobertis/onevcs` `origin/main` at `e0f307c`, the commit issue #1004 read. Every path and line this task cites was checked against that tree; if the tree has moved when you start, re-read what moved rather than trusting a cited line number.

Close `nickderobertis/ai-orchestrator` issue #1004 root cause 2: onevcs has no repairable
"unknown", so an unknown collapses to a terminal wrong answer. Two faces.

**Face A — a required check with no verdict is not a refusal.** `Check::red()`
(`crates/onevcs/src/host.rs:503-522`) is `settled() && !green()`, so a check GitHub reports
`completed` with conclusion `cancelled` — a concurrency-group kill, no verdict either way —
makes `watch` return `checks-failed` straight away (`crates/onevcs/src/publish.rs:2289-2295`,
reason built at `:2343-2351`). Change it so:

- A required check whose conclusion is `cancelled` or `stale` is **no verdict**: the
  publication does not refuse on it and keeps reading the change's checks, as it already does
  for a check not yet reported. A later verdict under the same check name decides — green
  lands under the policy as today, red refuses as `checks-failed` as today.
- With no later verdict inside the existing bound, the publication ends with the existing
  `checks-unsettled` kind, and its reason names the check and its no-verdict conclusion
  (so `unsettled()`, `publish.rs:2388-2399`, must stop describing it as settled).
- `failure`, `timed_out`, `action_required` and `startup_failure` remain refusals, as does any
  conclusion the code does not name as green or no-verdict.
- **No new `FailureKind` word.** That vocabulary is fixed across three libraries
  (`publish.rs:396-398`), and `checks-unsettled` already says "no verdict could be read".

**Face B — a landing onevcs did not witness can be given a release answer.** Baselines are
captured only at the two in-process landing sites (`publish.rs:1363`, `:1840`); a change the
host merges later (auto-merge completing after a 502 on `gh pr merge --auto`) is left with no
baseline, `release status` answers not-answered for ever, and `onevcs release acknowledge`
refuses an automated target by name (`crates/onevcs/src/release.rs:844-850`).

**Decision, departing from the issue's first suggestion.** Do **not** capture a baseline when
a landing is discovered. onevcs's own contract (`docs/contract.md:832-844`;
`crates/onevcs/src/releases.rs:440-442`) states why a late probe is unsound: if the release
carrying the change came out between the merge and the discovery, it would be recorded *as*
the baseline and the change reported unreleased until a further release that may never come —
and it says no automatic recovery should be added. Implement the issue's backstop instead,
which is sound because a person states the answer and the record says so:

**The contract** (consumed by `onepipeline-link-fixes`, which links this release into the
engine, and by the manager, who runs the verb; state it in `docs/contract.md`'s amendments
section and the usage blocks the contract drift gates read):

```
onevcs release acknowledge <REFERENCE> --target <NAME> --version <VERSION> [--supersede] [--json]
```

- For an **automated** target, it is accepted when the landing's baseline record for that
  target is absent or `Unestablished`, and records `<VERSION>` as the release that carries
  the landing. It is still refused, naming the probe, when that landing has an established
  probed baseline for the target.
- After it, `onevcs release status <REFERENCE> --target <NAME>` and the library's
  `release_status` answer released at `<VERSION>`, and both say the answer was
  **acknowledged rather than probed** (a field in `--json`, and words in the human form).
- `--supersede` replaces an earlier acknowledgement of a different version, keeping the old
  one in the record's history, exactly as it does for a human-step target today; without it a
  different version is refused.
- Human-step targets behave exactly as today.
- The not-answered message for a missing baseline (`unsound()`, `release.rs:660-676`) names
  this acknowledgement as the remedy, where it now says "land again, or adopt fast".
- The acknowledgement emits the release event it already emits for a human-step target.

## Why

In the run that closed #855, a required check cancelled by GitHub's concurrency group settled
a node `checks-failed` and cost a publication attempt and a retry; and a change request whose
auto-merge completed after a 502 never got a baseline, so a `published` dependent was held with
no timeout on `onepipeline-cli` 0.29.0 while that wheel was on PyPI the whole time. Both
remedies onevcs offered were unavailable: the landing had already happened, and `fast` cannot
land on a `local-direct` identity. The operator does not want a publication spent on a check
that never answered, or a node held for ever on a release that exists.

## Acceptance criteria

- A publication whose required check concludes `cancelled` or `stale` does not settle
  `checks-failed`: when the same check later reports green it proceeds under its policy, when
  it later reports a failing conclusion it settles `checks-failed`, and when nothing further
  arrives inside the bound it ends `checks-unsettled` with a reason naming that check and its
  conclusion — each proven end to end against the real binary with the repository's host
  double.
- A required check concluding `failure`, `timed_out`, `action_required` or `startup_failure`
  still settles `checks-failed`, and so does a required check whose conclusion is one the code
  names as neither green nor no verdict (a conclusion string the host double reports that onevcs
  does not recognise) — each proven end to end against the real binary with the host double — and
  the `FailureKind` vocabulary is unchanged.
- The Face B incident is reproduced end to end: a change request merged by the host with no
  onevcs process observing the merge has no baseline, `release status` for its automated target
  is not-answered naming the acknowledgement remedy, `onevcs release acknowledge … --version`
  is accepted, and `release status` then answers released at that version and marks the answer
  acknowledged — through the CLI (human and `--json`) and through the library's `release_status`.
  The same holds end to end for a landing whose baseline record for that target is
  `Unestablished`, and each accepted acknowledgement of an automated target emits the release
  event an acknowledgement of a human-step target emits, read back from onevcs's event stream.
- `release acknowledge` on an automated target whose landing has an established probed
  baseline is still refused naming the probe; a different version without `--supersede` is
  refused; `--supersede` replaces it and keeps the earlier one in history.
- No read path probes a baseline after a landing, and the tests that hold baselines to landing
  time stay green.
- `docs/contract.md`'s amendments and the usage blocks the contract drift gates read state the
  changed `acknowledge` surface and the no-verdict reading, and those gates pass.
- The tests covering `host.rs`, `publish.rs`, `release.rs` and `releases.rs` pass over the
  finished tree, and the judged lint over this node's diff leaves no finding unanswered.
- Every claim the dispatch makes about the finished work is true of the tree as it finally
  stands.

## Additional info

- Read first: `AGENTS.md`, `docs/contract.md` (the release sections around lines 805-844 and
  the amendments section), `docs/inferred-surface.md`, `crates/onevcs/src/host.rs`,
  `publish.rs` (`watch`, `await_checks`, `unsettled`, the two `record_baselines` sites),
  `release.rs` (`status`, `automated_status`, `unsound`, `acknowledge`), `releases.rs`.
- Tests nearest these changes: `crates/onevcs/tests/e2e/releases.rs` (the host-merged capture
  near `:718`, the no-probe case near `:1774`), the publish journeys over
  `crates/onevcs-testing/src/remote.rs`'s host double, `crates/onevcs/tests/contract.rs`'s
  drift gates. `just test-one <name>` runs one test; `just lint-llm-diff <base>` is the judged
  tier. The merge path runs the full gate.
- If the host double cannot yet report a `cancelled` or `stale` conclusion or a later re-run
  under the same name, extending `onevcs-testing` is part of this node.
- Whether the acknowledged record needs a new persisted shape or a new registry schema version
  is yours to decide. If it bumps the `$ONEVCS_HOME` schema, say so plainly in the change
  request description: a host adopting it migrates every checkout at once.
- The acknowledge contract above is consumed by another node. If you find it wrong, surface a
  proposal to the manager over `$ORCHESTRATOR_ASK_MANAGER` and keep building against it.
- The title must stay a releasing type: the host adopts this through a release.
...
{"onepipeline.id":"onevcs-no-verdict","onepipeline.max_turns":220,"onepipeline.persona":"engineer","onetaskgraph.item_kind":"task","orchestrator.plan-review":{"by":"review-plan","key":"123371d2f8c86a9f1d767a9bd0f9a14bd5cb6a0694c99089e6b34888788f5913","reviewed_at":"2026-09-13T17:31:53.682770+00:00"}}
-->
