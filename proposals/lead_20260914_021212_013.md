...
**feat(service): sign the agent's harness in once as the service user**

*Project description:*
## What

Give printobserver's supervising agent a production sign-in procedure that works on a fresh
install.

`scripts/install-service.sh` creates the service user with
`useradd --system --no-create-home --shell /usr/sbin/nologin`, and the systemd unit it writes sets
`ProtectHome=true` and `ReadWritePaths=<state_dir>`. On every event, the server invokes the harness
named by `supervisor.harness` (the template says `claude-code`) through
`crates/printobserver-oneharness`. `agent_for` in `crates/printobserver-server/src/server.rs` hands
that harness `harness_env: Vec::new()`. A harness keeps its configuration and its authentication in
a directory that one environment variable selects. oneharness-core 0.12.3 names the variable for
each harness in `oneharness_core::io::usage`: `CLAUDE_IDENTITY_ENV = "CLAUDE_CONFIG_DIR"` and
`CODEX_IDENTITY_ENV = "CODEX_HOME"`. The service user has nowhere to keep that directory, and nothing
tells an operator how to create it. The README's setup step 8 (as PR #31 wrote it) reports this gap.

This node owns the following contract.

1. **One table, in the adapter crate.**
   - `printobserver-oneharness` declares the harness identities printobserver can sign in. For each
     one it declares:
     - the environment variable selecting that harness's configuration directory, taken from
       `oneharness_core::io::usage`'s constants rather than spelled again;
     - the harness program and the arguments of its own interactive sign-in.
   - The entries are at least `claude-code` and `codex`.
   - Establish each sign-in invocation from that harness's own command-line help. The adapter only
     declares these as data: its "nothing in this crate spawns" rule stands, and `just check-repo`
     enforces it.
   - The server and the CLI import this table. Nothing else names these variables.
2. **One directory per harness, under the state directory.** It is
   `<state_dir>/harness/<harness identity>`, created with mode `0700` by whichever of these runs
   first:
   - the sign-in command below;
   - a server start whose configured harness is in the table.

   It is writable under the unit's `ReadWritePaths` and invisible to `ProtectHome`. For a harness in
   the table, `agent_for` passes `<VARIABLE>=<that directory>` to every supervision turn through
   `SupervisorConfig::harness_env`. A harness not in the table is handed nothing extra, as today.
3. **One command, `printobserver sign-in`.**
   - It sits beside `server` as a second command that is not a request to a running server. It
     takes only the global options, and reads the server configuration named by `--config`
     (default `/etc/printobserver/config.toml`) for `state_dir` and `supervisor.harness`.
   - It creates the directory above if needed, then runs the harness's declared sign-in invocation
     with the variable set to that directory, inheriting the terminal. It exits with the harness's
     own exit status.
   - It starts no server, opens no listener, writes no `client.toml`, runs no supervision turn, and
     sends no request to OctoPrint or Obico.
   - For a harness that is not in the table, it refuses before running anything, naming the
     configured harness and the ones it supports.
   - It needs only the configuration values it reads to be valid. An operator can sign the agent in
     before or after filling in the OctoPrint and Obico values.
   - The documented invocation runs it as the service user:
     `sudo -u printobserver /usr/local/lib/printobserver/printobserver sign-in`, or the program on
     that user's path.

Also make the installer and unit give the harness what it needs as that user on a fresh machine. If
the harness needs a writable home directory beside its configuration directory, both the service and
the sign-in command get one under the state directory. The installer still enables and starts
nothing.

Update `AGENTS.md`'s "The agent's and the operator's surface". It currently says the server command
is the one command beside the client commands; it must now describe both. If the sign-in becomes a
step of the end-user install path, add it to `AGENTS.md`'s "The end-user install path" section first
and derive every other statement from there. That section is the one source, and `just check-repo`'s
install-path rule (`tools/repo-checks/src/repo_checks/install_path.py`, applied from `checks_ci.py`)
holds the README to it.

Rewrite the README's step 8 into this procedure:
- install the harness program where the service user's path finds it;
- run the sign-in command once as the service user;
- let the service's turns use that sign-in.

Reorder steps if signing in belongs before enabling the service. The command surface reference and
its generated manifest (`docs/reference/command-surface.md`, `docs/reference/surface.json`, written
by `just docs-generate`) gain the new command.

**If an interactive sign-in as the service user proves impossible for a supported harness**, do not
document copying the operator's own harness authentication directory into the service directory as
the procedure. Put the finding — which harness, what failed, and the evidence — to your manager over
`$ORCHESTRATOR_ASK_MANAGER`. That route is an escalation, never a default.
...
enforce, the first read a person is told to make needs an ID nothing tells them, and the agent the
whole tool exists to run has no way to authenticate on the machine beside the printer. The README is
now honest about all three, and honest documentation of an unusable path is not the goal. This node
closes the last of those gaps: until the harness can be signed in as the user the service runs as,
no supervision turn can run on a real install, and the tool supervises nothing.

## Acceptance criteria

- `printobserver-oneharness` declares, in one table, each harness identity printobserver can sign
  in — at least `claude-code` and `codex`. Each entry carries the configuration-directory variable,
  taken from `oneharness_core::io::usage`, and the harness's own sign-in invocation. The server and
  the CLI read that table, and no other source in the tree names those variables.
- A server started with a harness in that table creates `<state_dir>/harness/<harness identity>`
  with mode `0700` if it is absent. Every supervision turn it runs is handed that harness's variable
  naming that directory. An end-to-end test proves this with the adapter's deterministic responder,
  or a recording stand-in for the harness program; either replaces only the paid provider process.
- `printobserver sign-in --config <configuration>`, run as a real binary with a recording stand-in
  harness program first on its path:
  - runs that harness's declared sign-in invocation with the variable naming
    `<state_dir>/harness/<harness identity>`, and creates that directory with mode `0700`;
  - hands the harness program the caller's own standard input, output and error (the terminal,
    when run from one), so an interactive sign-in can prompt and be answered; the test proves the
    stand-in received the invoking process's stdin and wrote to its stdout;
  - exits with the stand-in's exit status;
  - reads only `state_dir` and `supervisor.harness` from the configuration: a configuration whose
    OctoPrint and ingress settings are absent or unreachable still signs in, and the command
    reaches OctoPrint nowhere and posts to no Obico address (the test binds nothing at those
    addresses and asserts no connection was attempted);
  - starts no server, opens no listener, writes no `client.toml`, and runs no supervision turn.

  An end-to-end test through the real binary drives each of those properties.
- `printobserver sign-in` under a configuration naming a harness outside the table refuses without
  running anything: an end-to-end test through the real binary, with a recording stand-in first on
  the path for every harness in the table, proves no stand-in was invoked, no directory was created
  under `<state_dir>/harness`, the exit status is the CLI's configuration-refusal status, and the
  message names the configured harness and every supported one.
- On a root the real `scripts/install-service.sh` installed into, the service's supervision turns
  and the sign-in command both use the same harness directory under the installed state directory,
  and that directory is writable by the service user. A test proves this from the installed unit's
  own start command and configuration.
- The procedure the README documents works on a fresh install, and one installed-service journey
  proves the whole of it without authenticating a paid provider: on a root the real
  `scripts/install-service.sh` installed into, with a recording stand-in harness program that
  writes a simulated sign-in state file into the directory named by its variable, the sign-in
  command run under the installed unit's `User=` and its filesystem restrictions (`ProtectHome=true`
  and every other hardening directive the unit carries, applied by systemd itself — the unit or
  `systemd-run` — where the test host has it; where it does not, the test applies, and asserts it
  applied, a substitute that enforces each restriction relevant to authentication storage: the
  process runs as the service user; `HOME` is either unset or names a directory under
  `<state_dir>` that the service user owns at mode `0700`; every path under `/home`, `/root` and
  `/run/user` is unreadable and unwritable to it; and the only writable location it is handed is
  `<state_dir>`; a substitute that leaves any of those unrestricted fails the journey) leaves that state at
  `<state_dir>/harness/<harness identity>`, and a subsequent supervision turn started from the
  unit's own start command under the same user and restrictions is handed the same directory,
  reads that state file, and can write beside it. The journey asserts:
  - the service user is the one the installer creates; it has no home directory under `/home`,
    and any home the service or the sign-in command gives it is under `<state_dir>`;
  - the unit keeps `ProtectHome=true`;
  - the harness directory and the state file in it are owned by the service user with no access
    for others.
- The installer still enables and starts nothing, and the repository checks' service-install rule
  holds.
- Signing in starts no supervision turn and no service.
- No harness authentication material, and no credential, appears in the systemd unit text, the
  configuration template, the tree, or any output of the sign-in command other than what the
  harness itself prints to the terminal.
- The README describes the working procedure: installing the harness program where the service
  user's path finds it, signing it in once as the service user with the sign-in command, and what
  the service does with that sign-in. It no longer says that the repository provides no production
  procedure for installing or signing in the harness. Copying the operator's own harness
  authentication directory is not documented as the procedure.
- `AGENTS.md`'s "The agent's and the operator's surface" describes the sign-in command beside the
  server command.
- If the install path gained a step, it is stated in `AGENTS.md`'s "The end-user install path" and
  derived from there, with the repository checks' install-path rule green.
- `docs/reference/command-surface.md` and `docs/reference/surface.json` carry the new command as
  their generators write them.
- The tests of every crate this change touches and the repository's own deterministic repository checks are green over the finished
  tree.
- Every site-scoped suppression directive this change writes — added, moved or rewritten — has its
  `suppressions.toml` entry (rule, file, site and a non-empty reason) in the same change, and every
  directive this change removes has its entry removed with it. The completion report accounts for
  every one of those directives and for every site-scoped suppression already standing over a line
  the diff changes, each with its site and its reason.
...
- `cargo test --locked -p printobserver-oneharness`.
- `cargo test --locked -p printobserver-server`.
- `cargo test --locked -p printobserver`, including `--test service`, `--test surface` and
  `--test journeys`. Use the features those targets declare.
...
`just test-printer-smoke --run` or `just test-obico`. Never run a real harness sign-in against a paid
provider account from this dispatch. The recording stand-in is the proof, and it replaces only the
paid provider process.
...
`api-credential` runs beside this node and also edits `crates/printobserver-server/src/server.rs`,
`crates/printobserver/tests/service.rs`, the installer's configuration template and the README's
step 5. Keep your edits to those files to what this contract needs, so the two merge cleanly.
`print-discovery` stacks on this node and regenerates the command surface after it. The table, the
directory layout and the command under `## What` are the agreed contract. If building them shows you
a departure you want, put the proposal to your manager over `$ORCHESTRATOR_ASK_MANAGER`, and keep
building against the contract as written until you get an answer.
...
{"onepipeline.execution_checkout":"nickderobertis__printobserver-isolated","onepipeline.id":"agent-sign-in","onepipeline.persona":"engineer","onetaskgraph.item_kind":"task"}
-->
