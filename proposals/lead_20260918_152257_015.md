...
**fix(subprocess): hand a hosted plugin the directory of the configuration document that named it**

*Project description:*
## What

Resolve https://github.com/nickderobertis/onetaskgraph/issues/1427, accepted at severity
`high`: a relative path in a subprocess-hosted plugin's settings ignores the configuration
document's directory.

The document-relative rebasing rule (`crates/onetaskgraph-core/src/config/relative.rs`,
`README.md`'s "Relative paths in a configuration document") stops at the `subprocess`
seam: `SubprocessPlugin` declares no document-relative settings and hands its nested
`settings` to the child untouched, so `settings.config.root: store` is rebased for an
in-process `local-md` source and reaches a subprocess child as the literal `store`,
resolved against the child's working directory. The same relative root therefore selects
a different directory depending on which side of the seam the source runs, and moving a
source across it can pick the wrong store. Here "subprocess plugin" is core's transport
adapter for any separately packaged source; it lives in core because the protocol and the
process lifecycle are shared infrastructure.

**The contract this establishes, which the node states rather than re-decides.** The
`initialize` parameters of `docs/plugin-protocol.md` §3 gain one optional member,
`document_dir`: the absolute directory holding the configuration document that supplied
this source's block, and absent when that block came from no document — the environment
layer or a flag — in which case a relative path keeps resolving against the process
working directory exactly as the README's rule already says for those layers. It is added
without a protocol version bump, under the protocol's own §2.1 rule that a later version
may add a member an earlier plugin ignores. `docs/plugin-protocol.md` §3 is the one
statement of the member; `crates/onetaskgraph-core/src/subprocess/wire.rs` follows it.
Core keeps nested `settings` opaque and rebases nothing inside them. A hosted plugin —
the reference host `serve` runs, and each registered plugin behind it — resolves only the
relative fields its own `document_relative_paths()` declares against the supplied
directory, exactly as the in-process path resolves them against the document; a plugin
declaring none resolves nothing. Core needs to know, at the point it builds a subprocess
source, which document supplied the block: carry that origin from the configuration
layer to the plugin's `build` in whatever way the layer's own `Origin` tracking supports
without turning an engine-owned fact into a user-visible setting, and say which in your
report.

Bring `README.md`'s statement that the rule "stops at the `subprocess` seam" into
agreement, and `docs/local-md.md` where it points a plugin's reader at the rule. Prove it
with a CLI journey through the reference subprocess host (`onetaskgraph-source`, which
`crates/onetaskgraph/tests/common` and the e2e fixtures already drive over a real pipe):
one configuration document naming a `local-md` root relative to itself, read in process
and through the subprocess host from a different working directory, both reading the same
store; and one supplied through the environment layer, resolving against the working
directory on both sides.

## Why

The user accepted this at severity `high` because this host's stores are configured by
relative roots in one document and read from many working directories — a planner's
worktree, a manager's checkout, a run's scratch — and a source that resolves differently
behind the subprocess seam is a store that silently vanishes when it is moved there. It
is not fixed for this host until the plan-store executable this checkout spawns carries
it, which a sibling node of this plan adopts.

## Acceptance criteria

- A `local-md` root written relative to a configuration document resolves to the same
  directory whether the source runs in process or behind the subprocess host, from any
  working directory.
- A relative root supplied through the environment layer or a flag resolves against the
  process working directory on both sides of the seam, and the `initialize` parameters
  then carry no document directory.
- A hosted plugin resolves only the fields it declares as document-relative, and a plugin
  declaring none receives the member and resolves nothing.
- A plugin speaking the protocol before this member ignores it and behaves exactly as
  today; the protocol version is unchanged.
- `docs/plugin-protocol.md` §3 states the member, its meaning and when it is absent, and
  the wire definition and the reference host agree with it; `README.md` and
  `docs/local-md.md` no longer say the rule stops at the subprocess seam.
- The branch's history carries a closing reference naming
  https://github.com/nickderobertis/onetaskgraph/issues/1427, so the change that carries the work closes it.
- Every behaviour this change adds is proven by CLI journeys driving the real binary and
  the real reference subprocess host over a real pipe rather than by inspection, and no
  test substitutes a double for the layer under test.
- The tests that cover the crates and documents this change touches are green over the
...
Iterate with this repository's diff-scoped judged lint, the core crate's configuration
and subprocess tests, and the CLI configuration journeys (`crates/onetaskgraph/tests/configuration.rs`
already holds the journey asserting a root behind the seam is left to the child — that
assertion is the one this change makes untrue, so it changes with it) rather than the
whole gate; the required checks run downstream on the change this branch becomes.

The member's name and meaning above are the agreement with the SDKs and any plugin
written against the protocol, so if you find you cannot satisfy it as stated, ask over
`$ORCHESTRATOR_ASK_MANAGER` rather than choosing a different shape; a shared interface in
this plan is never changed unilaterally.

This repository's releases are cut from conventional commits, and this host's plan-store
pin adopts the release that carries this fix, so a commit type that cuts no release would
leave that adoption waiting. `fix` is the type.
...
{"onepipeline.execution_checkout":"nickderobertis__onetaskgraph","onepipeline.id":"otg-subprocess-document-dir","onepipeline.persona":"engineer","onetaskgraph.delivers":["followups:I_kwDOUBBgxs8AAAABRp2wQA"],"onetaskgraph.item_kind":"task"}
-->
