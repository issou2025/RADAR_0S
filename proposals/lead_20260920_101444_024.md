...
Adopt this workstream's cargo build contract in `onejudge` (github.com/nickderobertis/onejudge; checkout `nickderobertis__onejudge` (registered)).

onejudge is a Cargo workspace whose one member is `crates/onejudge`; no crate outside the workspace.

**What exists today:** no `.cargo/config.toml`; the root `Cargo.toml` carries `[profile.dev] opt-level = 0` with a comment (the e2e suite spawns the test-double binaries many times, so a cheap debug build beats optimizing them). That table and its comment stay exactly where they are; `debug = 1` goes in the config file like everywhere else, and the two merge. Add one sentence to that manifest comment pointing at `.cargo/config.toml` for the debuginfo level, so a reader of the manifest's `[profile.dev]` finds the other half.
...
- `python/onejudge-sdk/test/test_sdk.py` and `package_e2e.py` resolve `target/debug/onejudge`, `onejudge-echo-provider` and `onejudge-fake-oneharness` at the clone root; `just python-sdk-check` writes `target/python-sdk.coverage`
- `crates/onejudge/src/bin/support/coverage.rs` and `crates/onejudge/tests/coverage.rs` handle `.profraw` files under the directory `cargo llvm-cov` merges from; `just test` is `cargo llvm-cov nextest` with `--fail-under-lines 95`
- `.github/workflows/ci.yml` runs `./target/release/onejudge schema`; `release-binaries.yml`, `release-pypi.yml`
...
- The checks that read the target directory, spawn a built binary or run under coverage are green over the finished tree (their commands are under `## Additional info`): the `coverage` integration test, which handles `.profraw` files under the directory the coverage tool merges from; the Python SDK check, a reader of `target/debug/onejudge` and its test doubles.
...
- the `coverage` integration test, which handles `.profraw` files under the directory the coverage tool merges from: `cargo nextest run --features fake-provider,cli,sdk-schema --test coverage`
- the Python SDK check, a reader of `target/debug/onejudge` and its test doubles: `cargo build --locked --features fake-provider,cli,sdk-schema && just python-sdk-check`
- coverage: a scoped run through the crate's coverage interface reporting per-file line results: `cargo llvm-cov nextest --features fake-provider,cli,sdk-schema --ignore-filename-regex 'src/bin/' -E 'kind(lib)' --summary-only`
...
{"onepipeline.id":"onejudge","onepipeline.persona":"engineer","onetaskgraph.item_kind":"task","orchestrator.plan-review":{"by":"planning-closeout","key":"fd9bb7300fb82b732c5405221b3db7d90f899243c1f930a6363de310084f8074","reviewed_at":"2026-09-20T09:43:09.643180+00:00"}}
-->
