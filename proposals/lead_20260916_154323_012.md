...
**fix(release): print the resolvable reference in the recovery command**

*Project description:*
## What

Ticket: https://github.com/nickderobertis/onevcs/issues/150 — *release recovery hint can emit an
unresolvable commit*.

**Root cause (from the ticket).** When release status has no landing baseline, its recovery text
names the landing commit (`crates/onevcs/src/release.rs:669-685` at basis
`ed930a64a48f95225e970571886df8655bc16609`, the `unsound` reason, formatting `onevcs release
acknowledge {commit} …`). But reference resolution (`crates/onevcs/src/status.rs:1914-1966`,
`resolve`) accepts a commit only when `by_commit` finds a registered branch still carrying it. An
externally squash-merged commit is therefore printed as the remedy and refused by the remedy
itself. For onepipeline pull request 301, release status printed `onevcs release acknowledge
d552450…`, that command answered `names no work this host knows`, and the same acknowledgement
with `https://github.com/nickderobertis/onepipeline/pull/301` succeeded.

**The accepted fix, with the user's ruling.** The user commented *"Yes carry reference"*. So:
carry the resolvable work reference that release status was obtained with — the reference as the
caller gave it and `resolve` accepted it (a change-request URL, a session token, a branch or a
commit) — through the status result and into the recovery message, and print that same reference
in the `release acknowledge` command. Do not reconstruct the remedy from the landing commit. Where
the status is reached by a caller that supplied no reference of its own (a consumer iterating
landings, for example), carry the most resolvable reference that caller holds for the work — the
change request's URL when one exists — and never a bare landing commit that `resolve` would refuse.

Add an end-to-end case for an externally squash-merged change whose branch no longer contains the
resulting base commit, proving the printed change-request URL is accepted unchanged by `release
acknowledge`.
...
An operator following the exact recovery command onevcs prints cannot acknowledge a verified
release, so a `published`-adoption hold stays unanswered until somebody discovers that the
change-request URL works where the printed commit does not. This host holds every adoption of its
own repository `published`, so that hold is on the path of every release adoption the manager
supervises.

## Acceptance criteria

- When release status has no usable landing baseline, the recovery text's `onevcs release
  acknowledge` command names the reference the status was resolved from (for a status asked with
  a change-request URL, that URL), not the landing commit.
- The status result carries that reference, so the message is composed from what was resolved
  rather than reconstructed from the landing commit.
- An end-to-end test drives an externally squash-merged change whose branch does not contain the
  resulting base commit: release status prints the acknowledgement command, and running exactly
  that command, unchanged, is accepted by `release acknowledge` and records the acknowledgement.
...
**Downstream.** `op-relink` moves onepipeline onto the onevcs release this change becomes (waiting
on its `onevcs-testing` crate, which is published beside it), and `aio-adopt-engine` moves this
host's onevcs pin to it. The engine relays release-hold reasons to the manager, so the message
text's shape is read by people, not parsed; keep it one sentence per fact.
...
{"onepipeline.id":"ovcs-150","onepipeline.max_turns":200,"onepipeline.persona":"engineer","onetaskgraph.delivers":["followups:I_kwDOTx3GGM8AAAABRkzu_g"],"onetaskgraph.item_kind":"task","orchestrator.plan-review":{"by":"planning-closeout","key":"f932b6e2a1864708e173a42beefc86004d2f63286c5c7ff4d376f5bc443abff8","reviewed_at":"2026-09-16T15:22:01.120097+00:00"}}
-->
