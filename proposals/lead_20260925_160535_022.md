...
**feat(task): report the backend's human-facing key beside the stable id**

*Project description:*
## What

Give a task the short handle its backend shows people, beside the opaque id it already
reports. Every task keeps `id` — the source's own stable identifier, what everything
stores and matches on — and gains an optional `key`.

- **Linear**: the issue's `identifier`, `ENG-123`. Today `graphql::ISSUE` and
  `graphql::ISSUES` select `id title description url createdAt updatedAt state labels
  project` and no `identifier`, and `identifier` is not in the pinned
  `crates/onetaskgraph-linear/tests/fixtures/schema.graphql` either. Both move.
- **GitHub Projects**: the issue **number alone**, as a decimal string — `1043`, never
  `owner/repo#1043`. The native id there is the issue's GraphQL node id (`I_kw…`) and the
  number appears only inside `url`. There is one place to select it: the `BoardIssue`
  fragment in `crates/onetaskgraph-github-projects/src/lib.rs`, which `SEARCH_ISSUES`,
  `ISSUE`, `SUB_ISSUES` and the board query all compose.
- **`local-md`, `in-memory`, and any source with no separate handle**: absent. Never a
  copy of the id.

It has to reach every surface a task is read through: the contract type, the JSON Schema
bundle `onetaskgraph schema` prints, the plugin protocol, the CLI's JSON and text output,
and both SDKs under `sdks/`.

**Contract 1 of the project record `human-readable-branches` is the agreement, and it is
the source rather than this paragraph** — the field's shape, its read-only meaning, the
sentence the doc comment owes about the key being human-facing and liable to change, the
per-backend values, and the decision that `Project` gets no `key` in this work and why.
Read it before writing anything. Two things it settles that would otherwise look like
gaps: resolving a task **by** its key is not part of this and nothing added here may imply
it, and `ItemWrite` gains nothing because a key is derived by the source and never written.

Read `AGENTS.md` first — "The plugin contract", "Invariant: no work data outside a
plugin", "Tests" — and `docs/plugin-protocol.md` §2.1 with the passage recording that
`location` was added without a protocol bump, which is the precedent this follows.
...
`ENG-123` is the half of that this repository owns. The ticket handle people actually say
out loud is not something onetaskgraph exposes today: a consumer that wants to put it in
front of a person has to dig it out of a URL and parse it, which is why nothing does. The
user asked for it in its own right as well — the tracker's human id available beside the
stable one — and a later node of this plan renders a branch name from it.

## Acceptance criteria

1. A task read from a **Linear** source reports the issue's `identifier` as its `key`, in
   the CLI's JSON output and in its text rendering, with the native `id` unchanged beside
   it. Proven against that plugin's own offline lane —
   `crates/onetaskgraph-linear/tests/plugin.rs`, which builds the plugin through its real
   interface and answers over a real loopback socket from the fixtures — rather than
   against a credential this dispatch does not hold.
2. A task read from a **GitHub Projects** source reports the issue's number alone, as a
   decimal string, as its `key`, in the same two places and with its node id unchanged
   beside it. Proven the same way, against
   `crates/onetaskgraph-github-projects/tests/plugin.rs`.
3. A task read from **`local-md`** reports **no** `key` — absent, not empty and not a copy
   of its id — and the same holds for the `in-memory` source. Proven through the compiled
   binary the way that source's journeys already drive it.
4. `crates/onetaskgraph-linear/tests/fixtures/schema.graphql` carries `Issue.identifier`
   with the type Linear's published schema states, and
   `crates/onetaskgraph-linear/tests/fixtures/README.md` records it under that file's own
   provenance convention: which published artifact it was read from, identified the way
   that README identifies its other pins, and the date it was read. Completeness is that
   the member is present with its real type and its provenance line names a source a
   reader can open — a placeholder, a `TODO`, or a type guessed to make a test pass does
   not satisfy it. **The check that compares this pin against the real API is this
   repository's own credentialed Linear lane, which runs on the merge path; its result is
   not this dispatch's bar**, because every dispatch on the calling host has its forge and
   tracker credentials masked and that lane skips rather than runs here.
5. The plugin protocol carries `key` as an added optional member with a documented
   default, **with no protocol version bump**, and `docs/plugin-protocol.md` records it in
   the same passage that records `location` and the `rate-limited` `message` — so the next
   reader finds the reasoning where the other two additive members already keep theirs.
6. The JSON Schema bundle `onetaskgraph schema` prints carries `key` on `Task`, and both
   SDKs under `sdks/` carry it **as generated content**: each generated file in the
   finished tree is exactly what regenerating it from that bundle produces, so each SDK
   project's own `generate-check` target — `sdk-python:generate-check` and
   `sdk-typescript:generate-check` — is green over the finished tree.
7. `Project` carries no `key`, and the api crate's exported type inventory still
   reconciles: `scripts/check-contract-inventory.sh` is green over the finished tree.
8. Every test that exercises what this change touches is green over the finished tree —
   the two plugin lanes above, the source and CLI tests over the crates you edited, the
   SDK tests, and the GitHub Projects cost and node-count gates, which read every document
   that fragment composes. The judged lint over this branch's own diff against its merge
   base is clean. The repository's complete bar is the merge path's to run, not this
   dispatch's.
9. Every claim the dispatch's own report makes about the finished work — what it ran, what
   it saw, and on which commit — is true of the tree as it finally stands.

## Additional info

- **Contract 1 is fixed for this run.** A departure you want — a different field shape, a
  different per-backend value, projects carrying a key after all — is a **proposal**: raise
  it over `"$ORCHESTRATOR_ASK_MANAGER"` and keep building against the contract as written
  while you wait. Three other nodes are reading the same words.
- The `location` field on `Task` is the precedent for everything about this addition:
  where it sits, how it is documented, how it defaults, and how it reached both SDKs. Copy
  its shape rather than inventing one.
- The GitHub Projects plugin's `BoardIssue` fragment is composed into four documents, and
  `tests/point_cost.rs` and `tests/node_count.rs` pin what each of those costs. A scalar
  selection should move neither number; confirm that rather than assuming it, and if one
  moves, say so in your report rather than re-blessing a pin quietly.
- The Linear queries' variables and selections are held to the pinned schema by that
  plugin's own contract tests. Adding the selection without adding the member to the pinned
  schema fails there, which is the gate working.
- Run the crate-scoped tests and the lint over your diff as you iterate; the two plugin
  lanes are the ones that actually exercise criteria 1 and 2.
...
{"onepipeline.id":"onetaskgraph-task-key","onepipeline.max_turns":200,"onepipeline.persona":"engineer","onetaskgraph.item_kind":"task","orchestrator.plan-review":{"by":"review-plan","key":"3aa69bb92ae878a17216771523011a78fd17eae289d594eec498b418482a1873","reviewed_at":"2026-09-25T13:54:40.066471+00:00"}}
-->
